...
DSA kernel changes must preserve architecture fallback and run the matching
tests under `test/python/fe_api/dsa/` on every affected architecture. Keep
experimental variants and performance artifacts outside the production package;
the historical SM100/Rubin exploration is retained on
`archive/dsa_bwd_rubin_experiments_20260816`.

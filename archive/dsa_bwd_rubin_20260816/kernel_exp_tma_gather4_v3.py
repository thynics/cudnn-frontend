"""Rubin experiment: score-K TMA gather4 on the two-CTA candidate.
...
Only complete rank-owned N32 slices use gather4.  A dynamic-length
boundary slice falls back to the original masked cp.async gather, retaining
the established ragged, hole, and zero-length semantics.
"""
...
class FlashAttentionDSABackwardSm100TwoCTA(FlashAttentionDSABackwardSm100):
    ...
    def __call__(
        ...
        # Gather4 requires a rank-2 data tensor.  KV is shared across the
        # batch mode, so this view is address-identical to the canonical one.
        mKV_gather = cute.make_tensor(
            mKV.iterator,
            cute.make_layout(
        ...
        # The nonzero stride selects sparse rows; D is broadcast.  The
        # pointer value is irrelevant while constructing the TMA atom.
        gather_coord_layout = cute.make_layout(
            (mTopkIdxs.shape[0], self.D_HEAD),
            stride=(1, 0),
        )
        gather_coords = cute.make_tensor(
            mTopkIdxs.iterator,
            gather_coord_layout,
        )
        ...
        gather_tile = (self.N_TILE_CTA, self.K_CHUNK)
        gather_smem_base = sm100_utils.make_smem_layout(
            OperandMajorMode.K,
            gather_tile,
            self.element_dtype,
            self.K_CHUNKS,
        )
        # Generic make_tiled_tma_atom accepts either rank(tiler) or
        # rank(tiler)+1.  The canonical staged layout is exactly the latter;
        # the helper itself strips its pipeline-stage mode for the atom.
        assert cute.rank(gather_tile) == 2
        assert cute.rank(gather_smem_base) == 3
        gather_smem_tiled = cute.tiled_divide(
            gather_smem_base,
            gather_tile,
        )
        # tiled_divide profile: ((N32,K128), RestN=1, RestK=1, Stage=4).
        assert cute.rank(gather_smem_tiled) == 4
        assert cute.size(gather_smem_tiled, mode=[0]) == (
            self.N_TILE_CTA * self.K_CHUNK
        )
        assert cute.size(gather_smem_tiled, mode=[1]) == 1
        assert cute.size(gather_smem_tiled, mode=[2]) == 1
        assert cute.size(gather_smem_tiled, mode=[3]) == self.K_CHUNKS
        gather_smem_layout = gather_smem_base
        assert cute.cosize(gather_smem_layout) == (
            self.N_TILE_CTA * self.K_CHUNK * self.K_CHUNKS
        )
        assert gather_smem_layout.inner == score_b_layout_staged.inner
        assert cute.cosize(gather_smem_layout) == cute.cosize(
            score_b_layout_staged
        )
        ...
        tma_atom_k_gather, tma_tensor_k_gather = (
            cpasync.make_tiled_tma_atom(
                cpasync.CopyBulkTensor2DGather4G2SOp(),
                mKV_gather,
                gather_smem_layout,
                gather_tile,
                gmem_coord_tensor=gather_coords,
            )
        )
    ...
    def _store_dq_epi_tma_v12(
        ...
        if mtx < self.MATH_THREADS_PER_CTA:
            ...
            coordinate_tile = rank_coordinates[
                ((None, None), 0, 0)
            ]
            epi_tile = (
                self.D_TILE_CTA,
            ...
            for subtile_index in cutlass.range(
                ...
                for value_index in cutlass.range_constexpr(
                    ...
                    s_dq_epi[head, local_d] = self.element_dtype(
                        thread_values[value_index]
                    )
        cute.arch.fence_view_async_shared()
    ...
    def kernel(
        ...
        tma_atom_k_gather: cute.CopyAtom,
        tma_tensor_k_gather: cute.Tensor,
        mQ: cute.Tensor,
        mKV: cute.Tensor,
        mdO: cute.Tensor,
        mdQ: cute.Tensor,
        ...
        gather_smem_tiled: cute.ComposedLayout,
...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    def _specialize_shared_storage(
        ...
        class SharedStorageV2:
            ...
            score_gather_mbar: cutlass.Int64
            tmem_dealloc_mbar: cutlass.Int64
            tmem_holding_buf: cutlass.Int32
            ...
            direct_q: cute.struct.Align[
                cute.struct.MemRange[element_dtype, 16384], 1024
            ]
            direct_do: cute.struct.Align[
                cute.struct.MemRange[element_dtype, 16384], 1024
            ]
            loan_do: cute.struct.Align[
                cute.struct.MemRange[element_dtype, 16384], 1024
            ]

        assert SharedStorageV2.size_in_bytes() == 329_728
    ...
    @cute.jit
    def _load_score_kv_gather4(
        self,
        mKV: cute.Tensor,
        mTopkIdxs: cute.Tensor,
        destination: cute.Tensor,
        t_ks_gather: cute.Tensor,
        t_kg_gather: cute.Tensor,
        t_ki_gather: cute.Tensor,
        tma_atom_k_gather: cute.CopyAtom,
        token_idx: Int32,
        batch_idx: Int32,
        tile_index: Int32,
        ...
        copy_atom: cute.CopyAtom,
        thread_copy: cute.TiledCopy,
        score_gather_mbar: cute.Pointer,
        phase: Int32,
    ) -> Int32:
        """Load a score K tile with gather4; retain the ragged fallback."""

        row_begin = (
            tile_index * Int32(self.N_TILE)
            + rank * Int32(self.N_TILE_CTA)
        )
        next_phase = phase
        if row_begin + Int32(self.N_TILE_CTA) <= topk:
            row_tile = tile_index * Int32(2) + rank
            if warp_idx == Int32(0):
                with cute.arch.elect_one():
                    cute.arch.mbarrier_arrive_and_expect_tx(
                        score_gather_mbar,
                        self.N_TILE_CTA * self.D_HEAD * 2,
                    )
                for chunk in cutlass.range_constexpr(self.K_CHUNKS):
                    cute.copy(
                        tma_atom_k_gather,
                        [
                            t_kg_gather[None, (row_tile, chunk)],
                            t_ki_gather[None, (row_tile, chunk)],
                        ],
                        t_ks_gather[None, 0, 0, chunk],
                        tma_bar_ptr=score_gather_mbar,
                    )
                cute.arch.mbarrier_wait(score_gather_mbar, phase)
            self.gather_barrier.arrive_and_wait()
            cute.arch.fence_view_async_shared()
            next_phase = Int32(1) - phase
        else:
            self._load_score_kv(
                mKV,
                mTopkIdxs,
    ...
    def kernel(
        ...
        score_gather_mbar = storage.score_gather_mbar.ptr
        ...
        # As in the official HCA gather4 path, partition TMA through a
        # canonical logical (N32,K128,stage4) view of the same bytes.  The
        # MMA-specific nested view above remains the only GEMM operand view.
        k_n_gather = storage.score_kv.get_tensor(
            gather_smem_tiled.outer,
            swizzle=gather_smem_tiled.inner,
        )
        loan_quad = (
            cute.make_tensor(
                cute.recast_ptr(
        ...
        # ------------------------------------------------------------------
        gather_tile = (self.N_TILE_CTA, self.K_CHUNK)
        g_k_gather = cute.zipped_divide(
            tma_tensor_k_gather,
            gather_tile,
        )
        token_indices = mTopkIdxs[None, (token_idx, batch_idx)]
        gather_coords = cute.make_tensor(
            token_indices.iterator,
            cute.make_layout(
                (mTopkIdxs.shape[0], self.D_HEAD),
                stride=(1, 0),
            ),
        )
        g_i_gather = cute.zipped_divide(gather_coords, gather_tile)
        gather_tile_elements = self.N_TILE_CTA * self.K_CHUNK
        assert cute.size(k_n_gather, mode=[0]) == gather_tile_elements
        assert cute.size(g_k_gather, mode=[0]) == gather_tile_elements
        assert cute.size(g_i_gather, mode=[0]) == gather_tile_elements
        t_ks_gather, t_kg_gather, t_ki_gather = cpasync.tma_partition(
            tma_atom_k_gather,
            0,
            cute.make_layout(1),
            k_n_gather,
            [g_k_gather, g_i_gather],
        )
        ...
        direct_q_fragment_a = dkv_tiled_mma.make_fragment_A(
            direct_q_quad[0]
        )
        direct_q_fragment_b = dkv_tiled_mma.make_fragment_A(
            direct_q_quad[1]
        )
        direct_do_fragment_a = dkv_tiled_mma.make_fragment_A(
            direct_do_quad[0]
        )
        direct_do_fragment_b = dkv_tiled_mma.make_fragment_A(
            direct_do_quad[1]
        )
        ...
        if tidx == Int32(0):
            ...
            cute.arch.mbarrier_init(score_gather_mbar, 1)
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            if tile_count > Int32(0):
                ...
                score_gather_phase = self._load_score_kv_gather4(
                    mKV,
                    mTopkIdxs,
                    k_n,
                    t_ks_gather,
    ...
    def _issue_score_v2(
        ...
        page_pipeline,
        page_consumer_state: pipeline.PipelineState,

...
- **Attention boundary tests must check both normalization terms and masked arithmetic.** Finite saved LSE and sink values can overflow when converted to log2 units; cover either term dominating and a large finite tie, whose normalization correction can disappear when added to the reference logit. For ignored sparse slots, zero-filled K/V or a bounded exponent alone does not prove zero gradients: include finite dO large enough to expose intermediate overflow, and assert that the intended backend actually executes.

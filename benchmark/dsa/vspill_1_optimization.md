...
| A10 | 2026-08-14 | The remaining atomic anomaly may be the synchronized 16-warp injection width rather than same-address collision; two WG waves keep each epoch at 64 warp REDG while halving simultaneous writers. | All eight reducer warps retain their original T2R fragment. `cohort = wg_idx ^ rank`; one 4-warp WG/CTA executes all eight vector atomics per phase, then all reducers rendezvous on the existing barrier before the complementary phase. | PENDING | PENDING | RUNNING | Predeclared static gate: 16 vector-atomic sites, REG96/STACK16, and reducer local spill sites must not worsen. Accept only at >=2% paired end-to-end speedup, then correctness plus a second run. |

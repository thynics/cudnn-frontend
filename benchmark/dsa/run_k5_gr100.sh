#!/usr/bin/env bash
# K5: post-K1a cliff-tax repricing (one-click, GR100).
#
...
#
# Runner contract:
#   cd <repo> && bash benchmark/dsa/run_k5_gr100.sh
#   -> success prints  K5_OK / K5_ARTIFACTS <dir>
#   -> failure prints  K5_FAILED stage=<name> exit=<rc>
#   Artifacts dir: benchmark/dsa/k5_out (override K5_OUT).
#
# Knobs: K5_TOPKS (128,512,2048), K5_WARMUP (20), K5_REPEAT (300),
# K5_ALLOW_ANY_CC=1.

set -Eeuo pipefail

REPO="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
OUT="${K5_OUT:-${REPO}/benchmark/dsa/k5_out}"
WARMUP="${K5_WARMUP:-20}"
REPEAT="${K5_REPEAT:-300}"
TOPKS="${K5_TOPKS:-128,512,2048}"
...
STAGE=init
trap 'rc=$?; echo "K5_FAILED stage=${STAGE} exit=${rc} (artifacts: ${OUT})"' ERR
trap 'exec 1>&- 2>&-; wait "${TEE_PID}" 2>/dev/null || true' EXIT

echo "K5_START utc=$(date -u +%Y-%m-%dT%H:%M:%SZ) host=$(hostname) out=${OUT}"
echo "K5_CONFIG warmup=${WARMUP} repeat=${REPEAT} topks=${TOPKS}"
...
K5_ALLOW_ANY_CC="${K5_ALLOW_ANY_CC:-0}" \
...
topks = sorted(set(arms["ck"]) & set(arms["r2"]))
assert len(topks) >= 2, (
...
#   <= -0.05  r2_effective      -> pantry pays; iterate (Q-pantry next)
#   <  +0.05  r2_neutral        -> structure wins eaten by cliff fee
#   >= +0.05  r2_regression     -> analyze (pantry TMA? first-dP? kdq gate?)
delta = period["r2"] - period["ck"]
if delta <= -0.05:
    verdict = "r2_effective"
elif delta < 0.05:
    verdict = "r2_neutral"
else:
    verdict = "r2_regression"
...
print("K5_SUMMARY " + json.dumps({
    "periods": summary["steady_period_us_per_tile"],
    "delta_r2_vs_ck": summary["delta_r2_vs_ck"],
}, sort_keys=True))
print("K5_VERDICT " + verdict)
PY

echo "K5_OK"
echo "K5_ARTIFACTS ${OUT}"

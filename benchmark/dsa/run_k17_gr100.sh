#!/usr/bin/env bash
# K17: post-K1a cliff-tax repricing (one-click, GR100).
#
...
#
# Runner contract:
#   cd <repo> && bash benchmark/dsa/run_k17_gr100.sh
#   -> success prints  K17_OK / K17_ARTIFACTS <dir>
#   -> failure prints  K17_FAILED stage=<name> exit=<rc>
#   Artifacts dir: benchmark/dsa/k17_out (override K17_OUT).
#
# Knobs: K17_TOPKS (128,512,2048), K17_WARMUP (20), K17_REPEAT (300),
# K17_ALLOW_ANY_CC=1.

set -Eeuo pipefail

REPO="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
OUT="${K17_OUT:-${REPO}/benchmark/dsa/k17_out}"
WARMUP="${K17_WARMUP:-20}"
REPEAT="${K17_REPEAT:-300}"
TOPKS="${K17_TOPKS:-128,512,2048}"
...
STAGE=init
trap 'rc=$?; echo "K17_FAILED stage=${STAGE} exit=${rc} (artifacts: ${OUT})"' ERR
trap 'exec 1>&- 2>&-; wait "${TEE_PID}" 2>/dev/null || true' EXIT

echo "K17_START utc=$(date -u +%Y-%m-%dT%H:%M:%SZ) host=$(hostname) out=${OUT}"
echo "K17_CONFIG warmup=${WARMUP} repeat=${REPEAT} topks=${TOPKS}"
...
K17_ALLOW_ANY_CC="${K17_ALLOW_ANY_CC:-0}" \
...
profile_check() {
    ...
    env -u DSA_RUBIN1_B200_COMPAT -u DSA_RUBIN1_E3PAD -u DSA_RUBIN1_REG_K1A -u DSA_RUBIN1_SPIN_K2 -u DSA_RUBIN1_MIX51 -u DSA_RUBIN1_PRODUCER_REGS -u DSA_RUBIN1_REDUCE_REGS -u DSA_RUBIN1_SLIM51 -u DSA_RUBIN1_KV2 -u DSA_RUBIN1_TAIL "$@" \
        PYTHONPATH="${REPO}/python${PYTHONPATH:+:${PYTHONPATH}}" \
        K17_ARM="${arm}" K17_EXP_P="${exp_p}" K17_EXP_R="${exp_r}" \
        python3 - <<'PY'
import json
import os

arm = os.environ["K17_ARM"]
exp_p = int(os.environ["K17_EXP_P"])
exp_r = int(os.environ["K17_EXP_R"])
...
assert K.SLIM51
assert not K.KV2
assert K.TAIL == (arm == "m51st")
PY
}
STAGE=profile_m51s
profile_check m51s 64 120 DSA_RUBIN1_MIX51=1 DSA_RUBIN1_SLIM51=1 DSA_RUBIN1_REG_K1A=1 | tee "${OUT}/profiles.log"
STAGE=profile_m51st
profile_check m51st 64 120 DSA_RUBIN1_MIX51=1 DSA_RUBIN1_SLIM51=1 DSA_RUBIN1_TAIL=1 DSA_RUBIN1_REG_K1A=1 | tee -a "${OUT}/profiles.log"
...
ARM_TIMEOUT="${K17_ARM_TIMEOUT:-600}"
run_arm() {
...
run_arm m51st rubin_1 DSA_RUBIN1_MIX51=1 DSA_RUBIN1_SLIM51=1 DSA_RUBIN1_TAIL=1 DSA_RUBIN1_REG_K1A=1
...
# k17 acceptance: per-topk candidate_ms ratios (the fixed tail is
# invisible in the steady slope but large in real latency).
tail_gain = {
    t: round(
        1.0
        - arms["m51st"][t]["candidate_ms"] / arms["m51s"][t]["candidate_ms"],
        4,
    )
    for t in topks
}
deltas = {"m51st": period["m51st"] - period["m51s"]}
best_gain = max(tail_gain.values())
if best_gain >= 0.02:
    verdict = "tail_effective"
elif best_gain > -0.02:
    verdict = "tail_neutral"
else:
...
print("K17_SUMMARY " + json.dumps({
    "periods": summary["steady_period_us_per_tile"],
    "deltas_vs_m51s": summary["deltas_vs_m51s"],
    "tail_gain_by_topk": summary["tail_gain_by_topk"],
}, sort_keys=True))
print("K17_VERDICT " + verdict)
PY

echo "K17_OK"
echo "K17_ARTIFACTS ${OUT}"

...
v_s1: dO slice-streaming + loan retirement (base = final, spec =
V_S1_SPEC.md).  Fixed GQA128 / D512 shape.  One 2-CTA cluster per
token; all five GEMMs run cta_group::2.  The Q panel stays
token-resident in SMEM; the dO panel is replaced by a 2-stage x 16KB
chunk stream (dP consumes chunk c from slot c%2), its four gradient
quadrants come from GMEM TMA like today's peer halves, and the
score_kv dO_r0 loan is fully retired (kscore back to one generation
per tile).  The score K tile is gathered per KV tile; gradient
operands stream through a two-slot round ring (ten generations per
tile).  dQ accumulates in TMEM across all KV tiles and stores through
a staged TMA epilogue; dKV drains per tile via red.global.add into an
f32 workspace.
"""
...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    def _issue_dp_streamed_v_s1(
        ...
        cross the dynamic stage branch).
        """
    ...
    def kernel(
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            if tile_count > Int32(0):
                ...
                # per-chunk through the stream (gather warp 1).
                with cute.arch.elect_one():
                    cute.arch.mbarrier_arrive_and_expect_tx(

"""vk_2_trace: vk_2 with the K2-pricing span set (refined trace).

Instrumentation-only sibling of vk_2.  Two name trades inside the fixed
31-name IKET budget:
  swap 1 (from vk_1_trace): RK_ACQ(i) / MAT_ACQ(m,q) / MAT_WAIT(m,q) in
  (kscore credit wait, ring credit wait, TMA completion wait -- the
  supply-row decomposition); LOAD_QDO / LOAD_STATS / MATH_BAR1 out.
  swap 2 (the observation debt): PDS_WAIT(i) and GRAD_SUP_WAIT(i) in --
  the leader's two previously unmarked waits at the grads-block head
  (the pds publication wait and the composite P-landing + dkv-credit +
  loan-kscore wait); WAIT_S(i) / WAIT_dP(i) out (near-zero since the
  vg_2/vg_5 exp band).
Purpose: price K2 on the vk_2 base -- confirm W17 saturation
(ROUTE_K + MAT_QDO vs period) and put numbers on the two waits K2's
de-rotation would move.  RK_ACQ payloads are constant 0 by design
(runtime tile_count branches); MAT_* payloads are loop_iter; several
spans per tile share payloads -- bucket by timestamp, not payload.

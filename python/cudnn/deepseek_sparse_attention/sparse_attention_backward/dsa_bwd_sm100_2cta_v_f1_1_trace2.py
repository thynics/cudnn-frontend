"""v_f1_1_trace2: v_f1_1 with the composite gate SPLIT (diagnosis rev).

v_f1_1's paired reading showed the loop routing around the removed pds
segment: PDS_WAIT collapsed 0.506->0.140 but GRAD_SUP_WAIT grew
0.260->0.438 with a flat period.  This rev splits that composite gate
into its four code constituents so the next binding anchor gets a name:
LAND_P_WAIT (relay_mbars, the P-landing conversion), DKV_ACQ_WAIT
(dkv_done producer_acquire vs the reducer), KS2_WAIT (the loan
generation's kscore consumer_wait) and LAND_DS_WAIT (relay_mbars+1
before pass 3).  Budget holds at 31: GRAD_SUP_WAIT is replaced by its
parts; DQ_EPI/TAIL (tail chapter closed by triple-null) and the
MATH_PD envelope (reconstructible from its component spans) retire.

Instrumentation-only sibling.  Name trade inside the 31 budget: retire
WAIT_S/WAIT_dP (near-zero, recoverable from T2R starts) and the
prologue-only LOAD_QDO/LOAD_STATS plus MATH_BAR1; add RK_ACQ(i)/
MAT_ACQ(m,q)/MAT_WAIT(m,q) (the supply-wait decomposition) and
PDS_WAIT(i)/GRAD_SUP_WAIT(i) (the leader's two unmarked gates, matching
the vk_2 K2-pricing span placement for comparability).  The pre-
registered pair-reading lives here: PDS_WAIT collapse AND WAIT_dQ
inflation judged together.

Single lever.  The relay previously issued both DSM routes and only then
called pds producer_commit, serializing ~0.4us of routing (wakeup + two
arrive_and_expect_tx + two bulk issues) into the leader's grads-head pds
gate (PDS_WAIT measured 0.506/tile on the vk_2 K2-pricing trace).  This
rev moves the commit to the instant the count-128 pds_ready mbar clears:
the leader's gate now tracks math's publish directly and the routing
proceeds in the relay's own shadow.

Safety was adjudicated before build: (1) dQ's B is purely local
(make_fragment_B(ds_image), no DSM-landed bytes), so the earlier gate
cannot expose unlanded data to dQ; (2) all landed-data consumers (dV
pass on peer-P, dK passes on peer-dS) remain gated by the separate
relay/landing mbars, which the relay still arms and converts after
routing; (3) the outbound-read WAR (math(t+1) overwriting p_xchg/
ds_image while routes read them) keeps its existing temporal-margin
class -- routes complete ~1.4us into the window, the overwrite gate
(leader's UMMA-tracked pds release) clears at ~6.5us; the reorder does
not shorten that margin's protocol chain.

Pre-registered pair-reading (binding gates drift between bases): judge
PDS_WAIT collapse AND WAIT_dQ inflation together on the trace sibling;
a null release read with PDS_WAIT collapsed + WAIT_dQ inflated means
the kdq gate absorbed the win (next lever: gather schedule), not a
falsified mechanism.

Inherited vk_2 notes: split publish retired (validated C form).
...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    * The gradient GEMMs of tile p are issued inside tile t=p-1's S/dP
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            if tidx % Int32(32) == Int32(0):
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    # v_f1_1 (R2): commit the instant math's publish is
                    # visible -- the leader's pds gate no longer waits for
                    # the routing below.
    ...
    def _issue_prev_grads_head_v2(
        ...
        # published: invariant I1's only gate.
        ...
        land_p_token = _iket.range_start(
            "LAND_P_WAIT(i)",
            issue_seq,
        )
        ...
        dkv_acq_token = _iket.range_start(
            "DKV_ACQ_WAIT(i)",
            issue_seq,
        )
        ...
        land_ds_token = _iket.range_start(
            "LAND_DS_WAIT(i)",
            issue_seq,
        )
        _mbarrier_wait_acquire_cluster(

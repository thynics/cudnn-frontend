...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    def kernel(
        ...
        # EMPTY = pds_mbars_b[1] @ +0x88).  This ablation releases dS at
        # the gradient tail after dK-first has finished the following dV.
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            for loop_iter in cutlass.range(tile_count):
                ...
                # P-empty (pds_mbars[1] @ +0x78) is released after the
                # previous tile's final dV read, at the dK-first tail.
                ...
                # dS-empty (pds_mbars_b[1] @ +0x88) is also released at
                # the previous tile's gradient tail.  P stores above may
                # issue while that tile's dK/dV passes are still running.
            ...
            if is_leader_cta:
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    if has_prev:
                        ...
                        # P was released inside the tail after the last dV
                        # pass; release dS now that the full tail is issued.
            ...
            if tile_count > Int32(0):
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    # g2..g7: Q_r0, Q_r1, dO_r1 panel fills, pipelined
                    ...
                    for flat_gen in cutlass.range_constexpr(6):
                        ...
                        # TMA box.  G4/Q_r1/H0 is the exception: its CTA0
                        ...
                        if cutlass.const_expr(tensor_kind == 0):
                            ...
                            if cutlass.const_expr(h_half == 0):
                                if cutlass.const_expr(
                                    self.OWN_HALF_BULK and flat_gen != 2
                                ):
                                    if rank == Int32(0):
                                        with cute.arch.elect_one():
                                            _cpasync_bulk_s2cluster(
    ...
    def _issue_prev_grads_head_v2(
        ...
        """Gradient block, first half: dQ rounds + round-0 dK/dV passes.
        ...
        # dK-first ablation: consume Q_r0 from g2/g3 before borrowing the
        # score buffer for dV_r0.  relay_mbars+1 publishes the complete dS
        # image used by these two passes.
        _mbarrier_wait_acquire_cluster(
            relay_mbars + 1,
            relay_phase,
        )
        ...
        # dV_r0 retains aug6's borrowed dO source and is accumulated after
        # dK into the same dKV slot.
    ...
    def _issue_prev_grads_tail_v2(
        ...
        """Gradient block, second half: round-1 dK/dV passes.
        ...
        dkv_done_pipeline.producer_acquire(dkv_producer_state)
        # With the load ring ordered Q_r1 then dO_r1, g4/g5 feed dK first.
        round_pipeline.consumer_wait(round_consumer_state)
        self._issue_dkv_pass_v2(
            dkv_tiled_mma,
            t_dkv_1,
        ...
        # g6/g7 now carry dO_r1, so the final two passes are dV.
        round_pipeline.consumer_wait(round_consumer_state)
        self._issue_dkv_pass_v2(
            dkv_tiled_mma,
            t_dkv_1,
        ...
        # Last P read (round-1 dV on p_fragment_1) just issued.  Preserve
        # aug6's multicast pipeline release; the caller advances the state.

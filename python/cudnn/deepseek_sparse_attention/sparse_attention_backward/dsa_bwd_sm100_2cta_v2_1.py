...
v2_1 (Rubin capacity build): direct v12 lineage, with the supply ring
deepened 2 -> 5 and the P/dS publish path tile-parity double-buffered per
the design spec in V21_RUBIN_DESIGN.md (same directory).
DSA_V21_B200_COMPAT=1 (read at import time) selects the ring-2 /
single-buffer degenerate tier (semantics ~= v12) for B200 regression of
the generalized machinery; the default tier targets Rubin's 327 KB usable
SMEM (cap 334,848 B per CTA).  Both tiers share ONE kernel code path
(const_expr-pruned on class attributes).
...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    """v2_1: Rubin-capacity build on the v12 base (V21_RUBIN_DESIGN.md).

    Two keys against the pacing ring's two walls, funded purely by the
    Rubin SMEM budget (327 KB usable per CTA):

    * Supply ring depth 2 -> 5 (phase law: 10 gens/tile mod 5 == 0, one
      of the only legal static depths {2, 5, 10}).  gen -> slot is the
      flat generation number mod ROUND_STAGES; the kdq pair keeps slots
      0/1 and the eight panel fills walk slots 2,3,4,0,1,2,3,4.  The
      v12 deferred-commit lattice, per-slot TMA mbars/phases, and the
      W17/gather kdq handshake are preserved; every slot, mbar offset
      and fragment view is compile-time static.
    * P/dS publish double-buffer (pipe_pds num_stages 1 -> 2): the whole
      publish set (p_blocks + p_xchg + ds_image + ds_blocks) exists per
      tile parity; the dead ds_xchg field is retired in BOTH sets.  The
      leader/math/W18 tile loops are 2-tile macro-expanded (even body =
      buf0 static views, odd body = buf1, odd-count leftover handled
      separately) so no buffer index is ever dynamic; the leader's
      rotated schedule means grads(t) consumes parity t%2, published by
      math(t) with the same parity.

    Tiering: DSA_V21_B200_COMPAT=1 (env, read at import) degrades to
    ring-2 + single publish set (semantics ~= v12, cap 232,448) so the
    generalized machinery can be regression-proven on B200; the default
    tier asserts <= 334,848 with the actual struct size echoed.  Both
    tiers run the SAME kernel path, pruned by const_expr on class
    attributes.
    ...
    * The gradient GEMMs of tile p are issued inside tile t=p-1's S/dP
    ...
    # v2_1 tier switch, read ONCE at import time (class definition).
    # COMPAT=0 (default, Rubin): ring 5 + P/dS double buffer, cap 334,848.
    # COMPAT=1 (B200 regression): ring 2 + single buffer (~= v12 semantics),
    # cap 232,448.  Both tiers share one const_expr-pruned kernel path.
    # Default = COMPAT while the fleet is B200 (the harness cannot pass
    # env); flip the default (or export DSA_V21_B200_COMPAT=0) when the
    # Rubin toolchain lands.
    B200_COMPAT = os.environ.get("DSA_V21_B200_COMPAT", "1") == "1"

    # Round-region generations per tile (fixed order, one producer/consumer):
    # g0 kdq_r0 g1 kdq_r1 g2 dO_r0h0 g3 dO_r0h1 g4 Q_r0h0
    # g5 Q_r0h1 g6 dO_r1h0 g7 dO_r1h1 g8 Q_r1h0 g9 Q_r1h1
    # gen -> ring slot = flat gen mod ROUND_STAGES (kdq pair -> slots 0/1;
    # ring-5 panels walk slots 2,3,4,0,1,2,3,4).  Phase law: legal static
    # depths are exactly {2, 5, 10} (10 mod N == 0).
    ROUND_GENS_PER_TILE = 10
    ROUND_STAGES = 2 if B200_COMPAT else 5
    assert ROUND_GENS_PER_TILE % ROUND_STAGES == 0

    # v2_1: P/dS publish sets == pipe_pds stages (tile-parity double
    # buffer on the Rubin tier; the buffer-set index is parity %
    # PDS_STAGES while the landing/relay/pds_ready mbars stay parity-
    # split in BOTH tiers so the phase bookkeeping is tier-invariant).
    PDS_STAGES = 1 if B200_COMPAT else 2

    # Per-CTA SMEM cap override (base class launches with the struct's
    # actual size; this is the tier's upper bound, asserted with the
    # actual value echoed -- E3 lesson: never assert exact struct sizes).
    MAX_SMEM_BYTES = 232_448 if B200_COMPAT else 334_848
    ...
    def _specialize_shared_storage(
        ...
        # v2_1 byte-exact plan (per CTA), hand ledger:
        #
        # RUBIN tier (B200_COMPAT=0, cap 334,848):
        #   stationary Q + dO      2 x 65,536  = 131,072
        #   score K stage (epi alias)              32,768
        #   round region           5 x 16,384  =  81,920
        #   P sets   2 x (blocks 8,192 + xchg 4,096) = 24,576
        #   dS sets  2 x (image 8,192 + blocks 8,192) = 32,768
        #   softmax stats                             512
        #   ---------------------------------------------
        #   data total                            303,616
        #   + mbar/holding header (<= 1,024 pad)  304,640 <= 334,848
        #
        # B200_COMPAT tier (cap 232,448):
        #   round region           2 x 16,384  =  32,768
        #   single P set 12,288 + single dS set 16,384
        #   data total 225,792 + header pad    -> 226,816 <= 232,448
        #
        # ds_xchg (4,096, dead since v12 P1b) is retired in BOTH tiers:
        # the dS DSM send sources the whole-image half directly.
        assert cute.cosize(score_a_layout_staged) <= 32768
        assert cute.cosize(score_b_layout_staged) <= 16384
        ...
        # Host-side tier specialization (this method is pure host code;
        # the KERNEL path stays single, const_expr-pruned on the class
        # attributes).  Mbar layout notes shared by both variants:
        #   round_mbars     : 2 * ROUND_STAGES (full+empty per stage)
        #   pds_mbars       : 2 * PDS_STAGES
        #   round_tma_mbars : ROUND_STAGES (one per ring slot)
        #   landing/relay   : tile-parity split in BOTH tiers,
        #                     index = 2*parity + {0: P, 1: dS}
        #   pds_ready_mbars : tile-parity split in BOTH tiers (count-128)
        if self.B200_COMPAT:
            ...
            class SharedStorageV2:
                ...
                landing_mbars: cute.struct.MemRange[cutlass.Int64, 4]
                relay_mbars: cute.struct.MemRange[cutlass.Int64, 4]
            ...
            class SharedStorageV2:
                ...
                # Five 16,384 B ring slots, each 1024-aligned (slot s is
                # the flat-gen-mod-5 target; kdq pair -> a/b, panels walk
                # c,d,e,a,b,c,d,e within a tile).
                round_buf_a: cute.struct.Align[
                    cute.struct.MemRange[element_dtype, 8192],
                    1024,
                ]
                ...
                round_buf_e: cute.struct.Align[
                    cute.struct.MemRange[element_dtype, 8192],
                    1024,
                ]
                # P/dS publish set, tile parity 0 (even tiles).
                p_blocks_0: cute.struct.Align[
                    cute.struct.MemRange[element_dtype, 4096],
                    1024,
                ]
                ...
                # P/dS publish set, tile parity 1 (odd tiles).
                p_blocks_1: cute.struct.Align[
                    cute.struct.MemRange[element_dtype, 4096],
                    1024,
                ]
                p_xchg_1: cute.struct.Align[
                    cute.struct.MemRange[element_dtype, 2048],
                    1024,
                ]
                ds_image_1: cute.struct.Align[
                    cute.struct.MemRange[element_dtype, 4096],
                    1024,
                ]
                ds_blocks_1: cute.struct.Align[
                    cute.struct.MemRange[element_dtype, 4096],
                    1024,
                ]
                stats: cute.struct.Align[
        ...
        smem_bytes = SharedStorageV2.size_in_bytes()
        # Upper-bound assert with the actual value echoed (E3 lesson:
        # cute.struct padding never gets an exact-size assert).
        assert smem_bytes <= self.MAX_SMEM_BYTES, (
            f"v2_1 SharedStorageV2 {smem_bytes} B exceeds tier cap "
            f"{self.MAX_SMEM_BYTES} B (B200_COMPAT="
            f"{int(self.B200_COMPAT)})"
        )
        print(
            f"[dsa_bwd v2_1] SharedStorageV2 = {smem_bytes} B "
            f"(cap {self.MAX_SMEM_BYTES}, "
            f"B200_COMPAT={int(self.B200_COMPAT)}, "
            f"ROUND_STAGES={self.ROUND_STAGES}, "
            f"PDS_STAGES={self.PDS_STAGES})"
        )
    ...
    def kernel(
        ...
        # v2_1: ring-slot raw pointers, one static tuple entry per slot
        # (compile-time indexed everywhere; PHASE_DYNAMIC_INDEX is the
        # standing kill).
        if cutlass.const_expr(self.ROUND_STAGES == 5):
            round_buf_raws = (
                storage.round_buf_a.data_ptr(),
                storage.round_buf_b.data_ptr(),
                storage.round_buf_c.data_ptr(),
                storage.round_buf_d.data_ptr(),
                storage.round_buf_e.data_ptr(),
            )
        else:
            round_buf_raws = (
                storage.round_buf_a.data_ptr(),
        ...
        # v2_1: per-slot fragment views, compile-time bound
        # (round_kd[0..R-1] / round_quad[0..R-1]; the kdq pair only ever
        # CONSUMES kd slots 0/1, but the full tuples keep the slot
        # algebra uniform -- unused views cost nothing after DCE).
        if cutlass.const_expr(self.ROUND_STAGES == 5):
            round_kd = (
                ...
                storage.round_buf_c.get_tensor(
                    dq_a_layout_staged.outer,
                    swizzle=dq_a_layout_staged.inner,
                ),
                storage.round_buf_d.get_tensor(
                    dq_a_layout_staged.outer,
                    swizzle=dq_a_layout_staged.inner,
                ),
                storage.round_buf_e.get_tensor(
                    dq_a_layout_staged.outer,
                    swizzle=dq_a_layout_staged.inner,
                ),
            )
        ...
        # v2_1: P/dS publish sets per tile parity (PDS_STAGES tuples; all
        # indices below are compile-time constants; view-set index for
        # parity p is p % PDS_STAGES).
        if cutlass.const_expr(self.PDS_STAGES == 2):
            p_blocks_raws = (
                storage.p_blocks_0.data_ptr(),
                storage.p_blocks_1.data_ptr(),
            )
            ds_blocks_raws = (
                storage.ds_blocks_0.data_ptr(),
                storage.ds_blocks_1.data_ptr(),
            )
            ds_image_raws = (
                storage.ds_image_0.data_ptr(),
                storage.ds_image_1.data_ptr(),
            )
            ds_images = (
                storage.ds_image_0.get_tensor(
                    dq_b_layout_staged.outer,
                    swizzle=dq_b_layout_staged.inner,
                ),
                storage.ds_image_1.get_tensor(
                    dq_b_layout_staged.outer,
                    swizzle=dq_b_layout_staged.inner,
                ),
            )
        else:
            p_blocks_raws = (storage.p_blocks_0.data_ptr(),)
            ds_blocks_raws = (storage.ds_blocks_0.data_ptr(),)
            ds_image_raws = (storage.ds_image_0.data_ptr(),)
            ds_images = (
                storage.ds_image_0.get_tensor(
                    dq_b_layout_staged.outer,
                    swizzle=dq_b_layout_staged.inner,
        ...
        p_block_stage = p_blocks[0][0][None, None, None, 0]
        assert (
        ...
        if cutlass.const_expr(self.PDS_STAGES == 2):
            p_xchg_flats = (
                storage.p_xchg_0.get_tensor(
                    flat_pds_block_layout
                ),
                storage.p_xchg_1.get_tensor(
                    flat_pds_block_layout
                ),
            )
        else:
        ...
        # v2_1: one TMA smem partition per ring slot (any slot can host a
        # QT or dOT panel under the mod-ROUND_STAGES walk).
        t_qt_smem_0, t_qt_gmem = cpasync.tma_partition(
            tma_atom_qt,
            block_coord_vmnk[2],
            a_cta_layout,
        ...
        t_qt_smem_1, _ = cpasync.tma_partition(
            tma_atom_qt,
            block_coord_vmnk[2],
            a_cta_layout,
        ...
        t_dot_smem_0, t_dot_gmem = cpasync.tma_partition(
            tma_atom_dot,
            block_coord_vmnk[2],
            a_cta_layout,
            cute.group_modes(round_quad[0], 0, 3),
        ...
        t_dot_smem_1, _ = cpasync.tma_partition(
            tma_atom_dot,
            block_coord_vmnk[2],
            a_cta_layout,
            cute.group_modes(round_quad[1], 0, 3),
        ...
        if cutlass.const_expr(self.ROUND_STAGES == 5):
            t_qt_smem_2, _ = cpasync.tma_partition(
                tma_atom_qt,
                block_coord_vmnk[2],
                a_cta_layout,
                cute.group_modes(round_quad[2], 0, 3),
                cute.group_modes(rank_g_qt, 0, 3),
            )
            t_qt_smem_3, _ = cpasync.tma_partition(
                tma_atom_qt,
                block_coord_vmnk[2],
                a_cta_layout,
                cute.group_modes(round_quad[3], 0, 3),
                cute.group_modes(rank_g_qt, 0, 3),
            )
            t_qt_smem_4, _ = cpasync.tma_partition(
                tma_atom_qt,
                block_coord_vmnk[2],
                a_cta_layout,
                cute.group_modes(round_quad[4], 0, 3),
                cute.group_modes(rank_g_qt, 0, 3),
            )
            t_dot_smem_2, _ = cpasync.tma_partition(
                tma_atom_dot,
                block_coord_vmnk[2],
                a_cta_layout,
                cute.group_modes(round_quad[2], 0, 3),
            ...
            t_dot_smem_3, _ = cpasync.tma_partition(
                tma_atom_dot,
                block_coord_vmnk[2],
                a_cta_layout,
                cute.group_modes(round_quad[3], 0, 3),
            ...
            t_dot_smem_4, _ = cpasync.tma_partition(
                tma_atom_dot,
                block_coord_vmnk[2],
                a_cta_layout,
                cute.group_modes(round_quad[4], 0, 3),
            ...
            t_qt_smems = (t_qt_smem_0, t_qt_smem_1)
            t_dot_smems = (t_dot_smem_0, t_dot_smem_1)
        ...
        # v2_1: publish-side fragments per tile parity, ring fragments per
        # slot -- all compile-time bound.
        if cutlass.const_expr(self.PDS_STAGES == 2):
            dq_ds_fragments = (
                dq_tiled_mma.make_fragment_B(ds_images[0]),
                dq_tiled_mma.make_fragment_B(ds_images[1]),
            )
            p_fragments = (
                (
                    dkv_tiled_mma.make_fragment_B(p_blocks[0][0]),
                    dkv_tiled_mma.make_fragment_B(p_blocks[0][1]),
                ),
                (
                    dkv_tiled_mma.make_fragment_B(p_blocks[1][0]),
                    dkv_tiled_mma.make_fragment_B(p_blocks[1][1]),
                ),
            )
            ds_fragments = (
                (
                    dkv_tiled_mma.make_fragment_B(ds_blocks[0][0]),
                    dkv_tiled_mma.make_fragment_B(ds_blocks[0][1]),
                ),
                (
                    dkv_tiled_mma.make_fragment_B(ds_blocks[1][0]),
                    dkv_tiled_mma.make_fragment_B(ds_blocks[1][1]),
                ),
            )
        else:
            dq_ds_fragments = (
        ...
        if cutlass.const_expr(self.ROUND_STAGES == 5):
            quad_fragments = (
                dkv_tiled_mma.make_fragment_A(round_quad[0]),
                dkv_tiled_mma.make_fragment_A(round_quad[1]),
                dkv_tiled_mma.make_fragment_A(round_quad[2]),
                dkv_tiled_mma.make_fragment_A(round_quad[3]),
                dkv_tiled_mma.make_fragment_A(round_quad[4]),
            )
        else:
            quad_fragments = (
                dkv_tiled_mma.make_fragment_A(round_quad[0]),
        ...
        if tidx == Int32(0):
            ...
            # v2_1: landing/relay/pds_ready are tile-parity split in BOTH
            # tiers (index 2*parity + {0: P, 1: dS}); only the buffer
            # sets collapse in the compat tier.
            for mbar_idx in cutlass.range_constexpr(4):
                cute.arch.mbarrier_init(
                    landing_mbars + mbar_idx,
                    1,
                )
                cute.arch.mbarrier_init(
                    relay_mbars + mbar_idx,
                    2,
                )
            for mbar_idx in cutlass.range_constexpr(2):
                cute.arch.mbarrier_init(
                    pds_ready_mbars + mbar_idx,
                    self.MATH_THREADS,
                )
            # One TMA mbar per ring slot.
            for mbar_idx in cutlass.range_constexpr(
                self.ROUND_STAGES
            ):
                cute.arch.mbarrier_init(
                    round_tma_mbars + mbar_idx,
                    1,
                )
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            if tile_count > Int32(0):
                ...
                # Epilogue: the last tile's kdq (no next score gather).
                self._gather_kdq_v8(
                    mKV,
                    mTopkIdxs,
                    gather_kd_rows_0,
            ...
            owns_n = n_owner == rank
            # v2_1: one publish-view set per tile parity (pub_views =
            # (p_local, ds_local, p_xchg, ds_image) store tiles); compat
            # aliases both parities to set 0.  The v12 alignment lessons
            # (16-byte assumed alignment through make_ptr/toint) live in
            # _math_pub_views_v21; ds_xchg is retired in both sets.
            pub_views_0 = self._math_pub_views_v21(
                thread_copy_r2s,
                n_owner,
                p_blocks[0][0],
                ds_blocks[0][0],
            ...
            if cutlass.const_expr(self.PDS_STAGES == 2):
                pub_views_1 = self._math_pub_views_v21(
                    thread_copy_r2s,
                    n_owner,
                    p_blocks[1][0],
                    ds_blocks[1][0],
            ...
            # v2_1: 2-tile macro expansion.  Even bodies publish into
            # view set 0, odd bodies into set 1 (compat aliases both to
            # set 0); the pds_ready mbar index is the TRUE tile parity in
            # both tiers.  All per-tile machinery lives in
            # _math_tile_v21 (verbatim v12 body, ds_xchg retired).
            pair_count = tile_count // Int32(2)
            for pair_iter in cutlass.range(pair_count):
                t_even = pair_iter * Int32(2)
                s_state, dp_state, pds_state = self._math_tile_v21(
                    t_even,
                    scale_softmax,
                    rank,
                    pipe_s_done,
            ...
            # Odd-count leftover: one final even-parity body.
            if (tile_count % Int32(2)) == Int32(1):
                s_state, dp_state, pds_state = self._math_tile_v21(
                    tile_count - Int32(1),
                    scale_softmax,
                    rank,
            ...
            if is_leader_cta:
                ...
                # v2_1: parity-static ring/publish algebra (host ints;
                # ring slot for panel gen g is g % ROUND_STAGES, publish
                # view set for parity p is p % PDS_STAGES).
                ring = self.ROUND_STAGES
                pv_even = 0
                pv_odd = 1 % self.PDS_STAGES
                relay_phase_0 = Int32(0)
                relay_phase_1 = Int32(0)
                # v2_1: 2-tile macro expansion of the v7 rotated schedule.
                # S(t)/dP(t) issue BEFORE grads(t-1) as in v12; grads(t)
                # consumes parity t%2 (the buffers math(t) published), so
                # the EVEN body drains parity-1 grads and the ODD body
                # drains parity-0 grads.
                pair_count = tile_count // Int32(2)
                for pair_iter in cutlass.range(pair_count):
                    t_even = pair_iter * Int32(2)
                    t_odd = t_even + Int32(1)

                    # ---- even body: S/dP(t_even), grads(t_even-1).
                    ...
                    if pair_iter > Int32(0):
                        dq_acc = t_even != Int32(1)
                        (
                            round_cons,
                            dkv_prod,
                            pds_cons,
                            dq_done_prod,
                        ) = self._leader_grads_v21(
                            dq_tiled_mma,
                            dkv_tiled_mma,
                            t_dq[0],
                            t_dq[1],
                            ...
                            dq_kd_fragment_a,
                            dq_kd_fragment_b,
                            dq_ds_fragments[pv_odd],
                            quad_fragments[2 % ring],
                            quad_fragments[3 % ring],
                            quad_fragments[4 % ring],
                            quad_fragments[5 % ring],
                            quad_fragments[6 % ring],
                            quad_fragments[7 % ring],
                            quad_fragments[8 % ring],
                            quad_fragments[9 % ring],
                            p_fragments[pv_odd][0],
                            p_fragments[pv_odd][1],
                            ds_fragments[pv_odd][0],
                            ds_fragments[pv_odd][1],
                            dq_acc,
                            relay_phase_1,
                            relay_mbars + 2,
                            pipe_round,
                        ...
                        relay_phase_1 = Int32(1) - relay_phase_1

                    # ---- odd body: S/dP(t_odd), grads(t_even).
                    pipe_kscore.consumer_wait(kscore_cons)
                    s_prod = self._issue_score_v2(
                        score_tiled_mma,
                        t_score,
                    ...
                    (
                        ...
                        quad_fragments[8 % ring],
                        quad_fragments[9 % ring],
                        p_fragments[pv_even][0],
                        p_fragments[pv_even][1],
                        ds_fragments[pv_even][0],
                        ds_fragments[pv_even][1],
                        dq_acc,
                        relay_phase_0,
                        relay_mbars,
                        pipe_round,
                    ...
                    relay_phase_0 = Int32(1) - relay_phase_0

                # ---- odd-count leftover: one final even-parity body.
                if (tile_count % Int32(2)) == Int32(1):
                    t_last = tile_count - Int32(1)
                    pipe_kscore.consumer_wait(kscore_cons)
                    s_prod = self._issue_score_v2(
                        score_tiled_mma,
                        t_score,
                    ...
                    # tile_count == 1: the dO gate has not been waited in
                    # the (empty) pair loop yet.
                    if t_last == Int32(0):
                        _mbarrier_wait_acquire_cluster(
                            stationary_ready_mbar + 1,
                            Int32(0),
                ...
                # Drain the final tile's gradients.  Its parity is
                # tile_count-1 mod 2 -- a runtime value -- so the TAIL is
                # a dynamic two-arm branch whose arms are each statically
                # bound to one parity's buffers (both arms advance every
                # pipeline state identically, so the join is uniform).
                if tile_count > Int32(0):
                    ...
                    if (tile_count % Int32(2)) == Int32(1):
                        # Last tile index is even -> parity 0.
                        (
                            round_cons,
                            dkv_prod,
                            pds_cons,
                        ...
                        # Last tile index is odd -> parity 1.
                        (
                            round_cons,
                            dkv_prod,
                            pds_cons,
                    ...
                    # v12 (S1): the dq_done commit moved into the head
            ...
            # v3: the eight panel TMA fills are software-pipelined over
            # per-slot rotating barriers (issue gen q, then wait/commit gen
            # q-1) and the two K_dQ rounds share one fused gather pass with
            # a single index read per row, so at most one fill latency per
            # tile stays exposed instead of ten (the measured v2 supply-
            # chain wall).  v2_1: ROUND_STAGES slots/mbars; since 10 mod
            # ROUND_STAGES == 0, every tile starts its walk at slot 0.
            _iket.mark("ROLE_KV_LOAD", rank)
            ...
            # v2_1: one TMA-mbar phase scalar per ring slot (slots 2..4
            # are dead constants on the compat tier -- never touched by
            # its slot walk -- and cost nothing).
            tma_phase_0 = Int32(0)
            tma_phase_1 = Int32(0)
            tma_phase_2 = Int32(0)
            tma_phase_3 = Int32(0)
            ...
            if tile_count > Int32(0):
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    # g2..g9: panel TMA fills, v12's deferred-commit
                    # lattice generalized to the mod-ROUND_STAGES slot
                    # walk (panel gen g = flat_gen + 2 -> slot g % R:
                    # ring-5 walks 2,3,4,0,1,2,3,4; ring-2 walks
                    # 0,1,0,1,... == the v12 h_half mapping).  The commit
                    # for gen q-1 happens after gen q's TMA is in flight;
                    # a slot's mbar was last waited at least one gen
                    # before its re-arm, so it is never re-armed while
                    # pending.  Slot, mbar offset and phase scalar are
                    # ALL compile-time static per unrolled iteration.
                    ...
                    for flat_gen in cutlass.range_constexpr(8):
                        ...
                        h_half = flat_gen % 2
                        slot = (flat_gen + 2) % self.ROUND_STAGES
                        ...
                        # Ownership keys on h_half (content); the
                        # DESTINATION keys on the ring slot.
                        ...
                        if cutlass.const_expr(flat_gen > 0):
                            # Deferred commit of gen q-1: static slot
                            # dispatch onto that slot's phase scalar.
                            prev_slot = (
                                flat_gen + 1
                            ) % self.ROUND_STAGES
                            if cutlass.const_expr(prev_slot == 0):
                                cute.arch.mbarrier_wait(
                                    round_tma_mbars,
                                    tma_phase_0,
                                )
                                tma_phase_0 = Int32(1) - tma_phase_0
                            if cutlass.const_expr(prev_slot == 1):
                                cute.arch.mbarrier_wait(
                                    round_tma_mbars + 1,
                                    tma_phase_1,
                                )
                                tma_phase_1 = Int32(1) - tma_phase_1
                            if cutlass.const_expr(prev_slot == 2):
                                cute.arch.mbarrier_wait(
                                    round_tma_mbars + 2,
                                    tma_phase_2,
                                )
                                tma_phase_2 = Int32(1) - tma_phase_2
                            if cutlass.const_expr(prev_slot == 3):
                                cute.arch.mbarrier_wait(
                                    round_tma_mbars + 3,
                                    tma_phase_3,
                                )
                                tma_phase_3 = Int32(1) - tma_phase_3
                            if cutlass.const_expr(prev_slot == 4):
                                cute.arch.mbarrier_wait(
                                    round_tma_mbars + 4,
                                    tma_phase_4,
                                )
                                tma_phase_4 = Int32(1) - tma_phase_4
                            with cute.arch.elect_one():
                                pipe_round.producer_commit(round_com)
                            round_com.advance()

                    # Drain the last in-flight fill (gen 9 = flat 7,
                    # slot (7 + 2) % ROUND_STAGES).
                    if cutlass.const_expr(
                        9 % self.ROUND_STAGES == 1
                    ):
                        cute.arch.mbarrier_wait(
                            round_tma_mbars + 1,
                            tma_phase_1,
            ...
            # wait the math handoff (release/acquire pair via the count-128
            # pds_ready mbar), issue BOTH DSM sends, commit pds (this lane
            # is the pipeline's single-thread producer group), then convert
            # the DSM landings into leader-visible arrives as before.
            # Deadlock shape: my sends depend only on my math; my landing
            # waits depend on the peer's sends; the graph is the same
            # cross-pattern as v11's relay, plus one strictly-local edge.
            # v2_1: 2-tile macro expansion -- buffer set, landing target,
            # and every mbar are statically selected by tile parity (the
            # per-tile machinery lives in _relay_tile_v21).
            if tidx % Int32(32) == Int32(0):
                pv_even = 0
                pv_odd = 1 % self.PDS_STAGES
                landing_phase_0 = Int32(0)
                landing_phase_1 = Int32(0)
                ready_phase_0 = Int32(0)
                ready_phase_1 = Int32(0)
                ...
                for pair_iter in cutlass.range(pair_count):
                    ...
                    (
                        ...
                        landing_mbars,
                        relay_mbars,
                        p_xchg_flats[pv_even].iterator,
                        ds_image_raws[pv_even],
                        p_blocks_raws[pv_even],
                        ds_blocks_raws[pv_even],
                        pipe_pds,
                        pds_com,
                        ready_phase_0,
                        landing_phase_0,
                    ...
                    (
                        ...
                        landing_mbars + 2,
                        relay_mbars + 2,
                        p_xchg_flats[pv_odd].iterator,
                        ds_image_raws[pv_odd],
                        p_blocks_raws[pv_odd],
                        ds_blocks_raws[pv_odd],
                        pipe_pds,
                        pds_com,
                        ready_phase_1,
                        landing_phase_1,
    ...
    def _issue_prev_grads_head_v2(
        ...
        quad_frag_g2: cute.Tensor,
        quad_frag_g3: cute.Tensor,
        quad_frag_g4: cute.Tensor,
        quad_frag_g5: cute.Tensor,
        ...
        v2_1: the four dV/dK passes consume panel gens 2..5, whose ring
        slots are gen % ROUND_STAGES -- the caller passes the four slot
        fragments explicitly (quad_frag_g2..g5), so every buffer binding
        stays compile-time static under any legal ring depth.  The
        relay_mbars argument is the caller's parity pair base (P at +0,
        dS at +1).

        v12 (S1): when ``commit_dq`` is set (the TAIL block only), the
        dq_done generation commits IMMEDIATELY after the two dQ issues --
        tcgen05.commit tracks their completion, and the math epilogue
        reads only the dQ TMEM columns [128,384), disjoint from the dKV
        columns [384,512) the remaining passes write -- so the epilogue
        starts a full grads block earlier in the per-token tail.
        """
        ...
        # v12 (S1): early dq_done commit for the TAIL block -- see the
        # docstring.  The commit is issue-ordered after the dQ gemms above
        # and tracks their completion.
        if cutlass.const_expr(commit_dq):
            dq_done_pipeline.producer_commit(dq_done_state)
            dq_done_state.advance()

        # v12 (P2ii): the relay waits split -- the P landing gates pass 1
        # (the first p_fragment consumer); the dS landing wait moves down
        # to pass 3 (the first ds_fragment consumer), shaving the dS DSM
        # latency off the block head.
        ...
        round_pipeline.consumer_release(round_consumer_state)
        round_consumer_state.advance()
        # v12 (P2ii): dS landing gate, deferred from the block head to the
        # first ds_fragment consumer.
        _mbarrier_wait_acquire_cluster(
            relay_mbars + 1,
            relay_phase,
        )
    ...
    def _issue_prev_grads_tail_v2(
        ...
        dkv_tiled_mma: cute.TiledMma,
        t_dkv_1: cute.Tensor,
        quad_frag_g6: cute.Tensor,
        quad_frag_g7: cute.Tensor,
        quad_frag_g8: cute.Tensor,
        quad_frag_g9: cute.Tensor,
        p_fragment_0: cute.Tensor,
        p_fragment_1: cute.Tensor,
        ds_fragment_0: cute.Tensor,
        ds_fragment_1: cute.Tensor,
        ...
        v2_1: consumes panel gens 6..9; the caller passes the four ring
        slot fragments (gen % ROUND_STAGES) explicitly, as in the head.
    ...
    # ------------------------------------------------------------------
    # v2_1 helpers: parity/slot-static bodies for the 2-tile macro
    # expansion (leader / math / W18) and the per-parity view builders.
    # ------------------------------------------------------------------

    @cute.jit
    def _pds_block_pair_v21(
        self,
        blocks_raw: cute.Pointer,
        dkv_b_layout_staged: cute.ComposedLayout,
    ):
        """Both 4 KiB partition-B block views of one publish set."""
        ...
        return (
            ...
            cute.make_tensor(
                cute.recast_ptr(
                    blocks_raw + self.PDS_BLOCK_ELEMENTS,
                    dkv_b_layout_staged.inner,
                    dtype=self.element_dtype,
                ),
                dkv_b_layout_staged.outer,
    ...
    @cute.jit
    def _math_pub_views_v21(
        self,
        thread_copy_r2s,
        n_owner: Int32,
        p_block_0: cute.Tensor,
        ds_block_0: cute.Tensor,
        p_xchg_flat: cute.Tensor,
        ds_image_store: cute.Tensor,
        score_store_layout,
        score_store_domain,
    ):
        """Publish store tiles for ONE P/dS set (v12 machinery verbatim).

        Returns (p_local, ds_local, p_xchg, ds_image) R2S store tiles.
        ds_xchg is retired in both tiers (v12 P1b: the dS DSM send
        sources the byte-identical whole-image half directly).
        """
        ...
        aligned_p_blocks_ptr = cute.make_ptr(
            self.element_dtype,
            p_block_0.iterator.toint(),
            p_block_0.memspace,
            assumed_align=16,
        )
        aligned_ds_blocks_ptr = cute.make_ptr(
            self.element_dtype,
            ds_block_0.iterator.toint(),
            ds_block_0.memspace,
            assumed_align=16,
        )
        p_local_store = cute.make_tensor(
        ...
        aligned_p_xchg_ptr = cute.make_ptr(
            self.element_dtype,
            p_xchg_flat.iterator.toint()
            - n_owner * Int32(self.PDS_BLOCK_BYTES),
            p_xchg_flat.memspace,
            assumed_align=16,
        )
        p_xchg_store = cute.make_tensor(
            cute.recast_ptr(
    ...
    def _math_tile_v21(
        ...
        dp_state: pipeline.PipelineState,
        pipe_pds,
        pds_state: pipeline.PipelineState,
        score_copy,
        score_source,
        score_copy_pp,
        score_source_pp,
        ...
        owns_n,
        pub_views,
        pds_ready_mbar: cute.Pointer,
        r_score: cute.Tensor,
        r_dp: cute.Tensor,
        r_p: cute.Tensor,
        r_ds: cute.Tensor,
    ):
        """One math tile body (verbatim v12 loop body; v2_1 pulls the
        parity-selected publish views and pds_ready mbar in as
        arguments so the 2-tile macro expansion stays static)."""

        t_rs_p_local_tile = pub_views[0]
        t_rs_ds_local_tile = pub_views[1]
        t_rs_p_xchg_tile = pub_views[2]
        t_rs_ds_tile = pub_views[3]
        ...
        # v8 sub-spans decompose MATH_PD so the spill-fix
        # differential is directly readable from the span tables:
        # SOFTMAX (register math), PDS_ACQ (upstream wait, only
        # meaningful for i>=1), STORE (the SASS-confirmed spill
        # site), BAR1 (straggler detector).
        ...
        if owns_n:
            ...
            # v12 (P1b): the dS xchg copy is retired -- the DSM
            # send now sources the byte-identical (1-rank) half of
            # the whole dS image, which every warp already writes.
        ...
        return s_state, dp_state, pds_state

    @cute.jit
    def _leader_grads_v21(
        self,
        dq_tiled_mma: cute.TiledMma,
        dkv_tiled_mma: cute.TiledMma,
        t_dq_0: cute.Tensor,
        ...
        """One full gradient block (head + tail + pds release).

        Every fragment argument is already parity/slot-selected by the
        caller (the 2-tile macro expansion), so this body is byte-for-
        byte the v12 grads sequence; relay_mbars is the caller's parity
        pair base (P at +0, dS at +1).
        """
    ...
    def _relay_tile_v21(
        ...
        relay_mbars: cute.Pointer,
        p_xchg_src: cute.Pointer,
        ds_image_raw: cute.Pointer,
        p_blocks_raw: cute.Pointer,
        ds_blocks_raw: cute.Pointer,
        pipe_pds,
        pds_com: pipeline.PipelineState,
        ready_phase: Int32,
        landing_phase: Int32,
    ):
        """One relay/publish tile body (verbatim v12 W18 sequence with
        every mbar/buffer pointer parity-selected by the caller).

        Landing targets inside the peer: block ``rank`` of ITS parity
        p_blocks/ds_blocks set (block 1 lives at +PDS_BLOCK_ELEMENTS).
        """
        ...
        # v12 (P1b): the send sources the byte-identical (1-rank)
        # half of the whole dS image; ds_xchg is retired.
        if rank == Int32(0):
            _cpasync_bulk_s2cluster(
        ...
        return pds_com, ready_phase, landing_phase

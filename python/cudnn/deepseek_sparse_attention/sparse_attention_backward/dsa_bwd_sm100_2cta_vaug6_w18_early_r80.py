...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    P/dS across the cluster and issues dQ; warp 19 commits ring TMA
    completions.

    Steady-state schedule per KV tile (rotated): W16 issues S(t), dP(t),
    then tile t-1's eight dV/dK passes, while W18 overlaps dQ(t) with its
    in-flight P/dS DSM relay. The kdq rendezvous hands the gather-filled dQ-A
    ...
    def kernel(
        ...
        # Budget-neutral follow-up to the W18-early overlap experiment:
        # reducers donate 8 registers each (120 -> 112), funding 16 more
        # registers for each control warp (64 -> 80). The weighted CTA total
        # remains 61,440 registers, so this isolates W18/control-warp register
        # pressure without changing occupancy or the overlap schedule.
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            if is_leader_cta:
                ...
                # W18 owns g0/g1 (the two dQ rounds). W16 starts each tile at
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    if has_prev:
                        ...
                        # skip W18's g0/g1 ownership and resume at g2.
                        round_cons.advance()
                        round_cons.advance()
            ...
            for loop_iter in cutlass.range(tile_count):
                ...
                # Both DSM copies are in flight and local P/dS FULL is
                # published. Re-form the whole warp, then issue dQ while the
                # copies land instead of serializing dQ behind their waits.
                cute.arch.sync_warp()
                if is_leader_cta:
                ...
                # The issuing warp is uniform again when the dQ helper
                # returns, so lane 0 can publish the two DSM landings without
                # paying for another warp barrier.
                if tidx % Int32(32) == Int32(0):
                    _mbarrier_wait_acquire_cluster(
                        landing_mbars,
                        landing_phase,
            ...
            if tile_count > Int32(0):
                # Publish dQ completion before either relay producer tail can
                # wait on W16, preserving the early dQ epilogue handoff.
                if is_leader_cta:
    ...
    def _issue_prev_dkv_head_v3(
        ...
        W18 owns dQ and g0/g1. W16 enters here at g2; the separate

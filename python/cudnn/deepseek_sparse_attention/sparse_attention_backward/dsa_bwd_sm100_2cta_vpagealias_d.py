...
class FlashAttentionDSABackwardSm100TwoCTA(FlashAttentionDSABackwardSm100):
    ...
    @cute.jit
    def _issue_score_chunk_vd(
        self,
        tiled_mma: cute.TiledMma,
        accumulator: cute.Tensor,
        a_fragment: cute.Tensor,
        b_fragment: cute.Tensor,
        a_chunk: cutlass.Constexpr[int],
        b_chunk: cutlass.Constexpr[int],
        accumulate: cutlass.Constexpr[bool],
    ) -> None:
        """Issue one D128 score chunk with an explicit A-stage mapping."""

        mma = tiled_mma.with_()
        mma.set(tcgen05.Field.ACCUMULATE, accumulate)
        k_blocks = cute.size(a_fragment, mode=[2])
        for k_block in cutlass.range(0, k_blocks, unroll=4):
            cute.gemm(
                mma,
                accumulator,
                a_fragment[None, None, k_block, a_chunk],
                b_fragment[None, None, k_block, b_chunk],
                accumulator,
            )
            mma.set(tcgen05.Field.ACCUMULATE, True)

    @cute.jit
    def _fill_score_overlay_pair_vd(
        self,
        first_chunk: cutlass.Constexpr[int],
        tma_atom: cute.CopyAtom,
        t_chunk_gmem: cute.Tensor,
        t_chunk_smem: cute.Tensor,
        stationary_raw: cute.Pointer,
        round_buf_a_raw: cute.Pointer,
        overlay_tma_mbars: cute.Pointer,
        round_pipeline,
        round_acq: pipeline.PipelineState,
        rank: Int32,
        score_a_stage_bytes: cutlass.Constexpr[int],
    ) -> pipeline.PipelineState:
        """Fill two score chunks into the shared round-buffer overlay."""

        for slot in cutlass.range_constexpr(2):
            chunk = first_chunk + slot
            round_pipeline.producer_acquire(round_acq)
            round_acq.advance()
            with cute.arch.elect_one():
                cute.arch.mbarrier_arrive_and_expect_tx(
                    overlay_tma_mbars + slot,
                    score_a_stage_bytes,
                )
            if rank == Int32(slot):
                with cute.arch.elect_one():
                    _cpasync_bulk_s2cluster(
                        stationary_raw + 8192 * chunk,
                        round_buf_a_raw + 8192 * slot,
                        overlay_tma_mbars + slot,
                        score_a_stage_bytes,
                        rank,
                    )
                ...
                cute.copy(
                    tma_atom,
                    t_chunk_gmem[None, chunk],
                    t_chunk_smem[None, slot],
                    tma_bar_ptr=overlay_tma_mbars + slot,
                )
        return round_acq

    @cute.jit
    def _fill_k1_pair_vd(
        self,
        tma_atom_qt: cute.CopyAtom,
        t_qt_gmem: cute.Tensor,
        t_qt_smem_a: cute.Tensor,
        t_qt_smem_b: cute.Tensor,
        stationary_q_raw: cute.Pointer,
        round_buf_b_raw: cute.Pointer,
        round_tma_mbars: cute.Pointer,
        round_pipeline,
        round_acq: pipeline.PipelineState,
        rank: Int32,
        grad_a_stage_bytes: cutlass.Constexpr[int],
    ) -> pipeline.PipelineState:
        """Fill the two Q-r1/K1 panels after the dQ pair."""
    ...
    def __call__(
        ...
        # D: reuse the two 16 KiB round buffers as a two-stage score-A
        # overlay while the stationary Q/dO pages remain GradFull.
        overlay_a_layout_staged = sm100_utils.make_smem_layout_a(
            score_tiled_mma,
            score_tiler,
            self.element_dtype,
            2,
        )
        assert cute.cosize(overlay_a_layout_staged) == 16384
        assert overlay_a_layout_staged.inner == score_a_layout_staged.inner
        assert str(
            cute.select(overlay_a_layout_staged, mode=[0, 1, 2])
        ) == str(score_a_layout)
        tma_atom_q_chunk, tma_tensor_q_chunk = (
...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    def _specialize_shared_storage(
        ...
        class SharedStorageV2:
            ...
            overlay_tma_mbars: cute.struct.MemRange[cutlass.Int64, 2]
    ...
    def kernel(
        ...
        tma_atom_q_chunk: cute.CopyAtom,
        tma_tensor_q_chunk: cute.Tensor,
        tma_atom_do_chunk: cute.CopyAtom,
        tma_tensor_do_chunk: cute.Tensor,
        overlay_a_layout_staged: cute.ComposedLayout,
    ):
        ...
        if warp_idx == Int32(self.LOAD_WARP):
            ...
            cpasync.prefetch_descriptor(tma_atom_qt)
            cpasync.prefetch_descriptor(tma_atom_dot)
            cpasync.prefetch_descriptor(tma_atom_q_chunk)
        ...
        overlay_tma_mbars = storage.overlay_tma_mbars.data_ptr()
        ...
        score_overlay = cute.make_tensor(
            cute.recast_ptr(
                round_buf_a_raw,
                overlay_a_layout_staged.inner,
                dtype=self.element_dtype,
            ),
            overlay_a_layout_staged.outer,
        )
        ...
        g_q_chunk = cute.local_tile(
            tma_tensor_q_chunk,
            cute.select(
                (self.H_TILE_CLUSTER, self.N_TILE, self.K_CHUNK),
                mode=[0, 2],
            ),
            (None, None, (token_idx, batch_idx)),
        )
        rank_g_q_chunk = rank_score_mma.partition_A(g_q_chunk)
        g_do_chunk = cute.local_tile(
            tma_tensor_do_chunk,
            cute.select(
                (self.H_TILE_CLUSTER, self.N_TILE, self.K_CHUNK),
                mode=[0, 2],
        ...
        rank_g_do_chunk = rank_dp_mma.partition_A(g_do_chunk)
        t_qchunk_smem, t_qchunk_gmem = cpasync.tma_partition(
            tma_atom_q_chunk,
            block_coord_vmnk[2],
            a_cta_layout,
            cute.group_modes(score_overlay, 0, 3),
            cute.group_modes(rank_g_q_chunk, 0, 3),
        )
        t_qchunk_gmem = t_qchunk_gmem[None, 0, None]
        t_dochunk_smem, t_dochunk_gmem = cpasync.tma_partition(
            tma_atom_do_chunk,
            block_coord_vmnk[2],
            a_cta_layout,
            cute.group_modes(score_overlay, 0, 3),
            cute.group_modes(rank_g_do_chunk, 0, 3),
        )
        ...
        score_q_overlay_fragment = score_tiled_mma.make_fragment_A(
            score_overlay
        )
        score_do_overlay_fragment = dp_tiled_mma.make_fragment_A(
            score_overlay
        )
        ...
        if tidx == Int32(0):
            ...
            cute.arch.mbarrier_init(overlay_tma_mbars, 1)
            cute.arch.mbarrier_init(overlay_tma_mbars + 1, 1)
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            if is_leader_cta:
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    sdp_prod, round_cons = self._issue_score_overlay_vd(
                        score_tiled_mma,
                        t_score,
                        t_score_pp,
                        score_q_fragment,
                        score_q_overlay_fragment,
                        score_k_fragment,
                        pipe_s_done,
                        sdp_prod,
                        pipe_round,
            ...
            if tile_count > Int32(0):
                ...
                # Ablation A: overwrite only the off-diagonal Q-r0 page
                # before publishing stationary readiness. No restore.
                with cute.arch.elect_one():
                    cute.arch.mbarrier_arrive_and_expect_tx(
                        stationary_tma_mbars,
                        grad_a_stage_bytes,
                ...
                # V-r1 uses O2/O3; again only the peer page needs a TMA.
                with cute.arch.elect_one():
                    cute.arch.mbarrier_arrive_and_expect_tx(
                        stationary_tma_mbars + 1,
                        grad_a_stage_bytes,
                ...
                # D: score overlay first, then the preceding gradient.
                # This is the only ordering that cannot fill both round
                # stages with future score data while the leader waits grad.
                for loop_iter in cutlass.range(tile_count):
                    round_acq = self._fill_score_overlay_pair_vd(
                        0, tma_atom_q_chunk, t_qchunk_gmem,
                        t_qchunk_smem, stationary_q_raw,
                        round_buf_a_raw, overlay_tma_mbars,
                        pipe_round, round_acq, rank,
                        score_a_stage_bytes,
                    )
                    round_acq = self._fill_score_overlay_pair_vd(
                        2, tma_atom_do_chunk, t_dochunk_gmem,
                        t_dochunk_smem, stationary_do_raw,
                        round_buf_a_raw, overlay_tma_mbars,
                        pipe_round, round_acq, rank,
                        score_a_stage_bytes,
                    )
                    if loop_iter > Int32(0):
                        ...
                        round_acq.advance()
                        self.kdq_barrier_b.arrive_and_wait()
                        round_acq = self._fill_k1_pair_vd(
                            tma_atom_qt, t_qt_gmem,
                            t_qt_smem_a, t_qt_smem_b,
                            stationary_q_raw, round_buf_b_raw,
                            round_tma_mbars, pipe_round, round_acq,
                            rank, grad_a_stage_bytes,
                        )

                # Final gradient has no following score iteration.
                pipe_round.producer_acquire(round_acq)
                round_acq.advance()
                self.kdq_barrier.arrive_and_wait()
                pipe_round.producer_acquire(round_acq)
            ...
            overlay_phase_0 = Int32(0)
            overlay_phase_1 = Int32(0)
            w19_phase_0 = Int32(0)
            w19_phase_1 = Int32(0)
            kdq_phase_0 = Int32(0)
            kdq_phase_1 = Int32(0)
            for loop_iter in cutlass.range(tile_count):
                # Q overlay and dO overlay: two A/B pairs per score tile.
                for _ in cutlass.range_constexpr(2):
                    for slot in cutlass.range_constexpr(2):
                        if cutlass.const_expr(slot == 0):
                            ...
                            overlay_phase_0 = Int32(1) - overlay_phase_0
                        else:
                            cute.arch.mbarrier_wait(
                                overlay_tma_mbars + 1,
                                overlay_phase_1,
                            )
                            overlay_phase_1 = Int32(1) - overlay_phase_1
                        with cute.arch.elect_one():
                            pipe_round.producer_commit(commit_com)
                        commit_com.advance()
                if loop_iter > Int32(0):
                    cute.arch.mbarrier_wait(kdq_ready_mbar, kdq_phase_0)
                    kdq_phase_0 = Int32(1) - kdq_phase_0
                    cute.arch.fence_view_async_shared()
                    with cute.arch.elect_one():
                        pipe_round.producer_commit(commit_com)
                    commit_com.advance()
                    cute.arch.mbarrier_wait(kdq_ready_mbar_b, kdq_phase_1)
                    kdq_phase_1 = Int32(1) - kdq_phase_1
                    cute.arch.fence_view_async_shared()
                    with cute.arch.elect_one():
                        pipe_round.producer_commit(commit_com)
                    commit_com.advance()
                    cute.arch.mbarrier_wait(round_tma_mbars, w19_phase_0)
    ...
    @cute.jit
    def _issue_score_chunk_stage_vd(
        self,
        tiled_mma: cute.TiledMma,
        ...
        if stage_index == Int32(0):
            self._issue_score_chunk_vd(
                tiled_mma, accumulator_0, a_fragment, b_fragment,
                a_chunk, b_chunk, accumulate,
            )
        else:
            self._issue_score_chunk_vd(
                tiled_mma, accumulator_1, a_fragment, b_fragment,
                a_chunk, b_chunk, accumulate,
            )

    @cute.jit
    def _issue_score_overlay_vd(
        self,
        tiled_mma: cute.TiledMma,
        accumulator_0: cute.Tensor,
        accumulator_1: cute.Tensor,
        resident_fragment: cute.Tensor,
        overlay_fragment: cute.Tensor,
        b_fragment: cute.Tensor,
        done_pipeline,
        ...
        """Issue resident-safe chunks, then consume two overlay stages."""
        if cutlass.const_expr(do_acquire):
            done_pipeline.producer_acquire(producer_state)
        stage_index = producer_state.index
        if cutlass.const_expr(is_dp):
            ...
            overlay_first = 0
        for pair_index in cutlass.range_constexpr(2):
            chunk = resident_first + pair_index
            self._issue_score_chunk_stage_vd(
                tiled_mma, accumulator_0, accumulator_1,
                resident_fragment, b_fragment, stage_index,
                chunk, chunk, pair_index != 0,
            )
        for slot in cutlass.range_constexpr(2):
            round_pipeline.consumer_wait(round_state)
            self._issue_score_chunk_stage_vd(
                tiled_mma, accumulator_0, accumulator_1,
                overlay_fragment, b_fragment, stage_index,
                slot, overlay_first + slot, True,
            )
        ...
        if cutlass.const_expr(advance_state):
            producer_state.advance()
        return producer_state, round_state

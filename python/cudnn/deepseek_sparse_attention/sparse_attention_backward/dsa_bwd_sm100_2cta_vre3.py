...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    * The gradient GEMMs of tile p are issued inside tile t=p-1's S/dP
    ...
    def _specialize_shared_storage(
        ...
        # vre_3 byte-exact plan (per CTA), cap 232,448:
        #   stationary Q                   65,536
        #   score K stage (epi alias)      32,768
        #   round region (4 x 16,384)      65,536
        #   P blocks: local+xchg+landing   12,288
        #   dS image + local+xchg+landing  20,480
        #   softmax stats                     512
        #   mbarriers / holding buf        <= 640
        # (stationary dO RETIRED: dP's score-A streams through ring slots
        # 0..3; round_buf_a..d MUST stay declaration-contiguous -- the
        # 4-stage stationary layout is laid over their combined span.)
        assert cute.cosize(score_a_layout_staged) <= 32768
        assert cute.cosize(score_b_layout_staged) <= 16384
        ...
        class SharedStorageV2:
            ...
            # vre_3: four 16 KiB ring slots; a..d are declaration-
            # contiguous (each 16,384 B, Align 1024 adds no padding), so
            # the 4-stage stationary score-A layout laid at round_buf_a
            # spans exactly a..d with stage == slot.
    ...
    def kernel(
        ...
        round_buf_d_raw = storage.round_buf_d.data_ptr()
        ...
        # vre_3: slots c/d (Q quadrants land on c/d under the mod-4 map).
        ...
        t_qt_smem_d, _ = cpasync.tma_partition(
            tma_atom_qt,
            block_coord_vmnk[2],
            a_cta_layout,
        ...
        # vre_3 (mod-4 slot map): dO quadrants land on slots a/b, Q
        # quadrants on c/d; grads passes rotate a,b,c,d per round.
        quad_fragment_c = dkv_tiled_mma.make_fragment_A(
            round_quad[2]
        )
        quad_fragment_d = dkv_tiled_mma.make_fragment_A(
            round_quad[3]
        )
        p_fragments = (
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            if is_leader_cta:
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    # vre_3: dP(t) streams its score-A through ring slots
                    ...
                    if has_prev:
                        ...
                        # vre_3: dQ rounds at the tile tail (kdq gens at
                        # ring positions 12/13, slots a/b -- fragment
                        # bindings unchanged).
                        ...
                        # vre_3: pad gens 14/15 -- wait+release only.
                        ...
                        pipe_round.consumer_release(round_cons)
                        round_cons.advance()
                        # vre_3: the pds release follows the dQ issues --
                        # dQ reads the local dS_H image, and this UMMA-
                        # tracked release is the WAR gate against
                        # math(t+1)'s publish overwriting it.
                ...
                if tile_count > Int32(0):
                    ...
                    # vre_3: TAIL dQ rounds at the tile tail, then the
                    # dq_done commit (the v12 (S1) early-commit head
                    # start is retired; one-time ~2.5us per token).
                    round_cons = self._issue_dq_rounds_v2(
                        dq_tiled_mma,
                        t_dq[0],
                        t_dq[1],
                    ...
                    # vre_3: pad gens -- wait+release only.
                    pipe_round.consumer_wait(round_cons)
                    pipe_round.consumer_release(round_cons)
                    round_cons.advance()
                    pipe_round.consumer_wait(round_cons)
                    pipe_round.consumer_release(round_cons)
                    round_cons.advance()
                    # vre_3: pds release after the dQ issues (WAR gate
                    # for the dS_H image, same as the loop body).
                    pipe_pds.consumer_release(pds_cons)
                    pds_cons.advance()
            ...
            tma_phase_1 = Int32(0)
            if tile_count > Int32(0):
                # vre_3: the stationary prologue loads ONLY the Q panel;
                # dP's score-A streams through ring slots 0..3 every tile
                # (the dOscore whole-panel fill at each iteration's head).
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    for flat_gen in cutlass.range_constexpr(8):
                        ...
                        if cutlass.const_expr(tensor_kind == 0):
                            # vre_3: the dO own-half bulk source
                            # (stationary_do) is retired -- both dO
                            # halves are plain GMEM TMA (L2-hot; the
                            # panel bytes were just streamed by the
                            # dOscore fill this same tile).
                            ...
                            # vre_3 (mod-4 slot map): Q quadrants land on
                            # slots c/d; the own-half bulk survives (its
                            # source stationary_q is still resident).
                            if cutlass.const_expr(h_half == 0):
                    ...
                    # vre_3 g12/g13: both K_dQ rounds in one fused gather
                    # pass (frozen machine untouched; slots a/b, so the
                    # dq_kd fragment bindings are unchanged).  Depth 4
                    # frees these credits at the leader's qdo passes 4/5,
                    # so the fill overlaps the grads tail instead of
                    # serializing at the window head.
                    ...
                    # vre_3 g14/g15: pad gens (slots c/d) -- empty
                    # placeholders keeping 16 mod 4 == 0 so every slot
                    # mapping stays compile-time static.  No fill, no
                    # mbar: acquire then commit immediately.
                    pipe_round.producer_acquire(round_acq)
                    round_acq.advance()
                    with cute.arch.elect_one():
                        pipe_round.producer_commit(round_com)
            ...
            if tidx % Int32(32) == Int32(0):
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    # vre_3 (B-lite): the pds commit moves BEFORE the DSM
                    # sends -- its semantics are "both CTAs' math have
                    # published" (the local publishes this warp just
                    # observed); the sends' LANDINGS are separately gated
                    # by the landing mbars at the leader's grads passes.
                    # This shaves the two send-issue segments off the
                    # leader's pds edge.
    ...
    def _issue_dp_streamed_vre3(
        ...
        """vre_3: dP over four STREAMED score-A chunks (ring slots a..d).

        Chunk c's bytes live in ring slot c (the 4-stage stationary layout
        laid over round_buf_a..d; the dOscore whole-panel fill lands all
        four at once, so waits 1..3 clear instantly).  Each chunk is
        consumer-waited before its GEMM sub-chain and released right after
        (UMMA-tracked), freeing the slot for the tile's quadrant train.
        ACCUMULATE chains across the four chunks like the resident form.
        """
        ...
        done_pipeline.producer_commit(producer_state)
        producer_state.advance()
        return producer_state, round_consumer_state
    ...
    def _issue_prev_grads_head_v2(
        ...
        vre_3: the dQ rounds MOVED OUT to the caller's tile tail (the kdq
        gens sit at ring positions 12/13 under the new 16-gen order); the
        dq-related parameters stay in the signature (unused here) to
        minimise the fork's diff surface.  The four passes rotate quad
        fragments a,b,c,d (mod-4 slot map: dO on a/b, Q on c/d).  The
        TAIL's early dq_done commit moved with the dQ call.
        """

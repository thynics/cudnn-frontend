"""v_gpt_1: vk_2 + K_dQ fills via TMA tile::gather4 (K2-G4).

Retires the second sparse gather: the two K_dQ rounds ([N64 x D128] dQ-A
images, same topk rows as score's K but a different column window and the
MN_SW128 operand layout) were filled by 128 gather threads behind a
160-thread double rendezvous with W17.  Now W17 alone issues 32 TMA
tile::gather4 transactions per round ([4 rows x 64 elem] boxes; the
TensorMap carries the operand's own SW128 swizzle, whose XOR is
absolute-address based, so boxes land phase-correct in both halves of the
8-row atom).  Invalid rows (holes' -1, ragged tail) are clamped to an
always-OOB row and zero-filled by the TensorMap's OOB rule -- the same
bytes the retired zero-fill path produced; operand bytes are identical.
The kdq_barrier rendezvous and the gather warps' kdq duty are retired;
ring protocol, leader schedule and every MMA descriptor are untouched.
Design + desk-check evidence: V_GPT1_K2G4_DESIGN.md.

Inherited vk_2 notes: vg_5 with split publish retired (the validated C
form promoted).
...
def _cpasync_gather4_g2s(
    ...
    *,
    ...
    """One TMA tile::gather4: four sparse rows x one 64-element column box.

    Coordinate order per PTX ISA 9.3 (and the CUTLASS SM100 header): the
    dimension-0 (column) coordinate first, then the four gathered row
    coordinates.  The TensorMap operand must live in param/const/global
    space; callers pass the 128-byte global-memory home that the kernel
    prologue filled via copy_tensormap.
    """

    desc_i64 = tensormap_ptr.toint(loc=loc, ip=ip).ir_value()
    ...
    llvm.inline_asm(
        ...
        [
            ...
            desc_i64,
            barrier_i32,
            Int32(column).ir_value(loc=loc, ip=ip),
            Int32(row_0).ir_value(loc=loc, ip=ip),
            Int32(row_1).ir_value(loc=loc, ip=ip),
            Int32(row_2).ir_value(loc=loc, ip=ip),
            Int32(row_3).ir_value(loc=loc, ip=ip),
        ],
        (
...
class FlashAttentionDSABackwardSm100TwoCTA(FlashAttentionDSABackwardSm100):
    ...
    def __call__(
        ...
        # v_gpt_1: TMA tile::gather4 tensormap for the K_dQ fills.  The
        # gathered tensor is the plain 2D KV table (the host view above
        # rebuilt mKV with a dummy batch mode).  The box is one
        # [1 row x 64 elem] line composed under the dq-A operand's own
        # SW128 swizzle, so the hardware reproduces the MN_SW128 image
        # bytes exactly; boxDim[1] = 1 per the gather4 ISA contract.  The
        # descriptor's 128-byte device home is carved from the
        # workspace_LSE_OdO tail: the interface over-allocates that buffer
        # by contract (flat-offset carving; the tail is invisible to
        # impls that do not use it) and this family only reads the first
        # 2 * H * roundup(q, 8) * 4 bytes via get_workspace_tensor.
        mkv_gather_view = cute.make_tensor(
            mKV.iterator,
            cute.make_layout(
                (mKV.shape[0], mKV.shape[1]),
                stride=(mKV.stride[0], mKV.stride[1]),
            ),
        )
        gather4_box_layout = cute.composition(
            dq_a_layout_staged,
            cute.make_layout((1, 64), stride=(0, 1)),
        )
        tma_atom_kdq, tma_tensor_kdq = cpasync.make_tiled_tma_atom(
...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    * The gradient GEMMs of tile p are issued inside tile t=p-1's S/dP
    ...
    # v_gpt_1: row coordinate guaranteed out-of-bounds for any real S_kv,
    # so the gather4 TensorMap's OOB rule zero-fills clamped invalid rows.
    KDQ_OOB_ROW = 1 << 30
    ...
    def _specialize_shared_storage(
        ...
        class SharedStorageV2:
            ...
            kdq_tma_mbars: cute.struct.MemRange[cutlass.Int64, 2]
    ...
    def kernel(
        ...
        stationary_tiled_mma: cute.TiledMma,
        stationary_a_layout_staged: cute.ComposedLayout,
        tma_atom_kdq: cute.CopyAtom,
        gather4_desc_ptr: cute.Pointer,
    ):
        ...
        kdq_tma_mbars = storage.kdq_tma_mbars.data_ptr()
        ...
        if tidx == Int32(0):
            ...
            cute.arch.mbarrier_init(kdq_tma_mbars, 1)
            cute.arch.mbarrier_init(kdq_tma_mbars + 1, 1)
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            if tile_count > Int32(0):
                ...
                if tile_count == Int32(1):
                    ...
                    # v_gpt_1: kdq fill moved to W17 (TMA gather4).
                    pipe_kscore.producer_acquire(gather_state)
            ...
            tma_phase_0 = Int32(0)
            tma_phase_1 = Int32(0)
            kdq_tma_phase = Int32(0)
            if tile_count > Int32(0):
                # v_gpt_1: materialize the gather4 TensorMap once into its
                # 128-byte global home (the tensormap operand must live in
                # param/const/global space).  Every CTA writes identical
                # bytes, so the cross-CTA race is benign; each CTA fences
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    pipe_round.producer_acquire(round_acq)
                    round_acq.advance()
                    # v_gpt_1: both K_dQ rounds arrive by TMA
                    # tile::gather4 instead of the 160-thread rendezvous +
                    # per-row cp.async gather.  Lanes 0..15 each own four
                    # KV rows (n = 4*lane .. 4*lane+3) and issue one
                    # [4 x 64-elem] box per (round, d-half): 32
                    # transactions and 16 KiB expected-tx per round mbar.
                    # The arm precedes every issue in warp program order;
                    # completion visibility for the dQ MMA descriptor read
                    # follows the same mbar-tracked TMA rule the panel
                    # generations already rely on (no extra fence).
                    with cute.arch.elect_one():
                        cute.arch.mbarrier_arrive_and_expect_tx(
                            kdq_tma_mbars,
                            16384,
                    ...
                    if lane_idx < Int32(16):
                        base_n = tile_index * Int32(
                            self.N_TILE
                        ) + lane_idx * Int32(4)
                        # vg_4 discipline: all four index loads issue
                        # before any dependent use.  Invalid rows (holes'
                        # -1 and the ragged tail) clamp to the always-OOB
                        # row so the TensorMap zero-fills them.
                        kdq_row_0 = Int32(self.KDQ_OOB_ROW)
                        kdq_row_1 = Int32(self.KDQ_OOB_ROW)
                        kdq_row_2 = Int32(self.KDQ_OOB_ROW)
                        kdq_row_3 = Int32(self.KDQ_OOB_ROW)
                        if base_n < topk:
                            row_value_0 = mTopkIdxs[
                                base_n,
                                (token_idx, batch_idx),
                            ]
                            if row_value_0 >= Int32(0):
                                kdq_row_0 = row_value_0
                        if base_n + Int32(1) < topk:
                            row_value_1 = mTopkIdxs[
                                base_n + Int32(1),
                                (token_idx, batch_idx),
                            ]
                            if row_value_1 >= Int32(0):
                                kdq_row_1 = row_value_1
                        if base_n + Int32(2) < topk:
                            row_value_2 = mTopkIdxs[
                                base_n + Int32(2),
                                (token_idx, batch_idx),
                            ]
                            if row_value_2 >= Int32(0):
                                kdq_row_2 = row_value_2
                        if base_n + Int32(3) < topk:
                            row_value_3 = mTopkIdxs[
                                base_n + Int32(3),
                                (token_idx, batch_idx),
                            ]
                            if row_value_3 >= Int32(0):
                                kdq_row_3 = row_value_3
                        # Destination algebra (measured dq_a layout, see
                        # V_GPT1_K2G4_DESIGN.md): half-plane major --
                        # addr = Sw(2B*(d%64) + 128B*n + 8192B*(d//64)).
                        # Element units: 4096 per d-half, 256 per 4-row
                        # lane group.
                        smem_group = lane_idx * Int32(256)
                        for kdq_round in cutlass.range_constexpr(2):
                            if cutlass.const_expr(kdq_round == 0):
                                kd_dest_raw = round_buf_a_raw
                            else:
                                kd_dest_raw = round_buf_b_raw
                            for d_half in cutlass.range_constexpr(2):
                                _cpasync_gather4_g2s(
                                    gather4_desc_ptr,
                                    kd_dest_raw
                                    + Int32(4096 * d_half)
                                    + smem_group,
                                    kdq_tma_mbars + kdq_round,
                                    Int32(
                                        256 * kdq_round + 64 * d_half
                                    )
                                    + Int32(128) * rank,
                                    kdq_row_0,
                                    kdq_row_1,
                    ...
                    with cute.arch.elect_one():
                        pipe_round.producer_commit(round_com)
                    round_com.advance()
                    kdq_tma_phase = Int32(1) - kdq_tma_phase
                    _iket.range_end(
                        route_k_token,
                        loop_iter,
                    )
                    ...
                    # Drain the last in-flight fill (panel gen 5, barrier 1).
                    cute.arch.mbarrier_wait(
                        round_tma_mbars + 1,
                        tma_phase_1,
                    )

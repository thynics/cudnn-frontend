"""vc_5: vc_2 with eager A/B-pair publication in the round supplier.

The only scheduling change from vc_2 is in W17's six Q/dO round-panel
generations.  Each A/B pair is now fully launched, waited, and published
before W17 tries to acquire the next A slot.  This prevents a completed B
panel from being held behind the next A acquire and tests whether the odd
gradient gaps are software head-of-line blocking rather than fill rate.
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            if tile_count > Int32(0):
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    # gather role.
                    # vc_5: launch both halves of each logical operand, then
                    # wait and publish the A/B pair together.  In vc_2 the
                    # completed B half was not published until after the
                    # next A acquire; that acquire itself waited for the A
                    # consumer and structurally delayed every odd pass.
                    # Pair-eager publication keeps the generation order and
                    # byte traffic unchanged while removing that dependency.
                    ...
                    for flat_gen in cutlass.range_constexpr(6):
                        ...
                        if cutlass.const_expr(flat_gen % 2 == 1):
                            cute.arch.mbarrier_wait(
                                round_tma_mbars,
                                tma_phase_0,
                            )

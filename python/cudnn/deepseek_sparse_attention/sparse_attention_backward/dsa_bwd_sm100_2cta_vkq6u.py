"""DeepSeek Sparse Attention backward, two-CTA SERIAL chain, vkq6u.
...
Below: the vkq6t build docstring.

DeepSeek Sparse Attention backward, two-CTA SERIAL chain, vkq6t.
...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    dK r0/r1.  The four-slot true-K32 ring carries 20 generations/tile:
    eight dO, four gather-owned K_dQ, then eight Q generations.  P is
    relayed first on warp 18 so its exchange lands under the dP shadow;
    the same lane relays dS afterward.  score_kv is released after dP and
    immediately hosts the next tile's score K.  dQ stays TMEM-resident
    across all tiles and stores through a two-round staged TMA epilogue.
    """
    ...
    # Explicit per-tile ring ledger.  Four independent K32 slots publish
    # each old K64 panel as two H32 micro-generations.  All three owners
    # advance over the same 8 + 4 + 8 logical generations.
    ROUND_PANELS_PER_TILE = 8
    ROUND_DV_GENS = 8
    ROUND_KDQ_GENS = 4
    ROUND_DK_GENS = 8
    ROUND_KDQ_BEGIN = ROUND_DV_GENS
    ROUND_DK_BEGIN = ROUND_DV_GENS + ROUND_KDQ_GENS
    ROUND_TMA_GENS_PER_TILE = ROUND_DV_GENS + ROUND_DK_GENS
    ROUND_TOTAL_GENS_PER_TILE = (
        ROUND_DV_GENS + ROUND_KDQ_GENS + ROUND_DK_GENS
    )
    ROUND_STAGES = 4
    ...
    def _issue_dq_rounds_kq(
        ...
        """Issue both dQ rounds off ring generations u8..u11 (vkq6t).
        ...
        for round_index in cutlass.range_constexpr(self.D_ROUNDS):
            ...
            for half in cutlass.range_constexpr(2):
                ...
                # One logical dQ round is split by the four-slot K_dQ ring
                # into two non-overlapping enqueue intervals.  Reuse the
                # round payload for both halves; do not include the intervening
                # ring wait in an issue span.
    ...
    def kernel(
        ...
        # preserves that.
        # vkq6t: K_dQ leaves the score_kv loan.  The two 16KB dq panels
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            if tile_count > Int32(0):
                ...
                # rides ring gens u8..u11: all four slots are acquired
                ...
                for score_iter in cutlass.range(
                    ...
                    self.gather_barrier.arrive_and_wait()
                    for _ in cutlass.range_constexpr(self.ROUND_KDQ_GENS):
                        if warp_idx == Int32(0):
                            with cute.arch.elect_one():
                        ...
                        gather_ring_com.advance()
                    for _ in cutlass.range_constexpr(self.ROUND_DK_GENS):
                        gather_ring_com.advance()
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    for flat_gen in cutlass.range_constexpr(
                        ...
                        if cutlass.const_expr(flat_gen < 2):
                            ...
                            tensor_kind = 1  # Q_r1
                        h_half = flat_gen % 2
                        # vkq6t: W17 is the only producer-acquire agent.
            ...
            for loop_iter in cutlass.range(tile_count):
                for tma_gen in cutlass.range_constexpr(
                    self.ROUND_TMA_GENS_PER_TILE
                ):
                    # Generations u8..u11 are committed by the
                    # gather warps; advance past them between the dO
                    # and Q generations.
                    if cutlass.const_expr(
                        tma_gen == self.ROUND_KDQ_BEGIN
                    ):
                        for _ in cutlass.range_constexpr(
                            self.ROUND_KDQ_GENS
                        ):
                            commit_com.advance()
                    round_slot = tma_gen % self.ROUND_STAGES
                    cute.arch.mbarrier_wait(
                        round_tma_mbars + round_slot,

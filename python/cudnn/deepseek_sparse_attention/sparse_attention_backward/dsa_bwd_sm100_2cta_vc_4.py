"""vc_4: vc_2 with a second score-K loan for round-0 Q.

The active V2 kernel keeps vc_2's score, math, reducer, and persistent-TMEM
design.  During each gradient head it reinterprets the dead 32-KiB score-K
stage first as two 16-KiB dO operands for round-0 dV and then as two 16-KiB
Q operands for round-0 dK.  The A/B round ring therefore carries only
kdq/dO(round1)/Q(round1), while the one-stage kscore pipeline carries
K -> borrowed dO -> borrowed Q -> K.  This is a single-variable temporal
alias experiment: no SMEM allocation, math, reducer, or P/dS change.
...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    * vc_4 keeps only K_dQ, round-1 dO, and round-1 Q in the rank-symmetric
      A/B ring.  Round-0 dO and Q successively occupy the released score-K
      allocation.
    ...
    * The gradient GEMMs of tile p are issued inside tile t=p-1's S/dP
    ...
    # vc_4: round A/B carries neither dO nor Q round 0; both pairs
    # successively borrow the score-K allocation.  The remaining sequence
    # is phase closed at depth 2:
    # g0/g1 kdq, g2/g3 dO_r1, g4/g5 Q_r1.
    ...
    def _specialize_shared_storage(
        ...
        # vc_4 borrows the score-B allocation as two complete DKV-A
        # operands, first for dO-r0 and then for Q-r0.  Keep the raw
        # half-buffer offset below byte-exact even if either layout is
        # changed by a later specialization.
        assert (
            cute.cosize(score_b_layout_staged)
        ...
        class SharedStorageV2:
            ...
            # vc_4: completion barriers reused by the two 16-KiB dO-r0
            # panels and the following two 16-KiB Q-r0 panels staged over
            # score_kv.
    ...
    def _fill_score_loan_q_r0_vc4(
        ...
        tma_atom_qt: cute.CopyAtom,
        t_qt_gmem: cute.Tensor,
        t_qt_loan_smem_a: cute.Tensor,
        t_qt_loan_smem_b: cute.Tensor,
        grad_a_stage_bytes: cutlass.Constexpr[int],
    ) -> Int32:
        """Overwrite the released dO loan with the round-0 Q pair.

        This mirrors the proven vc_2 dO fill: the rank-owned panel uses one
        local bulk copy from stationary Q and the peer panel uses its L2-hot
        GMEM TMA box.  The collective kscore commit occurs only after both
        raw transfers complete and every gather thread fences the image.
        """
    ...
    def kernel(
        ...
        # vc_4: byte-exact DKV-A views over the two 16-KiB halves of the
        # 32-KiB score-K stage.  These views are live only after S/dP has
        # released K and successively hold the dO-r0 and Q-r0 operands
        # before the next K gather acquires the stage.
        loan_quad = (
            cute.make_tensor(
                cute.recast_ptr(
                    score_kv_raw,
        ...
        t_qt_loan_smem_a, _ = cpasync.tma_partition(
            tma_atom_qt,
            block_coord_vmnk[2],
            a_cta_layout,
            cute.group_modes(loan_quad[0], 0, 3),
        ...
        t_qt_loan_smem_b, _ = cpasync.tma_partition(
            tma_atom_qt,
            block_coord_vmnk[2],
            a_cta_layout,
            cute.group_modes(loan_quad[1], 0, 3),
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            if tile_count > Int32(0):
                ...
                if tile_count == Int32(1):
                    ...
                    # dO-r0 and Q-r0 in consumer order once score releases
                    # K(0).
                    self._gather_kdq_v8(
                        mKV,
                        mTopkIdxs,
                    ...
                    pipe_kscore.producer_acquire(gather_state)
                    do_loan_phase = self._fill_score_loan_do_r0_vc2(
                        rank,
                        warp_idx,
                        stationary_do_raw,
                        score_kv_raw,
                    ...
                    gather_state.advance()
                    pipe_kscore.producer_acquire(gather_state)
                    self._fill_score_loan_q_r0_vc4(
                        rank,
                        warp_idx,
                        stationary_q_raw,
                        score_kv_raw,
                    ...
                    # borrowed dO and Q generations.  The following
                    # generation is K(i+1), except in the final iteration
                    # where the tail tile receives its own dO/Q pair.
                    for score_iter in cutlass.range(
                        ...
                        # The same score stage changes ownership only after
                        # both dV-r0 readers have released the dO generation.
                        # Publish Q-r0 for the following dK-r0 pair.
                        pipe_kscore.producer_acquire(gather_state)
                        self._fill_score_loan_q_r0_vc4(
                            rank,
                            warp_idx,
                        ...
                        if score_iter == tile_count - Int32(1):
                            # After grads(i-1) releases Q-r0, overwrite the
                            # stage with tail dO-r0 before entering the
                            # potentially blocking last-kdq rendezvous.  The
                            # tail Q-r0 fill must stay AFTER that rendezvous:
                            # waiting for tail dV before kdq would form the
                            # hard cycle Q-acquire -> tail dV -> dQ -> kdq.
                            pipe_kscore.producer_acquire(gather_state)
                            tail_do_loan_phase = (
                                self._fill_score_loan_do_r0_vc2(
                                    rank,
                                    warp_idx,
                                    stationary_do_raw,
                                    score_kv_raw,
                ...
                # producer_tail observes the final borrowed Q generation's
            ...
            if is_leader_cta:
                ...
                if tile_count > Int32(0):
                    ...
                    pipe_pds.consumer_release(pds_cons)
                    pds_cons.advance()
                    # vc_4 keeps the early dQ commit; loan_epi_safe_mbar
            ...
            # --- load: stationary TMA once, then 6 A/B gens per tile.
            # v3: the panel TMA fills are software-pipelined over two
            ...
            if tile_count > Int32(0):
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    # g2..g5: dO_r1 and Q_r1 panel fills, pipelined depth 2.
                    # Both dO_r0 and Q_r0 are supplied through score_kv by
                    # the gather role.
                    ...
                    # Drain the last in-flight fill (panel gen 3, barrier 1).
                    cute.arch.mbarrier_wait(
                        round_tma_mbars + 1,
                        tma_phase_1,
                    )
    ...
    def _issue_prev_grads_head_v2(
        ...
        """Gradient block, first half: dQ plus double-loan dV/dK-r0.
        ...
        tail, protects score_kv from the dQ epilogue until the final
        borrowed dK/Q source reads have truly completed.
        """
        ...
        # vc_4: K(i) was released after dP; the gather role first republishes
        # this 32-KiB stage as the two round-0 dO panels.
        ...
        # first ds_fragment consumer.  vc_4 then waits for the second loan
        # generation, where Q-r0 has overwritten the released dO-r0 image.
        _mbarrier_wait_acquire_cluster(
            relay_mbars + 1,
            relay_phase,
        )
...
# File-based harness registration: V2 is vc_4's active implementation.

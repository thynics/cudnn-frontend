"""DeepSeek Sparse Attention backward, two-CTA SERIAL chain, kq5.

final_ser_kq5 = kq2 + the two adjudicated winners of the kq4 round:
packed math (kq4c, -1.58%) and deferred releases (kq4b, -0.33%).
kq4a (fused T2R, +3.68%) and kq4d (early xchg send, +1.12%) are
adjudicated dead and excluded.  Expected ~9.55-9.65ms; this is the
serial form's practical floor -- every relay-0 segment now has a
silicon verdict (reorder x, close mechanics x, skew x, math harvested,
release harvested, transport = floor).
    ...
    def kernel(
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            for loop_iter in cutlass.range(tile_count):
                ...
                cute.arch.fence_view_async_tmem_load()
                # kq5 (from kq4b): S release deferred past the P math.
                softmax_scale_log2_e = scale_softmax * Float32(
                    math.log2(math.e)
                )
                ...
                cute.arch.fence_view_async_tmem_load()
                # kq5 (from kq4b): dP release deferred past the dS math.
                if cutlass.const_expr(self.SOFTMAX_GROUPED_STATS):
                    for h_group in cutlass.range_constexpr(4):
                        delta = softmax_stats[

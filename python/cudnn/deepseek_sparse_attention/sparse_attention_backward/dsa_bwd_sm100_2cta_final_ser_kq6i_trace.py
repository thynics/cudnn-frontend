"""IKET-only derivative of final_ser_kq6i.

final_ser_kq6i = final_ser_kq6f(r1) + ring half-granularity (C.1), single variable.
...
def _nanosleep_u32(
    ...
    *,
    ...
    """Warp nanosleep hint (sm70+).  kq6i: paces the reducer atomic
...
class FlashAttentionDSABackwardSm100TwoCTA(FlashAttentionDSABackwardSm100):
    ...
    TMEM_COLUMNS = 512
    # kq6i: inter-chunk pacing (ns) for the reducer atomic bursts.
    ...
    def __call__(
        ...
        round_fragment_half_offset = cute.cosize(round_a_layout_staged)
        assert round_fragment_half_offset == self.ROUND_HALF_ELEMENTS
        assert (
            cute.cosize(dkv_a_layout_staged)
            == 2 * round_fragment_half_offset
        )
        ...
        assert round_stage_bytes == self.ROUND_HALF_BYTES
        local_bulk_half_offset = cute.cosize(round_a_layout_staged)
        assert local_bulk_half_offset == self.ROUND_HALF_ELEMENTS
        assert (
            cute.cosize(score_a_layout_staged)
            == 8 * local_bulk_half_offset
        )
...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    ROUND_TMA_TILER = (256, 64, 32)
    ROUND_HALF_ELEMENTS = 4_096
    ROUND_HALF_BYTES = 8_192
    H_PASSES = 2
    ...
    # kq6i C.1: retain two complete H64 physical slots/fragments, but
    # publish each old panel as two H32 micro-generations.  The static
    # micro-gen order is G0-h0,G0-h1,...,G7-h0,G7-h1; 16 % 4 == 0
    # keeps the stage/raw-barrier map tile-invariant.
    ...
    def kernel(
        ...
        round_half_ptrs = (
            cute.make_ptr(
                self.element_dtype,
                round_buf_a_raw.toint(),
                round_buf_a_raw.memspace,
                assumed_align=1024,
            ),
            cute.make_ptr(
                self.element_dtype,
                round_buf_a_raw.toint() + Int32(self.ROUND_HALF_BYTES),
                round_buf_a_raw.memspace,
                assumed_align=1024,
            ),
            cute.make_ptr(
                self.element_dtype,
                round_buf_b_raw.toint(),
                round_buf_b_raw.memspace,
                assumed_align=1024,
            ),
            cute.make_ptr(
                self.element_dtype,
                round_buf_b_raw.toint() + Int32(self.ROUND_HALF_BYTES),
                round_buf_b_raw.memspace,
                assumed_align=1024,
            ),
        )
        ...
        # H32 ring-only TMA partitions.  The full H64 round_quad views above
        # remain the UMMA fragments; these four views address their two
        # contiguous 4096-bf16 halves in micro-generation order A0,A1,B0,B1.
        g_qt_round = cute.local_tile(
            round_tma_tensor_qt,
            cute.select(self.ROUND_TMA_TILER, mode=[0, 2]),
            (None, None, (token_idx, batch_idx)),
        )
        ...
        t_qt_round_smem_0, t_qt_round_gmem = cpasync.tma_partition(
            ...
            block_coord_vmnk[2],
            a_cta_layout,
            cute.group_modes(round_halves[0], 0, 3),
        ...
        t_qt_round_smem_1, _ = cpasync.tma_partition(
            ...
            block_coord_vmnk[2],
            a_cta_layout,
            cute.group_modes(round_halves[1], 0, 3),
            cute.group_modes(rank_g_qt_round, 0, 3),
        )
        t_qt_round_smem_2, _ = cpasync.tma_partition(
            ...
            block_coord_vmnk[2],
            a_cta_layout,
            cute.group_modes(round_halves[2], 0, 3),
            cute.group_modes(rank_g_qt_round, 0, 3),
        )
        t_qt_round_smem_3, _ = cpasync.tma_partition(
            ...
            block_coord_vmnk[2],
            a_cta_layout,
            cute.group_modes(round_halves[3], 0, 3),
            cute.group_modes(rank_g_qt_round, 0, 3),
        )
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            if tile_count > Int32(0):
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    # G0..G7 retain their old panel order, but each complete
                    # H64 A/B slot is published h0 then h1 as two contiguous
                    # H32 micro-generations.  Raw slots are A0,A1,B0,B1.
                    for flat_gen in cutlass.range_constexpr(
                        ...
                        for k_half in cutlass.range_constexpr(2):
                            ...
                            if cutlass.const_expr(h_half == 0):
                                round_dst_raw = (
                                    round_buf_a_raw
                                    + self.ROUND_HALF_ELEMENTS * k_half
                                )
                            else:
                                round_dst_raw = (
                                    round_buf_b_raw
                            ...
                            local_src_offset = self.ROUND_HALF_ELEMENTS * (
    ...
    def _issue_grads_kq_v1(
        ...
        dO/Q quadrants stream through 8 H64 panels / 16 H32 ring
        micro-generations; the K_dQ
    ...
    def _drain_dkv_v8(
        ...
        for i in cutlass.range_constexpr(8):
            ...
            # kq6i: pace the burst (concentration knife).

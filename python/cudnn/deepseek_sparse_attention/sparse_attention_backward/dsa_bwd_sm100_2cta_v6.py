...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    """v6: baseline-verbatim dKV reducer on the CG2 slots.
    ...
    * The gradient GEMMs of tile p are issued inside tile t=p-1's S/dP
    ...
    def _drain_dkv_v6(
        ...
        """Drain one rank-owned dKV slot with the baseline reducer verbatim.

        The slot shape ([D128, N64] FP32) matches the baseline's per-panel
        accumulator exactly, so this uses the production store_dKV path:
        Ld16x256b(Rep 4) T2R split across two warp groups, register-gathered
        FP32x4 quads (no shuffles), preloaded KV indices, and the
        thread-group-addressed 16B red.global whose within-panel column
        scramble is decoded by the baseline convert kernel (see the
        convert_canonical override).  Panel index = 2*round + rank, so each
        CTA covers its own D quarters: half the baseline's atomic scalars
        with disjoint pair addresses.
        """
        ...
        # overlap the T2R (baseline reducer pattern).
        ...
        sub_tile_idx = Int32(2 * round_index) + rank
        for i in cutlass.range_constexpr(8):
            coord_base = i * 2 - i % 2

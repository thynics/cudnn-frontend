"""vk_1: vg_5 + independent producer state per pds pipeline (traceable).

Semantically identical to vg_5 -- the only change is relay-side pipeline
state ownership: ``pipe_pds_ds`` keeps ``pds_com``, ``pipe_pds_p`` gets its
own ``pds_p_com``; each state advances after its own commit and tails its
own pipeline.  CUTLASS 4.5.0 permits the shared state (producer_commit does
not consult phase; producer_tail at num_stages=1 does not mutate state),
but the shared form's lowered code shape deadlocks under IKET runtime
injection: bisect at 9c11a29 pinned the first trigger to vg_1's split
publish, and the exclusive variants proved separating this state (S1) or
removing split publish outright (C) both restore trace capture, while
baseline/vc_2 never hung.  This file is byte-identical to the validated S1
variant except this docstring; that variant passed correctness 4/4, perf
(9.854 @ 8.508) and full candidate IKET capture at 9c11a29.

Split publish's net perf contribution inside the vg_5 composition measured
indistinguishable from same-day baseline drift (C 1.1497 vs S1 1.1582 with
baselines 2.6%% apart); whether to drop it entirely is adjudicated by the
C-vs-S1 decoded-trace A/B, not by this file.  vk_1's role is the traceable
vg_5-equivalent base.
    ...
    def kernel(
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            if tidx % Int32(32) == Int32(0):
                ...
                pds_p_com = pipeline.make_pipeline_state(
                    pipeline.PipelineUserType.Producer,
                    1,
                )
                for loop_iter in cutlass.range(tile_count):
                    ...
                    pipe_pds_p.producer_commit(pds_p_com)
                    pds_p_com.advance()
                    _mbarrier_wait_acquire_cluster(
                        landing_mbars,
                        landing_phase,
                ...
                if tile_count > Int32(0):
                    ...
                    pipe_pds_p.producer_tail(pds_p_com)

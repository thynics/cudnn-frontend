...
class FlashAttentionDSABackwardSm100TwoCTA(FlashAttentionDSABackwardSm100):
    ...
    def _load_chase_piece_v32(
        ...
        ruling-B geometry); the eight pieces of a bundle stream through
        the 2-slot chase ring.  `destination_rows` is the [64, K_CHUNK]
        row view of the ring slot.  V == K: this single fetch feeds all
        four score-plane consumers (G1 c0/c1, G2 c0/c1).
        """
...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    * The leader is dr-major; the chase pieces for bundle t+1 are pinned
    ...
    # v3.2 T3-64 score-transposed plane grid.
    #
    # G1/G2 score plane (S^T = K.Q^T / dP^T = V.dO^T), CG2:
    #   (M,N,K) = (kv128, h64-chunk, D64-piece), K-outer: each chased
    #   K/V piece [own-kv64 x D64] feeds G1(c0), G1(c1), G2(c0), G2(c1)
    #   before its ring slot is released (V==K single fetch).  B is the
    #   resident Q/dO panel through a K-major zero-copy view (each CTA
    #   supplies its own N-half: 32 heads per chunk).
    ...
    # v3.2 TMEM 512-column map (all f32, per CTA), allocated UNGUARDED:
    #   dQ^T [0,256): persistent across the query tile, 128 cols per
    #                 D-round (M256 CG2: 128 lanes x full N=128);
    #   S pp [256,320) / dP pp [320,384): 2 stages x 32 cols each (M128
    #                 CG2 fold: UMMA_2SM M64 Interleaved atom
    #                 (64,(N/2,2)):(1,(128,64)) => 128 DP x N/2 columns);
    #   dV [384,448) / dK [448,512): one [kv128 x D128] block each
    #                 (128 DP x 64 cols), reused across the 4 D-rounds.
    TMEM_DQ0_OFFSET = 0
    TMEM_DQ1_OFFSET = 128
    TMEM_S_OFFSET = 256
    TMEM_S1_OFFSET = 288
    ...
    # slot 1 = dK; v3.2 slots are per-tensor, not ping-pong).
    ...
    # Round-region generations per bundle (fixed order, one producer/one
    # consumer, 2-stage ring; 12 mod 2 == 0 keeps the phase law):
    # g0  kdq_r0w0(A)  g1  kdq_r0w1(B)   [G5 D-round 0, kv waves 0/1]
    # g2  dO_r0(A)     g3  Q_r0(B)       [G3/G4 D-round 0]
    # g4  dO_r1(A)     g5  Q_r1(B)       [G3/G4 D-round 1]
    # g6  dO_r2(A)     g7  Q_r2(B)       [G3/G4 D-round 2]
    # g8  dO_r3(A)     g9  Q_r3(B)       [G3/G4 D-round 3]
    # g10 kdq_r1w0(A)  g11 kdq_r1w1(B)   [G5 D-round 1, kv waves 0/1]
    ...
    def kernel(
        ...
        # v3.2 zero-copy score-B panel views.  Panel stage c (one 32-row
        # [h32 x D512] K-major box, 32,768 B) is byte-identical to the
        # staged score-B layout (8 x [n32 x k64] canonical K-major
        # blocks with contiguous 2,048-element stages), so the B view
        # for h-chunk c binds the staged-B layout directly at the panel
        # stage base (+c*16,384 elements).
        ...
        # v3.2 round-gen B views: one gen = [h128 x own-D64] (16,384 B),
        # bound with the 2-stage dkv-B layout so stage index == h-chunk
        # window (contiguous 8,192 B halves, natural head order).
        round_grad = (
            storage.round_buf_a.get_tensor(
                dkv_b_layout_staged.outer,
                swizzle=dkv_b_layout_staged.inner,
        ...
        # v3.2 per-CTA round-gen B TMA partitions: mQT/mdOT ([D,H] views)
        # tiled (N=D128-round, K=h64-chunk); the D-tile coordinate walks
        # the four D-rounds, the K-tile coordinate the two h-chunks
        # (= dkv-B stages), and the CTA takes its own N-half columns.
        g_qt = cute.local_tile(
            tma_tensor_qt,
            cute.select(self.DKV_MMA_TILER, mode=[1, 2]),
            (None, None, (token_idx, batch_idx)),
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            # --- gather, v3.2: the score-A chase -- eight rank-owned
            # [own-kv64 x D64] pieces per bundle through the 2-slot ring
            # (V == K single fetch) -- PLUS the kdq image fills
            # offloaded from the load warp (128 threads vs 32).  Per
            # iteration the order is: chase(b) -> kdq-r1 handshake of
            # the PREVIOUS bundle -> kdq-r0 handshake of THIS bundle,
            # so the late r1 credits (freed near the end of grads(prev))
            # never delay the chase, and the r0 images are ready for
            # the G5 r0 waves right after the score plane.
            ...
            if tile_count > Int32(0):
                # v3.2 chase: eight [own-kv64 x D64] pieces per bundle
                # stream through the 2-slot ring (slot = piece % 2).  The
                # ring credit itself paces the chase against the leader's
                # K-outer release edge (a slot frees only after all four
                # score consumers of its piece have issued -- kill-list
                # entry 3: the release edge is IN the graph, piece
                # readiness is never treated as exogenous).  Bundle t+1's
            ...
            # v3.2 publish targets: each chunk publish is a WHOLE
            # [own-kv64 x h64] sub-image (the CG2 accumulator is M-half
            # x FULL N, so every math warp owns local rows only; the
            # v12 n-owner/xchg machinery is retired -- the only
            # cross-CTA payload left is the relay's dq_b peer push).
            ...
            assert cute.size(t_rs_p_0, mode=[4]) == 1
            assert cute.size(t_rs_ds_0, mode=[4]) == 1
            assert cute.size(t_rs_dqb, mode=[4]) == 1
            t_rs_p_tiles = (
            ...
            t_rs_dqb_tile = t_rs_dqb[None, None, None, None, 0]
            r_score = cute.make_rmem_tensor(
                score_coordinates.shape,
                self.acc_dtype,
            )
            ...
            # v3.2: one 128-token bundle per iteration with TWO chunk
            # phases inside (chunk == done-pipeline stage, so the T2R
            # source select is STATIC).  The dq_b free wait rides the
            # pipe_dqb_free pipeline once per bundle (the leader pre-
            # arms an initial-free commit, so no first-bundle branch).
            for loop_iter in cutlass.range(tile_count):
                for chunk in cutlass.range_constexpr(
                    self.SCORE_H_CHUNKS
                ):
                    chunk_payload = loop_iter * Int32(
                        2 * self.SCORE_H_CHUNKS
                    ) + Int32(2 * chunk)
                    ...
                    # v3.2 math: same packed f32x2 pair schedule as
                    # v17a/E2, but the constants index the COLUMN
                    # (head) axis: head = chunk*64 + n (coordinate
                    # mode [1]).  Packed pairs are column-adjacent, so
                    # the pair constants are DISTINCT (lse[h0],
                    # lse[h1]) -- the assumption-free shape; the v9.3
                    # per-group hoist is structurally invalid here
                    # (heads vary within each (a1, L) group).
                    # pds protocol unchanged: ONE producer_acquire per
                    # bundle (chunk 0), one count-128 pds_ready arrive
                    # after the LAST chunk publish, num_stages=1.
                    ...
                    if cutlass.const_expr(chunk == 0):
                        # ONE pds acquire per bundle: the slab pair is
                        # free once the leader's last G4 of the
                        # previous bundle and the relay's DSM push have
                        # released it.
                        ...
                        # below; the relay's peer push is transitively
                        # gated by pds_ready).  The leader's group
                        # commit after the bundle's last G5 wave covers
                        # both D-rounds and both waves (errata #1).
                    ...
                    # Publish the WHOLE [own-kv64 x h64] P sub-image of
                    # this chunk (no n-owner split in v3.2: the CG2
                    # fragment is M-half x full N).
                    r_p_store = thread_copy_r2s.retile(r_p)
                    assert t_rs_p_tiles[0].shape == r_p_store.shape
                    cute.copy(
                        tiled_copy_r2s,
                        r_p_store,
                        t_rs_p_tiles[chunk],
                    ...
                    r_ds_store = thread_copy_r2s.retile(r_ds)
                    # Whole-image dS chunk publish: sub_img[chunk] of
                    # the dS slab (also the G4-A window; sub[1-rank]
                    # doubles as the relay's 8,192 B DSM payload).
                    assert t_rs_ds_tiles[0].shape == r_ds_store.shape
                    cute.copy(
                        tiled_copy_r2s,
                        r_ds_store,
                        t_rs_ds_tiles[chunk],
                    )
                    # dq_b own-half SECOND image (addendum section 1):
                    # own-H64 == chunk `rank`'s columns, so the same
                    # register fragment re-stores into dq_b
                    # sub_img[rank] -- zero TMEM re-read.  The free
                    # gate was taken at the chunk-0 phase.
                    if Int32(chunk) == rank:
                        cute.copy(
                            tiled_copy_r2s,
                            r_ds_store,
                ...
                # v12 (P2i) inheritance, once per BUNDLE: the DSM push,
                # dqb gate arming, and the pds commit belong to the
                # relay warp (W18).  Each math thread hands off with one
                # release-semantics mbarrier arrive after its own fenced
                # stores (covers P, dS, and the dq_b own image).
            ...
            # v3.2: eight [own-kv64 x D128] blocks per bundle, in the
            # leader's commit order -- per D-round, dV then dK.  Both
            # tensors of a round merge into the same GMEM quadrant
            # (V == K).
            for loop_iter in cutlass.range(tile_count):
                ...
                for d_round in cutlass.range_constexpr(
                    ...
                    dkv_wait, dkv_rel = self._drain_dkv_block_v32(
                        ...
                        rtx,
                        rank,
                        loop_iter * Int32(8) + Int32(2 * d_round),
                        pipe_dkv_done,
                        dkv_wait,
                        dkv_rel,
                    )
            ...
            if is_leader_cta:
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    # (3) gradient rounds: per D-round, G3 (dV, dO gen)
                    # then G4 (dK, Q gen); the 2-stage dkv_done ring
                    # alternates dV/dK stages, so each tensor's next
                    # block acquire waits exactly on its previous
                    # block's drain.
                    ...
                    for d_round in cutlass.range_constexpr(
                        ...
                        pipe_dkv_done.producer_commit(dkv_prod)
                        dkv_prod.advance()
                    # Slab release: math(t+1)'s publish may proceed
                    # once the last G4 completes (UMMA-tracked).
                    pipe_pds.consumer_release(pds_cons)
                    pds_cons.advance()

                    # (4) G5 D-round 1 (the LAST dq_b consumers), then
                    # the free group-commit -- it tracks every issued
                    # MMA, so it covers both D-rounds and both waves
                    # (errata #1).
                    for wave in cutlass.range_constexpr(
                        self.DQ_KV_WAVES
                    ):
            ...
            # --- load: stationary TMA once (2 half-chunk boxes per
            # panel), then 12 round gens per bundle: g0/g1 kdq D-round 0
            # (gather-warp handshake), g2..g9 the four (dO, Q) gradient
            # rounds (TMA, software-pipelined over two rotating
            # barriers), g10/g11 kdq D-round 1 (second handshake, whose
            # credits free near the end of grads(t)).
            _iket.mark("ROLE_KV_LOAD", rank)
            ...
            if tile_count > Int32(0):
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    # g0/g1: kdq D-round 0, both kv-wave images in one
                    # gather pass (both stage credits held).
                    ...
                    # g2..g9: round-gen B TMA fills, one gen per
                    # (D-round, tensor): dO -> buf A (barrier 0), Q ->
                    # buf B (barrier 1).  Each gen carries BOTH h-chunk
                    # stage boxes on one barrier (expect_tx = 16,384 B,
                    # GMEM-natural [H,D] rows through the transposed
                    # [D,H] view).  Software-pipelined exactly like
                    # v17a: issue gen q, then wait/commit gen q-1 on
                    # the other barrier, so it is never re-armed while
                    # pending.  (OWN_HALF_BULK is structurally dead in
                    # v3.2: a gen's own-head half is not a contiguous
                    # panel slice under the [h128 x own-D64] geometry.)
                    ...
                    # Drain the last in-flight fill (gen 9, barrier 1).
                    cute.arch.mbarrier_wait(
                        round_tma_mbars + 1,
                        tma_phase_1,
                    )
                    ...
                    # g10/g11: kdq D-round 1 handshake (both kv-wave
                    # images).  Its stage credits free when the leader
                    # consumes g8/g9 late in grads(t), which is why the
                    # gather side runs this handshake AFTER the next
                    # bundle's chase.
            ...
            # (3) commit pds (single lane; count-2 producer group);
    ...
    def _issue_score_pieces_v32(
        ...
        q_fragment_0: cute.Tensor,
        q_fragment_1: cute.Tensor,
        do_fragment_0: cute.Tensor,
        do_fragment_1: cute.Tensor,
        kscore_pipeline,
        ...
        """v3.2 score plane: K-outer chase-piece streaming, 32 atoms.

        For each of the eight [own-kv64 x D64] chase pieces (ring slot =
        piece % 2) issue the FOUR consumers -- G1(c0), G1(c1), G2(c0),
        G2(c1) -- then release the slot (V == K single fetch forces the
        K-outer order; the release edge frees the gather's next piece).
        ...
        # piece 0 onward, so BOTH must be acquired before the first
        # atom (math may still be T2R-ing the previous bundle's stage).
        s_state_c1 = s_producer_state.clone()
        s_state_c1.advance()
        dp_state_c1 = dp_producer_state.clone()
        dp_state_c1.advance()
        ...
        for piece in cutlass.range_constexpr(self.SCORE_D_PIECES):
            ...
            for k_block in cutlass.range_constexpr(k_blocks):
                # G1(c0), G1(c1): S^T chunk accumulators.
                cute.gemm(
                    s_mma,
                    t_s_0,
                    k_fragment[None, None, k_block, piece % 2],
                    q_fragment_0[None, None, k_block, piece],
                    t_s_0,
                )
                cute.gemm(
                    s_mma,
                ...
                cute.gemm(
                    ...
                    do_fragment_0[None, None, k_block, piece],
                    t_dp_0,
                )
                cute.gemm(
                    dp_mma,
                    t_dp_1,
                    dp_k_fragment[None, None, k_block, piece % 2],
                    do_fragment_1[None, None, k_block, piece],
                    t_dp_1,
                )
                s_mma.set(tcgen05.Field.ACCUMULATE, True)
                dp_mma.set(tcgen05.Field.ACCUMULATE, True)
            # Chase release edge: the ring slot frees when its four
            # consumers complete (UMMA-tracked release).
        ...
        cute.arch.fence_view_async_tmem_store()
        # Chunk-stage commits: stage 0 = chunk 0, stage 1 = chunk 1
        # (both were acquired up front; the caller's states advance by
        # two per bundle).
        s_done_pipeline.producer_commit(s_producer_state)
        s_producer_state.advance()
        s_done_pipeline.producer_commit(s_state_c1)
        s_producer_state.advance()
    ...
    def _issue_dkv_round_v32(
        ...
        """v3.2 G3/G4: one (tensor, D-round) block, both h-chunks.

        A = the P or dS slab (fragment stage == h-chunk sub-image),
        B = the streamed round gen (fragment stage == h-chunk window).
        The block accumulator starts fresh (accumulate=False on the
        first atom) because every (tensor, round) block is drained
        before its TMEM slot is reused.
        """

        mma = dkv_tiled_mma.with_()
        mma.set(tcgen05.Field.ACCUMULATE, False)
        k_blocks = cute.size(slab_fragment, mode=[2])
        for chunk in cutlass.range_constexpr(self.H_PASSES):
            for k_block in cutlass.range_constexpr(k_blocks):
                cute.gemm(
                    mma,
                    t_acc,
                    slab_fragment[None, None, k_block, chunk],
                    gen_fragment[None, None, k_block, chunk],
                    t_acc,
                )
                mma.set(tcgen05.Field.ACCUMULATE, True)

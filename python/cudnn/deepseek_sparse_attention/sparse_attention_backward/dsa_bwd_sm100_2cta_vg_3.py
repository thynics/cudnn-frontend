"""vg_3: vg_2 + dual-issue S/dP T2R under one fence.

The two tcgen05 loads ran as issue-fence-release, issue-fence-release, so
the dP load could not start until the S load had fully landed and its
credit had been returned.  vg_3 issues both loads back-to-back and takes a
single fence_view_async_tmem_load before returning both credits: the two
TMEM read latencies overlap and one fence disappears.  Credit safety is
unchanged -- both releases still trail the fence that proves the registers
hold the data, so no producer can overwrite a slot that is still in flight.
    ...
    def kernel(
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            for loop_iter in cutlass.range(tile_count):
                ...
                # vg_3: issue only -- the fence and both credit returns
                # move below the dP load so the two TMEM reads overlap.
                ...
                # One fence covers both loads; only after it do the two
                # TMEM slots go back to the leader.
                cute.arch.fence_view_async_tmem_load()
                ...
                if cutlass.const_expr(self.SOFTMAX_GROUPED_STATS):
                    ...
                    for h_group in cutlass.range_constexpr(4):
                        ...
                        lse = softmax_stats[local_h, 0]
                        delta = softmax_stats[local_h, 1]
                        # vg_2 (M1): issue the whole band's EX2 first so
                        # eight MUFU ops are in flight, then run the
                        # dependent chain.  Same calls, same operands,
                        # same per-value op order -- pure scheduling.
                        band_indices = [
                        ...
                        band_p = [
                            ...
                            for value_index in band_indices
                        ]
                        for j in cutlass.range_constexpr(8):
                            value_index = band_indices[j]
                            p_value = band_p[j]
                            ds_value = (

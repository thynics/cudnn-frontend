...
def _cpasync_bulk_g2s(
    ...
    *,
    ...
    """One 1D global-to-shared bulk copy of a contiguous byte blob.

    TMA-less form: both sides must be 16B aligned and the size a
    multiple of 16B (v15 uses 1KB-aligned SMEM and 8KB-aligned ring
    offsets).  Completion is mbarrier complete_tx on the LOCAL mbar,
    so the caller arms expect_tx exactly like the round-region fills.
    """
...
def _cpasync_bulk_s2g(
    ...
    *,
    ...
    """One 1D shared-to-global bulk copy of a contiguous byte blob.

    Completion is bulk-group based: the caller follows up with
    cp_async_bulk_commit_group() and cp_async_bulk_wait_group()
    exactly like the dQ-epilogue TMA store path (b244255 precedent).
    """
...
class FlashAttentionDSABackwardSm100TwoCTA(FlashAttentionDSABackwardSm100):
    ...
    def __call__(
        ...
        ``workspace_pds`` (v15 L3): uint8 ring of shape
        ``_get_workspace_size_pds(total_S_q)``; required only when the
        active class runs with V15_L2X on (see V15_RUNNER_NOTES.md).
        Trailing-with-default so pre-v15 harness call sites keep working.
        """
...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    * The gradient GEMMs of tile p are issued inside tile t=p-1's S/dP
    ...
    # ------------------------------------------------------------------
    # v15 bisect flags (env-overridable at import time: DSA_V15_*).
    # With every flag off/0 the kernel must reproduce v12 bit-exactly.
    # ------------------------------------------------------------------
    # L3: L2-staged P/dS exchange (pds_stage + HBM ring + W18 G2S fills;
    # DSM sends, pds_ready handshake and math's slot park all retire).
    V15_L2X = os.environ.get("DSA_V15_L2X", "1") != "0"
    # Register swap.  setmaxnreg is WARPGROUP-granular (PTX .aligned over
    # 4 warps; divergent values within warps 16..19 are illegal), so both
    # variants stay warpgroup-uniform:
    #   0 = off (v12 split: gather 48 / W16-19 48 / math 128 / reduce 128)
    #   1 = variant B: W16-19 uniform 64, funded by reduce 128->120
    ...
    # cycle; the releases stay staggered after each round's issue.
    V15_DQ_MERGE = os.environ.get("DSA_V15_DQ_MERGE", "1") != "0"
    ...
    # 64-row TMA box; False falls back to the pure TMA path (v15 L4 takes
    # that fallback deliberately -- see V15_ALLTMA).
    OWN_HALF_BULK = not V15_ALLTMA

    # v15 (L3) HBM exchange ring geometry.  Per token:
    #   [depth 2][cta 2][kind 2: dS=0, P=1] x PDS_IMAGE_BYTES.
    # Hot set = 74 resident clusters x 64KB ~= 4.9MB (L2-resident by
    # construction: every blob is re-read within <= 2 tile periods).
    PDS_IMAGE_BYTES = 8192
    PDS_RING_TOKEN_BYTES = 2 * 2 * 2 * PDS_IMAGE_BYTES

    @staticmethod
    def _get_workspace_size_pds(total_S_q: int):
        """Byte shape of the v15 L2-staged exchange ring (uint8 alloc)."""

        return (
            total_S_q,
            FlashAttentionDSABackwardSm100TwoCTAV2.PDS_RING_TOKEN_BYTES,
        )
    ...
    def _specialize_shared_storage(
        ...
        if self.V15_L2X:
            # v15 (L3): the DSM exchange dies.  p_xchg (4,096B, live) and
            # ds_xchg (4,096B, write-dead since P1b) fund the 8,192B
            # pds_stage math publishes through -- byte-exact net zero.
            # pds_ready_mbars (count-128 handoff) retires; l2x_mbars adds
            # (review fix #1: gen_ready mbars are slot ping-ponged so a
            # publish for tile t+2 can never alias tile t's phase --
            # math's arrives are gated by ring_credit(t) which requires
            # W18(t)'s gen wait to have PASSED, restoring the structural
            # <=1-ahead bound v12 had):
            # [0]=gen_ready_ds_s0 [1]=gen_ready_ds_s1
            # [2]=gen_ready_p_s0  [3]=gen_ready_p_s1
            # [4]=ring_credit_s0  [5]=ring_credit_s1
            # [6]=w18 third landing group (+56B).
            @cute.struct
            class SharedStorageV2:
                ...
                l2x_mbars: cute.struct.MemRange[cutlass.Int64, 7]
    ...
    def _issue_dq_rounds_v2(
        ...
        if cutlass.const_expr(self.V15_DQ_MERGE):
            # v15 (DQ_MERGE): both kdq credits are committed together by
            # ROUTE_K before the leader ever waits, so the two waits can
            # run back-to-back (no producer cycle -- unlike the condemned
            # dVdK merge, whose producer commits gen q only after
            # acquiring gen q+1).  Releases stay staggered after each
            # round's issue so the fill lattice sees v12 timing.
            ...
            state_g0 = round_consumer_state.clone()
            ...
            cute.arch.fence_view_async_tmem_store()
            round_pipeline.consumer_release(state_g0)
            dq_issue_token = _iket.range_start(
                "dQ_ISSUE(i,r)",
                packed_issue_1,
            )
    ...
    def kernel(
        ...
        if cutlass.const_expr(self.V15_L2X):
            # v15 (L3): [0/1]=gen_ready_ds s0/s1, [2/3]=gen_ready_p s0/s1,
            # [4/5]=ring_credit s0/s1, [6]=w18 landing group 3.
            l2x_mbars = storage.l2x_mbars.data_ptr()
        else:
        ...
        if cutlass.const_expr(self.V15_L2X):
            pds_stage_raw = storage.pds_stage.get_tensor(
                cute.make_layout(
                    (2 * self.PDS_BLOCK_ELEMENTS,),
                    stride=(1,),
                )
            )
        ...
        if tidx == Int32(0):
            ...
            if cutlass.const_expr(self.V15_L2X):
                # gen_ready_{ds,p} per depth-2 slot (tile-pinned
                # ping-pong): one elected math lane per CTA arrives
                # local + remote (both CTAs' blobs feed every fill).
                cute.arch.mbarrier_init(l2x_mbars, 2)
                cute.arch.mbarrier_init(l2x_mbars + 1, 2)
                cute.arch.mbarrier_init(l2x_mbars + 2, 2)
                cute.arch.mbarrier_init(l2x_mbars + 3, 2)
                # ring_credit per depth-2 slot: both CTAs' W18s arrive
                # (each reads both blobs of the slot's generation).
                cute.arch.mbarrier_init(l2x_mbars + 4, 2)
                cute.arch.mbarrier_init(l2x_mbars + 5, 2)
                # W18's third local landing group (expect_tx, count 1).
                cute.arch.mbarrier_init(l2x_mbars + 6, 1)
            else:
                cute.arch.mbarrier_init(
                    pds_ready_mbars,
                    self.MATH_THREADS,
        ...
        if warp_idx < Int32(self.MATH_WARP_BEGIN):
            # Gather warpgroup (warps 0-3).
            if cutlass.const_expr(self.V15_REGSWAP == 2):
                # Variant C: gather funds the leader warpgroup.
                # Pool: 128*40 + 128*56 + 128*128 + 256*128 = 61,440.
                cute.arch.setmaxregister_decrease(40)
            else:
                cute.arch.setmaxregister_decrease(48)
        elif warp_idx >= Int32(self.MMA_WARP):
            # Leader/load/relay/idle warpgroup (warps 16-19); setmaxnreg
            # is warpgroup-granular so the value is uniform across all
            # four warps regardless of role.
            if cutlass.const_expr(self.V15_REGSWAP == 2):
                cute.arch.setmaxregister_decrease(56)
            elif cutlass.const_expr(self.V15_REGSWAP == 1):
                # Variant B: reduce funds the leader warpgroup.
                # Pool: 128*48 + 128*64 + 128*128 + 256*120 = 61,440.
            ...
            if warp_idx < Int32(self.REDUCE_WARP_BEGIN):
                ...
                if cutlass.const_expr(self.V15_REGSWAP == 1):
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            if cutlass.const_expr(not self.V15_L2X):
                # v12 path: math holds the pds producer-acquire side.
                # Under L3 the whole producer role lives on W18.
            ...
            if cutlass.const_expr(self.V15_L2X):
                # v15 (L3): math publishes whole P/dS images through the
                # 8KB pds_stage, one image per wave; the n-owner xchg
                # machinery retires with the DSM path.  Stage base is a
                # 1024-aligned field with no runtime bias.
                stage_store = cute.make_tensor(
                    cute.recast_ptr(
                        cute.make_ptr(
                            self.element_dtype,
                            pds_stage_raw.iterator.toint(),
                            pds_stage_raw.memspace,
                            assumed_align=16,
                        ),
                        score_store_layout.inner,
                        dtype=self.element_dtype,
                ...
                t_rs_stage = thread_copy_r2s.partition_D(
                    stage_store
                )
                assert cute.size(t_rs_stage, mode=[4]) == 1
                t_rs_stage_tile = t_rs_stage[
                    None, None, None, None, 0
                ]
            else:
            ...
            for loop_iter in cutlass.range(tile_count):
                ...
                if cutlass.const_expr(self.V15_L2X):
                    # v15 (L3): two-wave publish through pds_stage into
                    # the per-token HBM ring.  The slot park
                    # (MATH_PDS_ACQ), both DSM sends and the count-128
                    # handoff are gone; MATH_STORE now brackets the whole
                    # publish (stmatrix waves + S2G + gen_ready arrives).
                    ...
                    r_p_store = thread_copy_r2s.retile(r_p)
                    r_ds_store = thread_copy_r2s.retile(r_ds)
                    assert t_rs_stage_tile.shape == r_ds_store.shape
                    assert t_rs_stage_tile.shape == r_p_store.shape
                    slot_kind_off = (
                        (loop_iter % Int32(2)) * Int32(32768)
                        + rank * Int32(16384)
                    )
                    # Wave 1: whole dS image (dQ consumes dS first).
                    cute.copy(
                        tiled_copy_r2s,
                        r_ds_store,
                        t_rs_stage_tile,
                    ...
                    self.math_barrier.arrive_and_wait()
                    if mtx == Int32(0):
                        # Ring credit: this slot's blobs were last read
                        # by both W18s at tile t-2; the first two tiles
                        # start pre-free (depth-2 ring).
                        if loop_iter >= Int32(2):
                            ring_parity = (
                        ...
                        _cpasync_bulk_s2g(
                            pds_stage_raw.iterator,
                            ws_pds_token_base
                            + cutlass.Int64(slot_kind_off),
                            self.PDS_IMAGE_BYTES,
                        )
                        cute.arch.cp_async_bulk_commit_group()
                        cute.arch.cp_async_bulk_wait_group(0)
                        if (loop_iter % Int32(2)) == Int32(0):
                            cute.arch.mbarrier_arrive(l2x_mbars, rank)
                            cute.arch.mbarrier_arrive(
                                l2x_mbars,
                                peer_rank,
                            )
                    ...
                    self.math_barrier.arrive_and_wait()
                    # Wave 2: whole P image through the same stage.
                    cute.copy(
                        tiled_copy_r2s,
                        r_p_store,
                        t_rs_stage_tile,
                    ...
                    # Publish P/dS with stmatrix. Each pair of warps owns
                    # one N32 half, so this branch is warp-uniform.
                    r_p_store = thread_copy_r2s.retile(r_p)
                    r_ds_store = thread_copy_r2s.retile(r_ds)
                    assert t_rs_p_local_tile.shape == r_p_store.shape
                    ...
                    if owns_n:
                        ...
                        # v12 (P1b): the dS xchg copy is retired -- the
                        # DSM send sources the byte-identical (1-rank)
                        # half of the whole dS image, which every warp
                        # already writes.
                    ...
                    # pair): invalid columns see S=dP=0 from zero-filled
                    # K rows, so P/dS stay finite; dQ is protected by
                    # zero-filled K_dQ rows and dKV garbage columns are
                    # dropped by the drain predicates (global_n < topk,
                    # kv_index >= 0).
                    ...
                    # v12 (P2i): the barrier rendezvous, both DSM sends,
                    # and the pds commit move to the relay warp (W18).
            ...
            if is_leader_cta:
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    if has_prev:
                        ...
                        round_cons, dkv_prod, pds_cons, dq_done_prod = (
            ...
            if tidx % Int32(32) == Int32(0):
                if cutlass.const_expr(self.V15_L2X):
                    # v15 (L3): W18 owns the whole pds producer role.
                    # Per tile: acquire the slot (the wait that was
                    # math's 2.2us MATH_PDS_ACQ park), then convert the
                    # HBM ring blobs into SMEM operands with five 1D
                    # G2S fills in consumption order:
                    #   L1 own dS blob -> ds_image, commit pds full
                    #      (the leader's dQ gate: bytes LANDED),
                    #   L2 P halves -> p_blocks[0/1], arrive p_ready,
                    #   L3 dS halves -> ds_blocks[0/1], arrive ds_ready,
                    # then release the ring slot to both CTAs' math.
                    ...
                    for loop_iter in cutlass.range(tile_count):
                        pipe_pds.producer_acquire(pds_com)
                        # Slot-pinned gen waits (review fix #1): slot
                        # mbar flips once per 2 tiles, parity (t//2)%2;
                        # cluster-acquire variant because the arrival
                        # set includes the peer CTA's remote arrive.
                        gr_parity = (
                            loop_iter // Int32(2)
                        ) % Int32(2)
                        if (loop_iter % Int32(2)) == Int32(0):
                        ...
                        w18_pds_token = _iket.range_start(
                            "W18_PDS(i)",
                            loop_iter,
                        )
                        slot_off = (
                        ...
                        _cpasync_bulk_g2s(
                            ws_pds_token_base
                            + cutlass.Int64(
                                slot_off + rank * Int32(16384)
                            ),
                            ds_image_raw,
                            landing_mbars,
                            self.PDS_IMAGE_BYTES,
                        )
                        # L2: P halves (kind offset 8192) of BOTH CTAs'
                        # blobs; reader rank r takes bytes
                        # [r*4096, (r+1)*4096) of each image.
                        if (loop_iter % Int32(2)) == Int32(0):
                            _mbarrier_wait_acquire_cluster(
                                l2x_mbars + 2,
                                gr_parity,
                        ...
                        # L3: dS halves (kind offset 0) of both blobs.
                        cute.arch.mbarrier_arrive_and_expect_tx(
                            l2x_mbars + 6,
                            self.PDS_IMAGE_BYTES,
                        )
                        ...
                        # Landing conversions in consumption order.
                        _mbarrier_wait_acquire_cluster(
                            landing_mbars,
                            landing_phase,
                        )
                        ...
                        land3_phase = Int32(1) - land3_phase
                        cute.arch.mbarrier_arrive(
                            relay_mbars + 1,
                            Int32(0),
                        )
                        # Ring credit: this W18's reads of the slot's
                        # blobs are all complete; release to both CTAs'
                        # math (count 2 per mbar).  The last two tiles'
                        # arrives are never consumed -- benign, the
                        # common-tail cluster arrive/wait keeps both
                        # CTAs resident until delivery.
                        if (loop_iter % Int32(2)) == Int32(0):
                            cute.arch.mbarrier_arrive(
                                l2x_mbars + 4,
                                rank,

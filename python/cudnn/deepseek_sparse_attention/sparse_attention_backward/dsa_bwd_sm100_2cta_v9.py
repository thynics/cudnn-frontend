...
def _mbarrier_arrive_release_cluster(
    ...
    *,
    ...
    """Remote mbarrier arrive with explicit .release.cluster ordering.

    The DSL's mbarrier_arrive(peer_cta_rank_in_cluster=...) emits CTA
    scope; the v9 push-credit protocol needs the arrive to ORDER the
    receiver's preceding expect_tx arm against the sender's push (arm ->
    credit(release.cluster) -> credit-wait(acquire.cluster) -> push), so
    the release scope is load-bearing, not cosmetic.
    """
    ...
    llvm.inline_asm(
        ...
        [barrier_i32],
        (
            "mbarrier.arrive.release.cluster.shared::cluster"
            ".b64 _, [$0];"
        ),
        "r",
        has_side_effects=True,
...
@dsl_user_op
def _cp_reduce_bulk_add_f32(
    gmem_dst: cute.Pointer,
    smem_src: cute.Pointer,
    copy_bytes: int | Int32,
    *,
    loc=None,
    ip=None,
) -> None:
    """One bulk f32 add-reduction from SMEM into GMEM (bulk_group).

    Element-wise atomic RMW; completion is tracked source-side with
    cp.async.bulk.commit_group / wait_group (no mbarrier, no receiver).
    Addresses and size must be 16-byte multiples.
    """

    dst_i64 = gmem_dst.toint(loc=loc, ip=ip).ir_value()
    src_i32 = smem_src.toint(loc=loc, ip=ip).ir_value()
    llvm.inline_asm(
        None,
        [
            dst_i64,
...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    """v9: peer-push quadrant supply + bulk-reduce drain (the two walls).

    v8 measured 11.945 ms (1.48x baseline) with full sub-span attribution.
    The two remaining structural walls and their v9 levers:

    * Quadrant supply (MAT_QDO 2x4.83 us/tile inclusive): the 4 h!=rank
      GMEM TMA fills become credit-gated DSM pushes from the peer's
      stationary (byte-identity holds for c==peer_rank: every quadrant
      panel starts 16KB-aligned in the stationary concatenation, the same
      alignment class v4 proved on hardware).  The receiver's W17 arms its
      mbar (existing unconditional arm) then emits a .release.cluster
      credit; the peer's W19 (previously idle) waits the credit with
      cluster-acquire and pushes 16KB.  Arm -> credit -> push causality is
      provable; the 2-stage round recurrence bounds outstanding pushes to
      one per direction (count-1 credit mbar parity safe).  W17 keeps sole
      ownership of every round-pipeline op; its delayed waits upgrade to
      cluster-acquire (the hardware-proven P/dS landing visibility chain).
    * dKV drain (REDUCE busy 6.66 us/tile vs baseline 4.41; atomic issue
      measured volume-independent since v6): CONFIG B.  Each slot's rows
      stage through the dS-image region -- provably dead in the drain
      window: its tile-t dQ reads retire at the very head commit that
      wakes the drain, and the math warps' next overwrite waits a new
      count-8 stage_mbars gate (lead <= 1) whose tax hides inside the
      measured 2.2 us MATH_PDS_ACQ slack -- then 128 cp.reduce.async.bulk
      512-byte row reductions per tile replace 4,096 red.global.v4
      thread atomics.  Staged bytes replicate the scramble exactly;
      convert_canonical untouched.  Landing halves are NOT part of the
      pool, so no new cross-CTA timing surface exists.

    Known remaining wall (v9.1, diagnosed from v8 SASS): the P/dS publish
    stores lower to 96 scalar STS.U16 + 96 PRMT per warp per tile instead
    of stmatrix (~4.6 us real) -- get_smem_store_op layout compatibility,
    tracked separately.
    ...
    * The gradient GEMMs of tile p are issued inside tile t=p-1's S/dP
    ...
    def _specialize_shared_storage(
        ...
        class SharedStorageV2:
            ...
            # v9: peer-push credit (count-1, provable lead <= 1) and the
            # CONFIG-B staging gate (count-8: one arrive per reduce warp).
            push_credit_mbars: cute.struct.MemRange[cutlass.Int64, 1]
            stage_mbars: cute.struct.MemRange[cutlass.Int64, 1]
    ...
    def kernel(
        ...
        push_credit_mbars = storage.push_credit_mbars.data_ptr()
        stage_mbars = storage.stage_mbars.data_ptr()
        ...
        if tidx == Int32(0):
            ...
            cute.arch.mbarrier_init(push_credit_mbars, 1)
            cute.arch.mbarrier_init(stage_mbars, 8)
        ...
        if (
            ...
            # v8: rebalanced budgets.  The v7 SASS proved the math publish
            # spills at 128 regs (STACK=1384 B/thread; 39 STL + 43 LDL
            # between the fragment materialization and the first math
            # barrier).  The setmaxnreg pool is the CTA's LAUNCH allocation
            # (640 thr * 96 regs = 61,440), NOT the SM register file: .inc
            # can only consume what .dec released.  Exact balance (the same
            # invariant v7 satisfied with 48/128):
            #   dec supply: 8 warps * (96-48) * 32            = 12,288
            #   inc demand: 4*(176-96)*32 + 8*(104-96)*32     = 12,288
            #   totals: 256*48 + 128*176 + 256*104 = 61,440 = 640*96.
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            # v9 CONFIG B: phase of the staging-free gate (one count-8
            # mbarrier, one reducer arrive per tile; lead <= 1 because the
            # reducer's arrive for tile t transitively requires this warp
            # to have consumed the tile t-1 phase).
            stage_phase = Int32(0)
            r_p = cute.make_rmem_tensor(
                score_coordinates.shape,
                self.element_dtype,
            ...
            for loop_iter in cutlass.range(tile_count):
                ...
                # v9 CONFIG B: the dS whole-image region doubles as the
                # dKV drain's bulk-reduce staging pool; wait until the
                # engine has read tile t-1's staged rows before the image
                # store below overwrites them.  Skipped on the first tile
                # (no prior drain exists).
                if loop_iter > Int32(0):
                    cute.arch.mbarrier_wait(
                        stage_mbars,
                        stage_phase,
                    )
                    stage_phase = Int32(1) - stage_phase
            ...
            for loop_iter in cutlass.range(tile_count):
                ...
                dkv_wait, dkv_rel = self._drain_dkv_v9(
                    t_dkv[0],
                    t_dkv[1],
                    mdKV_acc,
                    mTopkIdxs,
            ...
            # CONFIG B epilogue: no in-flight bulk-reduce may outlive the
            # kernel; a FULL wait (not .read) before the common tail.
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    for flat_gen in cutlass.range_constexpr(8):
                            ...
                            if cutlass.const_expr(h_half == 0):
                                if cutlass.const_expr(self.OWN_HALF_BULK):
                                    if rank == Int32(0):
                                        ...
                                        # v9: panel arrives by peer push;
                                        # the credit tells the peer's W19
                                        # "gen acquired + mbar armed".
                                        with cute.arch.elect_one():
                        ...
                        if cutlass.const_expr(flat_gen > 0):
                            # v9: cluster-acquire waits -- half the fills
                            # are now DSM pushes from the peer CTA, and
                            # the async-proxy visibility chain must match
                            # the hardware-proven P/dS landing pattern.
                            if cutlass.const_expr(
                                (flat_gen - 1) % 2 == 0
                            ):
                                _mbarrier_wait_acquire_cluster(
            ...
            # --- push engine (v9): stream this CTA's stationary H-half
            # panels into the PEER's round-buffer generations.  Each push
            # is credit-gated: the receiver's W17 emits the credit only
            # AFTER its producer_acquire (buffer provably free: the
            # leader's UMMA release multicasts to both CTAs) and its
            # expect_tx arm, so the bytes can never beat the arm.  The
            # 2-stage round recurrence bounds outstanding pushes to one
            # per direction (credit k+1 needs the leader to have consumed
            # the gen filled by push k), so the count-1 credit mbar's
            # phase parity is safe (lead <= 1).  Tied to OWN_HALF_BULK:
            # the pure-TMA fallback path emits no credits, and an
            # unguarded W19 would deadlock waiting for them.
            if cutlass.const_expr(self.OWN_HALF_BULK):
                if tidx % Int32(32) == Int32(0):
                    if tile_count > Int32(0):
                        cute.arch.mbarrier_wait(
                        ...
                        credit_phase = Int32(0)
                        for push_k in cutlass.range(
                            tile_count * Int32(4)
                        ):
                            _mbarrier_wait_acquire_cluster(
                                push_credit_mbars,
                            ...
                            credit_phase = Int32(1) - credit_phase
                            # Push order per tile: dO_r0, Q_r0, dO_r1, Q_r1
                            # (matches the receiver's peer-gen order).  The
                            # source is the byte-identical stationary slice
                            # for the RECEIVER's D-quarter (c = peer_rank);
                            # dst/mbar are the sender-rank-indexed local
                            # names, mapa'd to the peer by the helper.
                            push_round = (
                                push_k % Int32(4)
                            ) // Int32(2)
                            src_off = Int32(4096) * (
                            ...
                            if push_k % Int32(2) == Int32(0):
                                if rank == Int32(0):
                                    _cpasync_bulk_s2cluster(
                                        stationary_do_raw + src_off,
                                        round_buf_a_raw,
                                        round_tma_mbars,
                                        grad_a_stage_bytes,
                                        peer_rank,
    ...
    @cute.jit
    def _drain_stage_slot_v9(
        self,
        thread_values: cute.Tensor,
        thread_coordinates: cute.Tensor,
        stage_f32: cute.Tensor,
        mdKV_acc: cute.Tensor,
        mTopkIdxs: cute.Tensor,
        tile_base: Int32,
        topk: Int32,
        ...
        """Drain one dKV slot via four 16-row bulk-reduce waves (CONFIG B).

        Each wave: all 256 reduce threads deposit their in-wave FP32x4
        quads into the 16-row staging pool (bytes identical to what the
        old red.global path wrote, so convert_canonical is unchanged),
        fence the async proxy, rendezvous, then one lane per warp issues
        two 512-byte cp.reduce.async.bulk row-reductions and drains its
        bulk group (.read) before the pool rows are reused.  The two
        named barriers per wave are correctness-critical: the first
        orders every thread's STS before any engine read, the second
        orders the engine reads before the next wave's STS.
        """

        # Pool = the 8 KiB dS image EXACTLY (16 rows x 512 B).  The review
        # panel proved a 32-row pool overruns into ds_blocks -- whose
        # peer-landed half is written by the OTHER CTA's ROUTE_dS push
        # with no ordering against this drain -- so the wave size is a
        # correctness bound, not a tuning knob.
        col_base = (dp_idx // Int32(4)) * Int32(4)
        for quarter in cutlass.range_constexpr(4):
            for i in cutlass.range_constexpr(8):
                ...
                pool_row = local_row - Int32(16 * quarter)
                if pool_row >= Int32(0):
                    if pool_row < Int32(16):
                        stage_f32[
                            pool_row,
                            col_base,
                        ] = thread_values[coord_base]
                        stage_f32[
                            pool_row,
                            col_base + Int32(1),
                        ] = thread_values[coord_base + 2]
                        stage_f32[
                            pool_row,
                            col_base + Int32(2),
                        ] = thread_values[coord_base + 16]
                        stage_f32[
                            pool_row,
                            col_base + Int32(3),
                        ] = thread_values[coord_base + 18]
            cute.arch.fence_view_async_shared()
            self.reduce_sync_barrier.arrive_and_wait()
            if rtx % Int32(32) == Int32(0):
                warp_slot = rtx // Int32(32)
                for j in cutlass.range_constexpr(2):
                    pool_row = warp_slot * Int32(2) + Int32(j)
                    global_row = (
                        tile_base
                        + Int32(16 * quarter)
                        + pool_row
    ...
    def _drain_dkv_v9(
        ...
        rank: Int32,
        issue_seq: Int32,
        stage_pool_raw: cute.Pointer,
        stage_mbar: cute.Pointer,
        done_pipeline,
        wait_state: pipeline.PipelineState,
        release_state: pipeline.PipelineState,
    ):
        """Drain both rank-owned dKV slots (v9 CONFIG B).

        T2R structure is v8's (per-slot generations, split wait/release
        states, slot 0 off the head commit).  The GMEM reduction is new:
        instead of 4,096 per-thread red.global.v4 atomics (measured
        issue-bound at ~2.6 us/slot), each slot is staged through the
        dS-image pool (dead in this window: its tile-t reads retire at
        the head commit that gates this very drain, and math overwrites
        it only after the stage_mbars gate) and reduced into mdKV_acc by
        128 cp.reduce.async.bulk 512-byte row operations per tile.  The
        staged bytes replicate the old scramble exactly, so
        convert_canonical is untouched.
        """
        ...
        # v9: no per-thread index preload -- the wave-issuing lanes load
        # the eight KV indices they need per wave directly (hidden under
        # the staging stores).
        ...
        # (16, 128) f32 = 8,192 bytes = the ds_image field EXACTLY; the
        # pool must never spill into the adjacent ds_blocks (its landed
        # half is written by the peer's ROUTE_dS push, unordered vs this
        # drain).
        stage_f32 = cute.make_tensor(
            cute.recast_ptr(
                stage_pool_raw,
                None,
                dtype=self.acc_dtype,
        ...
        # All staged rows of both slots have been read by the engine
        # (each wave ended with wait_group.read): open the gate for the
        # math warps' next dS-image overwrite.
        if rtx % Int32(32) == Int32(0):
            cute.arch.mbarrier_arrive(stage_mbar, rank)

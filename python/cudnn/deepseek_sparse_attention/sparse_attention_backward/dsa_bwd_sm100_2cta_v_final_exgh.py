...
v_final_exgh (P-early split exchange, forked off final): the math
publish splits into a P phase and a dS phase with separate count-128
ready barriers; the relay flies the P DSM as soon as the P phase lands,
so the dS compute+publish overlap the P flight.  Single pds pipeline,
single relay producer state, and the per-consumer landing gates
(relay0 gates dV, relay1 gates dK) are unchanged -- deliberately NOT
the vg_1-era shared-producer-state shape.
"""
    ...
    def kernel(
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            if tidx % Int32(32) == Int32(0):
                ...
                for loop_iter in cutlass.range(tile_count):
                    # exgh: P flies as soon as the math P phase lands.
                    cute.arch.mbarrier_wait(
                        pds_ready_mbars,
                        ready_phase,
                    )
                    ...
                    # exgh: dS follows its own ready phase; the P DSM
                    # flight overlaps the math dS compute+publish.
                    cute.arch.mbarrier_wait(
                        pds_ready_mbars + 1,
                        ds_ready_phase,
                    )

"""vg_6: vg_5 + double-buffered dQ epilogue (round overlap).

The per-token dQ epilogue ran its two [H128 x D128] rounds fully serially:
T2R -> scalar scatter -> barrier -> 32 KiB TMA -> wait_group(read) ->
barrier, twice, because both rounds staged through the same dead score-K
allocation.  vg_6 stages round 1 in the round ring (32 KiB, equally dead
once dq_done commits: its last readers are the final dQ MMAs, tracked by
the same commit), so round 1's T2R and scatter overlap round 0's TMA
flight, and one source-read wait plus one barrier cover both rounds at the
end.  Bytes, addresses and the TMA boxes are unchanged.

Inherited vg_5 notes: vg_4 + full-width exp band.
...
class FlashAttentionDSABackwardSm100TwoCTA(FlashAttentionDSABackwardSm100):
    ...
    def _store_dq_epi_tma_v12(
        ...
        rank: Int32,
        mtx: Int32,
        wait_reads: cutlass.Constexpr[bool] = True,
    ):
        ...
        if mtx < Int32(32):
            ...
            # vg_6: rounds stage in DISJOINT buffers, so the source-read
            # wait is only needed once, before the kernel retires -- the
            # final call waits for every outstanding bulk group.
            if cutlass.const_expr(wait_reads):

...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    16 issues dQ, S/dP, and dV/dK and manages their pipeline credits; warp
    17 feeds the round ring from the stationary panels; warp 18 relays
    P/dS across the cluster; warp 19 commits ring TMA completions.

    Steady-state schedule per KV tile (rotated): W16 first issues dQ(t-1),
    releasing its two kdq ring slots, then S(t)/dP(t), and finally tile
    t-1's eight dV/dK passes. The kdq rendezvous hands the gather-filled dQ-A
    ...
    def kernel(
        ...
        # Keep aug6's exact 48/128/120/64 split for a resource-matched W16
        # scheduling ablation (pool-neutral vs 48/128/128/48).
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            if is_leader_cta:
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    # Retire dQ(t-1) before S/dP(t). This releases g0/g1 so
                    # W17 can prepare the six dV/dK generations while the
                    # same warp issues the current score-side MMAs.
                    if has_prev:
            ...
            for loop_iter in cutlass.range(tile_count):
                ...
                # per-tile reconvergence before W16 consumes the relay.
                cute.arch.sync_warp()
            ...
            for loop_iter in cutlass.range(tile_count):
                # g0/g1 are committed by W17 and consumed by W16 dQ; this
                # independent producer cursor owns only g2..g7.
                commit_com.advance()
                commit_com.advance()
    ...
    def _issue_prev_dkv_head_v3(
        ...
        W16 has already issued dQ from g0/g1 and enters here at g2; the

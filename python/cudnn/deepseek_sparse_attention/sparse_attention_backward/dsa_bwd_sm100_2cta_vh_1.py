...
Inherited vg_1 notes: vc_2 + split publish (dS-early / P-late).

The pds handoff splits into TWO independent credit chains.  math publishes
the dS image first and hands it off, then publishes P; W18 sends the dS DSM
block and commits pds_dS, then sends P and commits pds_P.  The leader gates
its dQ rounds (whose B operand is the dS image) on pds_dS alone, so the P
store, the P DSM send, and the P landing all overlap the dQ issue instead
of sitting in front of it -- ve_1 measured this edge at 0.77 us/tile
(MATH_PDS_ACQ 0.823 -> 0.056 under a depth-2 pds) without needing ve_1's
second publish face.  Everything else (loan alias, score, math arithmetic,
reducer, persistent dQ) is byte-for-byte vc_2.
...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    * The gradient GEMMs of tile p are issued inside tile t=p-1's S/dP
    ...
    # vh_1: the loan is retired -- dO round 0 streams through the ring
    # like every other quadrant -- and the ring deepens to three slots.
    # Twelve generations per tile (ten real + two pads) keep the slot map
    # tile-invariant (12 mod 3 == 0), so no macro unroll is needed:
    # g0/g1 kdq (slots 0,1), g2/g3 dO_r0 (2,0), g4/g5 Q_r0 (1,2),
    # g6/g7 dO_r1 (0,1), g8/g9 Q_r1 (2,0), g10/g11 pads (1,2).
    ROUND_GENS_PER_TILE = 12
    ROUND_STAGES = 3
    # vh_1: K segment ring -- the 32KB resident K tile becomes a 2-slot
    # ring of D128 segments (chunk c lives at slot c % 2).
    K_SEG_STAGES = 2
    ...
    def _specialize_shared_storage(
        ...
        # Byte-exact plan (per CTA), cap 232,448 (vh_1):
        #   stationary Q + dO             131,072
        #   score K segment ring           16,384
        #   round region (3 x 16,384)      49,152
        #   P blocks: local+xchg+landing   12,288
        #   dS image + local+xchg+landing  20,480
        #   softmax stats                     512
        #   mbarriers / holding buf        <= 512
        assert cute.cosize(score_a_layout_staged) <= 32768
        assert cute.cosize(score_b_layout_staged) <= 8192
        ...
        class SharedStorageV2:
            ...
            # vh_1: W17 publishes this after its ring producer_tail so the
            # dQ epilogue may stage over round_buf_a+b.
    ...
    def _load_score_kv_segment(
        ...
        """vh_1: gather ONE D128 segment of the rank-owned score B.

        Same per-row protocol and hoisted index loads as the full-tile
        gather; only the D range and the destination slot differ.  Chunk c
        lands in ring slot c % K_SEG_STAGES.
        """
        ...
        destination_rows = cute.composition(
            destination[None, None, None, slot],
            cute.make_layout((self.N_TILE_CTA, self.K_CHUNK)),
        )
        row_local_n = [
            row_iteration * self.KV_NUM_GROUPS + group_index
    ...
    def kernel(
        ...
        round_buf_c_raw = storage.round_buf_c.data_ptr()
        ...
        # vh_1: the dQ epilogue stages each [H128, D128] round across
        # round_buf_a + round_buf_b (32KB contiguous by declaration order
        # and 1024-alignment of 16384-byte fields), which are dead at
        # epilogue time -- every ring generation of the final tile has
        # been read and released before dq_done commits, and W17 arms the
        # epi-safe barrier after its producer_tail proves it.
        s_dq_epi = cute.make_tensor(
            cute.recast_ptr(
        ...
        t_qt_smem_c, _ = cpasync.tma_partition(
            tma_atom_qt,
            block_coord_vmnk[2],
            a_cta_layout,
        ...
        t_dot_smem_c, _ = cpasync.tma_partition(
            tma_atom_dot,
            block_coord_vmnk[2],
            a_cta_layout,
            cute.group_modes(round_quad[2], 0, 3),
        ...
        quad_fragment_c = dkv_tiled_mma.make_fragment_A(
            round_quad[2]
        )
        p_fragments = (
        ...
        pipe_pds_p = pipeline.PipelineAsyncUmma.create(
            num_stages=1,
            producer_group=pds_commit_group,
            consumer_group=leader_group,
            barrier_storage=storage.pds_mbars.data_ptr() + 2,
            cta_layout_vmnk=cluster_layout_vmnk,
            defer_sync=True,
        )
        ...
        if tidx == Int32(0):
            ...
            cute.arch.mbarrier_init(round_tma_mbars + 2, 1)
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            # --- gather, vh_1: the score B streams as four D128 segments
            # through a 2-slot ring (chunk c -> slot c % 2), so the leader
            # can start S/dP on segment 0 while segment 1 is still landing
            # and the resident 32-KiB K tile disappears.  The per-tile kdq
            # handshake keeps its v8 shape and stays one tile ahead.
            ...
            if tile_count > Int32(0):
                for gather_iter in cutlass.range(tile_count):
                    ...
                    for chunk in cutlass.range_constexpr(self.K_CHUNKS):
                        ...
                        self._load_score_kv_segment(
                            mKV,
                            mTopkIdxs,
                            k_n,
                            token_idx,
                            batch_idx,
                            tile_count - Int32(1) - gather_iter,
                            topk,
                            rank,
                            tidx,
                            chunk,
                    ...
                    # kdq for the tile whose gradients run next; its round
                    # credits free near the end of the previous grads block.
                    self._gather_kdq_v8(
                        mKV,
                        mTopkIdxs,
                        gather_kd_rows_0,
            ...
            pds_p_state = pipeline.make_pipeline_state(
                pipeline.PipelineUserType.Producer,
                1,
            )
            ...
            for loop_iter in cutlass.range(tile_count):
                ...
                pipe_pds_ds.producer_acquire(pds_state)
                pipe_pds_p.producer_acquire(pds_p_state)
                _iket.range_end(
                    math_pds_acq_token,
                    loop_iter,
                )
                ...
                # vf_1 (split publish): the dS side goes out FIRST -- it
                # is what the leader's dQ rounds consume -- so its store,
                # fence and handoff all precede the P store.  The dS DSM
                # send sources the byte-identical (1-rank) half of the
                # whole image (v12 P1b), so there is no dS xchg copy.
                assert t_rs_ds_tile.shape == r_ds_store.shape
                cute.copy(
                    tiled_copy_r2s,
                    r_ds_store,
                ...
                # P side second: its store, DSM send and landing now all
                # overlap the leader's dQ issue.
                if owns_n:
                    cute.copy(
                        tiled_copy_r2s,
                        r_p_store,
            ...
            if is_leader_cta:
                ...
                pds_p_cons = pipeline.make_pipeline_state(
                    pipeline.PipelineUserType.Consumer,
                    1,
                )
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    # vh_1: the K tile streams as four D128 segments, so
                    # S and dP advance together one segment at a time.
                    # Both stationary halves must be ready before the very
                    # first segment (the v12 split readiness -- S gating on
                    # Q alone -- retires with the resident K tile).
                    ...
                    (
                        ...
                        s_prod,
                        dp_prod,
                    ) = self._issue_score_pair_segments_vh1(
                        score_tiled_mma,
                        dp_tiled_mma,
                        t_score,
                        t_score_pp,
                    ...
                    if has_prev:
                        ...
                        # vh_1: retire the tile's two pad generations so
                        # producer and consumer both count twelve.
                        for _pad in cutlass.range_constexpr(2):
                            pipe_round.consumer_wait(round_cons)
                            pipe_round.consumer_release(round_cons)
                            round_cons.advance()
                        pipe_pds_ds.consumer_release(pds_cons)
                        pds_cons.advance()
                        pipe_pds_p.consumer_release(pds_p_cons)
                        pds_p_cons.advance()
            ...
            # vh_1: one rotating TMA phase per ring slot.
            tma_phase = [Int32(0) for _ in range(self.ROUND_STAGES)]
            if tile_count > Int32(0):
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    # vh_1: eight real panel generations (dO_r0 is back
                    # in the ring now that the loan is gone) followed by
                    # two pads, so the tile consumes exactly twelve slots
                    # and the 3-slot map repeats every tile.  Generation g
                    # lands in buffer (2 + g) % 3 -- kdq took slots 0/1 --
                    # and uses the TMA barrier of that same slot.  The
                    # commit still lags one generation so a barrier is
                    # never re-armed while its fill is pending.
                    ...
                    for flat_gen in cutlass.range_constexpr(8):
                        if cutlass.const_expr(flat_gen < 2):
                            ...
                            tensor_kind = 1  # Q_r1
                        h_half = flat_gen % 2
                        slot = (2 + flat_gen) % self.ROUND_STAGES
                        prev_slot = (
                            1 + flat_gen
                        ) % self.ROUND_STAGES
                        ...
                        # The h==rank panel is a byte-identical 16 KiB
                        # slice of this CTA's own stationary tensor: one
                        # local bulk copy.  The h==peer panel keeps the
                        # GMEM TMA (L2-hot).
                        if cutlass.const_expr(tensor_kind == 0):
                            stationary_raw = stationary_do_raw
                            tma_atom_panel = tma_atom_dot
                            t_panel_gmem = t_dot_gmem
                            t_panel_smem = (
                                t_dot_smem_a,
                                t_dot_smem_b,
                                t_dot_smem_c,
                            ...
                            stationary_raw = stationary_q_raw
                            tma_atom_panel = tma_atom_qt
                            t_panel_gmem = t_qt_gmem
                            t_panel_smem = (
                                t_qt_smem_a,
                                t_qt_smem_b,
                                t_qt_smem_c,
                        ...
                        if cutlass.const_expr(self.OWN_HALF_BULK):
                            ...
                            cute.copy(
                                ...
                                t_panel_smem[slot][None, 0],
                                tma_bar_ptr=round_tma_mbars + slot,
                            )
                        if cutlass.const_expr(flat_gen > 0):
                            cute.arch.mbarrier_wait(
                                round_tma_mbars + prev_slot,
                                tma_phase[prev_slot],
                            )
                            tma_phase[prev_slot] = Int32(1) - tma_phase[
                                prev_slot
                            ]
                            with cute.arch.elect_one():
                                pipe_round.producer_commit(round_com)
                    ...
                    # Drain the last in-flight fill (generation 7).
                    last_slot = (2 + 7) % self.ROUND_STAGES
                    cute.arch.mbarrier_wait(
                        round_tma_mbars + last_slot,
                        tma_phase[last_slot],
                    )
                    tma_phase[last_slot] = Int32(1) - tma_phase[
                        last_slot
                    ]
                    with cute.arch.elect_one():
                        pipe_round.producer_commit(round_com)
                    ...
                    # Two pad generations close the tile at twelve so the
                    # slot map is tile-invariant; the leader takes and
                ...
                # vh_1: the dQ epilogue stages over round_buf_a+b, so the
                # ring's own producer publishes the safety edge -- the tail
                # observes every generation's true UMMA source-read
                # completion.  (Field name kept from the retired loan.)
                with cute.arch.elect_one():
                    cute.arch.mbarrier_arrive(
                        loan_epi_safe_mbar,
                        rank,
            ...
            if tidx % Int32(32) == Int32(0):
                ...
                for loop_iter in cutlass.range(tile_count):
                    # vf_1: dS leg first -- it releases the leader's dQ.
                    cute.arch.mbarrier_wait(
                        pds_ready_mbars,
                        ready_phase,
                    )
                    ...
                    pipe_pds_ds.producer_commit(pds_com)

                    # P leg second: overlaps the leader's dQ issue.
                    cute.arch.mbarrier_wait(
                        pds_ready_mbars + 1,
                        ready_phase,
                    )
                    ...
                    pipe_pds_p.producer_commit(pds_com)
                    pds_com.advance()
                    _mbarrier_wait_acquire_cluster(
                        landing_mbars,
                        landing_phase,
                ...
                if tile_count > Int32(0):
                    pipe_pds_ds.producer_tail(pds_com)
                    pipe_pds_p.producer_tail(pds_com)
    ...
    @cute.jit
    def _issue_score_pair_segments_vh1(
        self,
        score_tiled_mma: cute.TiledMma,
        dp_tiled_mma: cute.TiledMma,
        t_score_0: cute.Tensor,
        t_score_1: cute.Tensor,
        t_dp_0: cute.Tensor,
        t_dp_1: cute.Tensor,
        q_fragment: cute.Tensor,
        ...
        kscore_pipeline,
        kscore_state: pipeline.PipelineState,
        s_done_pipeline,
        ...
        """vh_1: issue S(t) and dP(t) interleaved over four K segments.

        The K tile no longer lives in SMEM as a whole: segment c arrives in
        ring slot c % K_SEG_STAGES and must be read by BOTH score GEMMs
        before the gather may refill that slot, so the two accumulator
        chains advance together, one segment at a time.  The chains are
        mutually independent (distinct TMEM accumulators), so interleaving
        is legal and lets the two atoms of a segment issue back-to-back.
        ...
        for chunk in cutlass.range_constexpr(self.K_CHUNKS):
            ...
            if s_producer_state.index == Int32(0):
                for k_block in cutlass.range_constexpr(
                    ...
                    cute.gemm(
                        s_mma,
                        t_score_0,
                        q_fragment[None, None, k_block, chunk],
            ...
            kscore_pipeline.consumer_release(kscore_state)
            kscore_state.advance()
        _iket.range_end(
            score_issue_token,
            issue_seq,
        )
    ...
    def _issue_prev_grads_head_v2(
        ...
        pds_ds_pipeline,
        pds_ds_state: pipeline.PipelineState,
        pds_p_pipeline,
        pds_p_state: pipeline.PipelineState,
        dkv_done_pipeline,
        ...
        vh_1: the round-0 dV operands now arrive through the ring like
        every other quadrant (slots 2 and 0 of the vh_1 slot map), so the
        loan wait is gone and every pass takes a plain round credit.
        """

        # vf_1: only the dS leg gates the block head -- the dQ rounds
        # below read the dS image.  The P leg is waited just before its
        # first consumer (the round-0 dV pass), so the P store, DSM send
        # and landing all overlap the dQ issue.
        pds_ds_pipeline.consumer_wait(pds_ds_state)
        ...
        pds_p_pipeline.consumer_wait(pds_p_state)

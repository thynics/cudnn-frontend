"""DeepSeek Sparse Attention backward, two-CTA SERIAL chain, kq6e.

final_ser_kq6e = final_ser_kq6c + SPLIT-REDUCE PHASE GATE only (single
variable): the reducer's second atomic burst (slot 1) waits for the
NEXT tile's dS publish close before firing, so the REDG.E.ADD.F32x4
storm runs parallel with the grad train instead of over the P/dS math
phase.  Burst 0 keeps riding the S/dP window; slot recycling untouched
(both T2Rs and releases stay in place, values parked in registers).

Mechanism (adjudicated 20260812): the storm's LSU/L2 pressure inflates
every memory-pipe instruction in the math phase (LDL 11->327ns,
LDS 21->95, STSM 26->91, FENCE 36->80) while arithmetic/TMEM stay
flat; tile 0 (no storm yet) runs the whole P in 0.33us vs 0.997
steady.  Gate = local mbar, count MATH_WARPS, armed once per tile at
the dS close; drain(g) waits close #(g+1) via parity (g+1)&1, skipped
on the last generation.  Pacing note: reduce stays within one phase of
the math loop (burst+next T2R ~1.65us << 3.4us to the next close), so
the parity wait cannot alias.

Pre-registered acceptance: P/dS math spans -> tile-0 levels
(T2R+P_MATH ~0.65-0.7 solo); WAIT_KSCORE must stay < 0.3 (score-K
fill shares L2 with the relocated burst); expected wall -2..4%; if
ring/kscore waits absorb the entire gain -> L2-level pressure
conservation proven, escalate to REDG volume reduction.
...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    def _specialize_shared_storage(
        ...
        class SharedStorageV2:
            ...
            reduce_gate_mbar: cute.struct.MemRange[cutlass.Int64, 1]
    ...
    def kernel(
        ...
        reduce_gate_mbar = storage.reduce_gate_mbar.data_ptr()
        ...
        if tidx == Int32(0):
            ...
            # kq6e: reduce phase gate -- math warps arrive once per
            # tile at the dS close; the reducer's second atomic burst
            # waits this gate so the P/dS math phase stays REDG-free.
            cute.arch.mbarrier_init(
                reduce_gate_mbar,
                self.MATH_WARPS,
            )
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            for loop_iter in cutlass.range(tile_count):
                ...
                with cute.arch.elect_one():
                    ...
                    # kq6e: open the reduce gate for this tile.
                    cute.arch.mbarrier_arrive(
                        reduce_gate_mbar,
                    )
                pds_state.advance()
    ...
    def _drain_dkv_v8(
        ...
        # kq6e: split-reduce phase gate.  Burst 0 rode the S/dP window;
        # burst 1 waits the NEXT tile's dS close so its REDG storm runs
        # parallel with the grad train instead of over the P/dS math
        # phase.  TMEM slots are already released above (T2R done,
        # values parked in registers) -- slot recycling untouched.
        # Close #(g+1) completes mbar phase (g+1)&1; skip on the last
        # generation (no following close exists).
        if issue_seq != tile_count - Int32(1):
            cute.arch.mbarrier_wait(
                reduce_gate_mbar,
                (issue_seq + Int32(1)) & Int32(1),
            )

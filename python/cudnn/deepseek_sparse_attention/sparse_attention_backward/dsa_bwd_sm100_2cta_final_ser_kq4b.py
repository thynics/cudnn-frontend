"""DeepSeek Sparse Attention backward, two-CTA SERIAL chain, kq4b.

final_ser_kq4b = final_ser_kq2 + DEFERRED RELEASES only (single
variable): the s_done/dp_done consumer releases (cluster-visible
pipeline ops, measured inside the T2R spans) move to after the P/dS
math blocks respectively.  The S/dP ping-pong slots have an entire
serial chain of slack before reuse, so the deferral is free.
Target: the release tax inside T2R_S 0.352 / T2R_dP 0.256.
    ...
    def kernel(
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            for loop_iter in cutlass.range(tile_count):
                ...
                cute.arch.fence_view_async_tmem_load()
                # kq4b: the consumer release (cluster-visible pipeline
                # op) moves out of the T2R critical section to after
                # the P math -- the S ping-pong has a whole serial
                # chain of slack before S(t+1) needs the slot.
                softmax_scale_log2_e = scale_softmax * Float32(
                    math.log2(math.e)
                )
                ...
                cute.arch.fence_view_async_tmem_load()
                # kq4b: dP release deferred to after the dS math (see
                # the P-phase note).
                if cutlass.const_expr(self.SOFTMAX_GROUPED_STATS):
                    for h_group in cutlass.range_constexpr(4):
                        delta = softmax_stats[

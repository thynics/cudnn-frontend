...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    17 feeds and commits the round ring from the stationary panels; warp
    18 relays P/dS across the cluster; warp 19 issues dQ.

    Steady-state schedule per KV tile (rotated): W16 issues S(t), dP(t),
    then tile t-1's eight dV/dK passes, while W19 issues dQ(t) as soon as
    local dS and the two kdq rounds are ready, independently of W18's DSM
    landing waits. The kdq rendezvous hands the gather-filled dQ-A
    images to the ring through a count-128 hardware arrival; the score K tile is
    ...
    def kernel(
        ...
        # Keep aug6's exact 48/128/120/64 split for a resource-matched W19
        # ownership ablation (pool-neutral vs 48/128/128/48).
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            if is_leader_cta:
                ...
                # W19 owns g0/g1 (the two dQ rounds). W16 starts each tile at
                # g2 and consumes only the six dV/dK generations.
                round_cons.advance()
                round_cons.advance()
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    if has_prev:
                        ...
                        # Six dV/dK generations land on the next tile's g0;
                        # skip W19's g0/g1 ownership and resume at g2.
                        round_cons.advance()
                        round_cons.advance()
            ...
            if tile_count > Int32(0):
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    for flat_gen in cutlass.range_constexpr(6):
                        ...
                        # Keep two fills in flight: after issuing q, publish
                        # q-1 from the alternating raw completion barrier.
                    ...
                    # Drain g7, the final fill on barrier 1. W17 now owns all
                    # g2..g7 commits, leaving W19 as a pure dQ consumer.
                    cute.arch.mbarrier_wait(
                        round_tma_mbars + 1,
                        tma_phase_1,
                    )
            ...
            for loop_iter in cutlass.range(tile_count):
                ...
                # W18 is lane-0 driven, but its next loop iteration and the
                # common TMEM tail are warp collectives. Preserve the original
                # per-tile reconvergence even though dQ now runs on W19.
                cute.arch.sync_warp()
            ...
            dq_ds_observer = pipeline.make_pipeline_state(
                pipeline.PipelineUserType.Consumer,
                1,
            )
            ...
            for loop_iter in cutlass.range(tile_count):
                # This role is whole-warp and consumer-only. On the leader CTA
                # it consumes g0/g1 as soon as local dS is published,
                # independently of W18's DSM work and W17's g2..g7 commits.
                if is_leader_cta:
                    pipe_ds.consumer_wait(dq_ds_observer)
                    ...
                    # Observe dS FULL without releasing it. W16 remains the
                    # sole pipe_ds releaser after the last dK source read.
                    dq_ds_observer.advance()
                    for _ in cutlass.range_constexpr(
                        self.ROUND_GENS_PER_TILE - self.D_ROUNDS
                    ):
                        dq_round_cons.advance()
                    # Hand the final dQ generation to the math epilogue
                    # before waiting for this tile's unrelated g2..g7 TMA
                    # completions.
    ...
    def _issue_prev_dkv_head_v3(
        ...
        W19 owns dQ and g0/g1. W16 enters here at g2; the separate

"""vk_6: vk_5 + K2-swap -- the loan carries kdq; dO_r0 rides the ring.

The vc_2 loan (score_kv's dead window between K generations) carried the
two token-stationary dO_r0 panels; the kdq images (dQ's A operand, two
16KB [N64 x D128] tiles) rode ring gens g0/g1 behind the W17/gather
rendezvous that saturated the pacer chain (ROUTE_K 2.04us).  vk_6 swaps
the cargo:

  * the loan gen now carries BOTH kdq images (2 x 16KB == the 32KB
    borrow window exactly); the gather warps fill them with the same
    sparse per-row protocol (_fill_kdq_pair_v8), draining their own
    cp.async group -- no rendezvous, no W17 involvement.  The dO
    "recommit" trick on the tail tile dies (kdq is tile-varying): the
    tail gen is a real fill for grads tile 0.
  * dQ consumes the loan gen (one kscore wait for both rounds, released
    UMMA-tracked after both issues); its A fragments rebase from the
    ring slots to the score_kv images.  FIFO order holds: per window
    the leader consumes [K(t) at S/dP, loan(t-1) at dQ].
  * dO_r0 becomes two ordinary ring gens: W17's train grows 6 -> 8
    (order dO_r0, Q_r0, dO_r1, Q_r1 -- matching pass order), W19 relays
    8 commits, and ROUTE_K disappears from W17's chain entirely.
  * Ledgers are count-invariant: ring 8 gens/tile with 8 leader
    consumptions (passes 0-7); kscore 2 gens/tile with 2 consumptions
...
period -- the tightest edge of this design, and the first suspect if
lengths/holes regress or the period fails to move).
...
the wait+commit conversion moves to W19 -- the one fully idle warp:
  ...
  and moves on (no wait, no commit; in vk_6 W17 commits nothing at all).
...
on W17 unchanged in vk_5; retired entirely in vk_6 (the cargo swap).  Producer arrives per gen stay one-elect-per-CTA (count 2);
...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    * The gradient GEMMs of tile p are issued inside tile t=p-1's S/dP
    ...
    def _issue_dq_rounds_v2(
        ...
        vk_6 (K2-swap): both K_dQ images ride ONE kscore loan generation
        (they share the 32KB score_kv borrow window), so a single
        consumer wait covers both rounds and one UMMA-tracked release
        follows the second issue -- it is what lets the gather warps
        start the next K fill.
        """
        ...
        for round_index in cutlass.range_constexpr(self.D_ROUNDS):
            ...
            if cutlass.const_expr(
                round_index == self.D_ROUNDS - 1
            ):
                cute.arch.fence_view_async_tmem_store()
                kscore_pipeline.consumer_release(
                    kscore_consumer_state
                )
    ...
    def kernel(
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            gather_kd_rows_0 = self._kd_round_rows_v2(loan_kd[0])
            gather_kd_rows_1 = self._kd_round_rows_v2(loan_kd[1])
            if tile_count > Int32(0):
                ...
                if tile_count == Int32(1):
                    ...
                    # the loan generation -- which now carries the kdq
                    # images for grads tile (tile_count - score_iter).  The second generation is
                    ...
                    for score_iter in cutlass.range(
                        ...
                        if score_iter == tile_count - Int32(1):
                            # vk_6: the dO recommit trick dies with the
                            # cargo swap (kdq is tile-varying) -- the tail
                            # generation is a real fill for grads tile 0.
                            pipe_kscore.producer_acquire(gather_state)
                            self._fill_kdq_pair_v8(
                                mKV,
                                mTopkIdxs,
            ...
            if tile_count > Int32(0):
                ...
                for loop_iter in cutlass.range(tile_count):
                    # vk_6 (K2-swap): the kdq rendezvous is gone -- the
                    # images ride the kscore loan, filled by the gather
                    # warps in their own slack.  The train grows to eight
                    # gens with the ex-loan dO_r0 pair at its head.

                    # g0..g7: dO_r0, Q_r0, dO_r1, Q_r1 panel fills,
                    # pipelined depth 2, matching the leader's pass order.
                    ...
                    for flat_gen in cutlass.range_constexpr(8):
                        if cutlass.const_expr(flat_gen < 2):
                            grad_round = 0
                            tensor_kind = 0  # dO_r0 (vk_6: ex-loan cargo)
            ...
            w19_phase_1 = Int32(0)
            for loop_iter in cutlass.range(tile_count):
                # vk_6: all eight gens are TMA/bulk-armed quadrants now
                # (the kdq pair left the ring for the kscore loan).
                for flat_gen in cutlass.range_constexpr(8):
    ...
    def _issue_prev_grads_head_v2(
        ...
        # dQ both rounds back-to-back off the kscore loan; its single
        # release is what lets the gather warps start the next K fill.
        kscore_consumer_state = self._issue_dq_rounds_v2(
            dq_tiled_mma,
            t_dq_0,
            t_dq_1,
            dq_kd_fragment_a,
        ...
        # vk_6: the round-0 dO pair rides the ring now (gens g0/g1),
        # filled by W17's train like every other quadrant.

"""DeepSeek Sparse Attention backward, two-CTA rubin_1 capacity build.

rubin_1 = the ``final`` (vk-lineage) machine with the SMEM capacity
surgery for the Rubin generation (assumed 328 KB/SM, per-CTA cap taken
as 334,848 B; correct against the real toolchain when it lands):

  1. Loan retirement: the round-0 dO quadrants no longer lodge in
     score_kv -- they are ordinary round-ring generations (8 -> 10
     gens/tile), so kscore returns to one K generation per tile and
     LOAD_K(t+1) is gated by dP(t) release instead of by grads(t-1)
     reading the borrowed dO pair.
  2. Round ring depth 2 -> 5 (10 mod 5 == 0 keeps the static
     gen->slot map tile-invariant): the supply ring gains real
     lookahead, attacking the per-generation credit platform.
  3. P/dS publication double face (tile parity): math(t+1) publishes
     into the other face while grads(t) still read theirs, removing
     the single-stage pds anchor edge.

``DSA_RUBIN1_B200_COMPAT=1`` degrades to (ring depth 2, single P/dS
face, 232,448 B cap) so the generalized machine can be regression
tested on B200; the default profile targets Rubin and will not launch
on SM100.
...
through the round ring.  dQ accumulates in TMEM across all KV tiles and
stores through a staged TMA epilogue; dKV drains per tile via
...
# Profile switch, read once at import time (the harness passes env
# through; runtime toggling would violate the compile-time struct
...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    """Two-CTA rubin_1 kernel: 20 warps per CTA, five CG2 GEMMs per KV tile.
    ...
    the ring through a count-128 hardware arrival.  rubin_1: the score
    K tile is a single-tenant kscore generation (released after dP;
    the round-0 dO quadrants are ring generations g2/g3, not a
    score_kv loan), the ring carries 10 generations per tile at depth
    ROUND_STAGES, and P/dS publish through PDS_FACES parity faces.
    dQ stays TMEM-resident across all tiles and stores through a
    two-round staged TMA epilogue.
    """
    ...
    # rubin_1: dO_r0 h0/h1 join the ring (loan retired) -> 10 gens/tile.
    # Ring slots are named struct fields with a static gen->slot binding,
    # so ROUND_GENS_PER_TILE % ROUND_STAGES == 0 is a hard phase law.
    # The phase law admits {2, 5, 10}; this implementation builds storage
    ...
    def _specialize_shared_storage(
        ...
        if self.ROUND_STAGES == 5 and self.SLIM51:
            ...
            class SharedStorageV2:
                ...
                dkv_done_mbars: cute.struct.MemRange[cutlass.Int64, 4]
                dq_done_mbars: cute.struct.MemRange[cutlass.Int64, 2]
                # Raw barriers ([P, dS] per face where doubled).
                stationary_tma_mbars: cute.struct.MemRange[cutlass.Int64, 2]
                stationary_ready_mbar: cute.struct.MemRange[cutlass.Int64, 2]
        ...
        # the exact struct size against a hand computation).
    ...
    @cute.jit
    def _issue_dkv_pass_faced(
        self,
        dkv_tiled_mma: cute.TiledMma,
        t_dkv: cute.Tensor,
        a_fragment: cute.Tensor,
        b_fragment_f0: cute.Tensor,
        b_fragment_f1: cute.Tensor,
        use_face1: cutlass.Boolean,
        accumulate: cutlass.Constexpr[bool],
    ) -> None:
        """One dV/dK pass whose B operand lives in a parity face.

        The face branch wraps only the pure-side-effect GEMM issue (v7
        ping-pong rule); with a single face it is compile-time pruned.
        """

        if cutlass.const_expr(self.PDS_FACES == 1):
    ...
    def _issue_dq_rounds_v2(
        ...
        ds_fragment_f0: cute.Tensor,
        ds_fragment_f1: cute.Tensor,
        use_face1: cutlass.Boolean,
        accumulate: cutlass.Boolean,
        round_pipeline,
        ...
        can begin the panel refills.  The dS whole image lives in the P/dS
        face of the gradients' tile parity: the face branch wraps only the
        pure-side-effect GEMM helper (v7 ping-pong rule -- the pipeline
        state never crosses the dynamic branch).
        """
    ...
    def kernel(
        ...
        if cutlass.const_expr(self.ROUND_STAGES == 5):
            round_buf_raws = (
                ...
                storage.round_buf_3.data_ptr(),
                storage.round_buf_4.data_ptr(),
            )
            round_bufs = (
                storage.round_buf_0,
                storage.round_buf_1,
        ...
        # P/dS publication faces.  Face f serves tiles t with
        # t % PDS_FACES == f; every view below exists once per face.
        if cutlass.const_expr(self.PDS_FACES == 2):
            p_blocks_storages = (
                storage.p_blocks_0,
                storage.p_blocks_1,
            )
        ...
        t_dot_smem_slots = (t_dot_smem_0,) + tuple(
            cpasync.tma_partition(
                tma_atom_dot,
                block_coord_vmnk[2],
                a_cta_layout,
        ...
        # Per-face B operands.  In the single-face profile the face-1
        # names alias face 0 so every helper keeps a uniform signature;
        # the face-1 arms are compile-time pruned there.
        dq_ds_fragment_f0 = dq_tiled_mma.make_fragment_B(
            ds_image_f[0]
        )
        p_fragments_f0 = (
        ...
        if cutlass.const_expr(self.PDS_FACES == 2):
            dq_ds_fragment_f1 = dq_tiled_mma.make_fragment_B(
                ds_image_f[1]
            )
            p_fragments_f1 = (
                dkv_tiled_mma.make_fragment_B(p_blocks_f[1][0]),
                dkv_tiled_mma.make_fragment_B(p_blocks_f[1][1]),
            )
            ds_fragments_f1 = (
                dkv_tiled_mma.make_fragment_B(ds_blocks_f[1][0]),
                dkv_tiled_mma.make_fragment_B(ds_blocks_f[1][1]),
            )
        else:
            dq_ds_fragment_f1 = dq_ds_fragment_f0
            p_fragments_f1 = p_fragments_f0
            ds_fragments_f1 = ds_fragments_f0
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            if tile_count > Int32(0):
                # rubin_1: kscore is single-tenant -- exactly one K
                # generation per tile.  acquire(t+1) is gated by the
            ...
            for loop_iter in cutlass.range(tile_count):
                ...
                # Publish P/dS with stmatrix into this tile's parity face
                # (face == pds_state.index; both start at 0 and advance
                # once per tile).  Each pair of warps owns one N32 half,
                # so the owns_n branch inside the helper is warp-uniform;
                # the face branch wraps only pure copies (v7 rule).
                r_p_store = thread_copy_r2s.retile(r_p)
                ...
                if cutlass.const_expr(self.PDS_FACES == 1):
                    ...
                    if pds_state.index == Int32(1):
                        self._publish_pds_face(
                            tiled_copy_r2s,
                            r_p_store,
                            r_ds_store,
                            owns_n,
                            t_rs_p_local_tile_f[1],
                            t_rs_ds_local_tile_f[1],
                            t_rs_p_xchg_tile_f[1],
                            t_rs_ds_tile_f[1],
                        )
                    else:
                        self._publish_pds_face(
                ...
                if cutlass.const_expr(self.PDS_FACES == 1):
                    ...
                    cute.arch.mbarrier_arrive(
                        pds_ready_mbars + pds_state.index,
                        rank,
                    )
                pds_state.advance()
            ...
            if is_leader_cta:
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    if has_prev:
                        ...
                        # Face and per-face relay phase of grads(t):
                        # face = t % PDS_FACES; each face's raw relay
                        # mbars are armed once per PDS_FACES tiles.
                        grads_face = issue_seq % Int32(
                            self.PDS_FACES
                        )
                        grads_face1 = grads_face == Int32(1)
                        grads_relay_base = (
                            relay_mbars + Int32(2) * grads_face
                        )
                        grads_relay_phase = (
                            issue_seq // Int32(self.PDS_FACES)
                        ) % Int32(2)
                        (
                            round_cons,
                            dkv_prod,
            ...
            if tile_count > Int32(0):
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    # g2..g9: dO_r0, Q_r0, dO_r1, Q_r1 panel fills,
                    # pipelined at ring depth ROUND_STAGES (rubin_1: the
                    # round-0 dO pair rides the ring; the loan is gone).
                    # All 8 panel commits are delegated to W19.  Re-arm
                    # safety: gen G's arm follows producer_acquire(G),
                    # i.e. the leader's release of gen G-ROUND_STAGES,
                    # whose consumption ordered after W19's wait cleared
                    # this slot's previous completion (for slots 0/1 the
                    # chain passes through the mbar-less kdq rendezvous
            ...
            if tidx % Int32(32) == Int32(0):
                ...
                for loop_iter in cutlass.range(tile_count):
                    # face = t % PDS_FACES; each face's raw barriers flip
                    # once per PDS_FACES tiles.
                    relay_face = loop_iter % Int32(self.PDS_FACES)
                    face_phase = (
                        loop_iter // Int32(self.PDS_FACES)
                    ) % Int32(2)
                    if cutlass.const_expr(self.PDS_FACES == 1):
                        ...
                        if relay_face == Int32(1):
                            self._relay_send_pds_face(
                                ...
                                landing_mbars + 2,
                                face_phase,
                                p_xchg_raw_f[1].iterator,
                                p_block_raw_ptrs_f[1][0],
                                p_block_raw_ptrs_f[1][1],
                                ds_image_raw_f[1],
                                ds_block_raw_ptrs_f[1][0],
                                ds_block_raw_ptrs_f[1][1],
                            )
                        else:
                            self._relay_send_pds_face(
                                rank,
            ...
            # tile (evenly only over ROUND_GENS_PER_TILE gens), so the
            # per-slot named locals are the only safe carrier.  The
            # COMPAT profile only ever touches slots 0/1; its reads of
            # the higher slots are const_expr-pruned, so the locals are
            # only bound in the 5-slot profile.
            w19_phase_0 = Int32(0)
            w19_phase_1 = Int32(0)
            if cutlass.const_expr(self.ROUND_STAGES == 5):
                w19_phase_2 = Int32(0)
            ...
            for loop_iter in cutlass.range(tile_count):
                ...
                for flat_gen in cutlass.range_constexpr(8):
                    ...
                    if cutlass.const_expr(slot == 0):
                        ...
                        w19_phase_4 = Int32(1) - w19_phase_4
                    with cute.arch.elect_one():
    ...
    def _issue_prev_grads_head_v2(
        ...
        dq_ds_fragment_f0: cute.Tensor,
        dq_ds_fragment_f1: cute.Tensor,
        quad_fragment_dv_h0: cute.Tensor,
        quad_fragment_dv_h1: cute.Tensor,
        quad_fragment_dk_h0: cute.Tensor,
        quad_fragment_dk_h1: cute.Tensor,
        p_fragment_f0_0: cute.Tensor,
        p_fragment_f0_1: cute.Tensor,
        p_fragment_f1_0: cute.Tensor,
        p_fragment_f1_1: cute.Tensor,
        ds_fragment_f0_0: cute.Tensor,
        ds_fragment_f0_1: cute.Tensor,
        ds_fragment_f1_0: cute.Tensor,
        ds_fragment_f1_1: cute.Tensor,
        use_face1: cutlass.Boolean,
        ...
        rubin_1: every A operand is a ring generation (the score_kv loan
        is retired -- dO_r0 h0/h1 are gens g2/g3).  All P/dS B operands
        and the relay barriers belong to the gradients' parity face,
        selected by ``use_face1`` / ``relay_base``.  The tail keeps
        v12's early dQ completion commit; the epi_safe barrier
        (published after the kscore producer tail) protects score_kv
        from the dQ epilogue until dP of the last tile has truly read K.
        """
        ...
        # dV round-0: the dO_r0 quadrant pair from the ring.
        round_pipeline.consumer_wait(round_consumer_state)
        self._issue_dkv_pass_faced(
            dkv_tiled_mma,
            t_dkv_0,
            quad_fragment_dv_h0,
            p_fragment_f0_0,
    ...
    def _issue_prev_grads_tail_v2(
        ...
        generation and commits it at the end.  A operands are the ring
        generations g6..g9; B operands live in the gradients' parity
        face.
        """

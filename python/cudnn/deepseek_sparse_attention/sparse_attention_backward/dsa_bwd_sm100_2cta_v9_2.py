...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    """v9.2: peer-push quadrant supply; CONFIG B reverted (bisect).
    ...
    * dKV drain: v9's CONFIG B (bulk-reduce staging) is REVERTED here --
      v9 measured 24.63 ms (2.06x v8): the 512-byte cp.reduce engine ops
      cost microseconds of engine time per tile, serialized into the pds
      ring through the stage gate and contending with the peer-push
      traffic.  v9.2 restores the v8 red.global.v4 drain verbatim (runs
      in reduce-role slack, off every ring) and retires the four
      zero-information S/dP ACQUIRE/PUBLISH spans so the IKET name count
      is exactly 31 (the encoding limit) at the source.
    ...
    * The gradient GEMMs of tile p are issued inside tile t=p-1's S/dP
    ...
    def _specialize_shared_storage(
        ...
        class SharedStorageV2:
            ...
            # v9: peer-push credit (count-1, provable lead <= 1).
    ...
    def _issue_score_v2(
        ...
        # v9.2: the S/dP ACQUIRE and PUBLISH spans are retired (near-zero
    ...
    def _drain_dkv_v9(
        ...
        """Drain both rank-owned dKV slots (v8 atomic path, reinstated).

        v9's CONFIG B (bulk-reduce staging through the dS image) caused
        the 24.6 ms regression: the 512-byte cp.reduce engine ops cost
        several microseconds of engine time per tile, serialized into
        the pds ring through the stage gate and contending with the
        peer-push/quadrant traffic on the same async engine.  v9.2
        restores the v8 red.global.v4 drain verbatim -- it runs entirely
        inside the reduce role's slack, off every ring.
        """

"""DeepSeek Sparse Attention backward, two-CTA SERIAL chain, kq4d.

final_ser_kq4d = final_ser_kq2 + EARLY XCHG SEND only (single
variable): the P DSM source (p_xchg) is written by exactly the two
non-owner math warps, so a narrow count-2 close (p_xchg_ready) lets
W18 launch the 4KiB send while the owner warps are still publishing;
the DSM flight overlaps the full-close tail.  W18 re-checks the full
p_ready before its relay-0 arrive, so the downstream certification
contract (relay0 = both CTAs fully published + both landings) is
unchanged.  Upgrade path (pending a layout-contiguity probe): split
the send per non-owner warp (2 x 2KiB, incremental tx accounting).
...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    def _specialize_shared_storage(
        ...
        class SharedStorageV2:
            ...
            p_xchg_ready_mbar: cute.struct.MemRange[cutlass.Int64, 1]
    ...
    def kernel(
        ...
        p_xchg_ready_mbar = storage.p_xchg_ready_mbar.data_ptr()
        ...
        if tidx == Int32(0):
            ...
            # kq4d: the P DSM source (p_xchg) is written by exactly
            # the two non-owner math warps; this narrower close lets
            # W18 launch the send while the owner warps are still
            # publishing (flight overlaps the full-close tail).
            cute.arch.mbarrier_init(p_xchg_ready_mbar, 2)
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            for loop_iter in cutlass.range(tile_count):
                ...
                cute.arch.fence_view_async_shared()
                # kq4d: narrow xchg close -- non-owner warps certify
                # the DSM source early (warp-level, elected arrive).
                if not owns_n:
                    cute.arch.sync_warp()
                    with cute.arch.elect_one():
                        cute.arch.mbarrier_arrive(
            ...
            if relay_lane == Int32(0):
                ...
                p_xchg_phase = Int32(0)
                for loop_iter in cutlass.range(tile_count):
                    # kq4d: launch the DSM off the narrow xchg close;
                    # the full p_ready is re-checked before the relay-0
                    # arrive so the certification contract is intact.
                    cute.arch.mbarrier_wait(
                        p_xchg_ready_mbar,
                        p_xchg_phase,
                    )
                    p_xchg_phase = Int32(1) - p_xchg_phase
                    cute.arch.mbarrier_arrive_and_expect_tx(
                        landing_mbars,
                    ...
                    # kq4d: full local close before certifying relay 0
                    # (usually already flipped by now).
                    cute.arch.mbarrier_wait(
                        p_ready_mbars,
                        p_ready_phase,
                    )

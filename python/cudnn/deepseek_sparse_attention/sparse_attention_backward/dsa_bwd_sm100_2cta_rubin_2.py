"""DeepSeek Sparse Attention backward, two-CTA rubin_2 full-dO-panel build.

rubin_2 = rubin_1 with the dO supply machinery bought out by Rubin's
SMEM headroom (design: RUBIN2_dO全量面板设计_20260810.md).  Rubin-only,
single profile -- every rubin_1 env switch (COMPAT / E3PAD / REG_K1A /
SPIN_K2) is deleted; the K1a register layout (64/64/128/120) and the
334,848 B cap are the only layout.

  1. dO full panel: ``stationary_do`` grows 64 KB -> 128 KB per CTA.
     The first 32,768 elements stay the rank-owned H64 sub-panel
     (byte-identical to rubin_1: the dP A fragment reads it at offset
     0 through the same score_a view).  The second 32,768 elements are
     the dV-A pantry: four [D128, H64] quadrant slots in
     dkv_a_layout_staged form, holding THIS CTA's M-half of dO.T for
     the four dV passes (r, h), filled once per token by the same
     quadrant TMA that used to feed the round ring every tile.
  2. dV-A zero-copy: the four dV passes consume permanent
     ``make_fragment_A`` views of the pantry slots -- no ring waits,
     no releases, no per-tile dO transport of any kind.  Each pantry
     slot is a fixed-address clone of a rubin_1 round-ring slot (same
     layout object, same TMA partition construction), so the CG2
     shared-descriptor rank-same-address rule is satisfied exactly as
     the ring satisfied it: same local address in both CTAs, each CTA
     holding its own M-half bytes.
     NOTE (implementation ruling, see RUBIN2_IMPL_NOTES_20260811.md):
     the design memo's "identical bytes in both CTAs + static recast
     view" primary form is refuted by the same-address rule (rank 1
     would re-read rank 0's chunk); the pantry is the provable
     realization of the same byte budget and the same steady state.
  3. Supply ring 10 -> 6 generations per tile at depth 3
     (2 kdq + 4 Q quadrants; 6 mod 3 == 0 keeps the static gen->slot
     map tile-invariant).  W17 issues 4 panel fills per tile instead
     of 8; W19 waits 4 completions instead of 8.
  4. P/dS publication is single-face (PDS_FACES == 1); the rubin_1
     double-face arms are deleted, not disabled.
...
the score K tile is gathered per KV tile; dQ-A images and Q.T quadrants
stream through the round ring.  dQ accumulates in TMEM across all KV
...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    """Two-CTA rubin_2 kernel: 20 warps per CTA, five CG2 GEMMs per KV tile.
    ...
    the ring through a count-128 hardware arrival.  rubin_2: the score
    ...
    stationary dO panel (zero per-tile dO transport); the ring carries
    6 generations per tile (2 kdq + 4 Q quadrants) at depth 3; P/dS
    publish through a single face.  dQ stays TMEM-resident across all
    ...
    # rubin_2: the four dO quadrant generations leave the ring (they
    # are pantry views now) -> 6 gens/tile.  Ring slots are named
    # struct fields with a static gen->slot binding, so
    # ROUND_GENS_PER_TILE % ROUND_STAGES == 0 is a hard phase law
    # (6 % 3 == 0: the gen->slot map is tile-invariant).
    ROUND_GENS_PER_TILE = 6
    ROUND_STAGES = 3
    ...
    # g2..g5).  Consumption order == fill order (FIFO):
    #   Q_r0 h0/h1, Q_r1 h0/h1  ->  slots (2%3, 3%3, 4%3, 5%3).
    ...
    def _specialize_shared_storage(
        ...
        # 3, single P/dS face, 128 KB stationary_do (dP sub-panel +
        # dV-A pantry).  cute.struct bodies cannot carry conditional
        ...
        class SharedStorageV2:
            ...
            round_mbars: cute.struct.MemRange[cutlass.Int64, 6]
            ...
            round_tma_mbars: cute.struct.MemRange[cutlass.Int64, 3]
        ...
        # neighbourhood: ~308,736 B (~309 KB) per the rubin_2 design
        # byte ledger.
    ...
    def kernel(
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            if tile_count > Int32(0):
                ...
                # CTAs via the count-2 leader mbar.
                cute.arch.mbarrier_wait(
                    stationary_tma_mbars,
                    Int32(0),
                )
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    # g2..g5: Q_r0 h0/h1, Q_r1 h0/h1 panel fills,
                    # pipelined at ring depth ROUND_STAGES (rubin_2:
                    # the dO quadrants live in the pantry; only Q
                    # rides the ring).  All 4 panel commits are
                    # delegated to W19.  Re-arm safety: gen G's arm
                    # follows producer_acquire(G), i.e. the leader's
                    # release of gen G-ROUND_STAGES, whose consumption
                    # ordered after W19's wait cleared this slot's
                    # previous completion (for slots 0/1 the chain
                    # passes through the mbar-less kdq rendezvous
                    # commits of g0/g1) -- the mbar is never re-armed
                    # while pending.
            ...
            # One raw-mbar phase per ring slot: a slot's mbar flips once
            # per arm and the panel walk hits slots unevenly within a
            # tile (slot 2 twice, slots 0/1 once -- evenly only over
            # ROUND_GENS_PER_TILE gens), so the per-slot named locals
    ...
    def _issue_prev_grads_head_v2(
        ...
        rubin_2: the dQ K images (g0/g1) and the Q_r0 quadrants (g2/g3)
        are ring generations; the dV round-0 A operands are the
        permanent pantry fragments ``dv_fragment_h0/h1`` -- no ring
        wait or release surrounds their issue.  All P/dS B operands
        live in the single face.  The tail keeps v12's early dQ
    ...
    def _issue_prev_grads_tail_v2(
        ...
        rubin_2: the dV round-1 A operands are the permanent pantry
        fragments (no ring machinery); the dK round-1 A operands are
        the ring generations g4/g5 (Q_r1 h0/h1).  B operands live in
        the single P/dS face.
        """
        ...
        # dK round-1: the Q_r1 quadrant pair from the ring (g4/g5).
        round_pipeline.consumer_wait(round_consumer_state)
        self._issue_dkv_pass_v2(
            dkv_tiled_mma,
            t_dkv_1,

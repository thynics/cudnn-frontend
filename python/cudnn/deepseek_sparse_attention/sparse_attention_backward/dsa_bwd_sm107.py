"""DeepSeek Sparse Attention backward, Rubin SM107 two-CTA implementation.
...
class FlashAttentionDSABackwardSm100TwoCTA(FlashAttentionDSABackwardSm100):
    ...
    TMEM_COLUMNS = 512
    # Rubin exposes (328 - 1) KiB through the oversized shared-memory mode.
    ...
    def convert_canonical(
        ...
        if seq_id < seqlen:
            for d_block in cutlass.range_constexpr(self.D_HEAD // self.num_threads_D_convert):
                d = d_block * self.num_threads_D_convert + tidx
                mdKV[d, seq_id, (0, batch_idx)] = self.element_dtype(mdKV_acc[d, seq_id, (0, batch_idx)])
    ...
    def _issue_four_chunks(
        ...
        for flat_k_block in cutlass.range_constexpr(self.K_CHUNKS * k_blocks_per_chunk):
    ...
    def _load_grad_k(
        ...
        d_offset = round_index * self.D_TILE_CLUSTER + rank * self.D_TILE_CTA
        for row_iteration in cutlass.range_constexpr(rows_per_group):
    ...
    def _compute_pd_from_tmem(
        ...
        if tidx < self.MATH_THREADS_PER_CTA:
            ...
            for value_index in cutlass.range_constexpr(cute.size(r_score)):
                head = Int32(cute.get(score_coordinates[value_index], mode=[0]))
                n_index = Int32(cute.get(score_coordinates[value_index], mode=[1]))
                p_value = cute.math.exp2(
                    r_score[value_index] * softmax_scale_log2_e + scaled_lse[head, (token_idx, batch_idx)],
                    fastmath=True,
                )
                ds_value = (r_dp[value_index] + sum_odo[head, (token_idx, batch_idx)]) * p_value * scale_softmax
                ...
                ds_scratch[scratch_offset] = self.element_dtype(ds_value)
    ...
    def _issue_dv(
        ...
        mma.set(tcgen05.Field.ACCUMULATE, False)
        for k_block in cutlass.range_constexpr(cute.size(dout_fragment, mode=[2])):
            cute.gemm(
                mma,
                t_dkv,
    ...
    def _issue_dq(
        ...
        for k_block in cutlass.range_constexpr(cute.size(kv_fragment, mode=[2])):
            cute.gemm(
                mma,
                t_dq,
    ...
    def _atomic_dkv_from_tmem(
        ...
        if tidx < self.MATH_THREADS_PER_CTA:
            ...
            for value_index in cutlass.range_constexpr(cute.size(thread_values)):
                ...
                n_index = Int32(cute.get(thread_coordinates[value_index], mode=[1]))
                ...
                if topk_slot < topk:
                    ...
                    if kv_index >= 0:
                        d_index = round_index * self.D_TILE_CLUSTER + d_in_round
                        destination_ptr = mdKV_acc.iterator + d_index * mdKV_acc.stride[0] + kv_index * mdKV_acc.stride[1]
    ...
    def _store_dq_epi_tma_v12(
        ...
        """Store one rank-owned dQ round via split T2R and one bulk TMA.

        Drain the D128 x H128 accumulator as two 64-register/thread
        epilogue subtiles.  Both subtiles fill the same complete 32 KiB
        BF16 SMEM image, so the existing one-barrier/one-TMA protocol and
        round-to-round source-completion contract remain unchanged.
        """

        if mtx < self.MATH_THREADS_PER_CTA:
            dq_tile = t_dq[((None, None), 0, 0)]
            coordinate_tile = rank_coordinates[((None, None), 0, 0)]
            epi_tile = (
                self.D_TILE_CTA,
                self.H_TILE_CLUSTER,
            )
            dq_epi = cute.zipped_divide(dq_tile, epi_tile)
            coordinate_epi = cute.zipped_divide(
                coordinate_tile,
                epi_tile,
            )
            split_tmem_load = cute.make_copy_atom(
            ...
            thread_source = thread_t2r.partition_S(dq_epi)
            thread_coordinates = thread_t2r.partition_D(coordinate_epi)
            assert cute.size(thread_source, mode=[1]) == 2
            for subtile_index in cutlass.range(
                cute.size(thread_source, mode=[1]),
                unroll_full=True,
            ):
                source_subtile = thread_source[
                    None,
                    subtile_index,
                    0,
                ]
                coordinate_subtile = thread_coordinates[
                    None,
                    subtile_index,
                    0,
                ]
                thread_values = cute.make_rmem_tensor(
                    thread_coordinates[None, 0, subtile_index].shape,
                    self.acc_dtype,
                )
                assert cute.size(thread_values) == 64
                cute.copy(tiled_t2r, source_subtile, thread_values)
                cute.arch.fence_view_async_tmem_load()
                for value_index in cutlass.range_constexpr(cute.size(thread_values)):
                    d_in_round = Int32(
                        cute.get(
                            coordinate_subtile[value_index],
                            mode=[0],
                        )
                    )
                    head = Int32(
                    ...
                    s_dq_epi[head, local_d] = self.element_dtype(thread_values[value_index])
        cute.arch.fence_view_async_shared()
    ...
    def _exchange_peer_n32(
        ...
        if tidx == 0:
            ...
            _cpasync_bulk_s2cluster(
                source_scratch.iterator + peer_rank * self.QUADRANT_ELEMENTS,
                inbox.iterator,
                remote_full,
                self.QUADRANT_BYTES,
                peer_rank,
    ...
    def _materialize_dkv_b(
        ...
        if tidx < self.MATH_THREADS_PER_CTA:
            for slot in cutlass.range_constexpr(32):
                linear_index = tidx + slot * self.MATH_THREADS_PER_CTA
                source_coordinate = cute.idx2crd(
                    linear_index,
                    source_partition.shape,
                )
                logical_coordinate = coordinate_partition[source_coordinate]
                n_index = Int32(cute.get(logical_coordinate, mode=[0]))
                head = Int32(cute.get(logical_coordinate, mode=[1]))
                local_h = head % self.H_TILE_CTA
                value = self.element_dtype(0.0)
                if head // self.H_TILE_CTA == rank:
                    value = local_scratch[n_index * self.H_TILE_CTA + local_h]
                else:
                    value = remote_inbox[(n_index - rank * self.N_TILE_CTA) * self.H_TILE_CTA + local_h]
    ...
    def _materialize_ds_both(
        ...
        if tidx < self.MATH_THREADS_PER_CTA:
            for slot in cutlass.range_constexpr(32):
                ...
                dkv_logical_coordinate = dkv_coordinate_partition[dkv_source_coordinate]
                n_index = Int32(cute.get(dkv_logical_coordinate, mode=[0]))
                head = Int32(cute.get(dkv_logical_coordinate, mode=[1]))
                local_h = head % self.H_TILE_CTA
                dkv_value = self.element_dtype(0.0)
                if head // self.H_TILE_CTA == rank:
                    dkv_value = local_scratch[n_index * self.H_TILE_CTA + local_h]
                else:
                    dkv_value = remote_inbox[(n_index - rank * self.N_TILE_CTA) * self.H_TILE_CTA + local_h]
                ...
                dq_logical_coordinate = dq_coordinate_partition[dq_source_coordinate]
                head = Int32(cute.get(dq_logical_coordinate, mode=[0]))
                n_index = Int32(cute.get(dq_logical_coordinate, mode=[1]))
                dq_value = local_scratch[n_index * self.H_TILE_CTA + head % self.H_TILE_CTA]
    ...
    def kernel(
        ...
        rank_dkv_coordinates = rank_dkv_mma.partition_C(cute.make_identity_tensor(self.DKV_MMA_TILER[:2]))
        rank_dq_coordinates = rank_dq_mma.partition_C(cute.make_identity_tensor(self.DQ_MMA_TILER[:2]))

        dkv_b_identity = cute.local_tile(
            cute.make_identity_tensor((self.N_TILE, self.H_TILE_CLUSTER)),
            cute.select(self.DKV_MMA_TILER, mode=[1, 2]),
            (None, None),
        )
        dq_b_identity = cute.local_tile(
            cute.make_identity_tensor((self.H_TILE_CLUSTER, self.N_TILE)),
            cute.select(self.DQ_MMA_TILER, mode=[1, 2]),
            (None, None),
        )
        rank_dkv_b_coordinates = rank_dkv_mma.partition_B(dkv_b_identity)
        rank_dq_b_coordinates = rank_dq_mma.partition_B(dq_b_identity)
        ...
        score_c_shape = score_tiled_mma.partition_shape_C((self.H_TILE_CLUSTER, self.N_TILE))
        score_c_layout = score_tiled_mma.make_fragment_C(score_c_shape).layout
        dp_c_shape = dp_tiled_mma.partition_shape_C((self.H_TILE_CLUSTER, self.N_TILE))
        dp_c_layout = dp_tiled_mma.make_fragment_C(dp_c_shape).layout
        dkv_c_shape = dkv_tiled_mma.partition_shape_C(self.DKV_MMA_TILER[:2])
        dkv_c_layout = dkv_tiled_mma.make_fragment_C(dkv_c_shape).layout
        dq_c_shape = dq_tiled_mma.partition_shape_C(self.DQ_MMA_TILER[:2])
        ...
        for tile_ordinal in cutlass.range_constexpr(tile_count):
            ...
            if tidx >= self.KV_LOAD_THREAD_BEGIN and tidx < self.KV_LOAD_THREAD_BEGIN + self.KV_LOAD_THREADS:
...
class FlashAttentionDSABackwardSm107(FlashAttentionDSABackwardSm100TwoCTA):
    """Rubin production kernel: 20 warps per CTA, five CG2 GEMMs per KV tile.

    Per-CTA roles: warps 0-3 gather the sparse K tile and the dQ-A
    images, then drain the final dQ epilogue; warps 4-7 run the softmax
    backward and publish P/dS; warps
    ...
    across the cluster; warp 19 relays ring TMA completions and commits
    the one-shot direct Q/dO operand loads.
    ...
    quadrants.  dQ stays TMEM-resident across all tiles and stores through
    a two-round staged TMA epilogue after W4-W7 hand registers to W0-W3.
    """
    ...
    # Preserve the fast Rubin variant's native 576-column allocation.
    # Q-r1 is restored to SMEM-A, so columns 512..575 are intentionally
    # unused by this correctness-isolation variant.
    TMEM_COLUMNS = 576

    SCORE_DONE_STAGES = 2

    # Per tile the ring carries two KdQ generations followed by the two
    # Q-r1 H64 panels consumed by the dK-r1 passes.
    ROUND_STAGES = 2
    ...
    def __init__(
        ...
        # vfinal_aug_6: baseline-style pacing barrier for the dKV atomic
        # bursts -- rejoins all reducer warps every 4 red.global iterations
    ...
    def _specialize_shared_storage(
        ...
        class SharedStorageV2:
            ...
            # One-stage score/gradient ownership rings for the aliased Q
            # and dO pages.  FULL alternates ScoreFull/GradFull; EMPTY is
            # the true CG2 source-read completion from the leader.
            alias_q_mbars: cute.struct.MemRange[cutlass.Int64, 2]
            alias_do_mbars: cute.struct.MemRange[cutlass.Int64, 2]
            pds_mbars: cute.struct.MemRange[cutlass.Int64, 2]
            pds_mbars_b: cute.struct.MemRange[cutlass.Int64, 2]
            ...
            direct_q: cute.struct.Align[cute.struct.MemRange[element_dtype, 16384], 1024]
            direct_do: cute.struct.Align[cute.struct.MemRange[element_dtype, 16384], 1024]
            loan_do: cute.struct.Align[cute.struct.MemRange[element_dtype, 16384], 1024]

        assert SharedStorageV2.size_in_bytes() == 325_632
    ...
    def _fill_kdq_pair_vk7(
        ...
        """Copy one KdQ pair using indices prefetched before MMA issue.
    ...
    def kernel(
        ...
        loan_ready_mbar = storage.loan_ready_mbar.ptr
        ...
        direct_q_raw = storage.direct_q.data_ptr()
        direct_do_raw = storage.direct_do.data_ptr()
        loan_do_raw = storage.loan_do.data_ptr()
        ...
        # Rubin: K-r0 owns a dedicated two-quadrant Q operand.
        direct_q_quad = (
            cute.make_tensor(
                cute.recast_ptr(
                    direct_q_raw,
        ...
        # Rubin: V-r1 owns a dedicated two-quadrant dO operand.
        direct_do_quad = (
            cute.make_tensor(
                cute.recast_ptr(
                    direct_do_raw,
        ...
        stats_thread_copy = stats_tiled_copy.get_slice(lane_idx)
        ...
        rank_score_mma = score_tiled_mma.get_slice(rank)
        rank_dkv_mma = dkv_tiled_mma.get_slice(rank)
        peer_dkv_mma = dkv_tiled_mma.get_slice(peer_rank)
        rank_dq_mma = dq_tiled_mma.get_slice(rank)
        rank_score_coordinates = rank_score_mma.partition_C(cute.make_identity_tensor((self.H_TILE_CLUSTER, self.N_TILE)))
        rank_dq_coordinates = rank_dq_mma.partition_C(cute.make_identity_tensor(self.DQ_MMA_TILER[:2]))
        ...
        rank_g_qt = rank_dkv_mma.partition_A(g_qt)
        rank_g_dot = rank_dkv_mma.partition_A(g_dot)
        # The direct fill uses this CTA's D128 owner.  Restoring the score
        # page needs the opposite D128 owner, so it must use a peer-D source
        # partition even though the destination remains CTA-local.
        peer_rank_g_qt = peer_dkv_mma.partition_A(g_qt)
        peer_rank_g_dot = peer_dkv_mma.partition_A(g_dot)
        a_cta_layout = cute.make_layout(
            cute.slice_(
                cluster_layout_vmnk,
                (0, 0, None, 0),
        ...
        t_qt_direct_smem_a, _ = cpasync.tma_partition(
            tma_atom_qt,
            block_coord_vmnk[2],
            a_cta_layout,
            cute.group_modes(direct_q_quad[0], 0, 3),
            cute.group_modes(rank_g_qt, 0, 3),
        )
        t_qt_direct_smem_b, _ = cpasync.tma_partition(
            tma_atom_qt,
            block_coord_vmnk[2],
            a_cta_layout,
            cute.group_modes(direct_q_quad[1], 0, 3),
            cute.group_modes(rank_g_qt, 0, 3),
        )
        t_qt_restore_smem_a, t_qt_restore_gmem = cpasync.tma_partition(
            tma_atom_qt,
            block_coord_vmnk[2],
            a_cta_layout,
            cute.group_modes(direct_q_quad[0], 0, 3),
            cute.group_modes(peer_rank_g_qt, 0, 3),
        )
        t_qt_restore_smem_b, _ = cpasync.tma_partition(
            tma_atom_qt,
            block_coord_vmnk[2],
            a_cta_layout,
            cute.group_modes(direct_q_quad[1], 0, 3),
        ...
        _, t_dot_gmem = cpasync.tma_partition(
            tma_atom_dot,
            block_coord_vmnk[2],
            a_cta_layout,
            cute.group_modes(round_quad[0], 0, 3),
        ...
        t_dot_direct_smem_a, _ = cpasync.tma_partition(
            tma_atom_dot,
            block_coord_vmnk[2],
            a_cta_layout,
            cute.group_modes(direct_do_quad[0], 0, 3),
            cute.group_modes(rank_g_dot, 0, 3),
        )
        t_dot_direct_smem_b, _ = cpasync.tma_partition(
            tma_atom_dot,
            block_coord_vmnk[2],
            a_cta_layout,
            cute.group_modes(direct_do_quad[1], 0, 3),
            cute.group_modes(rank_g_dot, 0, 3),
        )
        t_dot_restore_smem_a, t_dot_restore_gmem = cpasync.tma_partition(
            tma_atom_dot,
            block_coord_vmnk[2],
            a_cta_layout,
            cute.group_modes(direct_do_quad[0], 0, 3),
            cute.group_modes(peer_rank_g_dot, 0, 3),
        )
        t_dot_restore_smem_b, _ = cpasync.tma_partition(
            tma_atom_dot,
            block_coord_vmnk[2],
            a_cta_layout,
            cute.group_modes(direct_do_quad[1], 0, 3),
        ...
        direct_q_fragment_a = dkv_tiled_mma.make_fragment_A(direct_q_quad[0])
        direct_q_fragment_b = dkv_tiled_mma.make_fragment_A(direct_q_quad[1])
        direct_do_fragment_a = dkv_tiled_mma.make_fragment_A(direct_do_quad[0])
        direct_do_fragment_b = dkv_tiled_mma.make_fragment_A(direct_do_quad[1])
        loan_quad_fragment_a = dkv_tiled_mma.make_fragment_A(loan_quad[0])
        loan_quad_fragment_b = dkv_tiled_mma.make_fragment_A(loan_quad[1])
        p_fragments = (
        ...
        pipe_alias_q = pipeline.PipelineAsyncUmma.create(
            num_stages=1,
            producer_group=load_elect_group,
            consumer_group=leader_group,
            barrier_storage=storage.alias_q_mbars.data_ptr(),
            cta_layout_vmnk=cluster_layout_vmnk,
            defer_sync=True,
        )
        pipe_alias_do = pipeline.PipelineAsyncUmma.create(
            num_stages=1,
            producer_group=load_elect_group,
            consumer_group=leader_group,
            barrier_storage=storage.alias_do_mbars.data_ptr(),
            cta_layout_vmnk=cluster_layout_vmnk,
            defer_sync=True,
        )
        ...
        # F1 split: pipe_p guards the P byte-set (p_blocks + p_xchg,
        # EMPTY = pds_mbars[1] @ struct +0x78, released after the last dV
        # read); pipe_ds guards the dS byte-set (ds_image + ds_blocks,
        # EMPTY = pds_mbars_b[1] @ +0x88, released after the last dK read).
        ...
        if tidx == Int32(0):
            ...
            cute.arch.mbarrier_init(loan_ready_mbar, 2)
            cute.arch.mbarrier_init(
                kdq_ready_mbar,
                self.GATHER_THREADS,
            )
        ...
        # Match the exact baseline traversal contract: mTopkLength remains a
        # per-query dynamic validity bound, while every role walks the
        # compile-time tile span of mTopkIdxs.  Invalid columns are still
        # predicated through `topk`; fixing only the trip count keeps the full
        # None/-1 and variable-length semantics without broadcasting a
        # runtime loop bound through all warp-specialized roles.
        tile_count = self.max_topk // self.N_TILE

        # Rubin register probe: all four control warps 64->96,
        # reducers 120->104. Pool neutral:
        # 4*48 + 4*128 + 8*104 + 4*96 = 1920.
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            if tile_count > Int32(0):
                ...
                if tile_count == Int32(1):
                    ...
                    # gradients.  loan_do is independent storage on Rubin,
                    # so fill it once while the leader consumes K(0).
                    self._gather_kdq_v8(
                        mKV,
                        mTopkIdxs,
                        gather_kd_rows_0,
                    ...
                    # Keep kscore a pure K stream.  K(1) is published before
                    # the one-time loan_do fill, so score(0/1) can overlap
                    # its TMA and cluster-copy traffic.
                    next_iter = Int32(1)
                    pipe_kscore.producer_acquire(gather_state)
                    self._load_score_kv(
                        mKV,
                    ...
                    # Each remaining producer generation carries only the
                    # next score K tile.  kdq remains gathered one tile ahead
                ...
                # Generation 1: producer_tail observes the final score K
                # source-read completion.  Publish that fact before the math
                # epilogue reuses score_kv as dQ staging.
                self.gather_barrier.arrive_and_wait()
                if warp_idx == Int32(0):
                    with cute.arch.elect_one():
                ...
                # Role move experiment: W0-W3 are now done with all K/KdQ
                # production.  Take the 128-register budget released by
                # W4-W7 below and drain the unchanged two 64-value dQ
                # subtiles while the reducer warps finish the last dKV slot.
                cute.arch.setmaxregister_increase(128)
            ...
            # --- math: stats and per-tile softmax + publication.
            ...
            for loop_iter in cutlass.range(tile_count):
                ...
                group_bases = [2 * (h_group % 2) + 16 * (h_group // 2) for h_group in range(4)]
                group_local_h = [
                    Int32(
                        cute.get(
                ...
                band_indices = [[group_base + (j % 2) + 4 * (j // 2) for j in range(8)] for group_base in group_bases]
                ...
                for h_group in cutlass.range_constexpr(4):
                    delta = softmax_stats[group_local_h[h_group], 1]
                    for value_index in band_indices[h_group]:
                ...
                # F1: P-empty (pds_mbars[1] @ +0x78) is released right
                # after the prev tile's last dV read -- this wait now
                # unblocks ~mid-grad instead of end-of-grads.
                ...
                # F1: dS-empty (pds_mbars_b[1] @ +0x88) is released only
                # after the prev tile's last dK read; waiting here lets the
                # P stmatrix stores issue while dK still reads dS.
            ...
            # Return this aligned warp group's 128-register allocation to
            # W0-W3.  Their matching increase can wait here without a named
            # barrier; setmaxnreg serializes the grant, and all producer
            # dependencies needed by this math loop have already completed.
            cute.arch.setmaxregister_decrease(48)
            ...
            if is_leader_cta:
                ...
                alias_cons = pipeline.make_pipeline_state(
                    pipeline.PipelineUserType.Consumer,
                    1,
                )
                if tile_count > Int32(0):
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    if has_prev:
                        ...
                        round_cons, dkv_prod = self._issue_prev_grads_tail_v2(
                            dkv_tiled_mma,
                            t_dkv[1],
                            direct_do_fragment_a,
                            direct_do_fragment_b,
                            quad_fragment_a,
                            quad_fragment_b,
                            p_fragments[0],
                            p_fragments[1],
                        ...
                        # last dV pass; only dS waits for the dK passes.
                ...
                if tile_count > Int32(0):
                    ...
                    ds_cons.advance()
                    p_cons.advance()
                    pipe_alias_q.consumer_release(alias_cons)
                    pipe_alias_do.consumer_release(alias_cons)
                    alias_cons.advance()
            ...
            if tile_count > Int32(0):
                ...
                for loop_iter in cutlass.range(tile_count):
                    # g0/g1: the two KdQ images used by dQ.
                    ...
                    # g2/g3: restore the original per-tile Q-r1 H64 panels.
                    # The consumer uses these MN-major round-buffer images
                    # directly as SMEM-A for dK-r1; no TMEM-A reinterpretation.
                    for h_half in cutlass.range_constexpr(self.H_PASSES):
                        pipe_round.producer_acquire(round_acq)
                        round_acq.advance()
                        with cute.arch.elect_one():
                            cute.arch.mbarrier_arrive_and_expect_tx(
                        ...
                        if cutlass.const_expr(h_half == 0):
                            # Match kernel.py g2: both ranks use the original
                            # rank-partitioned Q.T TMA for the first panel.
                            cute.copy(
                                tma_atom_qt,
                                t_qt_gmem[None, 1, 0],
                                t_qt_smem_a[None, 0],
                            ...
                            # kernel.py g3 lets rank 1 source its byte-identical
                            # stationary panel locally; rank 0 retains TMA.
                            if rank == Int32(1):
                                with cute.arch.elect_one():
                                    _cpasync_bulk_s2cluster(
                                        stationary_q_raw + 4096 * (4 + 2),
                                        round_buf_b_raw,
                                        round_tma_mbars + 1,
                                        grad_a_stage_bytes,
                                        rank,
                    ...
                    # W19 performs the two Q-r1 commits after raw completion.
                    round_com.advance()
                    round_com.advance()
            ...
            alias_acq = pipeline.make_pipeline_state(
                pipeline.PipelineUserType.Producer,
                1,
            )
            alias_com = pipeline.make_pipeline_state(
                pipeline.PipelineUserType.Producer,
                1,
            )
            alias_q_phase = Int32(0)
            alias_do_phase = Int32(0)
            q_r1_phase_0 = Int32(0)
            q_r1_phase_1 = Int32(0)
            if tile_count > Int32(0):
                # Wait until the stationary Q/dO prologue has released the
                # raw TMA barriers, then reuse them for dedicated operands.
                cute.arch.mbarrier_wait(
                    stationary_tma_mbars,
                    alias_q_phase,
                )
                alias_q_phase = Int32(1) - alias_q_phase
                cute.arch.mbarrier_wait(
                    stationary_tma_mbars + 1,
                    alias_do_phase,
                )
                alias_do_phase = Int32(1) - alias_do_phase
                # Dedicated Q/dO gradient operands are filled once and stay
                # immutable for every sparse tile.
                (
                    alias_acq,
                    alias_com,
                    alias_q_phase,
                    alias_do_phase,
                ) = self._transition_alias_pages_vb(
                    False,
                    rank,
                    tma_atom_qt,
                    t_qt_gmem,
                    ...
                    t_dot_direct_smem_b,
                    t_dot_restore_gmem,
                    t_dot_restore_smem_a,
                    t_dot_restore_smem_b,
                    stationary_tma_mbars,
                    alias_q_phase,
                    alias_do_phase,
                    pipe_alias_q,
            ...
            for _ in cutlass.range(tile_count):
                # g0/g1 were committed by W17 after the KdQ rendezvous.
                commit_com.advance()
                commit_com.advance()
                # Commit g2/g3 only after each Q-r1 TMA/bulk source completes.
                for h_half in cutlass.range_constexpr(self.H_PASSES):
                    if cutlass.const_expr(h_half == 0):
                        cute.arch.mbarrier_wait(
                            round_tma_mbars,
                        ...
                        q_r1_phase_0 = Int32(1) - q_r1_phase_0
                    else:
                        cute.arch.mbarrier_wait(
                            round_tma_mbars + 1,
                            q_r1_phase_1,
                        )
                        q_r1_phase_1 = Int32(1) - q_r1_phase_1
                    with cute.arch.elect_one():
                        pipe_round.producer_commit(commit_com)
                    commit_com.advance()
            if tile_count > Int32(0):
                alias_tail_q = alias_acq.clone()
                alias_tail_do = alias_acq.clone()
                pipe_alias_q.producer_tail(alias_tail_q)
                pipe_alias_do.producer_tail(alias_tail_do)
    ...
    @cute.jit
    def _transition_alias_pages_vb(
        self,
        restore_score: cutlass.Constexpr[bool],
        rank: Int32,
        tma_atom_qt: cute.CopyAtom,
        t_qt_gmem: cute.Tensor,
        t_qt_direct_smem_a: cute.Tensor,
        t_qt_direct_smem_b: cute.Tensor,
        t_qt_restore_gmem: cute.Tensor,
        t_qt_restore_smem_a: cute.Tensor,
        t_qt_restore_smem_b: cute.Tensor,
        tma_atom_dot: cute.CopyAtom,
        t_dot_gmem: cute.Tensor,
        t_dot_direct_smem_a: cute.Tensor,
        t_dot_direct_smem_b: cute.Tensor,
        t_dot_restore_gmem: cute.Tensor,
        t_dot_restore_smem_a: cute.Tensor,
        t_dot_restore_smem_b: cute.Tensor,
        raw_mbars: cute.Pointer,
        q_phase: Int32,
        do_phase: Int32,
        q_pipeline,
        do_pipeline,
        producer_acq: pipeline.PipelineState,
        producer_com: pipeline.PipelineState,
        stage_bytes: cutlass.Constexpr[int],
    ):
        """Switch both 16 KiB aliased pages between score and grad views."""

        q_pipeline.producer_acquire(producer_acq)
        do_pipeline.producer_acquire(producer_acq)
        producer_acq.advance()
        with cute.arch.elect_one():
            cute.arch.mbarrier_arrive_and_expect_tx(
                raw_mbars,
                2 * stage_bytes,
        ...
        if cutlass.const_expr(restore_score):
            if rank == Int32(0):
                cute.copy(
                    tma_atom_qt,
                    t_qt_restore_gmem[None, 0, 0],
                    t_qt_restore_smem_b[None, 0],
                    tma_bar_ptr=raw_mbars,
                )
                cute.copy(
                    tma_atom_dot,
                    t_dot_restore_gmem[None, 1, 0],
                    t_dot_restore_smem_b[None, 0],
                    tma_bar_ptr=raw_mbars + 1,
                )
            else:
                cute.copy(
                    tma_atom_qt,
                    t_qt_restore_gmem[None, 0, 1],
                    t_qt_restore_smem_a[None, 0],
                    tma_bar_ptr=raw_mbars,
                )
                cute.copy(
                    tma_atom_dot,
                    t_dot_restore_gmem[None, 1, 1],
                    t_dot_restore_smem_a[None, 0],
                    tma_bar_ptr=raw_mbars + 1,
                )
        else:
            if rank == Int32(0):
                cute.copy(
                    tma_atom_qt,
                    t_qt_gmem[None, 0, 0],
                    t_qt_direct_smem_a[None, 0],
                    tma_bar_ptr=raw_mbars,
                )
                cute.copy(
                    tma_atom_dot,
                    t_dot_gmem[None, 1, 0],
                    t_dot_direct_smem_a[None, 0],
                    tma_bar_ptr=raw_mbars + 1,
                )
                cute.copy(
                    tma_atom_qt,
                    t_qt_gmem[None, 0, 1],
                    t_qt_direct_smem_b[None, 0],
                    tma_bar_ptr=raw_mbars,
                )
                cute.copy(
                    tma_atom_dot,
                    t_dot_gmem[None, 1, 1],
                    t_dot_direct_smem_b[None, 0],
                    tma_bar_ptr=raw_mbars + 1,
                )
            else:
                cute.copy(
        ...
        cute.arch.mbarrier_wait(raw_mbars, q_phase)
        q_phase = Int32(1) - q_phase
        cute.arch.fence_view_async_shared()
        with cute.arch.elect_one():
            q_pipeline.producer_commit(producer_com)
        cute.arch.mbarrier_wait(raw_mbars + 1, do_phase)
        do_phase = Int32(1) - do_phase
        cute.arch.fence_view_async_shared()
        with cute.arch.elect_one():
            do_pipeline.producer_commit(producer_com)
        producer_com.advance()
        return producer_acq, producer_com, q_phase, do_phase
    ...
    def _issue_score_v2(
        ...
        do_acquire: cutlass.Constexpr[bool] = True,
    ) -> pipeline.PipelineState:
        """Issue one score-side CG2 GEMM over four resident D128 chunks.
        ...
        if cutlass.const_expr(do_acquire):
            done_pipeline.producer_acquire(producer_state)
    ...
    def _issue_score_chunks_v7(
        ...
        """One full-K score GEMM with the aliased page wait pushed late.

        Q aliases chunks 0/1, so S issues safe chunks 2/3 first.  dO
        aliases chunks 2/3, so dP issues safe chunks 0/1 first.  The page
        FULL wait then guards only the pair whose bytes can be in flight.
        """
        ...
        for local_chunk in cutlass.range_constexpr(2):
    ...
    def _issue_prev_grads_head_v2(
        ...
        loan_ready_mbar: cute.Pointer,
        round_pipeline,
        round_consumer_state: pipeline.PipelineState,
        kscore_consumer_state: pipeline.PipelineState,
        q_alias_pipeline,
        alias_consumer_state: pipeline.PipelineState,
        p_pipeline,
        ...
        # loan_do is filled once into Rubin-only independent storage.  This
        # readiness generation is unrelated to the pure-K kscore stream.
        _mbarrier_wait_acquire_cluster(
            loan_ready_mbar,
            Int32(0),
        )
        ...
        # The direct K-r0 page fill runs in parallel with dQ and V-r0.
        # Wait only at its first actual source use.
        q_alias_pipeline.consumer_wait(alias_consumer_state)
        self._issue_dkv_pass_v2(
            dkv_tiled_mma,
            t_dkv_0,
            quad_fragment_a,
    ...
    def _issue_prev_grads_tail_v2(
        ...
        dkv_tiled_mma: cute.TiledMma,
        t_dkv_1: cute.Tensor,
        direct_do_fragment_a: cute.Tensor,
        direct_do_fragment_b: cute.Tensor,
        quad_fragment_a: cute.Tensor,
        quad_fragment_b: cute.Tensor,
        p_fragment_0: cute.Tensor,
        p_fragment_1: cute.Tensor,
        ...
        """Gradient tail: round-1 dV/dK with Q-r1 restored to SMEM-A."""

        dkv_done_pipeline.producer_acquire(dkv_producer_state)
        # V-r1 is the first read of the aliased dO page.  Its TMA fill can
        # overlap the entire gradient head before this late ownership wait.
        do_alias_pipeline.consumer_wait(alias_consumer_state)
        self._issue_dkv_pass_v2(
            dkv_tiled_mma,
            t_dkv_1,
            direct_do_fragment_a,
        ...
        # F1: last P read (round-1 dV on p_fragment_1) just issued.
        # Same primitive as the old whole-set release: tcgen05.commit
        # multicast self|peer, CtaGroup.TWO -- P-empty fires in both CTAs
        # once every UMMA issued so far completes, i.e. before the four
        # round-1 dK passes below.  Caller advances p_consumer_state.

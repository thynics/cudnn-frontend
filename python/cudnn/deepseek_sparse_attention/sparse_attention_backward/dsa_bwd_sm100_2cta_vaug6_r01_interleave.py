...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    def _drain_dkv_v8(
        ...
        full grads-tail before slot 1.  This variant preserves both early
        T2R releases but parks slot 0 in registers until slot 1's FULL
        transition (the tile's final-dK completion), then emits the two
        rounds as adjacent atomic pairs.  WG0 orders each pair R0/R1 while
        WG1 orders it R1/R0, so the first issue wave covers both rounds;
        there is no mid-burst pacing, only one tile-end reducer barrier.
        Split wait/release pipeline states still let both releases trail
        their own fences (round_acq/round_com pattern).
        """
        ...
        # --- slot 1: tail-committed generation.  Its FULL transition is
        # the existing all-dK-end signal for this tile.  T2R and release it
        # before either atomic burst so both TMEM slots recycle promptly.
        done_pipeline.consumer_wait(wait_state)
        wait_state.advance()
        ...
        # Both round payloads are now parked in registers and every dK for
        # the tile has completed.  Pair the two independent destinations for
        # each KV row to expose the maximum back-to-back REDG issue window.

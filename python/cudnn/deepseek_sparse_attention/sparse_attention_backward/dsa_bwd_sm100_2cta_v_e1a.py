    ...
    def kernel(
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            if is_leader_cta:
                ...
                relay_phase = Int32(0)
                # E1a diagnostic: leader runs the BASE monolithic score
                # path (whole-tile kscore wait before S); the gather-side
                # E1 machinery (chunk ladder, fences, remote arrives)
                # stays live but unconsumed -- prices the gather side.
                for loop_iter in cutlass.range(tile_count):

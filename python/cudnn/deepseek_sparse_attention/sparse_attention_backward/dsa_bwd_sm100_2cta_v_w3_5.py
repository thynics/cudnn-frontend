"""v_w3_5: D1 conservative probe -- 640 -> 512 threads (reducers 8 -> 4 warps).

The last untested >0.1ms-class lever (multi-lens F16: the 20-warp
cohabitation tax -- SOFTMAX cold 1.728 vs steady 2.336 on identical
register-only code -- may dilute with fewer resident warps).
Conservative form: register budgets stay 48/128/128 (measures ONLY the
warp-count effect); the two spatial reducer warpgroups become one group
draining each dKV slot in two temporal halves, preserving v11's
32-value T2R liveness (the v9_3 spill window stays closed).  The
consumer_release still fires once per slot (after the second half's
tmem reads) so dkv_done credit counts are unchanged; the release is
later by one half-atomics burst, inside the drain's off-ring headroom.

Inherited v_w3_2 notes: kdq handshake B is a count-128 hardware arrival.

Barrier B (the 160-thread software rendezvous handing "both kdq images
written, drained, fenced" back to W17) is replaced by the PTX paired
completion protocol on one count-128 mbarrier (PTX ISA 9.3
9.7.14.16.18 Example 1; source ruling dsa-r2-handshake-ruling
CONDITIONAL PASS, patch form A):

  each gather thread, after its own copies/stores, unconditionally:
    cp.async.mbarrier.arrive.shared::cta.b64  [kdq_ready]   (no .noinc:
        +1 pending, -1 when ALL its prior cp.async ops complete)
    mbarrier.arrive (.release.cta default)                  (publishes
        its plain-STS zero fills; consumes one of the 128 init counts)
  W17: acquire-wait the phase, KEEP fence_view_async_shared (tcgen05
  reads via the async proxy; PTX 9.7.17.6.5), then the two commits.

The gather threads shed their per-thread cp_async_wait_group(0) drain
and the barrier-B convergence: they arrive and move on to the next
LOAD_K (whose commit_group sweeps any kdq stragglers -- harmless
over-wait on the gather side, off the pacer chain).  W17's B-side wait
now ends at the true last cp.async landing instead of the software
convergence after every thread's full drain.  Zero fills stay plain
vector stores, published by the release arrive (the R2-2 veto's
mandated fix).  Counting: 128 init = 128 release arrives; each
no-noinc arrive is +1/-1 net zero but forces phase completion to wait
for cp.async completion.  tx-count stays 0 (non-bulk family).

Inherited vk_7 notes: kdq index loads hoist ahead of barrier A (W2).

The rendezvous fill is conserved serial content on the pacer cycle
(the K2 family's five falsifications), so the only legal attack is to
shrink the fill itself.  Inside it sits a classic latency exposure:
the four per-thread topk index loads issue AFTER barrier A, so the
first row copy eats the full GMEM index latency inside the critical
section -- while before barrier A the gather warps sit idle waiting
for the load warp to show up with the ring credits.  W2 moves the
index loads (pure register state, no SMEM writes, no credit needed)
ahead of the barrier: their latency hides under the A-wait, and the
post-A section becomes address math + cp.async issues + data drain.

vg_4's exact playbook (the only 4-for-4 family this campaign), zero
protocol change: barriers, credits, copy protocol and visibility
fences are untouched -- only gather-side instruction scheduling moves.

Inherited vk_5 notes: W17 sheds TMA waits via the W19 commit relay.

The pacer decomposition (vk_2_trace): W17's 6.92us chain vs period 7.089
(97.6% duty) contains MAT_WAIT 1.368us of per-gen TMA-completion waits
under the lag-1 manual-commit discipline.  Directly TMA-arming the ring
pipeline's full barriers is impossible (TMA completion signals LOCAL
mbars only, while the pair's full barrier lives on the leading CTA), so
the wait+commit conversion moves to W19 -- the one fully idle warp:

  W17 per quadrant gen: acquire -> arm local tma mbar -> issue copy,
  and moves on (no wait, no commit; its round_com skip-advances the
  six quadrant positions after the train).
  W19 per tile: skip the two kdq positions, then six times [wait the
  local tma mbar (parity = gen%2) -> elect-one producer_commit].

Correctness chain (why a tma mbar is never re-armed while pending):
W17's acquire of gen n waits the leader's UMMA-tracked release of gen
n-2, which happens-after the leader consumed it, which happens-after
W19's commit of gen n-2, which happens-after W19's wait on mbar n%2 --
the ring's 2-slot credit and the 2 tma mbars share parity, so the
credit chain IS the re-arm guard.  The kdq rendezvous (ROUTE_K) stays
on W17 unchanged; its two commits keep their positions in round_com's
sequence.  Producer arrives per gen stay one-elect-per-CTA (count 2);
the pipeline counts arrives, not identities.

vd_1's shadow does not apply: vd_1 split the FILLS across two lanes in
an era when W17's chain was not the pacer (null); vk_5 moves only the
WAITS, in a regime where W17's chain IS the period (97.6% duty).

Expected: W17 chain 6.92 -> ~5.3-5.6; period toward the math chain
(5.84) -- and if W17 lands below it, the M2 receivable (vk_4, archived)
partially unlocks.

Inherited vk_2 notes: split publish retired (the validated C form).

Byte-identical (except this docstring) to the exclusive variant
vg5_nosplit_c that reverse-applied all 23 vc_2->vg_1 hunks onto vg_5:
the pds publish returns to vc_2's single pipeline and single count-128
ready mbar; the vg_2/vg_4/vg_5 math and gather surgeries are untouched.
That variant passed correctness 4/4, perf 9.896 @ 8.607 and full
candidate IKET capture at 9c11a29.

Why retire split publish instead of keeping vk_1's S1 fix: the C-vs-S1
trace A/B measured every split-publish envelope worse or equal
(MATH_PDS_ACQ 1.739 vs 2.008, ROUTE_P 0.304 vs 0.611, ROUTE_dS 0.355 vs
0.526, P+dS T2R/math 8.054 vs 8.946 us/tile), release ms within same-day
baseline drift, and the feature's original gain (-0.069 ms) was itself
inside drift.  It also carried the injection-hang trigger family (shared
producer state across two pipes; S2/a_advance hung, S1/s1a passed).
One pipeline less, one mbar less, one handoff less per tile.

Inherited vg_5 notes: vg_4 + full-width exp band (32 EX2 in flight).
...
r_score, the register that fed it and is dead immediately after (the v7
math form).  Phase one runs the 32 independent EX2 over all four h-groups,
phase two runs the dependent dS chain and both bf16 converts.  Live-register
count is unchanged -- no temporary array is created -- so the SASS spill
gate is not at risk.  The instruction set and per-value operation order are
identical; this is scheduling, not an approximation.

Inherited vg_4 notes: vg_2 + hoisted topk-index loads in the sparse gathers.

Both sparse gathers read their topk index inside the row loop and consumed
it in the very next statement, so every row paid a full GMEM index latency
before its own cp.async could issue.  vg_4 issues all of a thread's index
loads up front and only then walks the rows -- the same fix v5 applied to
the reducer after the baseline comparison ("the baseline preloads all eight
indices as independent loads up front").  Addresses, predicates and copy
order are unchanged.

Inherited vg_2 notes: vg_1 + M1 exp band software pipelining.

The grouped softmax body issued MUFU.EX2 and then immediately consumed its
result (p_value feeds ds_value in the next statement), so each of the 32
per-thread exponentials paid the full MUFU latency in program order -- the
"~1.3 us of the 1.87 us SOFTMAX span is latency tax" finding.  vg_2 splits
each eight-value group into two passes: all eight EX2 issue back-to-back
into independent SSA values, then the dependent FFMA/convert chain runs.
Eight exponentials are in flight at once and their latency overlaps the
band's own arithmetic.  The instruction SET is unchanged (same exp2 calls,
same operands, same order of floating-point operations per value), so this
is bit-identical, not an approximation swap.

Experiment C overlay: retain vg_2/vg_4/vg_5 math and gather scheduling while
reverting vg_1's split publish as one atomic change.  The P/dS handoff uses
vc_2's single pipeline, single count-128 ready mbar, P-then-dS relay order,
and single producer state.  This file is an isolated diagnostic candidate,
not a production implementation.

Inherited vc_2 notes: v12 with a conservative score-K/dO temporal alias.

The active V2 kernel keeps v12's score, math, reducer, and persistent-TMEM
design.  During each gradient head it temporarily reinterprets the dead
32-KiB score-K stage as two 16-KiB dO operands for round-0 dV.  The original
A/B round ring therefore carries kdq/Q(round0)/dO(round1)/Q(round1), while
the existing one-stage kscore pipeline carries K -> borrowed dO -> K.  This
creates four physical gradient operand slots without increasing SMEM.
...
class FlashAttentionDSABackwardSm100TwoCTA(FlashAttentionDSABackwardSm100):
    ...
    def _load_score_kv(
        ...
        # vg_4: every index load issues before the first row's copies, so
        # the loads overlap each other instead of serializing row by row.
        row_local_n = [
...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    * vc_2 keeps K_dQ/Q and round-1 dO in the rank-symmetric A/B ring, while
      round-0 dO temporarily occupies the released score-K allocation.
    ...
    * The gradient GEMMs of tile p are issued inside tile t=p-1's S/dP
    ...
    # Warp roles.  v_w3_5 (D1): reducers halve to 4 warps; every role
    # index above them shifts down by 4.
    GATHER_WARPS = 4
    MATH_WARP_BEGIN = 4
    MATH_WARPS = 4
    REDUCE_WARP_BEGIN = 8
    ...
    # vc_2: round A/B no longer carries dO round 0; that pair temporarily
    # lives in the score-K allocation.  The remaining sequence is phase
    # closed at depth 2:
    # g0/g1 kdq, g2/g3 Q_r0, g4/g5 dO_r1, g6/g7 Q_r1.
    ...
    def _specialize_shared_storage(
        ...
        # vc_2 borrows the score-B allocation as two complete DKV-A
        # operands.  Keep the raw half-buffer offset below byte-exact even
        # if either layout is changed by a later specialization.
        assert (
            cute.cosize(score_b_layout_staged)
        ...
        class SharedStorageV2:
            ...
            # vc_2: completion barriers for the two 16-KiB dO panels
            # temporarily staged over score_kv.
    ...
    def _fill_kdq_pair_v8(
        ...
        # vg_4: hoist the per-row index loads ahead of every copy.
        kdq_local_n = [
    ...
    def _gather_kdq_v8(
        ...
        # W2: the four index loads are pure register state -- no SMEM
        # write, no credit needed -- so they issue BEFORE barrier A and
        # their GMEM latency hides under the wait for the load warp.
        group_index = role_tidx // self.KV_GROUP_SIZE
        ...
        # R2 paired completion protocol (PTX 9.7.14.16.18 Example 1):
        # the no-noinc arrive forces phase completion to wait for this
        # thread's cp.async copies; the plain release arrive publishes
        # its STS zero fills and consumes one of the 128 init counts.
        # No per-thread drain, no software convergence: W17's wait ends
        # at the true last landing.
    ...
    def kernel(
        ...
        # vc_2: byte-exact DKV-A views over the two 16-KiB halves of the
        # 32-KiB score-K stage.  These views are live only after S/dP has
        # released K and before the next K gather acquires the stage.
        loan_quad = (
            cute.make_tensor(
                cute.recast_ptr(
                    score_kv_raw,
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            for loop_iter in cutlass.range(tile_count):
                ...
                if cutlass.const_expr(self.SOFTMAX_GROUPED_STATS):
                    ...
                    # vg_5 (M1 full width): phase one issues all 32 EX2
                    # over the four h-groups, each result landing back in
                    # the r_score slot that fed it (dead after the call),
                    # so 32 MUFU ops are in flight at zero register cost.
                    # Phase two runs the dependent dS chain.  Same calls,
                    # same operands, same per-value order.
                    group_bases = [
            ...
            if is_leader_cta:
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    # v7 reorder: S(t) and dP(t) both issue BEFORE the
                    # previous tile's gradient block.  dP depends only on
                    # the kscore buffer and its ping-pong TMEM credit, so
                    # issuing it early lets the K stage recycle immediately
                    # and gather(t+1) run concurrently with grads(t-1) and
                    # math(t) instead of serializing behind them.
                ...
                if tile_count > Int32(0):
                    ...
                    pipe_pds.consumer_release(pds_cons)
                    pds_cons.advance()
                    # vc_2 keeps the early dQ commit; loan_epi_safe_mbar
                    # separately protects the aliased score_kv staging.
            ...
            if tile_count > Int32(0):
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    self.kdq_barrier.arrive_and_wait()
                    # R2: handshake B is now the count-128 hardware
                    # arrival -- the phase completes at the true last
                    # cp.async landing (and after every thread's release
                    # arrive publishing its zero fills).  The async-proxy
                    # fence below is load-bearing (ruling R2-1 condition:
                    # tcgen05 reads via the async proxy) -- keep it.
                    cute.arch.mbarrier_wait(
                        kdq_ready_mbar,
                        kdq_ready_phase,
                    )
                    ...
                    # vk_5 (W1'): no completion waits, no commits here --
                    # W19 converts each local tma mbar into the pipeline
                    # commit.  round_com only needs its six quadrant
                    # positions skip-advanced to keep the kdq commits of
                    # the next tile on the right stage and phase.
            ...
            if tidx % Int32(32) == Int32(0):
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    # (1-rank) half of the whole dS image; the ds_xchg
                    # staging copy is retired (field kept, two-step plan).
                    if rank == Int32(0):
                        _cpasync_bulk_s2cluster(
            ...
            # --- W19, vk_5 (W1'): the quadrant commit relay.  W17 sheds
            # its six per-tile TMA-completion waits (MAT_WAIT, 1.368us of
            # the pacer chain); this otherwise-idle warp serially waits
            # each LOCAL round_tma mbar and converts the completion into
            # the pipeline commit the leader pair consumes.  A tma mbar
            # is never re-armed while pending: W17's acquire of gen n
            # waits the leader's UMMA-tracked release of gen n-2, which
            # happens-after this warp's commit of gen n-2, which
            # happens-after the wait below on the same-parity mbar.
    ...
    def _issue_prev_grads_head_v2(
        ...
        # vc_2: K(i) was released after dP; the gather role has republished
        # this same 32-KiB stage as two round-0 dO panels.
    ...
    def _drain_dkv_v8(
        ...
        # v_w3_5 (D1): one 128-thread group drains the full slot in two
        # temporal halves (the old spatial wg split becomes an unrolled
        # loop), preserving the 32-value T2R liveness.  Per half: its own
        # coordinate/source partitions, its own 8 topk indices (loaded
        # before the T2R to overlap, kept alive for slot 1).
        w35_coords = []
        w35_src0 = []
        w35_src1 = []
        w35_topk = []
        for w35_h in cutlass.range_constexpr(2):
            w35_coords.append(self.split_wg(
                thread_t2r_0.partition_D(c_dkv),
                2,
                Int32(w35_h),
            ))
            w35_src0.append(self.split_wg(
                thread_t2r_0.partition_S(t_dkv_core_0),
                2,
                Int32(w35_h),
            ))
            w35_src1.append(self.split_wg(
        ...
        for w35_h in cutlass.range_constexpr(2):
            r_topk_h = cute.make_rmem_tensor((8,), cutlass.Int32)
            for i in cutlass.range_constexpr(8):
                coord_base = i * 2 - i % 2
                local_row = Int32(
                    cute.get(
                        w35_coords[w35_h][coord_base],
                        mode=[1],
                    )
                )
                global_row = tile_base + local_row
                if global_row < topk:
                    r_topk_h[i] = mTopkIdxs[
                        global_row,
                        (token_idx, batch_idx),
                    ]
                else:
                    r_topk_h[i] = Int32(-1)
            w35_topk.append(r_topk_h)
        ...
        for w35_h in cutlass.range_constexpr(2):
            thread_values_0 = cute.make_rmem_tensor(
                w35_coords[w35_h].shape,
                self.acc_dtype,
            )
            cute.copy(tiled_t2r_0, w35_src0[w35_h], thread_values_0)
            cute.arch.fence_view_async_tmem_load()
            if cutlass.const_expr(w35_h == 1):
        ...
        for w35_h in cutlass.range_constexpr(2):
            ...
            cute.copy(tiled_t2r_1, w35_src1[w35_h], thread_values_1)
            cute.arch.fence_view_async_tmem_load()
            if cutlass.const_expr(w35_h == 1):
                done_pipeline.consumer_release(release_state)
                release_state.advance()
...
# File-based harness registration: V2 is vc_2's active implementation.

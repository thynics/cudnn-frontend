"""DeepSeek Sparse Attention backward, two-CTA SERIAL chain, kq4a.

final_ser_kq4a = final_ser_kq2 + FUSED T2R only (single variable):
both S and dP tmem loads issue back to back with ONE
fence_view_async_tmem_load, overlapping the two TMEM round trips (the
T2R span is latency-bound, not issue-bound -- atom investigation
2026-08-11); both consumer releases follow the single fence.  dP is
long done when the math warps wake (WAIT_dP median 0.224us = pure
wake), so the added dP wait costs nothing.  Register note: r_score
and r_dp are now live together through the P phase, as in the
original pre-split fused loop (128-reg math budget, known-good).
Target: T2R_S 0.352us on the relay-0 chain.
    ...
    def kernel(
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            for loop_iter in cutlass.range(tile_count):
                # ---- kq4 fused T2R: issue BOTH tmem loads back to
                # back and fence once, overlapping the two TMEM round
                # trips (the T2R span was measured latency-bound, not
                # issue-bound).  dP is long done by the time the math
                # warps wake (WAIT_dP measured 0.224us = pure wake), so
                # waiting it here costs nothing.  Both consumer
                # releases move BEHIND the fence, off the critical
                # chain (they were measured inside the T2R span).
                ...
                # ---- dS phase: dS math + publish (dP was T2R'd in the
                # fused load above).
                if cutlass.const_expr(self.SOFTMAX_GROUPED_STATS):
                    for h_group in cutlass.range_constexpr(4):
                        delta = softmax_stats[

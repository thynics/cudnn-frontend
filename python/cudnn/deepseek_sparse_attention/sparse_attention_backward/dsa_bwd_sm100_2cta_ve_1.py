"""ve_1: two-KV-tile A-stationary gradient batching on the vc_2 base.

The active V2 kernel keeps vc_2's stationary Q/dO panels, score path, math,
rank-owned reducer, and persistent dQ accumulator.  It changes the scheduling
unit from one KV tile to a two-tile macro: score/math publish two independent
P/dS images, then every D128 x H64 Q/dO gradient operand is materialized once
and consumed by both tiles back-to-back.  The second P/dS image aliases the
dead score-K allocation; the first tile's two dKV accumulators alias the dead
S/dP TMEM columns during the gradient phase.

At topk=2048 this cuts the per-pair operand generations from twenty to twelve
without adding SMEM or activating a second load warp.  The implementation uses
explicit two-stage P/dS landing/relay state and four-stage dKV completion state
so none of the aliases depends on timing alone.

The filename is the harness registration: ``--impl ve_1`` imports this module
without changing the canonical or vc_2 implementations.
"""
...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    """ve_1: two-tile A-stationary gradient macro on the vc_2 base.
    ...
    * ve_1 puts K_dQ plus every Q/dO panel in the rank-symmetric A/B ring;
      each Q/dO generation is shared by two KV tiles and score-K instead
      becomes the second tile's complete P/dS arena.
    ...
    * Score/math and gradients execute as a two-tile macro.  This sacrifices
      vc_2's cross-tile rotation to remove eight operand generations per
      pair; the B200 trace is the intended arbitration of that tradeoff.
    """
    ...
    # During a pair's gradient phase all four S/dP ping-pong images have
    # reached registers/SMEM P/dS.  Reuse those 128 columns for tile 0's
    # rank-owned round accumulators; tile 1 keeps vc_2's original slots.
    TMEM_PAIR0_DKV0_OFFSET = TMEM_S_OFFSET
    TMEM_PAIR0_DKV1_OFFSET = TMEM_DP_OFFSET

    # v7: S/dP TMEM double-buffer depth (cross-tile: S(t+1) no longer waits
    # for math's T2R of S(t)).
    SCORE_DONE_STAGES = 2

    # Per pair: two KdQ images per live tile, then four shared dO/Q images
    # per D256 round.  The two-slot physical ring is unchanged.
    ROUND_GENS_PER_PAIR = 12
    ROUND_STAGES = 2

    # Four tile-major completions per full pair: alias tile r0/r1 followed
    # by main tile r0/r1.  The reducer consumes the same order, so the next
    # pair's first two acquires are exact ownership gates for [S,dP] TMEM.
    MMA_DONE_STAGES = 4
    ...
    def _specialize_shared_storage(
        ...
        # score_kv is the byte-exact 32-KiB PDS1 arena during gradients;
        # retain the base layout equality as an alias-capacity build gate.
        assert (
            cute.cosize(score_b_layout_staged)
        ...
        class SharedStorageV2:
            ...
            # Two independent P/dS generations per macro.  Each stage owns
            # a P landing and a dS landing; relay mirrors the same indexing.
            ...
            # W18 owns both sides of the PDS producer pipeline.  These raw
            # barriers grant math permission to overwrite the physical
            # stage only after W18 has acquired its empty credit.
            pds_credit_mbars: cute.struct.MemRange[cutlass.Int64, 2]
    ...
    @cute.jit
    def _issue_pair_grad_round_ve1(
        self,
        dkv_tiled_mma: cute.TiledMma,
        t_pair0: cute.Tensor,
        t_pair1: cute.Tensor,
        a_fragment_0: cute.Tensor,
        a_fragment_1: cute.Tensor,
        p0_fragment_0: cute.Tensor,
        p0_fragment_1: cute.Tensor,
        ds0_fragment_0: cute.Tensor,
        ds0_fragment_1: cute.Tensor,
        p1_fragment_0: cute.Tensor,
        p1_fragment_1: cute.Tensor,
        ds1_fragment_0: cute.Tensor,
        ds1_fragment_1: cute.Tensor,
        has_second: cutlass.Boolean,
        round_index: cutlass.Constexpr[int],
        lane0_seq: Int32,
        lane1_seq: Int32,
        round_pipeline,
        round_state: pipeline.PipelineState,
    ) -> pipeline.PipelineState:
        """Consume four stationary A panels, issuing both pair tiles.

        The physical round generation is released only after both B
        operands have issued.  This is the structural ve_1 experiment:
        materialization cost is paid once while both dKV accumulators are
        resident in disjoint TMEM ranges.
        """

        for pass_index in cutlass.range_constexpr(4):
            round_pipeline.consumer_wait(round_state)
            if cutlass.const_expr(pass_index % 2 == 0):
                a_fragment = a_fragment_0
            else:
                a_fragment = a_fragment_1
            if cutlass.const_expr(pass_index == 0):
                b0_fragment = p0_fragment_0
                b1_fragment = p1_fragment_0
            elif cutlass.const_expr(pass_index == 1):
                b0_fragment = p0_fragment_1
                b1_fragment = p1_fragment_1
            elif cutlass.const_expr(pass_index == 2):
                b0_fragment = ds0_fragment_0
                b1_fragment = ds1_fragment_0
            else:
                b0_fragment = ds0_fragment_1
                b1_fragment = ds1_fragment_1

            packed0 = (
                lane0_seq * Int32(self.D_ROUNDS * 4)
                + Int32(round_index * 4 + pass_index)
            )
            ...
            _iket.range_end(issue_token, packed0)

            if has_second:
                packed1 = (
                    lane1_seq * Int32(self.D_ROUNDS * 4)
                    + Int32(round_index * 4 + pass_index)
                )
                issue_token = _iket.range_start(
                    "dVdK_ISSUE(i,r,p)",
                ...
                _iket.range_end(issue_token, packed1)

            round_pipeline.consumer_release(round_state)
            round_state.advance()
        return round_state
    ...
    def kernel(
        ...
        pds_credit_mbars = storage.pds_credit_mbars.data_ptr()
        kscore_safe_mbar = storage.kscore_safe_mbar.ptr
        score_alias_free_mbar = storage.score_alias_free_mbar.ptr
        round_tma_mbars = storage.round_tma_mbars.data_ptr()
        score_epi_safe_mbar = storage.score_epi_safe_mbar.ptr
        ...
        # ve_1 stage 1 is a byte-for-byte clone of the 32-KiB PDS0 arena in
        # score_kv after the odd tile's score source reads complete:
        #   P blocks 8K | P xchg 4K | dS image 8K | dS blocks 8K | spare 4K.
        p_blocks_alias_raw = score_kv_raw
        p_xchg_alias_raw_ptr = score_kv_raw + Int32(4096)
        ds_image_alias_raw = score_kv_raw + Int32(6144)
        ds_blocks_alias_raw = score_kv_raw + Int32(10240)
        p_blocks_alias = (
            cute.make_tensor(
                cute.recast_ptr(
                    p_blocks_alias_raw,
            ...
            cute.make_tensor(
                cute.recast_ptr(
                    p_blocks_alias_raw + self.PDS_BLOCK_ELEMENTS,
                    dkv_b_layout_staged.inner,
                    dtype=self.element_dtype,
                ),
                dkv_b_layout_staged.outer,
        ...
        ds_blocks_alias = (
            ...
            cute.make_tensor(
                cute.recast_ptr(
                    ds_blocks_alias_raw + self.PDS_BLOCK_ELEMENTS,
                    dkv_b_layout_staged.inner,
                    dtype=self.element_dtype,
                ),
                dkv_b_layout_staged.outer,
        ...
        ds_image_alias = cute.make_tensor(
            cute.recast_ptr(
                ds_image_alias_raw,
                dq_b_layout_staged.inner,
                dtype=self.element_dtype,
        ...
        p_xchg_alias_raw = cute.make_tensor(
            p_xchg_alias_raw_ptr,
            flat_pds_block_layout,
        )
        ...
        dq_ds_fragments = (
            dq_tiled_mma.make_fragment_B(ds_image),
            dq_tiled_mma.make_fragment_B(ds_image_alias),
        )
        ...
        p_fragments_alias = (
            dkv_tiled_mma.make_fragment_B(p_blocks_alias[0]),
            dkv_tiled_mma.make_fragment_B(p_blocks_alias[1]),
        )
        ds_fragments_alias = (
            dkv_tiled_mma.make_fragment_B(ds_blocks_alias[0]),
            dkv_tiled_mma.make_fragment_B(ds_blocks_alias[1]),
        )
        ...
        if tidx == Int32(0):
            ...
            for pds_stage in cutlass.range_constexpr(self.PDS_STAGES):
                cute.arch.mbarrier_init(
                    landing_mbars + pds_stage * 2,
                    1,
                )
                cute.arch.mbarrier_init(
                    landing_mbars + pds_stage * 2 + 1,
                    1,
                )
                cute.arch.mbarrier_init(
                    relay_mbars + pds_stage * 2,
                    2,
                )
                cute.arch.mbarrier_init(
                    relay_mbars + pds_stage * 2 + 1,
                    2,
                )
                cute.arch.mbarrier_init(
                ...
                cute.arch.mbarrier_init(
                    pds_credit_mbars + pds_stage,
                    1,
                )
            cute.arch.mbarrier_init(kscore_safe_mbar, 1)
        ...
        t_dkv_pair0 = (
            cute.make_tensor(
                tmem_ptr + self.TMEM_PAIR0_DKV0_OFFSET,
                dkv_c_layout,
            ),
            cute.make_tensor(
                tmem_ptr + self.TMEM_PAIR0_DKV1_OFFSET,
                dkv_c_layout,
            ),
        )
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            # --- ve_1 gather: two score-K generations and two fused KdQ
            # fills per pair.  The next pair cannot acquire score-K until
            # both the leader and W18 release the intervening PDS1 alias.
            ...
            pair_count = (
                tile_count + Int32(self.PAIR_TILES - 1)
            ) // Int32(self.PAIR_TILES)
            if tile_count > Int32(0):
                for pair_iter in cutlass.range(pair_count):
                    lane0_iter = pair_iter * Int32(self.PAIR_TILES)
                    lane1_iter = lane0_iter + Int32(1)
                    has_second = lane1_iter < tile_count
                    ...
                    _iket.range_end(load_k_token, lane0_iter)
                    ...
                    # A full pair repurposes score_kv as PDS1.  This count-2
                    # barrier joins leader consumer death with W18's outgoing
                    # DSM source drain before the next loop may acquire K.
                    if has_second:
                        _mbarrier_wait_acquire_cluster(
            ...
            t_rs_ds_alias = thread_copy_r2s.partition_D(
                ds_image_alias_store
            )
            assert cute.size(t_rs_ds, mode=[4]) == 1
            assert cute.size(t_rs_ds_alias, mode=[4]) == 1
            t_rs_ds_tile = t_rs_ds[None, None, None, None, 0]
            t_rs_ds_alias_tile = t_rs_ds_alias[
                None, None, None, None, 0
            ]
            ...
            aligned_p_blocks_alias_ptr = cute.make_ptr(
                self.element_dtype,
                p_blocks_alias[0].iterator.toint(),
                p_blocks_alias[0].memspace,
                assumed_align=16,
            )
            aligned_ds_blocks_alias_ptr = cute.make_ptr(
                self.element_dtype,
                ds_blocks_alias[0].iterator.toint(),
                ds_blocks_alias[0].memspace,
                assumed_align=16,
            )
            p_local_store = cute.make_tensor(
                cute.recast_ptr(
            ...
            p_local_alias_store = cute.make_tensor(
                cute.recast_ptr(
                    aligned_p_blocks_alias_ptr,
                    score_store_layout.inner,
                    dtype=self.element_dtype,
                ),
                score_store_domain,
            )
            ds_local_alias_store = cute.make_tensor(
                cute.recast_ptr(
                    aligned_ds_blocks_alias_ptr,
                    score_store_layout.inner,
                    dtype=self.element_dtype,
                ),
                score_store_domain,
            ...
            aligned_p_xchg_alias_ptr = cute.make_ptr(
                self.element_dtype,
                p_xchg_alias_raw.iterator.toint()
                - n_owner * Int32(self.PDS_BLOCK_BYTES),
                p_xchg_alias_raw.memspace,
                assumed_align=16,
            )
            ...
            p_xchg_alias_store = cute.make_tensor(
                cute.recast_ptr(
                    aligned_p_xchg_alias_ptr,
                    score_store_layout.inner,
                    dtype=self.element_dtype,
                ),
                score_store_domain,
            ...
            t_rs_p_local_alias = thread_copy_r2s.partition_D(
                p_local_alias_store
            )
            t_rs_ds_local_alias = thread_copy_r2s.partition_D(
                ds_local_alias_store
            )
            t_rs_p_xchg_alias = thread_copy_r2s.partition_D(
                p_xchg_alias_store
            )
            t_rs_ds_xchg = thread_copy_r2s.partition_D(
                ds_xchg_store
            ...
            assert cute.size(t_rs_p_xchg, mode=[4]) == 1
            assert cute.size(t_rs_ds_xchg, mode=[4]) == 1
            assert cute.size(t_rs_p_local_alias, mode=[4]) == 1
            assert cute.size(t_rs_ds_local_alias, mode=[4]) == 1
            assert cute.size(t_rs_p_xchg_alias, mode=[4]) == 1
            t_rs_p_local_tile = t_rs_p_local[
                None, None, None, None, 0
            ]
            ...
            t_rs_p_local_alias_tile = t_rs_p_local_alias[
                None, None, None, None, 0
            ]
            t_rs_ds_local_alias_tile = t_rs_ds_local_alias[
                None, None, None, None, 0
            ]
            t_rs_p_xchg_alias_tile = t_rs_p_xchg_alias[
                None, None, None, None, 0
            ]
            r_score = cute.make_rmem_tensor(
                score_coordinates.shape,
            ...
            for loop_iter in cutlass.range(tile_count):
                ...
                pds_stage = loop_iter % Int32(self.PDS_STAGES)
                pair_phase = (
                    loop_iter // Int32(self.PAIR_TILES)
                ) % Int32(2)
                _mbarrier_wait_acquire_cluster(
                    pds_credit_mbars + pds_stage,
                    pair_phase,
                ...
                assert (
                    t_rs_p_local_alias_tile.shape
                    == r_p_store.shape
                )
                assert (
                    t_rs_ds_local_alias_tile.shape
                    == r_ds_store.shape
                )
                assert (
                    t_rs_p_xchg_alias_tile.shape
                    == r_p_store.shape
                )
                assert t_rs_ds_alias_tile.shape == r_ds_store.shape
                if pds_stage == Int32(0):
                    if owns_n:
                        cute.copy(
                            tiled_copy_r2s,
                            r_p_store,
                    ...
                    if owns_n:
                        cute.copy(
                            tiled_copy_r2s,
                            r_p_store,
                            t_rs_p_local_alias_tile,
                        )
                        cute.copy(
                            tiled_copy_r2s,
                            r_ds_store,
                            t_rs_ds_local_alias_tile,
                        )
                    else:
                        cute.copy(
                            tiled_copy_r2s,
                            r_p_store,
                            t_rs_p_xchg_alias_tile,
                        )
                    cute.copy(
                        tiled_copy_r2s,
                        r_ds_store,
            ...
            for pair_iter in cutlass.range(pair_count):
                lane0_iter = pair_iter * Int32(self.PAIR_TILES)
                lane1_iter = lane0_iter + Int32(1)
                tile_index = tile_count - Int32(1) - lane0_iter
            ...
            # stationary Q/dO panel twice before releasing its round slot.
            ...
            if is_leader_cta:
                ...
                pds_wait = pipeline.make_pipeline_state(
                    pipeline.PipelineUserType.Consumer,
                    self.PDS_STAGES,
                )
                pds_rel = pipeline.make_pipeline_state(
                    pipeline.PipelineUserType.Consumer,
                    self.PDS_STAGES,
                )
                ...
                if tile_count > Int32(0):
                    ...
                    for pair_iter in cutlass.range(pair_count):
                        ...
                        # train.  A later ve revision may sink the dS waits.
                        _mbarrier_wait_acquire_cluster(
                            relay_mbars,
                            pair_phase,
                        )
                        ...
                        round_cons = self._issue_pair_grad_round_ve1(
                            ...
                            p_fragments_alias[0],
                            p_fragments_alias[1],
                            ds_fragments_alias[0],
                            ds_fragments_alias[1],
                            has_second,
                            0,
                            lane0_iter,
                            lane1_iter,
                        ...
                        # The PDS release is after every B-source issue in
                        # the pair.  Only a full pair aliases score_kv.
            ...
            if tile_count > Int32(0):
                ...
                for pair_iter in cutlass.range(pair_count):
                    ...
                    lane1_iter = lane0_iter + Int32(1)
                    has_second = lane1_iter < tile_count
                    # g0/g1 (and g2/g3 for a full pair): each fused KdQ
                    # gather owns both physical stage credits.
                    ...
                    if has_second:
                        ...
                        with cute.arch.elect_one():
                            pipe_round.producer_commit(round_com)
                        round_com.advance()
                        _iket.range_end(route_k_token, lane1_iter)

                    # The remaining eight generations are shared by both
                    # tiles: dO(h0,h1), Q(h0,h1), once per D round.
                    ...
                    # Drain the last in-flight fill (panel gen 7, barrier 1).
                    cute.arch.mbarrier_wait(
                        round_tma_mbars + 1,
                        tma_phase_1,
                    )
            ...
            # --- ve_1 relay/publisher.  W18 owns acquire+commit for PDS;
            # math receives an explicit write credit for each physical
            # arena.  Outgoing DSM source reads are drained before any arena
            # is granted to the next pair.
            if tidx % Int32(32) == Int32(0):
                pds_acq = pipeline.make_pipeline_state(
                    pipeline.PipelineUserType.Producer,
                    self.PDS_STAGES,
                )
                ...
                for pair_iter in cutlass.range(pair_count):
                    ...
                    pipe_pds.producer_acquire(pds_acq)
                    pds_acq.advance()
                    cute.arch.mbarrier_arrive(
                        pds_credit_mbars,
                        rank,
                    ...
                    _iket.range_end(route_p_token, lane0_iter)
                    ...
                    _iket.range_end(route_ds_token, lane0_iter)
                    cute.arch.cp_async_bulk_commit_group()
                    ...
                    if has_second:
                        ...
                        if rank == Int32(0):
                            _cpasync_bulk_s2cluster(
                                p_xchg_alias_raw.iterator,
                                p_block_alias_raw_ptrs[0],
                                landing_mbars + 2,
                                self.PDS_BLOCK_BYTES,
                                peer_rank,
                            )
                            ...
                            _cpasync_bulk_s2cluster(
                                p_xchg_alias_raw.iterator,
                                p_block_alias_raw_ptrs[1],
                                landing_mbars + 2,
                                self.PDS_BLOCK_BYTES,
                                peer_rank,
                            )
                        _iket.range_end(route_p_token, lane1_iter)
                        ...
                        if rank == Int32(0):
                            _cpasync_bulk_s2cluster(
                                ds_image_alias_raw + Int32(2048),
                                ds_block_alias_raw_ptrs[0],
                                landing_mbars + 3,
                                self.PDS_BLOCK_BYTES,
                                peer_rank,
                            )
                            ...
                            _cpasync_bulk_s2cluster(
                                ds_image_alias_raw,
                                ds_block_alias_raw_ptrs[1],
                                landing_mbars + 3,
                                self.PDS_BLOCK_BYTES,
                                peer_rank,
                            )
                        _iket.range_end(route_ds_token, lane1_iter)
                        cute.arch.cp_async_bulk_commit_group()
                        pipe_pds.producer_commit(pds_com)
                        pds_com.advance()
                    ...
                    # Landing proves peer->local completion only.  This wait
                    # separately proves local->peer source-read completion.
                    cute.arch.cp_async_bulk_wait_group(0, read=True)
                    if has_second:
                        cute.arch.mbarrier_arrive(
                            score_alias_free_mbar,
...
# File-based harness registration: V2 is ve_1's active implementation.

...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    * The gradient GEMMs of tile p are issued inside tile t=p-1's S/dP
    ...
    # v7: ONE fused dkv_done generation per tile (covers both D-round
    # slots), so a single stage is required for TMEM-alias safety: the
    # producer_acquire of tile i must guarantee tile i-1's slots were
    # drained before dV(i, r0) overwrites slot 0.  With the v6 layout (one
    # generation per slot) two stages provided exactly that guarantee; with
    # one generation per tile only depth 1 does.
    MMA_DONE_STAGES = 1
    ...
    def _specialize_shared_storage(
        ...
        class SharedStorageV2:
            ...
            dkv_done_mbars: cute.struct.MemRange[cutlass.Int64, 2]
    ...
    def kernel(
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            # --- reduce: one fused drain per tile (both dKV slots on a
            # single dkv_done generation: one wait, back-to-back T2Rs, one
            # fence, one release, then both atomic bursts).
    ...
    def _issue_prev_grads_head_v2(
        ...
        # v7 fused drain: this single acquire guards BOTH dKV slots for the
        # tile (one dkv_done generation per tile, committed after the
        # round-1 passes in the tail half).
    ...
    def _issue_prev_grads_tail_v2(
        ...
        v7 fused drain: no acquire here -- the head half's single acquire
        already guards both slots; this half ends with the tile's one
        dkv_done commit.
        """
    ...
    def _drain_dkv_v7(
        ...
        """Drain BOTH rank-owned dKV slots on one fused generation.
        ...
        column scramble decoded by convert_canonical).  v7 fuses the two
        per-round calls into one: a single dkv_done wait, back-to-back T2Rs
        of slot 0 and slot 1, ONE tmem-load fence, ONE release, then both
        atomic bursts from registers.  The KV-index preload is shared (both
        slots cover the same N rows; they differ only in the D panel:
        sub_tile = 2*round + rank).  v6's drain time was volume-independent
        -- fixed per-call costs dominate -- so halving the call count is
        the lever.  The inherited reduce_sync_barrier is dropped: each
        thread releases after its own fenced loads, and the producer's
        acquire counts all 256 consumer arrivals, so no extra intra-role
        barrier is needed.
        """

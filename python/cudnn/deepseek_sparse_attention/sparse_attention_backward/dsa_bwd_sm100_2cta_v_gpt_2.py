"""v_gpt_2: vk_7 + owner-push -- peer-h panel quadrants come from the
owner CTA's stationary panel over DSM, not from GMEM.

Every quadrant window (round r, M-half c) of a given h-half is a flat,
byte-identical 16 KiB slice of the OWNING CTA's stationary panel at
element offset 4096*(4r+2c) -- the S1 dual-view identity, machine-checked
over all four windows (probe_vgpt_dualview r2).  vk_5/7 already exploit
this for the own leg (one local bulk).  v_gpt_2 completes the symmetry:
the owner also pushes the PEER's M-half window into the peer's ring slot
as one flat s2cluster bulk, and the peer-h GMEM TMA leg retires.

Per generation (h constexpr, owner = CTA with rank == h):
  owner:  arm own mbar -> local bulk (own window) ->
          arm peer mbar remotely -> push (peer window)   [4 elect_one ops]
  other:  nothing (data and expect_tx both arrive from the owner)
Remote-arm-then-push in one thread's program order is the relay P/dS
discipline: expect_tx always lands before its bytes.  Phase freshness:
acquire(g) implies the UMMA-tracked multicast release of gen g-2 reached
BOTH CTAs, so the peer's tma mbar finished phase g-2 before any gen-g arm.

What it buys (v_gpt_1 autopsy evidence): -48 KiB/tile/CTA of GMEM round
trips (L2 bandwidth freed for the reducer atomics and LOAD_K) and a
shorter fill latency (DSM ~0.17us/4KB routing vs GMEM TMA round trip).
Engine op count per CTA per tile is unchanged (6 x 16 KiB).  Bytes are
bit-identical to the TMA path (same mQT/mdOT source data staged at token
start), so this is a pure transport swap under the precision iron rule.
W19 waiter/committer, leader, gather, kdq, loan: untouched.

Inherited vk_7 notes: vk_5 + W2 (kdq index loads hoist ahead of barrier A).
            ...
            if tile_count > Int32(0):
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    for flat_gen in cutlass.range_constexpr(6):
                        ...
                        pipe_round.producer_acquire(round_acq)
                        round_acq.advance()
                        # v_gpt_2 (owner-push): both CTAs' slots are
                        # supplied by the h-half OWNER from its stationary
                        # panel (windows at 4096*(4r+2c) elements, S1
                        # identity).  Remote arm precedes the push in this
                        # thread's program order; the non-owner issues
                        # nothing for this generation.
                        if cutlass.const_expr(tensor_kind == 0):
                            gen_panel_raw = stationary_do_raw
                        else:
                            gen_panel_raw = stationary_q_raw
                        if cutlass.const_expr(h_half == 0):
                            gen_slot_raw = round_buf_a_raw
                            gen_mbar = round_tma_mbars
                        else:
                            gen_slot_raw = round_buf_b_raw
                            gen_mbar = round_tma_mbars + 1

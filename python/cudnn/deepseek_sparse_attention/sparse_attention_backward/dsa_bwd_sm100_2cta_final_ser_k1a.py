"""DeepSeek Sparse Attention backward, two-CTA SERIAL chain, K1a regs.

final_ser_k1a = final_ser with ONLY the K1a register relayout
(48/64/128/120 for gather/producers/math/reduce, replacing the
pre-K1a 48/48/128/128 the final lineage shipped with).  Single
variable against final_ser for the B300 P0 spill finding (272 B stack
frames, dynamic local loads 65,536 -> 20,340,736); on GR100 the same
relayout cut the native build by 54% (ledger K1a, commit 4f683ca) and
k13 closed the register axis at this point.  If the spill is a shared
inflator, the ring inter-pass gaps (W17 producer hotspot) and the
reducer atomic burst (2M STL) should shrink alongside the publish
chain -- watch those spans, not just the wall.
...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    def kernel(
        ...
        # k1a: K1a register relayout 48/64/128/120 for gather/producers/
        # math/reduce (GR100 local optimum, rubin_1.py:102; the final
        # lineage shipped pre-K1a at 48/48/128/128 and the 48-reg
        # W16-19 producers spill hard -- B300 review: 272 B stack
        # frames, 20M dynamic local loads).  HARD CONSTRAINT
        # (v9_3.py:12440, re-learned via a setmaxregister deadlock on
        # B300): setmaxregister redistributes within the CTA LAUNCH
        # allocation, 640 x 96 = 61,440 -- not the 65,536 physical
        # file.  128x48 + 128x64 + 128x128 + 256x120 = 61,440 exactly.

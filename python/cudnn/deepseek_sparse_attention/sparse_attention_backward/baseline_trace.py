...
# baseline_trace: IKET-instrumented copy of the baseline (single-CTA) kernel
# for critical-path adjudication against the 2-CTA v12 trace.  24 range
# names + 4 role marks = 28 (cap 29).  Payloads: (i) = execution-order tile
# index; (i,part) = 2*i + part.  Discriminators for "is atomic dKV on the
# baseline critical path": DKV_SLOT(i,part) (mma waits reducer slot release)
# and T2R_BAR(i,part) (mma waits reducer T2R completion).  Only the
# same_hdim_kv paths are instrumented in reduce (the D512 shape).
#
# Span nesting/inclusion semantics (do NOT sum interior spans with parents):
# S_ISSUE/dP_ISSUE INCLUDE their producer_acquire (S/dP TMEM slot wait);
# DVDK_ISSUE(part0) nests T2R_BAR(part0) and WAIT_dS; DVDK_ISSUE(part1)
# nests T2R_BAR(part1) and the compute_mma_P consumer_release.
# Adjudication notes: DKV_SLOT payloads 0 and 1 hit initially-empty slots
# (2-stage pipeline) -- the reducer-gated signal starts at payload >= 2.
# T2R_BAR pairing is off by one on part0: T2R_BAR(2i) <-> R_T2R(2i-1)
# (t2r_dKV4 barrier, previous iteration part1); T2R_BAR(2i+1) <-> R_T2R(2i).
# Compute warps have small unattributed gaps at the two
# fence+compute_sync_barrier points (between SOFTMAX and P_STORE, and
# between DS_MATH and DS_STORE).
class FlashAttentionDSABackwardSm100:
    ...
    def bwd(
        ...
        if warp_idx == self.load_warp_id:
            ...
            load_qdo_token = _iket.range_start("LOAD_QDO", Int32(0))
            self.load(
                QK_tiled_mma,
                dOV_tiled_mma,
                tma_atom_Q,
            ...
            _iket.mark("ROLE_REDUCE", Int32(0))
            tmem.wait_for_alloc()
            tmem_ptr_base = tmem.retrieve_ptr(self.acc_dtype)
            ...
            _iket.mark("ROLE_LOADKV", Int32(0))
            self.load_KV(
                mKV,
                mTopkIdxs,
                sK,
    ...
    def load_KV(
        ...
        kv_acq_token = _iket.range_start("KV_ACQ(i)", Int32(0))
        load_mma_K_pipeline.producer_acquire(load_mma_K_producer_state)
        _iket.range_end(kv_acq_token, Int32(0))
        load_kv_token = _iket.range_start("LOAD_KV(i)", Int32(0))
        ...
        _iket.range_end(load_kv_token, Int32(0))
        tile_index -= 1
        ...
        while tile_index >= 0:
            ...
            iter_idx = tile_count - 1 - tile_index
            kv_acq_token = _iket.range_start("KV_ACQ(i)", iter_idx)
            load_mma_K_pipeline.producer_acquire(load_mma_K_producer_state)
            _iket.range_end(kv_acq_token, iter_idx)
            load_kv_token = _iket.range_start("LOAD_KV(i)", iter_idx)
            sK_slice = sK[(None, None), 0, (None, None), load_mma_K_producer_state.index]
            sK_slice = cute.composition(sK_slice, cute.make_layout((self.block_tile, self.head_dim)))
            ...
            load_mma_K_pipeline.producer_commit(load_mma_K_producer_state)
            load_mma_K_producer_state.advance()
            _iket.range_end(load_kv_token, iter_idx)
            tile_index -= 1
    ...
    def mma(
        ...
        while tile_index >= 0:
            ...
            wait_kv_token = _iket.range_start("WAIT_KV(i)", iter_idx)
            load_mma_K_pipeline.consumer_wait(load_mma_K_consumer_state)
            _iket.range_end(wait_kv_token, iter_idx)
            s_issue_token = _iket.range_start("S_ISSUE(i)", iter_idx)
            ...
            _iket.range_end(s_issue_token, iter_idx)

            # Gemm dP = dO @ V
            dp_issue_token = _iket.range_start("dP_ISSUE(i)", iter_idx)
            ...
            _iket.range_end(dp_issue_token, iter_idx)

            # Gemm dKV = dO @ P part1
            wait_p_token = _iket.range_start("WAIT_P(i)", iter_idx)
            compute_mma_P_pipeline.consumer_wait(compute_mma_P_consumer_state)
            _iket.range_end(wait_p_token, iter_idx)
            dkv_slot_token = _iket.range_start(
                "DKV_SLOT(i,part)", iter_idx * Int32(2)
            )
            mma_reduce_dKV_pipeline.producer_acquire(mma_reduce_dKV_producer_state)
            _iket.range_end(dkv_slot_token, iter_idx * Int32(2))
            dvdk_issue_token = _iket.range_start(
                "DVDK_ISSUE(i,part)", iter_idx * Int32(2)
            )
            ...
            if cutlass.const_expr(self.same_hdim_kv):
                if not is_first_mma:
                    t2r_bar_token = _iket.range_start(
                        "T2R_BAR(i,part)", iter_idx * Int32(2)
                    )
                    self.t2r_dKV4_done_barrier.arrive_and_wait()
                    _iket.range_end(t2r_bar_token, iter_idx * Int32(2))
            dOP_tiled_mma.set(tcgen05.Field.ACCUMULATE, False)
            for k_block in cutlass.range(0, cute.size(tdKVrP, mode=[2]), unroll=2):
                cute.gemm(
                    dOP_tiled_mma,
            ...
            wait_ds_token = _iket.range_start("WAIT_dS(i)", iter_idx)
            compute_mma_dS_pipeline.consumer_wait(compute_mma_dS_consumer_state)
            _iket.range_end(wait_ds_token, iter_idx)
            ...
            _iket.range_end(dvdk_issue_token, iter_idx * Int32(2))
            ...
            # Gemm dQ = K @ dS
            dq_issue_token = _iket.range_start("DQ_ISSUE(i)", iter_idx)
            ...
            _iket.range_end(dq_issue_token, iter_idx)
            # KV is used
            ...
            if cutlass.const_expr(not self.same_hdim_kv):
                self.t2r_dKV4_done_barrier.arrive_and_wait()
            dkv_slot_token = _iket.range_start(
                "DKV_SLOT(i,part)", iter_idx * Int32(2) + Int32(1)
            )
            mma_reduce_dKV_pipeline.producer_acquire(mma_reduce_dKV_producer_state)
            _iket.range_end(dkv_slot_token, iter_idx * Int32(2) + Int32(1))
            dvdk_issue_token = _iket.range_start(
                "DVDK_ISSUE(i,part)", iter_idx * Int32(2) + Int32(1)
            )
            # dKV2
            dOP_tiled_mma.set(tcgen05.Field.ACCUMULATE, False)
            for k_block in cutlass.range(0, cute.size(tdKVrP, mode=[2]), unroll=2):
            ...
            if cutlass.const_expr(self.same_hdim_kv):
                t2r_bar_token = _iket.range_start(
                    "T2R_BAR(i,part)", iter_idx * Int32(2) + Int32(1)
                )
                self.t2r_dKV01_done_barrier.arrive_and_wait()
                _iket.range_end(t2r_bar_token, iter_idx * Int32(2) + Int32(1))
            dOP_tiled_mma.set(tcgen05.Field.ACCUMULATE, False)
            for k_block in cutlass.range(0, cute.size(tdKVrP, mode=[2]), unroll=2):
                cute.gemm(
                    dOP_tiled_mma,
            ...
            mma_reduce_dKV_pipeline.producer_commit(mma_reduce_dKV_producer_state)
            mma_reduce_dKV_producer_state.advance()
            _iket.range_end(dvdk_issue_token, iter_idx * Int32(2) + Int32(1))
    ...
    def compute(
        ...
        while tile_index >= 0:
            iter_idx = tile_count - 1 - tile_index
            wait_s_token = _iket.range_start("WAIT_S(i)", iter_idx)
            mma_compute_S_pipeline.consumer_wait(mma_compute_S_consumer_state)
            _iket.range_end(wait_s_token, iter_idx)
            p_acq_token = _iket.range_start("P_ACQ(i)", iter_idx)
            compute_mma_P_pipeline.producer_acquire(compute_mma_P_producer_state)
            _iket.range_end(p_acq_token, iter_idx)
            softmax_token = _iket.range_start("SOFTMAX(i)", iter_idx)
            ...
            _iket.range_end(softmax_token, iter_idx)

            cute.arch.fence_view_async_tmem_load()
            self.compute_sync_barrier.arrive_and_wait()

            # ======= stsm ============
            p_store_token = _iket.range_start("P_STORE(i)", iter_idx)
            ...
            wait_dp_token = _iket.range_start("WAIT_dP(i)", iter_idx)
            mma_compute_dP_pipeline.consumer_wait(mma_compute_dP_consumer_state)
            _iket.range_end(wait_dp_token, iter_idx)
            ds_acq_token = _iket.range_start("DS_ACQ(i)", iter_idx)
            compute_mma_dS_pipeline.producer_acquire(compute_mma_dS_producer_state)
            _iket.range_end(ds_acq_token, iter_idx)
            ds_math_token = _iket.range_start("DS_MATH(i)", iter_idx)
            ...
            _iket.range_end(ds_math_token, iter_idx)
            ...
            ds_store_token = _iket.range_start("DS_STORE(i)", iter_idx)
        ...
        dq_epi_token = _iket.range_start("DQ_EPI", Int32(0))
        ...
        _iket.range_end(dq_epi_token, Int32(0))
    ...
    def reduce_dKV(
        ...
        while tile_index >= 0:
            ...
            iter_idx = tile_count - 1 - tile_index
            wait_dkv_token = _iket.range_start(
                "WAIT_DKV(i,part)", iter_idx * Int32(2)
            )
            mma_reduce_dKV_pipeline.consumer_wait(mma_reduce_dKV_consumer_state)
            _iket.range_end(wait_dkv_token, iter_idx * Int32(2))
            ...
            if cutlass.const_expr(not self.same_hdim_kv):
                ...
                r_t2r_token = _iket.range_start(
                    "R_T2R(i,part)", iter_idx * Int32(2)
                )
                rdKV1 = self.t2r_dKV(tdKVtdKV1)
                cute.arch.fence_view_async_tmem_load()
                self.t2r_dKV01_done_barrier.arrive_and_wait()
                _iket.range_end(r_t2r_token, iter_idx * Int32(2))
                r_atomic_token = _iket.range_start(
                    "R_ATOMIC(i,part)", iter_idx * Int32(2)
                )
                self.store_dKV(mdKV_acc, tdKVtdKV0, rTopkIdx, 0)
                self.reduce_dKV_from_reg(mdKV_acc, rdKV1, rTopkIdx, 1)
                _iket.range_end(r_atomic_token, iter_idx * Int32(2))
            ...
            wait_dkv_token = _iket.range_start(
                "WAIT_DKV(i,part)", iter_idx * Int32(2) + Int32(1)
            )
            mma_reduce_dKV_pipeline.consumer_wait(mma_reduce_dKV_consumer_state)
            _iket.range_end(wait_dkv_token, iter_idx * Int32(2) + Int32(1))

            if cutlass.const_expr(self.same_hdim_kv):
                r_t2r_token = _iket.range_start(
                    "R_T2R(i,part)", iter_idx * Int32(2) + Int32(1)
                )
                rdKV3 = self.t2r_dKV(tdKVtdKV3)
                cute.arch.fence_view_async_tmem_load()
                self.t2r_dKV4_done_barrier.arrive_and_wait()
                _iket.range_end(r_t2r_token, iter_idx * Int32(2) + Int32(1))
                r_atomic_token = _iket.range_start(
                    "R_ATOMIC(i,part)", iter_idx * Int32(2) + Int32(1)
                )
                self.store_dKV(mdKV_acc, tdKVtdKV2, rTopkIdx, 2)
                self.reduce_dKV_from_reg(mdKV_acc, rdKV3, rTopkIdx, 3)
                _iket.range_end(r_atomic_token, iter_idx * Int32(2) + Int32(1))
            else:
                self.store_dKV(mdKV_acc, tdKVtdKV2, rTopkIdx, 2)

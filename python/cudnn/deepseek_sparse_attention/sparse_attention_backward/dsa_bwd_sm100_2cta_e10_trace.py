...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    def _specialize_shared_storage(
        ...
        class SharedStorageV2:
            ...
            ds_ready_mbars: cute.struct.MemRange[cutlass.Int64, 1]
    ...
    def _issue_dq_rounds_v2(
        ...
        Round r consumes the round-region generation holding K_dQ(r).  Both
        waits front-run the issue pair (legal: W17 commits the two K_dQ
        credits back-to-back at one rendezvous point), the tcgen05 store
        fence runs once after both rounds, and the releases trail through a
        cloned state -- keeping the per-round fence/release serial segment
        out of the inter-round path; next-tile kdq refill anchors sit a
        full period away.
        """
    ...
    def kernel(
        ...
        ds_ready_mbars = storage.ds_ready_mbars.data_ptr()
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            for loop_iter in cutlass.range(tile_count):
                ...
                # Whole-image dS store for the dQ B operand, hoisted to
                # the front of the publish sequence: its own fence and
                # ds_ready arrive let W18 grant the leader's dQ gate
                # before the block/xchg stores and DSM pushes.
            ...
            if tidx % Int32(32) == Int32(0):
                ...
                for loop_iter in cutlass.range(tile_count):
                    # dS-early split gate: publish the whole-image dS
                    # stores to the leader's dQ rounds ahead of the full
                    # pds chain (block/xchg stores + DSM pushes).
                    cute.arch.mbarrier_wait(
                        ds_ready_mbars,
                        ds_ready_phase,
                    )
    ...
    def _issue_prev_grads_head_v2(
        ...
        # dS-early split gate: both CTAs' whole-image dS stores of the
        # previous tile are published -- the dQ rounds' only P/dS gate.
        # The full publication + DSM-issue proof (invariant I1) moves to
        # the pds wait ahead of the P/dS-block consumers below.
        ...
        # published (and its DSM sends issued): invariant I1's gate for
        # the P/dS-block consumers below.

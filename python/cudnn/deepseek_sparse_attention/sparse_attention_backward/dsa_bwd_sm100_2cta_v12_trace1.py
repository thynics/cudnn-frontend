# v12_trace1: the canonical refined-trace collection variant of v12.
# Byte-identical fork of vm5probe (c7a98f4): waits instrumented as separate
# spans (MAT_ACQ/MAT_WAIT/RK_ACQ/...) so they render as bubbles, plus the W19
# hardware-completion-edge probes (PROBE_GEN/PROBE_DONE) for true GEMM
# windows.  Collect with:  run_b200_pipeline.sh --impl v12_trace1 --mode all
# --download-raw;  render with the v12_trace1 bubble pipeline (work solid,
# waits hollow).
...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    * The gradient GEMMs of tile p are issued inside tile t=p-1's S/dP
    ...
    LOAD_WARP = 17
    RELAY_WARP = 18
    # vm5probe: W19 is the M5 hardware-completion probe (see the
    # IDLE_WARP branch in the kernel body).  Instrumentation-only
    # variant -- never a performance candidate.
    IDLE_WARP = 19
    # Bounded spin budget per observed flip; one timeout permanently
    # disarms the probe (fail-soft, no hang).  ~4.2M polls at ~20-40
    # ns/poll bounds the worst case near 100-170 ms, once per launch.
    PROBE_SPIN_CAP = 1 << 22
    ...
    def kernel(
        ...
        # vm5probe: mbar pointers watched by the W19 probe.  Extracted
        # here for the same reason as the raws above -- the storage
        # struct cannot cross a dynamic-if region (review fix #1).
        # Layout per pipeline create(): [full x stages][empty x stages].
        probe_round_empty_base = (
            storage.round_mbars.data_ptr() + self.ROUND_STAGES
        )
        probe_s_done_full_base = storage.s_done_mbars.data_ptr()
        probe_dp_done_full_base = storage.dp_done_mbars.data_ptr()
        probe_dkv_full_base = storage.dkv_done_mbars.data_ptr()
        probe_pds_empty_ptr = storage.pds_mbars.data_ptr() + 1
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            for loop_iter in cutlass.range(tile_count):
                # vm5probe: WAIT_S span retired (name budget handed to
                # the M5 probe set); the wait is unchanged and its
                # hardware edge is re-provided by PROBE_DONE(k=0).
                ...
                # vm5probe: WAIT_dP span retired; hardware edge
                # re-provided by PROBE_DONE(k=1).
                ...
                # vm5probe: MATH_PDS_ACQ span retired (settled: 2.20 us
                # full-run mean; its end edge is dual-certified against
                # WAIT_dK min-ends).  The acquire itself is unchanged.
                ...
                # vm5probe: MATH_BAR1 span retired; the
                # release-semantics arrive is unchanged.
                cute.arch.mbarrier_arrive(
                    pds_ready_mbars,
                    rank,
                )
            ...
            if tile_count > Int32(0):
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    # vm5probe: RK_ACQ isolates the kdq credit
                    # back-pressure (waits the leader's g8/g9 releases of
                    # tile t-1) from the gather handshake that follows.
                    ...
                    for flat_gen in cutlass.range_constexpr(8):
                        ...
                        # vm5probe: MAT_ACQ = round-credit back-pressure
                        # for this gen (waits the leader's completion-
                        # driven release of gen q-2).
                        ...
                        if cutlass.const_expr(flat_gen > 0):
                            # vm5probe: MAT_WAIT = raw fill-completion
                            # wait for gen q-1 (intrinsic TMA/L2 latency
                            # exposure, back-pressure excluded).
            ...
            # ===============================================================
            # vm5probe (M5 pipe-budget gate): W19 hardware-completion probe.
            #
            # Every watched arrive below is a tcgen05 commit on the pair's
            # single in-order UMMA pipe, so the mbarriers flip in exactly
            # the leader's issue order:
            #   iteration t: s_done(t), dp_done(t), then for the grads
            #   block of tile b = t-1: round gens g0..g5 (two dQ rounds +
            #   head passes p0..p3), dkv slot-0, g6..g9 (tail passes),
            #   dkv slot-1, pds release (== whole block's UMMA retired).
            # The probe only OBSERVES: mbarrier_try_wait is a parity read
            # and never arrives, so the watched pipelines cannot be
            # perturbed.  Each detection is bracketed in an IKET range
            # whose END edge is the hardware completion timestamp (plus
            # polling granularity, tens of ns -- far below the ~0.25 us
            # consumer wake latency this probe exists to remove).
            #
            # Parities are pure functions of the indices (no carried
            # phase state, so a model bug cannot desynchronize silently;
            # it times out instead):
            #   s/dp_done full  stage t%2   parity (t//2)%2   1 flip/tile
            #   round empty     stage g%2   parity (b+g//2)%2 10 flips/blk
            #   dkv_done full   stages 0,1  parity b%2         2 flips/blk
            #   pds empty       stage 0     parity b%2         1 flip/blk
            # Fail-soft: every wait is a bounded spin (PROBE_SPIN_CAP);
            # the first timeout permanently disarms the probe (alive=0),
            # so an order/parity error truncates the PROBE stream instead
            # of hanging the kernel at the common-tail rendezvous.
            #
            # Payloads: PROBE_GEN(i,g) = b*16+g; PROBE_DONE(i,k) = t*8+k
            # with k: 0=S(t) done, 1=dP(t) done, 2=dkv slot0(b),
            # 3=dkv slot1(b), 4=pds(b) released (k>=2 use b, the grads
            # tile, in the tile field).
            # ===============================================================
            round_empty_base = probe_round_empty_base
            s_done_full_base = probe_s_done_full_base
            dp_done_full_base = probe_dp_done_full_base
            dkv_full_base = probe_dkv_full_base
            pds_empty_ptr = probe_pds_empty_ptr
            alive = Int32(1)
            for probe_iter in cutlass.range(tile_count):
                sd_stage = probe_iter % Int32(2)
                sd_parity = (probe_iter // Int32(2)) % Int32(2)
                # Pointer offsets must stay constexpr (dynamic
                # cute.Pointer arithmetic needs the make_ptr/toint
                # pattern -- v9_3 lesson); select the ping-pong stage
                # with a dynamic branch over two constant pointers.
                probe_done_tok = _iket.range_start(
                    "PROBE_DONE(i,k)",
                    probe_iter * Int32(8),
                )
                if sd_stage == Int32(0):
                    alive = self._probe_wait_flip_v2(
                        s_done_full_base,
                        sd_parity,
                        alive,
                    )
                ...
                if probe_iter > Int32(0):
                    alive = self._probe_grads_block_v2(
                        round_empty_base,
                        dkv_full_base,
                        pds_empty_ptr,
                        probe_iter - Int32(1),
    ...
    @cute.jit
    def _probe_wait_flip_v2(
        self,
        mbar_ptr,
        parity: Int32,
        alive: Int32,
    ) -> Int32:
        """Bounded observe-only wait for one mbarrier phase flip.

        Pure parity read (mbarrier_try_wait) -- never an arrive, so the
        watched pipeline cannot be perturbed.  Returns alive=0 on
        timeout, which the caller uses to disarm the entire probe; the
        kernel then falls through to the common tail instead of hanging
        (worst case one spin budget, roughly 100 ms, once per launch).
        """
        new_alive = alive
        if alive > Int32(0):
            budget = Int32(self.PROBE_SPIN_CAP)
            hit = Int32(0)
            while budget > Int32(0):
                if _mbarrier_try_wait(mbar_ptr, parity):
                    hit = Int32(1)
                    budget = Int32(0)
                else:
                    budget = budget - Int32(1)
            if hit == Int32(0):
                new_alive = Int32(0)
        return new_alive

    @cute.jit
    def _probe_grads_block_v2(
        self,
        round_empty_base,
        dkv_full_base,
        pds_empty_ptr,
        ...
        """Observe the 13 hardware completions of one grads block.

        Flip order is the leader's issue order on the in-order tcgen05
        pipe: g0..g5 (dQ rounds + head passes p0..p3), dkv slot-0
        commit, g6..g9 (tail passes), dkv slot-1 commit, pds release.
        The round-empty flips are the leader's completion-driven
        consumer_releases -- exactly the credits W17's RK_ACQ/MAT_ACQ
        waits are gated on, timestamped with no wake latency.
        """
        blk_parity = block_iter % Int32(2)
        new_alive = alive
        for g in cutlass.range_constexpr(10):
            if cutlass.const_expr(g == 6):
                # dkv slot-0 commit lands between g5 and g6.
                probe_done_tok = _iket.range_start(
                    "PROBE_DONE(i,k)",
                    block_iter * Int32(8) + Int32(2),
                )
                new_alive = self._probe_wait_flip_v2(
                    dkv_full_base,
                    blk_parity,
                    new_alive,
                )
            ...
            probe_gen_tok = _iket.range_start(
                "PROBE_GEN(i,g)",
                block_iter * Int32(16) + Int32(g),
            )
            new_alive = self._probe_wait_flip_v2(
                round_empty_base + (g % 2),
                (block_iter + Int32(g // 2)) % Int32(2),
                new_alive,
            )
            _iket.range_end(
                probe_gen_tok,

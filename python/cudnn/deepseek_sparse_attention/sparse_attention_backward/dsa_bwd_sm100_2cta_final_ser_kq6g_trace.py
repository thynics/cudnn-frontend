"""IKET-only derivative of final_ser_kq6g.

The release twin's dKV issue path uses row-contiguous 512-byte
cp.reduce.async.bulk groups with two private SMEM row stages per reducer
warpgroup.  Existing IKET span names and boundaries remain unchanged.

IKET-only derivative (kq6c: 48/64/136/116 register relayout) of the two-CTA SERIAL-chain KQ build.

TWIN UPDATE 2026-08-11: now kq4c-equivalent (kq2 count-4 closes +
kq4c packed math) with the 9-name math span set -- capture gives the
post-kq4c P-chain decomposition directly.

Source provenance: SHA256 b89ccda6680c9c5359c17152da1d2f2988f6ed460207d90e775c20fe59cfa46b (dsa_bwd_sm100_2cta_final_ser_kq4c.py)
This file lives under ``outputs`` deliberately: annotations are diagnostic
and must never be used for release-performance measurements.  Apart from
the no-op-capable IKET proxy and ranges/role marks, it mirrors that source.

DeepSeek Sparse Attention backward, two-CTA SERIAL chain, kq build.
...
@dsl_user_op
def _cpasync_bulk_reduce_add_f32(
    source: cute.Pointer,
    destination: cute.Pointer,
    reduce_bytes: int | Int32,
    *,
    loc=None,
    ip=None,
) -> None:
    """Issue one FP32 shared-to-global async bulk add."""

    source_i32 = source.toint(loc=loc, ip=ip).ir_value()
    llvm.inline_asm(
        None,
        [
            destination.llvm_ptr,
            source_i32,
            Int32(reduce_bytes).ir_value(loc=loc, ip=ip),
        ],
        (
...
class FlashAttentionDSABackwardSm100TwoCTA(FlashAttentionDSABackwardSm100):
    ...
    DKV_REDUCE_WGS = 2
    DKV_REDUCE_STAGES = 2
    DKV_REDUCE_ROW_ELEMENTS = D_TILE_CTA
    DKV_REDUCE_ROW_BYTES = DKV_REDUCE_ROW_ELEMENTS * (Float32.width // 8)
    DKV_REDUCE_WG_STRIDE_ELEMENTS = (
        DKV_REDUCE_STAGES * DKV_REDUCE_ROW_ELEMENTS
    )
    DKV_REDUCE_STAGING_ELEMENTS = (
        DKV_REDUCE_WGS * DKV_REDUCE_WG_STRIDE_ELEMENTS
    )
    DKV_REDUCE_STAGING_BYTES = (
        DKV_REDUCE_STAGING_ELEMENTS * (Float32.width // 8)
    )
    DKV_REDUCE_ALIGN_BYTES = 16
    DKV_REDUCE_BARRIER_ID = 8
...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    chain of tile t issues in order S(t) -> dP(t) -> dV r0 -> dV r1 ->
    dK r0 -> dK r1 -> dQ both rounds, with every gradient A operand
    (dO0..dO3, Q0..Q3) streaming through the 16-micro-generation true-K32
    four-slot round ring.  P is published early (own p_ready arrival + immediate relay
    send) so the P exchange lands under the dP shadow; dS publishes
    under the dV window.  The kdq rendezvous hands the gather-filled
    dQ-A images to the ring through a count-128 hardware arrival at
    the END of the ring cycle; the score K tile is released after dP,
    after which score_kv immediately hosts the NEXT tile's score K
    gather (no dO loan in this build).  dQ stays TMEM-resident across
    ...
    # kq build: the K_dQ pair rides a kscore generation in score_kv.
    # kq6k B plan: four independent K32 slots publish each old K64 panel
    # as two H32 micro-generations.  Static order is
    # G0-k0,G0-k1,...,G7-k0,G7-k1; 16 % 4 == 0 keeps the stage/raw-barrier
    # map tile-invariant.
    ...
    def _specialize_shared_storage(
        ...
        # kq6g shape (a): two contiguous 512-byte row slabs per reducer
        # warpgroup.  The layout assertions are compile-time contracts for
        # the inline PTX alignment/size requirements and row contiguity.
        dkv_reduce_staging_layout = cute.make_layout(
            (
                self.DKV_REDUCE_WGS,
                self.DKV_REDUCE_STAGES,
                self.DKV_REDUCE_ROW_ELEMENTS,
            ),
            stride=(
                self.DKV_REDUCE_WG_STRIDE_ELEMENTS,
                self.DKV_REDUCE_ROW_ELEMENTS,
                1,
            ),
        )
        assert self.DKV_REDUCE_ALIGN_BYTES % 16 == 0
        assert self.DKV_REDUCE_ROW_BYTES % 16 == 0
        assert self.DKV_REDUCE_STAGING_BYTES % 16 == 0
        assert self.DKV_REDUCE_STAGING_BYTES <= 2 * 1024
        assert self.SOFTMAX_GROUPED_STATS
        assert self.H_TILE_CTA * 2 <= self.DKV_REDUCE_STAGING_ELEMENTS
        assert dkv_reduce_staging_layout.stride[2] == 1
        assert (
            cute.cosize(dkv_reduce_staging_layout)
            == self.DKV_REDUCE_STAGING_ELEMENTS
        )
        ...
        class SharedStorageV2:
            ...
            # Overlay the retired-after-hoist 512-byte stats image.  Math
            # copies those values to registers before any dKV generation
            # can reach the reducers, so the full 2 KiB tail is then free.
            dkv_reduce_staging: cute.struct.Align[
                cute.struct.MemRange[
                    Float32,
                    self.DKV_REDUCE_STAGING_ELEMENTS,
                ],
    ...
    def _gather_kdq_v8(
        ...
        # k1: no cta_id operand -- a local shared::cta arrive.  Passing
        # any cta_id (even self) forces mapa + a shared::cluster-encoded
        # arrive in the DSL, ~0.4us per 128-thread close (DSM review).
        cute.arch.mbarrier_arrive(kdq_ready_mbar)
    ...
    def _issue_dq_rounds_kq(
        ...
        """Issue both dQ rounds off ONE kscore generation (kq).

        Both K_dQ round panels live simultaneously in the two score_kv
        halves, so a single consumer wait covers both rounds; the
        release recycles score_kv for the next tile's score K gather.
        """

        packed_issue = issue_seq * Int32(self.D_ROUNDS)
        # KQ owns both K_dQ panels in one kscore generation, so there is
        # one real data wait (payload r=0), not a fabricated r=1 wait.
        ...
        for round_index in cutlass.range_constexpr(self.D_ROUNDS):
            round_issue = packed_issue + Int32(round_index)
    ...
    def kernel(
        ...
        dkv_reduce_staging_raw = storage.dkv_reduce_staging.data_ptr()
        ...
        softmax_stats = storage.dkv_reduce_staging.get_tensor(
            cute.make_layout(
                (self.H_TILE_CTA, 2),
        ...
        # kq6k compile-time release gates.  Fragments are built directly
        ...
        if tidx == Int32(0):
            ...
            # kq4c-equiv twin: count-4 closes (kq2).
            cute.arch.mbarrier_init(
                pds_ready_mbars,
                self.MATH_WARPS,
            )
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            if tile_count > Int32(0):
                ...
                # kq steady state: score_kv cycles TWO kscore
                # generations per tile -- K_dQ(t) (filled as soon as
                # dP(t) releases the score K, consumed mid-chain by
                # dQ(t)) and then the NEXT tile's score K (filled once
                # dQ(t) releases the kdq loan, covered by the dK
                # window).  No ring rendezvous: the kdq pair is an
                # ordinary kscore generation.
                for score_iter in cutlass.range(
                    Int32(0),
                    tile_count,
                ):
                    kdq_gather_token = _iket.range_start(
                        "KDQ_GATHER(i)",
                        score_iter,
                    )
                    pipe_kscore.producer_acquire(gather_state)
            ...
            # loop (loop-invariant, un-unioned SMEM, final after
            # math_barrier) -- kills the per-tile LDS -> R2UR
            # serialization head on the exposed relay-0 chain.
            softmax_scale_log2_e = scale_softmax * Float32(
                math.log2(math.e)
            )
            ...
            for loop_iter in cutlass.range(tile_count):
                ...
                _iket.range_end(ds_math_token, loop_iter)
                ...
                cute.arch.fence_view_async_shared()
                # kq2: warp-level close (see p_ready note).
                cute.arch.sync_warp()
                with cute.arch.elect_one():
                    cute.arch.mbarrier_arrive(
                        pds_ready_mbars,
            ...
            if is_leader_cta:
                ...
                relay_phase = Int32(0)
                for loop_iter in cutlass.range(tile_count):
                    wait_kscore_token = _iket.range_start(
                        "WAIT_KSCORE(i)",
                        loop_iter,
                    )
                    ...
                    if loop_iter == Int32(0):
                        wait_dp_input_token = _iket.range_start(
                            "WAIT_dP_INPUT(i)",
                            loop_iter,
                        )
                        _mbarrier_wait_acquire_cluster(
            ...
            if tile_count > Int32(0):
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    round_fill_token = _iket.range_start(
                        "ROUND_FILL(i)",
                        loop_iter,
                    )
                    ...
                    # per tile, all committed by the commit warp.
            ...
            # dS leg, fully independent.  This removes the old serial
            # hazard (the dS send queued behind the P landing wait)
            # without the reviewer-suggested reorder's pitfall (relay 0
            # must never wait on pds_ready, or dV blocks on the dS
            ...
            if relay_lane == Int32(0):
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    ds_landing_phase = Int32(1) - ds_landing_phase
                    cute.arch.mbarrier_arrive(
                        relay_mbars + 1,
                        Int32(0),
                    )
    ...
    def _issue_score_v2(
        ...
        if cutlass.const_expr(is_dp):
            packed_score = issue_seq * Int32(2) + Int32(1)
        else:
            packed_score = issue_seq * Int32(2)
        score_slot_token = _iket.range_start(
            "WAIT_SCORE_SLOT(i,p)",
            packed_score,
        )
        done_pipeline.producer_acquire(producer_state)
    ...
    def _issue_grads_kq_v1(
        ...
        """kq same-tile gradient chain: dV r0/r1 -> dQ -> dK r0/r1.
        ...
        # Payload packing matches final_ser_trace_ab: g=0..3 are dV
        # (r0p0,r0p1,r1p0,r1p1), g=4..7 are dK in the same order.
        ...
        dkv_slot_token = _iket.range_start(
            "WAIT_DKV_SLOT(i,r)",
            issue_seq * Int32(2),
        )
        ...
        # backpressure + ds_local_ready (both CTAs' local dS images
        # published).  Releasing the generation hands score_kv to the
        # gather warps for the NEXT tile's score K.
        dq_gate_token = _iket.range_start(
            "WAIT_dQ_GATE(i)",
            issue_seq,
        )
        ...
        # relay 1 now has dQ's execution window as extra cover.
        relay_wait_token = _iket.range_start(
            "WAIT_RELAY(i,p)",
            issue_seq * Int32(2) + Int32(1),
        )
    ...
    def _drain_dkv_v8(
        ...
        t_dkv_1: cute.Tensor,
        mdKV_acc: cute.Tensor,
        dkv_reduce_staging: cute.Pointer,
        mTopkIdxs: cute.Tensor,
        tile_index: Int32,
        topk: Int32,
        token_idx: Int32,
        ...
        The v8 T2R, per-slot generations, shared KV-index preload, and
        split wait/release states are unchanged.  kq6g replaces only the
        issue mechanism: each logical D128 row is assembled as one
        contiguous 512-byte SMEM slab and one warpgroup leader issues an
        elementwise-atomic FP32 async bulk reduce.  Two row stages per
        warpgroup bound the issuer queue while preserving the slot order.
        """
        ...
        for i in cutlass.range_constexpr(8):
            ...
            kv_index = r_topk[i]
            for row_in_chunk in cutlass.range_constexpr(4):
                stage_idx = row_in_chunk % self.DKV_REDUCE_STAGES
                row_offset = (
                    wg_idx * Int32(self.DKV_REDUCE_WG_STRIDE_ELEMENTS)
                    + Int32(stage_idx * self.DKV_REDUCE_ROW_ELEMENTS)
                )
                staging_row_ptr = cute.make_ptr(
                    Float32,
                    (dkv_reduce_staging + row_offset).llvm_ptr,
                    cute.AddressSpace.smem,
                    assumed_align=self.DKV_REDUCE_ALIGN_BYTES,
                )
                staging_row = cute.make_tensor(
                    staging_row_ptr,
                    cute.make_layout(
                        (self.DKV_REDUCE_ROW_ELEMENTS,),
                        stride=(1,),
                ...
                cute.arch.barrier(
                    barrier_id=self.DKV_REDUCE_BARRIER_ID + wg_idx,
                    number_of_threads=self.MATH_THREADS_PER_CTA,
                )

                if dp_idx % Int32(4) == Int32(row_in_chunk):
                    staging_frg_ptr = cute.make_ptr(
                        Float32,
                        (
                            staging_row.iterator
                            + (dp_idx // Int32(4)) * Int32(4)
                        ).llvm_ptr,
                        cute.AddressSpace.smem,
                        assumed_align=self.DKV_REDUCE_ALIGN_BYTES,
                    )
                    staging_frg = cute.make_tensor(
                        staging_frg_ptr,
                        cute.make_layout((4,), stride=(1,)),
                    )
                    cute.autovec_copy(rdkv_frg_0, staging_frg)
                ...
                row_kv_index = cute.arch.shuffle_sync(
                    kv_index,
                    row_in_chunk,
                )
                if dp_idx == Int32(0):
                    if row_kv_index >= Int32(0):
                        dkv_row = mdKV_acc[
                            None,
                            row_kv_index,
                            (0, batch_idx),
                        ...
                        target_frg_0 = tile_row_0[None, 0]
                        _cpasync_bulk_reduce_add_f32(
                            staging_row.iterator,
                            target_frg_0.iterator,
                            self.DKV_REDUCE_ROW_BYTES,
                        )
                    cute.arch.cp_async_bulk_commit_group()
        _iket.range_end(
            reduce_atomic_token,
        ...
        for i in cutlass.range_constexpr(8):
            ...
            for row_in_chunk in cutlass.range_constexpr(4):
                ...
                if dp_idx % Int32(4) == Int32(row_in_chunk):
                    ...
                    cute.autovec_copy(rdkv_frg_1, staging_frg)
                ...
                if dp_idx == Int32(0):
                    if row_kv_index >= Int32(0):
                        ...
                        target_frg_1 = tile_row_1[None, 0]
                        _cpasync_bulk_reduce_add_f32(
                            staging_row.iterator,

"""v_w3_4: v_w3_2 + prologue knife 1 -- the topk scalar chain hoists ahead of init.

Two confirmed prologue moves (final mining round, adversarially verified;
the head has NO rendezvous absorber, so the v_w3_1 ratchet null does not
apply here):
1. topk/tile_count (a two-level GMEM scalar chain) hoists from after the
   tmem-alloc barrier to right after token_idx -- its ~0.6-1.0us flight
   hides under the mbarrier-init + cta_barrier + pipeline_init +
   tmem-alloc sync window.  One move, every role benefits.
2. The gather warps issue gen0's two per-thread row-index loads BEFORE
   arriving at the cta_barrier (pure register state, no SMEM writes, no
   credits -- vk_7 W2's own justification verbatim).  Reuses the v_w3_1
   split helpers (null there on the steady loop; the prologue has no
   absorber).
...
class FlashAttentionDSABackwardSm100TwoCTA(FlashAttentionDSABackwardSm100):
    ...
    def _score_kv_row_indices(
        ...
        """Batched per-thread topk index loads for one score-K tile.

        W3: extracted from _load_score_kv so the caller can issue these
        two GMEM loads at the iteration top, hidden under the loan
        acquire + TMA flight window, instead of inside the credit-gated
        LOAD_K section.  Trace-time python lists of SSA values (the vg_4
        form -- comprehension plus static iteration, no dynamic-range
        list growth).
        """
        ...
        return row_local_n, row_kv_index

    @cute.jit
    def _load_score_kv_rows(
        self,
        mKV: cute.Tensor,
        ...
        """Copy loop of _load_score_kv over prefetched indices."""
...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    * The gradient GEMMs of tile p are issued inside tile t=p-1's S/dP
    ...
    def kernel(
        ...
        # v_w3_4 knife 1: the topk scalar chain issues here so its GMEM
        # flight hides under the whole init sync window below.
        if cutlass.const_expr(mTopkLength is not None):
            topk = mTopkLength[token_idx]
        else:

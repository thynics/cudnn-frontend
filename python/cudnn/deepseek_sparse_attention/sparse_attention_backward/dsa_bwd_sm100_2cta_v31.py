...
v3.1 (v19-MAXFUSE): the approved anchor-edge elimination surgery, per
CG2终局设计_20260731.md, built on the v18a base (its dual-ready W18 /
pds_ds machine and dQ-early leader carry a hardware-backed correctness
run, so rev2 starts here rather than re-deriving from v17a).  Two env
stages gate the surgery; the all-off build is v18a-identical:

* DSA_V19_CHASE (default 1): score_kv 4->3 chunks with chunk-3 chasing
  slot 0, plus the minimal S/dP interleave order.
* DSA_V19_DBUF (default 1): p_blocks x2 + ds_image x2 same-parity double
  buffering (pds/pds_ds acquire gates gain one tile of slack), ds_blocks
  retired as an alias into ds_image (+b*4096B halves, byte identity
  asserted on the production swizzle), ds_xchg revived as the outbound
  push staging (P1b inverted: the DSM send must not source the live
  image once landings overwrite it), the peer dS landing retargeted at
  the ds_image half it aliases, read-release on push sources, the
  dq_done landing gate on W18, and the dO-quadrant-first / dS-late
  pass reorder.
...
# v3.1 stage flags, read once at import (the runner uses a fresh process
...
class FlashAttentionDSABackwardSm100TwoCTA(FlashAttentionDSABackwardSm100):
    ...
    def __call__(
        ...
        # v3.1 CHASE: the dQ epilogue stages in the (dead) score-K
        # allocation, which CHASE shrinks 32,768 -> 24,576 B.  The
        # [H128, D128] single-wave round no longer fits, so the staging
    ...
    @cute.jit
    def _load_score_kv_chunk_v31(
        self,
        mKV: cute.Tensor,
        mTopkIdxs: cute.Tensor,
        destination_view: cute.Tensor,
        logical_chunk: cutlass.Constexpr[int],
        token_idx: Int32,
        batch_idx: Int32,
        tile_index: Int32,
        topk: Int32,
        ...
        """v3.1 CHASE: gather ONE logical D128 chunk into one slot view.

        The single-stage destination view carries the chase mapping
        (logical chunk 3 -> slot 0); the GMEM d-offset stays keyed to
        the LOGICAL chunk.  Per-row topk index re-read vs the fused
        4-chunk loader: 3 extra L2-hot index reads per row, noise.
        """
        ...
        for row_iteration in cutlass.range_constexpr(rows_per_group):
            ...
            if kv_index >= 0:
                self._copy_sparse_k_d128_row(
                    ...
                    kv_index,
                    batch_idx,
                    Int32(logical_chunk * self.K_CHUNK),
                    index_in_group,
                    copy_atom,
                    thread_copy,
                )
    ...
    def _store_dq_epi_tma_v12(
        ...
        if cutlass.const_expr(self.V19_CHASE):
            # CHASE: the score-K staging shrank to 24,576 B, so each
            # round leaves as TWO [H64, D128] waves.  Each wave redoes
            # the T2R (the epilogue runs once per query tile -- the
            # re-read is noise) and stores its head half
            # (wave == head // 64) into the halved staging, then
            # issues its own bulk TMA.  Every wave iteration is fully
            # self-contained: no name crosses a dynamic branch (DSL
            # lesson #6).
            ...
            for wave in cutlass.range_constexpr(2):
                if mtx < self.MATH_THREADS_PER_CTA:
                    ...
                    for value_index in cutlass.range_constexpr(
                        ...
                        head_wave = head // Int32(self.H_TILE_CTA)
                        if head_wave == Int32(wave):
                            s_dq_epi[
                                head
                                - Int32(wave * self.H_TILE_CTA),
                                local_d,
                            ] = self.element_dtype(
                                thread_values[value_index]
                            )
                ...
                # Source-side completion broadcast: the next wave (or
                # round) may overwrite the staging only after the
                # engine has READ this wave's bytes.
                self.math_barrier.arrive_and_wait()
        else:
            if mtx < self.MATH_THREADS_PER_CTA:
                tiled_t2r = tcgen05.make_tmem_copy(dq_tmem_load, t_dq)
            ...
            if mtx < Int32(32):
                ...
                cute.arch.cp_async_bulk_commit_group()
                cute.arch.cp_async_bulk_wait_group(0, read=True)
            # Source-side completion broadcast: round 1 may overwrite
            # the staging only after the engine has READ round 0's
            # bytes.
            self.math_barrier.arrive_and_wait()
...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    * The gradient GEMMs of tile p are issued inside tile t=p-1's S/dP
    ...
    # v3.1 DBUF: one same-parity copy of {2 blocks} / {whole image} per
    # publish surface; parity q of tile t is t % 2.
    PDS_PARITY_ELEMENTS = 4_096
    PDS_PARITY_BYTES = 8_192
    # v3.1 CHASE: one score-K chunk slot ([N_TILE_CTA, K_CHUNK] bf16).
    SCORE_STAGE_ELEMENTS = 4_096

    # 64-column-aligned TMEM map (the layout the v1 CG2 T2R code ran with).
    # v7: a CG2 accumulator with per-SM M_MMA=64 folds into 128 datapaths x
    # N/2 columns (CUTLASS mma_traits_sm100_frag.hpp, UMMA_2SM M64
    # Interleaved atom (64,(N/2,2)):(1,(128,64))), so S and dP really cover
    # columns [0,32) and [64,96).  The ping-pong stage-1 buffers live in
    # the [32,64) and [96,128) holes; dQ/dKV placements are unchanged.
    TMEM_S_OFFSET = 0
    TMEM_S1_OFFSET = 32
    TMEM_DP_OFFSET = 64
    TMEM_DP1_OFFSET = 96
    ...
    # v7: S/dP TMEM double-buffer depth (cross-tile: S(t+1) no longer waits
    ...
    # v9.3: hoist softmax lse/delta per (a1, L) h-group of the
    # Ld16x256b(Rep4) fragment; False falls back to assumption-free
    # per-value coordinate indexing.
    ...
    # v3.1 (v19-MAXFUSE) stage gates, captured from the module-level env
    # flags; staged code reads them through cutlass.const_expr like every
    # other class knob.  All-off is v18a-identical (see module docstring
    # and V31_SURGERY_SPEC.md).
    V19_CHASE = DSA_V19_CHASE
    V19_DBUF = DSA_V19_DBUF
    ...
    def _specialize_shared_storage(
        ...
        # v3.1 byte plans (per CTA), cap 232,448.
        #
        #   DBUF on (CHASE on):            DBUF off = v18a layout:
        #     stationary Q + dO   131,072    stationary Q + dO   131,072
        #     score K stage        24,576    score K stage        32,768
        #     round region         32,768    round region         32,768
        #     P blocks x2 parity   16,384    P blocks + xchg      12,288
        #     P xchg (outbound)     4,096    dS image + blocks    16,384
        #     dS image x2 parity   16,384    stats                   512
        #     dS xchg (outbound)    4,096    mbarrier pad      <=  1,024
        #     stats+mbars head  <=  1,024    total (v18a gate)   230,912
        #     total               230,400
        assert cute.cosize(score_a_layout_staged) <= 32768
        assert cute.cosize(score_b_layout_staged) <= 16384
        ...
        # CHASE: three resident score-K chunks; logical chunk 3 chases
        # slot 0 (static mapping -- the staged B layout stays 4-chunk;
        # the view split lives at the consumer/producer sites).
        score_kv_elements = 12288 if self.V19_CHASE else 16384

        if self.V19_DBUF:

            @cute.struct
            class SharedStorageV31:
                # stats first: offset 0 gives it maximal natural
                # alignment for free, and the single 1,024 B head slot
                # absorbs stats (512) plus every mbarrier below (the
                # approved "mbar+stats pack").
                ...
                # v3.1: pds/pds_dS run depth-2 (same-parity double
                # buffer -- math(t+1) publishes into parity(t+1) while
                # grads(t) consume parity(t); the producer acquire gate
                # relaxes to dkv(t-2)-done).
                pds_mbars: cute.struct.MemRange[cutlass.Int64, 4]
                pds_ds_mbars: cute.struct.MemRange[cutlass.Int64, 4]
                ...
                # v18a dual ready gates: [0] = ds_ready, [1] = p_ready.
                pds_ready_mbars: cute.struct.MemRange[cutlass.Int64, 2]
                # v3.1: outbound-staging read release.  W18 arrives once
                # cp.async.bulk.wait_group.read confirms both push
                ...
                # slot 0.  fill3: gather arrives once chunk 3 is filled
                # and fenced; the leader waits it before issuing the
                # chunk-3 atoms.
                chase_mbars: cute.struct.MemRange[cutlass.Int64, 2]
                fill3_ready_mbar: cute.struct.MemRange[cutlass.Int64, 1]
                ...
                score_kv: cute.struct.Align[
                    cute.struct.MemRange[element_dtype, score_kv_elements],
                    1024,
                ]
                ...
                # x2 same-parity copies; each 4,096-element copy holds
                # the two 2,048-element rank blocks (local + landing).
                ...
                # x2 same-parity copies.  ds_blocks is retired: block b
                # of parity q aliases ds_image bytes [q*8192 + b*4096,
                # +4096) -- byte identity carried by the production
                # swizzle asserts at the view-construction site.
                ds_image: cute.struct.Align[
                    cute.struct.MemRange[element_dtype, 8192],
                    1024,
                ]
                # Revived as the OUTBOUND dS push staging (P1b
                # inverted): with peer landings overwriting the live
                # image halves, the DSM send must source a stable copy.
                ds_xchg: cute.struct.Align[
                    cute.struct.MemRange[element_dtype, 2048],
                    1024,
                ]

            v31_actual = SharedStorageV31.size_in_bytes()
            assert v31_actual <= 230_912, (
                f"v3.1 DBUF SMEM exceeds the plan bound: {v31_actual}"
            )
            assert v31_actual <= self.MAX_SMEM_BYTES
            return SharedStorageV31
        ...
        class SharedStorageV2:
            ...
            # v18a (E3): second single-stage publish gate.  pds (now
            # the P gate) keeps the drain-coupled release; pds_dS
            # releases right after the dQ(t-1) issue + the ROUTE_dS
            # landing wait, breaking the zero-slack anchor edge
            # dVdK(t-1)-drain -> MATH_STORE(t).
            pds_ds_mbars: cute.struct.MemRange[cutlass.Int64, 2]
            dkv_done_mbars: cute.struct.MemRange[cutlass.Int64, 4]
            dq_done_mbars: cute.struct.MemRange[cutlass.Int64, 2]
            # Raw single-phase-per-tile barriers.
            stationary_tma_mbars: cute.struct.MemRange[cutlass.Int64, 2]
            ...
            # v18a (E3): two of them -- [0] = ds_ready (early, after
            # the whole dS-image store), [1] = p_ready (late, after the
            # P + local dS-block stores, on the original pds cadence).
            pds_ready_mbars: cute.struct.MemRange[cutlass.Int64, 2]
            # v3.1 CHASE gates (see the DBUF variant; allocated in both
            # so the flags stay independent).
            chase_mbars: cute.struct.MemRange[cutlass.Int64, 2]
            fill3_ready_mbar: cute.struct.MemRange[cutlass.Int64, 1]
            khot_seq: cute.struct.MemRange[cutlass.Int64, 1]
            tmem_dealloc_mbar: cutlass.Int64
            ...
            # v18a (E3): ds_xchg (4,096 B) retired.  Dead since v12
            # P1b: no store targeted it and ROUTE_dS sources the byte-
            # identical half of the whole ds_image.
            stats: cute.struct.Align[
    ...
    def kernel(
        ...
        if cutlass.const_expr(self.V19_DBUF):
            # v3.1: outbound-staging read-release gate (field exists
        ...
        if cutlass.const_expr(self.V19_CHASE):
            ...
            fill3_ready_mbar_ptr = (
                storage.fill3_ready_mbar.data_ptr()
            )
        ...
        if cutlass.const_expr(self.V19_CHASE):
            # CHASE: three resident chunk slots serve four logical
            # chunks; chunk 3 statically chases slot 0.  The full
            # 4-stage view would overhang the shrunken field, so every
            # consumer/producer works on per-chunk single-stage views
            # (per-stage layout via cute.select -- the dq_epi
            # precedent).  slot(c) = (0, 1, 2, 0).
            score_kv_raw = storage.score_kv.data_ptr()
            score_b_stage_layout = cute.select(
                score_b_layout_staged,
                mode=[0, 1, 2],
            )
            ...
            k_chunk_0 = cute.make_tensor(
                cute.recast_ptr(
                    score_kv_raw,
                    score_b_stage_layout.inner,
                    dtype=self.element_dtype,
                ),
                score_b_stage_layout.outer,
            )
            k_chunk_1 = cute.make_tensor(
                cute.recast_ptr(
                    score_kv_raw + self.SCORE_STAGE_ELEMENTS,
                    score_b_stage_layout.inner,
                    dtype=self.element_dtype,
                ),
                score_b_stage_layout.outer,
            )
            k_chunk_2 = cute.make_tensor(
                cute.recast_ptr(
                    score_kv_raw + 2 * self.SCORE_STAGE_ELEMENTS,
                    score_b_stage_layout.inner,
                    dtype=self.element_dtype,
                ),
                score_b_stage_layout.outer,
            )
            # The chase alias: logical chunk 3 lives in slot 0.
            k_chunk_3 = cute.make_tensor(
                cute.recast_ptr(
                    score_kv_raw,
                    score_b_stage_layout.inner,
                    dtype=self.element_dtype,
        ...
        # v12 (P4): the dQ epilogue stages each [H128, D128] round in the
        # score-K allocation, which is dead once dq_done commits (its last
        # reader is the final S/dP MMA pair, tracked by the same commit).
        s_dq_epi = cute.make_tensor(
            cute.recast_ptr(
        ...
        # Parity-independent: computed before the DBUF branch.
        score_store_layout = sm100_utils.make_smem_layout_epi(
            self.element_dtype,
            utils.LayoutEnum.COL_MAJOR,
            (self.H_TILE_CTA, self.N_TILE),
        ...
        if cutlass.const_expr(self.V19_DBUF):
            # v3.1 DBUF prologue views.  Legacy single-parity names are
            # deliberately NOT bound in this arm: any consumer that has
            # not been made parity-aware fails loudly at trace time
            # instead of silently reading parity 0.
            #
            # p_blocks parity copy q at +q*PDS_PARITY_ELEMENTS holds the
            # two rank blocks exactly as the v18a single copy did.
            # Constructions are fully explicit: nested defs and
            # comprehensions are unproven under the DSL preprocessor.
            p_blocks_q0 = (
                cute.make_tensor(
                    cute.recast_ptr(
                        p_blocks_raw,
            ...
            p_blocks_q1 = (
                cute.make_tensor(
                    cute.recast_ptr(
                        p_blocks_raw + self.PDS_PARITY_ELEMENTS,
                        dkv_b_layout_staged.inner,
                        dtype=self.element_dtype,
                    ),
                    dkv_b_layout_staged.outer,
            ...
            # ds_blocks is retired: block b of parity q aliases the
            # ds_image bytes [q*8192 + b*4096, +4096).  The swizzle
            # identity asserts above carry the byte identity between
            # the K64 partition-B block layout and the store image.
            ds_blocks_q0 = (
                cute.make_tensor(
                    cute.recast_ptr(
                        ds_image_raw,
                ...
                cute.make_tensor(
                    cute.recast_ptr(
                        ds_image_raw + self.PDS_BLOCK_ELEMENTS,
                        dkv_b_layout_staged.inner,
                        dtype=self.element_dtype,
                    ),
                    dkv_b_layout_staged.outer,
            ...
            ds_blocks_q1 = (
                cute.make_tensor(
                    cute.recast_ptr(
                        ds_image_raw + self.PDS_PARITY_ELEMENTS,
                        dkv_b_layout_staged.inner,
                        dtype=self.element_dtype,
                    ),
                    dkv_b_layout_staged.outer,
            ...
            ds_image_store_q0 = cute.make_tensor(
                cute.recast_ptr(
                    ds_image_raw,
                    score_store_layout.inner,
                    dtype=self.element_dtype,
            ...
            ds_image_store_q1 = cute.make_tensor(
                cute.recast_ptr(
                    ds_image_raw + self.PDS_PARITY_ELEMENTS,
                    score_store_layout.inner,
                    dtype=self.element_dtype,
            ...
            p_block_stage = p_blocks_q0[0][None, None, None, 0]
            assert (
            ...
            # DSM raw endpoints per parity.  P push: src = local p_xchg,
            # dst = peer p_blocks[q] block[rank].  dS push: src = local
            # ds_xchg (revived staging), dst = peer ds_image[q] half
            # [rank*PDS_BLOCK_ELEMENTS] -- the aliased block position.
            p_land_ptrs_q0 = (
                p_blocks_raw,
                p_blocks_raw + self.PDS_BLOCK_ELEMENTS,
            )
            ...
            # Preserve the exact nested/swizzled K64 partition-B byte
            # image.  A raw 4 KiB DSM copy is then layout-preserving
            # because every block has the same type and alignment at
            ...
            # v18a (E3): ds_xchg retired in this arm (dead field since
            # v12 P1b; ROUTE_dS sources the whole-image half).
        softmax_stats = storage.stats.get_tensor(
            cute.make_layout(
                (self.H_TILE_CTA, 2),
                stride=(1, self.H_TILE_CTA),
        ...
        if cutlass.const_expr(self.V19_CHASE):
            score_k_c0 = score_tiled_mma.make_fragment_B(k_chunk_0)
            score_k_c1 = score_tiled_mma.make_fragment_B(k_chunk_1)
            score_k_c2 = score_tiled_mma.make_fragment_B(k_chunk_2)
            score_k_c3 = score_tiled_mma.make_fragment_B(k_chunk_3)
            dp_k_c0 = dp_tiled_mma.make_fragment_B(k_chunk_0)
            dp_k_c1 = dp_tiled_mma.make_fragment_B(k_chunk_1)
            dp_k_c2 = dp_tiled_mma.make_fragment_B(k_chunk_2)
            dp_k_c3 = dp_tiled_mma.make_fragment_B(k_chunk_3)
        else:
        ...
        if cutlass.const_expr(self.V19_DBUF):
            # Leader-side B fragments, one set per publish parity; the
            # per-tile dynamic if at the issue sites selects a set (the
            # accumulator ping-pong precedent).
            dq_ds_fragment_q0 = dq_tiled_mma.make_fragment_B(
                ds_image_q0
            )
            dq_ds_fragment_q1 = dq_tiled_mma.make_fragment_B(
                ds_image_q1
            )
            p_fragments_q0 = (
                dkv_tiled_mma.make_fragment_B(p_blocks_q0[0]),
                dkv_tiled_mma.make_fragment_B(p_blocks_q0[1]),
            )
            p_fragments_q1 = (
                dkv_tiled_mma.make_fragment_B(p_blocks_q1[0]),
                dkv_tiled_mma.make_fragment_B(p_blocks_q1[1]),
            )
            ds_fragments_q0 = (
                dkv_tiled_mma.make_fragment_B(ds_blocks_q0[0]),
                dkv_tiled_mma.make_fragment_B(ds_blocks_q0[1]),
            )
            ds_fragments_q1 = (
                dkv_tiled_mma.make_fragment_B(ds_blocks_q1[0]),
                dkv_tiled_mma.make_fragment_B(ds_blocks_q1[1]),
            )
        else:
        ...
        # One committing W18 lane per CTA of the pair: BOTH CTAs' commits
        # arrive the leading CTA's full mbar, so the count is atom_thr_size
        # (= 2), mirroring load_elect_group -- a count of 1 would flip the
        # phase twice per tile and desynchronize the consumer.
        pds_commit_group = pipeline.CooperativeGroup(
            pipeline.Agent.Thread,
            atom_thr_size,
        )
        # v3.1 DBUF: the publish pipelines run depth 2 over the parity
        # copies -- math(t+1)'s producer_acquire relaxes from "grads(t)
        # drained" to "grads(t-1) drained" (the E3 induction that turns
        # the anchor-edge relocation into elimination).  Statement form
        # (not a ternary) inside staged code, per the const_expr rule.
        pds_publish_stages = 1
        if cutlass.const_expr(self.V19_DBUF):
            pds_publish_stages = 2
        pipe_pds = pipeline.PipelineAsyncUmma.create(
            num_stages=pds_publish_stages,
        ...
        # v18a (E3): pipe_pds is now the P-publish gate only (empty ==
        # every grads(t-1) MMA completed -- the old full-drain
        # semantics, demoted off the critical store).  pipe_pds_ds is
        # the new dS gate: FULL (W18 commit on ds_ready) admits the
        # leader's dQ(t); EMPTY (leader tcgen05-commit release placed
        # after dQ(t) + S/dP(t+1) issue + the ROUTE_dS(t) landing wait)
        # is math(t+1)'s early dS-image store front gate ==
        #   max(softmax(t+1) done [program order],
        #       dq_done(t)      [commit-tracked release],
        #       ROUTE_dS(t) landing [relay+1 waited before release]).
        pipe_pds_ds = pipeline.PipelineAsyncUmma.create(
            num_stages=pds_publish_stages,
            producer_group=pds_commit_group,
            consumer_group=leader_group,
            barrier_storage=storage.pds_ds_mbars.data_ptr(),
            cta_layout_vmnk=cluster_layout_vmnk,
            defer_sync=True,
        )
        ...
        if cutlass.const_expr(self.V19_CHASE):
            # CHASE gate: the leader commits right after the S/dP
        ...
        if tidx == Int32(0):
            ...
            # v18a (E3): p_ready -- the late, original-cadence handoff.
            cute.arch.mbarrier_init(
                pds_ready_mbars + 1,
                self.MATH_THREADS,
            )
            ...
            if cutlass.const_expr(self.V19_DBUF):
                # Single W18 lane arrives once per tile after the
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            # PLUS, in v8, the K_dQ image fills offloaded from the load
            # warp (128 threads vs 32).  The loop is software-pipelined one
            # tile ahead: K(t+1)'s score gather runs first (its kscore
            # credit frees right after dP(t)), and only then the tile-t kdq
            # handshake, whose round credits free near the end of
            # grads(t-1) -- so the credit-gated kdq never delays the next
            # score gather.  The khot advisory machinery is retired: the
            # kdq reads now run on the warps that just loaded the same KV
            # rows.
            ...
            if cutlass.const_expr(self.V19_CHASE):
                chase_cons = pipeline.make_pipeline_state(
                    pipeline.PipelineUserType.Consumer,
                    1,
                )
                if tile_count > Int32(0):
                    # Prologue: K(0) pass A (chunks 0-2).  The kscore
                    # commit means "three resident chunks ready"; the
                    # leader's chunk-3 atoms are gated by fill3 alone.
                    load_k_token = _iket.range_start(
                        "LOAD_K(i)",
                        Int32(0),
                    )
                    pipe_kscore.producer_acquire(gather_state)
                    self._load_score_kv_chunk_v31(
                        mKV,
                        mTopkIdxs,
                        k_chunk_0,
                        0,
                    ...
                    for loop_iter in cutlass.range(
                        ...
                        # Tail fill of the CURRENT executed tile: wait
                        # the leader's chase commit (its S/dP chunk-0
                        # atoms are hardware-complete), refill slot 0
                        # with logical chunk 3, then hand fill3 to the
                        # leader.  This MUST precede pass A of the next
                        # tile: the kscore-empty acquire below only
                        # opens once the current tile's chunk-3 atoms
                        # (gated by fill3) have completed.
                        if tidx == Int32(0):
                            pipe_chase.consumer_wait(chase_cons)
                    ...
                    # Epilogue: the last executed tile's tail fill +
                    # kdq (no next score gather).
                    if tidx == Int32(0):
                        pipe_chase.consumer_wait(chase_cons)
                    self.gather_barrier.arrive_and_wait()
                    self._load_score_kv_chunk_v31(
                ...
                if tile_count > Int32(0):
                    ...
                    # Epilogue: the last tile's kdq (no next score
                    # gather).
                    self._gather_kdq_v8(
                        mKV,
                        mTopkIdxs,
            ...
            # v18a (E3): producer state for the split dS-publish gate.
            pds_ds_state = pipeline.make_pipeline_state(
                pipeline.PipelineUserType.Producer,
                pds_publish_stages,
            )
            ...
            # n-half ownership under Ld16x256b(Rep4): every per-thread n
            # (n_base + {0,1,8,9,16,17,24,25}) lies in one 32-wide half,
            # warp-uniform (warps 4,5 -> half 0; 6,7 -> half 1) and
            # numerically identical to mtx // 64 -- but derived from the
            # coordinate tensor so it stays correct under any atom whose
            # n-half is warp-uniform.
            n_owner = cute.arch.make_warp_uniform(
                Int32(
                    cute.get(
                        score_coordinates[0],
            ...
            owns_n = n_owner == rank
            # v9.3 fix (applies to every biased pointer below): the
            # runtime n_owner bias degrades the DSL's alignment
            # inference to element width (16-bit), which the 128-bit
            # stmatrix store atom rejects at build time.  The bias is
            # n_owner * 4096 B off a 1024-aligned field, so a 16-byte
            # assumed alignment is provable -- declare it the same way
            # the local-block pointers do.
            ...
            if cutlass.const_expr(self.V19_DBUF):
                # DBUF math destinations, one set per parity.  The
                # owns_n dS block store is retired (the whole-image
                # store already covers the aliased own-half block);
                # the non-owning warps instead revive the ds_xchg
                # store -- the outbound push staging (P1b inverted).
                t_rs_ds_q0 = thread_copy_r2s.partition_D(
                    ds_image_store_q0
                )
                t_rs_ds_q1 = thread_copy_r2s.partition_D(
                    ds_image_store_q1
                )
                assert cute.size(t_rs_ds_q0, mode=[4]) == 1
                assert cute.size(t_rs_ds_q1, mode=[4]) == 1
                t_rs_ds_tile_q0 = t_rs_ds_q0[
                    None, None, None, None, 0
                ]
                t_rs_ds_tile_q1 = t_rs_ds_q1[
                    None, None, None, None, 0
                ]
                aligned_p_blocks_ptr_q0 = cute.make_ptr(
                    self.element_dtype,
                    p_blocks_q0[0].iterator.toint(),
                    p_blocks_q0[0].memspace,
                    assumed_align=16,
                )
                aligned_p_blocks_ptr_q1 = cute.make_ptr(
                    self.element_dtype,
                    p_blocks_q1[0].iterator.toint(),
                    p_blocks_q1[0].memspace,
                    assumed_align=16,
                )
                p_local_store_q0 = cute.make_tensor(
                    cute.recast_ptr(
                        aligned_p_blocks_ptr_q0,
                        score_store_layout.inner,
                        dtype=self.element_dtype,
                    ),
                    score_store_domain,
                )
                p_local_store_q1 = cute.make_tensor(
                    cute.recast_ptr(
                        aligned_p_blocks_ptr_q1,
                        score_store_layout.inner,
                        dtype=self.element_dtype,
                    ),
                    score_store_domain,
                )
                t_rs_p_local_q0 = thread_copy_r2s.partition_D(
                    p_local_store_q0
                )
                t_rs_p_local_q1 = thread_copy_r2s.partition_D(
                    p_local_store_q1
                )
                assert cute.size(t_rs_p_local_q0, mode=[4]) == 1
                assert cute.size(t_rs_p_local_q1, mode=[4]) == 1
                t_rs_p_local_tile_q0 = t_rs_p_local_q0[
                    None, None, None, None, 0
                ]
                t_rs_p_local_tile_q1 = t_rs_p_local_q1[
                    None, None, None, None, 0
                ]
            ...
            for loop_iter in cutlass.range(tile_count):
                ...
                # v18a (E3, on v17a/E2): split-publish, dS-early /
                # P-late.  COMPUTE order is unchanged from E2 (packed
                # f32x2 P softmax, then packed dS pair math -- dS
                # needs the fp32 P values), but the PUBLISH order
                # flips and the single pds gate splits:
                #   softmax P compute -> dS compute
                #   -> pds_dS producer_acquire  (EARLY gate: empty ==
                #      dQ(t-1) complete [tcgen05-commit-tracked
                #      release] AND ROUTE_dS(t-1) landing [leader
                #      waits relay+1 before releasing]; softmax(t)
                #      done by program order -- the spec's max())
                #   -> whole dS-image store -> ds_ready arrive (W18
                #      commits pds_dS -> leader dQ(t) unblocks)
                #   -> pds_P producer_acquire   (LATE gate: empty ==
                #      ALL grads(t-1) MMAs drained -- the old anchor
                #      edge, now gating only cheap trailing stores)
                #   -> P store + LOCAL dS-block store -> p_ready.
                # The local dS-block store rides the LATE leg: the
                # dVdK dS passes of tile t-1 still read ds_blocks
                # while the early leg runs, and the CG2 shared-
                # descriptor rule (one address for both CTAs) plus
                # the 232,448 B cap forbids a landing ping-pong
                # (4 x 4,096 B regions needed; only ds_xchg 4,096 +
                # slack 1,536 exist).  ROUTE_dS stays on W18's late
                # leg for the same WAR reason.
                #
                # IKET contract (name SET unchanged; E2's encoding
                # kept): MATH_SOFTMAX / MATH_STORE payloads stay
                # CONTENT-keyed -- 2*i = P phase, 2*i+1 = dS phase --
                # so per-phase cross-version comparison holds even
                # though the dS store now PRECEDES the P store.
                # MATH_PDS_ACQ / MATH_BAR1 adopt the same doubling:
                # payload 2*i = dS-side acquire / ds_ready arrive,
                # payload 2*i+1 = P-side acquire / p_ready arrive.
                #
                # Judge signature (E3):
                #   (a) MATH_STORE(2i+1).start decouples from
                #       G9(t-1) (v12/v17a: 6/6 tiles at +-0.032) and
                #       lands at MATH_SOFTMAX(2i+1).end (+<=0.1);
                #   (b) softmax_end -> dS-store_start wait
                #       1.44 -> <=0.2;
                #   (c) S_ISSUE spacing <= 6.2;
                #   negative control: dVdK per-pass cadence stays
                #   0.43-0.50 (supply ring untouched).
                #   Net period drop < 0.3 -> STOP and add the three
                #   probes (W18 P-leg / gate-A / gate-B) before any
                #   further merge (v16 lesson).
                phase_p_payload = loop_iter * Int32(2)
                phase_ds_payload = phase_p_payload + Int32(1)
                ...
                if cutlass.const_expr(self.SOFTMAX_GROUPED_STATS):
                    # Ld16x256b(Rep4) fragment (((2,2,4),1),2,1,1):
                    # value v = a0 + 2*a1 + 4*a2 + 16*L, and
                    # h = h_base + 8*a1 + 16*L is CONSTANT within each
                    # (a1, L) group of eight values -- so lse/delta
                    # hoist per group.  local_h comes from the
                    # coordinate tensor (ground truth under any atom),
                    # never from mtx arithmetic.  Group bases {0,2,16,
                    # 18}; in-group offsets {0,1,4,5,8,9,12,13}: the
                    # four groups partition v in [0,32) exactly.
                    #
                    # Packed-pairing proof (Rep4 dependent): pairs are
                    # (v0, v0 + 1) with v0 = group_base + 4*k, k in
                    # [0,4).  v0 and v0 + 1 differ only in a0 (bit 0),
                    # so they share (a1, L) -- same h row, same
                    # lse/delta, which makes the (lse, lse) broadcast
                    # pair valid -- and they are the n-adjacent column
                    # pairs (n_base + {0,1}, {8,9}, {16,17}, {24,25})
                    # of one even-aligned fragment register pair.
                    for h_group in cutlass.range_constexpr(4):
                        ...
                        for pair in cutlass.range_constexpr(4):
                            ...
                            # Keep FP32 P live for the dS phase (the
                            # baseline keeps tTR_rS the same way).
                            r_score[v0] = p_0
                            r_score[v1] = p_1
                    ...
                    # lookup with baseline-literal packed pairs (per-
                    # lane lse, no h-group constancy assumed).  Flip
                    # the class flag if the grouped arithmetic is ever
                    # suspect.
                    for pair in cutlass.range_constexpr(
                        ...
                        s_0, s_1 = cute.arch.fma_packed_f32x2(
                            ...
                            (
                                softmax_stats[local_h_0, 0],
                                softmax_stats[local_h_1, 0],
                            ),
                        )
                        p_0 = cute.math.exp2(s_0, fastmath=True)
                        p_1 = cute.math.exp2(s_1, fastmath=True)
                ...
                # v18a (E3): the P publish moves BELOW the dS phase
                # (dS-early / P-late) -- see the header comment.  The
                # dS pair math runs gate-free next: it reads only
                # registers (r_dp, fp32 r_score) and the delta stats.

                # --- dS phase: packed dS math first. ---
                math_softmax_token = _iket.range_start(
                    "MATH_SOFTMAX(i)",
                    phase_ds_payload,
                )
                if cutlass.const_expr(self.SOFTMAX_GROUPED_STATS):
                    # Same Rep4 grouping/pairing proof as the P phase;
                    # delta is reloaded here (4 smem loads) instead of
                    # staying live across the P publish.
                    for h_group in cutlass.range_constexpr(4):
                        group_base = (
                            2 * (h_group % 2) + 16 * (h_group // 2)
                        )
                    ...
                    for pair in cutlass.range_constexpr(
                        ...
                        ds_0, ds_1 = cute.arch.add_packed_f32x2(
                            (r_dp[v0], r_dp[v1]),
                            (
                                softmax_stats[local_h_0, 1],
                                softmax_stats[local_h_1, 1],
                            ),
                        )
                        ds_0, ds_1 = cute.arch.mul_packed_f32x2(
                            (ds_0, ds_1),
                ...
                # --- dS-EARLY publish: acquire the split dS gate,
                # then store ONLY the whole dS image (the dQ B operand
                # and the ROUTE_dS source).  WAR cover of ds_image of
                # tile t-1: dQ(t-1) reads are tcgen05-commit-tracked
                # into this empty; ROUTE_dS(t-1)'s source read is
                # complete because the leader waited its landing
                # (relay+1) before releasing.  The LOCAL ds_blocks
                # store deliberately does NOT happen here.
                math_pds_acq_token = _iket.range_start(
                    "MATH_PDS_ACQ(i)",
                    phase_p_payload,
                )
                pipe_pds_ds.producer_acquire(pds_ds_state)
                _iket.range_end(
                    math_pds_acq_token,
                    phase_p_payload,
                )
                ...
                r_ds_store = thread_copy_r2s.retile(r_ds)
                if cutlass.const_expr(self.V19_DBUF):
                    # Whole-image dS store into this tile's parity copy
                    # (the dQ B operand and the aliased own-half block).
                    assert t_rs_ds_tile_q0.shape == r_ds_store.shape
                    assert t_rs_ds_tile_q1.shape == r_ds_store.shape
                    if (loop_iter % Int32(2)) == Int32(0):
                        cute.copy(
                            tiled_copy_r2s,
                            r_ds_store,
                            t_rs_ds_tile_q0,
                ...
                # ds_ready: release-semantics arrive after each math
                # thread's own fenced stores.  W18 commits pds_dS on it
                # (EARLY, no DSM send) -- the edge that unpins the
                # leader's dQ(t) from the grads(t-1) drain.
                ...
                pds_ds_state.advance()

                # --- P-LATE publish: the original pds gate (empty ==
                # all grads(t-1) MMAs drained).  Everything the dVdK
                # passes of tile t-1 may still read during the early
                # leg is (over)written only here: p_blocks local +
                # p_xchg, and the LOCAL ds_blocks slot (its peer-
                # landing twin is only overwritten by ROUTE_dS, which
                # W18 also issues on this late leg).
                math_pds_acq_token = _iket.range_start(
                    "MATH_PDS_ACQ(i)",
                    phase_ds_payload,
                )
                ...
                # N32 half, so this branch is warp-uniform (identical
                # predicate to v12, now tested once per phase).
                r_p_store = thread_copy_r2s.retile(r_p)
                if cutlass.const_expr(self.V19_DBUF):
                    # xchg read-release: ROUTE_P/ROUTE_dS of tile t-1
                    # must have finished READING the x1 outbound
                    # stagings (p_xchg/ds_xchg) before this tile
                    # overwrites them.  W18 arrives once per tile after
                    # cp.async.bulk.wait_group.read; tile 0 has no
                    # prior push to wait for.
                    if loop_iter > Int32(0):
                        cute.arch.mbarrier_wait(
                            xchg_read_mbar_ptr,
                            (loop_iter + Int32(1)) % Int32(2),
                        )
                    assert (
                        t_rs_p_local_tile_q0.shape == r_p_store.shape
                    )
                    assert (
                        t_rs_p_local_tile_q1.shape == r_p_store.shape
                    )
                    assert t_rs_p_xchg_tile.shape == r_p_store.shape
                    assert t_rs_ds_xchg_tile.shape == r_ds_store.shape
                    if owns_n:
                        # The owns-side dS block store is retired: the
                        # aliased block IS the freshly stored image
                        # half.  Only P lands locally, per parity.
                        if (loop_iter % Int32(2)) == Int32(0):
                            cute.copy(
                                tiled_copy_r2s,
                                r_p_store,
                                t_rs_p_local_tile_q0,
                            )
                        else:
                            cute.copy(
                                tiled_copy_r2s,
                                r_p_store,
                                t_rs_p_local_tile_q1,
                            )
                    else:
                        cute.copy(
                            tiled_copy_r2s,
                        ...
                        # Revived outbound dS staging (P1b inverted):
                        # ROUTE_dS now sources this stable copy, since
                        # the live image half it used to read gets
                        # overwritten by the peer's landing.
                        cute.copy(
                            tiled_copy_r2s,
                            r_ds_store,
                            t_rs_ds_xchg_tile,
                    ...
                    if owns_n:
                        ...
                        # Local dS block for the dVdK dS passes (moved
                        # here from the dS phase; same owns_n predicate
                        # and the same r_ds_store retile as v12/v17a).
                        cute.copy(
                            tiled_copy_r2s,
                            r_ds_store,
                            t_rs_ds_local_tile,
                ...
                # p_ready: late handoff on the original cadence; W18
                # runs ROUTE_P + ROUTE_dS + the pds_P commit on it.
                math_bar1_token = _iket.range_start(
                    "MATH_BAR1(i)",
                    phase_ds_payload,
                )
            ...
            if is_leader_cta:
                ...
                # v18a (E3): consumer state for the split dS gate.
                pds_ds_cons = pipeline.make_pipeline_state(
                    pipeline.PipelineUserType.Consumer,
                    pds_publish_stages,
                )
                ...
                if cutlass.const_expr(self.V19_DBUF):
                    # v3.1: split acquire/commit producer states for the
                    # reordered grads block (dkv_prod doubles as the
                    # acquire state; commits trail on this one).
                ...
                if cutlass.const_expr(self.V19_CHASE):
                    chase_prod = pipeline.make_pipeline_state(
                        pipeline.PipelineUserType.Producer,
                        1,
                    )
                    fill3_phase = Int32(0)
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    # v18a (E3, partial v7 rollback): dQ(t-1) issues
                    # FIRST -- before S(t)/dP(t) -- because it depends
                    # only on the split pds_dS gate (both CTAs' whole
                    # dS images stored + W18 commits) and the kdq round
                    # generations, NOT on the S/dP issue window.  The
                    # round-ring consumption ORDER is unchanged (the
                    # kdq gens of tile t-1 still drain before its
                    # quadrant gens): W17 is untouched.
                    if has_prev:
                        dq_acc = loop_iter != Int32(1)
                        if cutlass.const_expr(self.V19_DBUF):
                            # Parity of the tile being consumed (t-1):
                            # select the matching publish-surface set
                            # (the accumulator ping-pong precedent for
                            # dynamic-if operand selection).
                            if (
                                (loop_iter - Int32(1)) % Int32(2)
                            ) == Int32(0):
                                round_cons, pds_ds_cons, dq_done_prod = (
                                    self._issue_prev_dq_v18a(
                                        dq_tiled_mma,
                                        t_dq[0],
                                        t_dq[1],
                                        dq_kd_fragment_a,
                    ...
                    # only on the kscore buffer and its ping-pong TMEM
                    # credit, so issuing it early lets the K stage
                    # recycle immediately and gather(t+1) run
                    # concurrently with grads(t-1) and math(t).
                    pipe_kscore.consumer_wait(kscore_cons)
                    if cutlass.const_expr(self.V19_CHASE):
                        # CHASE pairs the S/dP issue (interleave
                        # S0,dP0,S1,S2,S3,dP1-3); dP's chunk 0 comes
                        # early, so the first-tile dO gate hoists in
                        # front of the pair (cold-path only).
                        if loop_iter == Int32(0):
                            _mbarrier_wait_acquire_cluster(
                                stationary_ready_mbar + 1,
                                Int32(0),
                            )
                        s_prod, dp_prod, chase_prod = (
                            self._issue_score_pair_v31(
                                score_tiled_mma,
                                dp_tiled_mma,
                                t_score,
                                t_score_pp,
                        ...
                        fill3_phase = Int32(1) - fill3_phase
                    else:
                        ...
                        # dP(t) then early K recycle.  The first dP
                        # also gates on the dO half of the split
                        # stationary load.
                        if loop_iter == Int32(0):
                            _mbarrier_wait_acquire_cluster(
                                stationary_ready_mbar + 1,
                ...
                if tile_count > Int32(0):
                    ...
                    pipe_pds.consumer_release(pds_cons)
                    pds_cons.advance()
                    # v12 (S1) kept: the dq_done commit lives in the dQ
                    # helper, right after the TAIL's dQ issues.

                    pipe_s_done.producer_tail(s_prod)
                    pipe_dp_done.producer_tail(dp_prod)
                    if cutlass.const_expr(self.V19_DBUF):
                        # Commits ran on the split commit state.
            ...
            if tile_count > Int32(0):
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    for flat_gen in cutlass.range_constexpr(8):
                        if cutlass.const_expr(self.V19_DBUF):
                            # v3.1 gen reorder (paired 1:1 with the
                            # reordered leader pass consumption): all
                            # four dO quadrants first (feeding the dV
                            # half-blocks of both slots), then the four
                            # Q quadrants (the dK half-blocks) --
                            # kdq x2, dO x4, Q x4.
                            tensor_kind = flat_gen // 4
                            grad_round = (flat_gen // 2) % 2
                            h_half = flat_gen % 2
                        else:
            ...
            # --- relay, v18a (E3): dual-ready publish engine.  Per
            # tile:
            #   EARLY leg -- wait ds_ready (count-128; math whole-image
            #   dS stores fenced), then commit pds_dS ONLY (no DSM
            #   send): the leader's dQ(t) gate.  Both CTAs' commits
            #   arrive the leading CTA's full mbar (producer count 2).
            #   LATE leg -- wait p_ready (math P + local dS-block
            #   stores, original pds cadence), then issue BOTH DSM
            #   sends and commit pds_P.  ROUTE_dS deliberately rides
            #   the LATE leg: its landing overwrites the peer's
            #   ds_blocks slot, which the peer's dVdK dS passes of
            #   tile t-1 may still be reading during the early leg;
            #   p_ready(t) implies pds_P(t-1) was released, i.e. every
            #   grads(t-1) MMA completed, so the landing is WAR-safe
            #   without a second landing buffer (the CG2 shared-
            #   descriptor rule + the 232,448 B cap forbids the
            #   4-block ping-pong).  Then convert both DSM landings
            #   into leader-visible arrives, dS FIRST (the leader now
            #   consumes relay+1 before relay).
            # Deadlock shape: the early leg depends only on my math's
            # dS store; the late leg on my math's P store (gated by
            # the cluster-visible pds_P empty of t-1); my landing
            # waits depend on the peer's late leg of the SAME tile,
            # which is gated by the same t-1 facts -- every wait
            # resolves to tile t-1 events, so the graph stays the v11
            # cross-pattern, acyclic.
            if tidx % Int32(32) == Int32(0):
                ...
                pds_ds_com = pipeline.make_pipeline_state(
                    pipeline.PipelineUserType.Producer,
                    pds_publish_stages,
                )
                for loop_iter in cutlass.range(tile_count):
                    # EARLY leg: dS handoff -> pds_dS commit.
                    cute.arch.mbarrier_wait(
                        pds_ready_mbars,
                        ds_ready_phase,
                    )
                    ds_ready_phase = Int32(1) - ds_ready_phase
                    pipe_pds_ds.producer_commit(pds_ds_com)
                    pds_ds_com.advance()

                    # LATE leg: P handoff -> both DSM sends -> pds_P.
                    cute.arch.mbarrier_wait(
                        pds_ready_mbars + 1,
                        p_ready_phase,
                    )
                    ...
                    if cutlass.const_expr(self.V19_DBUF):
                        # src = the x1 outbound staging (bias-absorbed
                        # at the store); dst = the peer's parity copy,
                        # block[sender rank].
                        if (loop_iter % Int32(2)) == Int32(0):
                            if rank == Int32(0):
                                _cpasync_bulk_s2cluster(
                    ...
                    if cutlass.const_expr(self.V19_DBUF):
                        # P1b inverted: the send sources the revived
                        # ds_xchg staging (the live image half it used
                        # to read is itself a landing target now); the
                        # bias-absorbing store makes the source rank-
                        # symmetric.  dst = peer ds_image parity copy +
                        # sender_rank * block (the aliased position).
                        if (loop_iter % Int32(2)) == Int32(0):
                            if rank == Int32(0):
                                _cpasync_bulk_s2cluster(
                        ...
                        # v12 (P1b): the send sources the byte-identical
                        # (1-rank) half of the whole dS image (ds_xchg
                        # is retired in the v18a layout).
                        if rank == Int32(0):
                            _cpasync_bulk_s2cluster(
                    ...
                    pds_com.advance()
                    if cutlass.const_expr(self.V19_DBUF):
                        # Read-release: both pushes above are the only
                        # outstanding bulk ops in this thread's group;
                        # wait for their SOURCE reads and free the x1
                        # stagings for math(t+1).
                        cute.arch.cp_async_bulk_commit_group()
                        cute.arch.cp_async_bulk_wait_group(0, read=True)
                        cute.arch.mbarrier_arrive(
                            xchg_read_mbar_ptr,
                    ...
                    # Landing conversions, dS first (leader wait order).
                    _mbarrier_wait_acquire_cluster(
                        landing_mbars + 1,
                        landing_phase,
                    )
                ...
                if tile_count > Int32(0):
                    ...
                    pipe_pds_ds.producer_tail(pds_ds_com)
    ...
    def _issue_score_pair_chunks_v31(
        ...
        acc_s: cute.Tensor,
        acc_dp: cute.Tensor,
        score_q_fragment: cute.Tensor,
        score_do_fragment: cute.Tensor,
        s_c0: cute.Tensor,
        s_c1: cute.Tensor,
        s_c2: cute.Tensor,
        s_c3: cute.Tensor,
        ...
        mma_dp = dp_tiled_mma.with_()
        mma_dp.set(tcgen05.Field.ACCUMULATE, False)
        k_blocks_per_chunk = cute.size(score_q_fragment, mode=[2])
        for k_block in cutlass.range(0, k_blocks_per_chunk, unroll=4):
            cute.gemm(
                mma_s,
                acc_s,
                score_q_fragment[None, None, k_block, 0],
                s_c0[None, None, k_block],
                acc_s,
            )
        ...
        for k_block in cutlass.range(0, k_blocks_per_chunk, unroll=4):
            cute.gemm(
                mma_dp,
                acc_dp,
                score_do_fragment[None, None, k_block, 0],
                dp_c0[None, None, k_block],
                acc_dp,
            )
            mma_dp.set(tcgen05.Field.ACCUMULATE, True)
        chase_pipeline.producer_commit(chase_state)
        chase_state.advance()
        for k_block in cutlass.range(0, k_blocks_per_chunk, unroll=4):
            cute.gemm(
                mma_s,
                acc_s,
                score_q_fragment[None, None, k_block, 1],
                s_c1[None, None, k_block],
                acc_s,
            )
        for k_block in cutlass.range(0, k_blocks_per_chunk, unroll=4):
            cute.gemm(
                mma_s,
                acc_s,
                score_q_fragment[None, None, k_block, 2],
        ...
        for k_block in cutlass.range(0, k_blocks_per_chunk, unroll=4):
            cute.gemm(
                mma_dp,
                acc_dp,
                score_do_fragment[None, None, k_block, 1],
                dp_c1[None, None, k_block],
                acc_dp,
            )
        for k_block in cutlass.range(0, k_blocks_per_chunk, unroll=4):
            cute.gemm(
        ...
        for k_block in cutlass.range(0, k_blocks_per_chunk, unroll=4):
            cute.gemm(
                mma_dp,
                acc_dp,
                score_do_fragment[None, None, k_block, 3],
    ...
    def _issue_score_pair_v31(
        ...
        """CHASE paired S/dP issue (replaces two _issue_score_v2
        calls): shared ping-pong branch (s and dp stage indices always
        advance together), the approved minimal interleave inside."""

        pipe_s_done.producer_acquire(s_state)
        pipe_dp_done.producer_acquire(dp_state)
        chase_pipeline.producer_acquire(chase_state)
        mma_issue_token = _iket.range_start(
            "S_ISSUE(i)",
            issue_seq,
        )
        dp_issue_token = _iket.range_start(
            "dP_ISSUE(i)",
            issue_seq,
        )
        if s_state.index == Int32(0):
    ...
    def _issue_prev_dq_v18a(
        ...
        pds_ds_pipeline,
        pds_ds_consumer_state: pipeline.PipelineState,
        round_pipeline,
        round_consumer_state: pipeline.PipelineState,
        dq_done_pipeline,
        ...
        """v18a (E3): the dQ block, split out of the grads head.

        Gates ONLY on the split pds_dS pipeline (both CTAs' whole dS
        images stored + fenced; W18 committed on ds_ready) and the kdq
        round generations -- NOT on the P publish, NOT on the S/dP
        issue window, and NOT on the grads(t-1) drain: this is the E3
        anchor-edge break plus the (v) partial v7 rollback.

        The caller must NOT release pds_dS here: the release also has
        to certify the ROUTE_dS(t-1) source read via the relay dS
        landing wait, which the caller performs after the S/dP issue.

        v12 (S1) kept: when ``commit_dq`` is set (TAIL only), dq_done
        commits immediately after the two dQ issues (tcgen05.commit
        tracks their completion; the math epilogue reads only the dQ
        TMEM columns [128,384), disjoint from dKV [384,512)).
        """

        # dS image published on both CTAs (early gate): each CTA's
        # whole-image store is complete before the leader-issued CG2
        # dQ reads either side -- invariant I1 for the dQ B operand.
        pds_ds_pipeline.consumer_wait(pds_ds_consumer_state)

        # dQ both rounds back-to-back; releases free the round buffers
        # for the quadrant refills (ring order unchanged).
    ...
    def _issue_prev_grads_head_v2(
        ...
        v18a (E3): the dQ rounds moved out (``_issue_prev_dq_v18a``,
        issued before S/dP).  This half waits pds_P -- P blocks +
        p_xchg + the LOCAL dS block are published and both DSM sends
        are issued (W18 commits pds_P on the late p_ready leg) -- and
        the P DSM landing.  The dS landing (relay + 1) was already
        waited by the caller before it released pds_dS, which precedes
        this call in program order, so the dS passes below stay
        covered without a second wait.
        """

        # P published on the original cadence: invariant I1's only
        # gate for the dVdK B operands.
        pds_pipeline.consumer_wait(pds_consumer_state)

        # v12 (P2ii, adjusted): the P landing gates pass 0 (the first
        # p_fragment consumer).
        _mbarrier_wait_acquire_cluster(relay_mbars, relay_phase)

        # Round 0: dV(h0), dV(h1) overwrite+accumulate, then dK(h0), dK(h1).
        # v8: per-slot dkv_done generations again -- this acquire guards
        # slot 0 (vs. the reducer's release of tile i-1's slot 0) and the
        # head commits its generation below, giving the slot-0 T2R a full
        # grads-tail of head start.
        ...
        round_pipeline.consumer_release(round_consumer_state)
        round_consumer_state.advance()
        # v18a (E3): the dS landing wait (relay + 1) moved to the
        # leader body, BEFORE the pds_dS release (it now also
        # certifies the ROUTE_dS source read for math's early dS-image
        # store); program order keeps passes 2/3 behind it.
        round_pipeline.consumer_wait(round_consumer_state)
        dkv_issue_token = _iket.range_start(
            "dVdK_ISSUE(i,r,p)",
    ...
    def _issue_prev_grads_v31(
        ...
        """v3.1 (FATAL-2) reordered gradient block: dV halves first.

        Pass order becomes dV(slot0), dV(slot1), dK(slot0), dK(slot1):
        the first dS consumer moves from global pass 3 to pass 5,
        opening a two-pass window for the dq_done -> ROUTE_dS landing
        chain that the DBUF alias made load-bearing.  Each slot still
        receives its exact v18a pass set (A/B operands and accumulate
        flags unchanged), so the merged-slot drain semantics are
        untouched; only the inter-slot interleave differs.  The ring
        consumption order matches the v3.1 W17 gen order (kdq x2,
        dO x4, Q x4).

        Slot bookkeeping uses split acquire/commit producer states
        (the W17 round_acq/round_com delayed-commit precedent), NOT
        double up-front waits -- the v15 red team killed the two-wait
        head form against the W17 lattice.
        """

        pds_pipeline.consumer_wait(pds_consumer_state)
        # The P landing gates the first p_fragment consumer.
        _mbarrier_wait_acquire_cluster(relay_mbars, relay_phase)
        packed_issue = issue_seq * Int32(self.D_ROUNDS * 4)

        # --- dV half, slot 0 (acquire slot 0's generation).
        ...
        # --- dV half, slot 1 (acquire slot 1's generation only now:
        # just-in-time, so the reducer keeps its drain window).
        dkv_done_pipeline.producer_acquire(dkv_acquire_state)
        dkv_acquire_state.advance()
        round_pipeline.consumer_wait(round_consumer_state)
        dkv_issue_token = _iket.range_start(
        ...
        round_pipeline.consumer_release(round_consumer_state)
        round_consumer_state.advance()
        # Slot 0 complete (its dV and dK passes both issued): fence and
        # commit its generation so the reducer's slot-0 T2R can start
        # while slot 1's dK half is still issuing.
        cute.arch.fence_view_async_tmem_store()

...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    P/dS across the cluster; warp 19 issues dQ and commits ring TMA
    completions.
    ...
    def kernel(
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            # Match exact aug6: keep the entire relay lifetime under one
            # lane-0 branch.  The branch reconverges once before the common
            # TMEM tail, so no per-tile whole-warp sync is required.
            if tidx % Int32(32) == Int32(0):
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    # dQ consumes only the complete local dS image. Publish
                    # that readiness before issuing the independent DSM
                    # copies so W19 can overlap dQ with relay traffic.
            ...
            for loop_iter in cutlass.range(tile_count):
                # g0/g1 are committed by W17; point this independent
                # producer cursor at g2 before dQ releases the reused slots.
                commit_com.advance()
                commit_com.advance()

                # The commit role is already whole-warp. On the leader CTA it
                # consumes g0/g1 for dQ as soon as local dS is published,
                # independently of W18's DSM landing waits.
                if is_leader_cta:

"""DeepSeek Sparse Attention backward, two-CTA SERIAL chain, k1 close fix.
    ...
    def kernel(
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            for loop_iter in cutlass.range(tile_count):
                ...
                cute.arch.fence_view_async_shared()
                # k1: local shared::cta arrive (consumer = same-CTA
                # relay lane; cluster-scope release rides the relay's
                # own remote arrive downstream).
                cute.arch.mbarrier_arrive(
                    p_ready_mbars,
                )
                ...
                cute.arch.fence_view_async_shared()
                # k1: local shared::cta arrive (see p_ready note).
                cute.arch.mbarrier_arrive(
                    pds_ready_mbars,
                )
                pds_state.advance()

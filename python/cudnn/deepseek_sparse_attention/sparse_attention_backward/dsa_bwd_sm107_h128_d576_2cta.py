...
"""Rubin (SM107) two-CTA sparse-attention backward for BF16 H128 D576/V512.
...
@dsl_user_op
def _tcgen05_commit_smem_a_read(
    barrier: cute.Pointer,
    ...
    *,
    ...
    """Signal a CG2 barrier once prior MMAs finish reading SMEM operand A."""

    nvvm.tcgen05_commit(
        barrier.llvm_ptr,
        multicast_mask=cutlass.Int16(multicast_mask).ir_value(loc=loc, ip=ip),
        smem_a_read=True,
...
@dsl_user_op
def _prefetch_kv_row_l2(source: cute.Pointer, *, loc=None, ip=None) -> None:
    """Protect one next-tile KV line against intervening atomic churn."""
    source_i64 = source.toint(loc=loc, ip=ip).ir_value()
    llvm.inline_asm(
        None,
        [source_i64],
...
def _load_bf16x16(source: cute.Pointer, *, loc=None, ip=None) -> U64x4:
    """Load one immutable O or dO segment with streaming retention.

    Every caller reads these O (and rare-path dO) rows exactly once per
    call, immediately after their advisory L2 warm, so the evict-first
    policy releases the ~0.5-1 GB/call one-pass stream from L2 instead of
    letting it displace the KV rows and FP32 dKV workspace lines the
    steady-state tile loop and atomic drain keep re-touching."""
    source_i64 = source.toint(loc=loc, ip=ip).ir_value()
    result_type = llvm.StructType.get_literal([T.i64()] * 4)
    value = llvm.inline_asm(
        result_type,
        [source_i64],
        "ld.global.cs.v4.u64 {$0, $1, $2, $3}, [$4];",
        "=l,=l,=l,=l,l",
        has_side_effects=False,
        is_align_stack=False,
        asm_dialect=llvm.AsmDialect.AD_ATT,
...
def _dot_bf16x16_serial(
    ...
    *,
    ...
    result = llvm.inline_asm(
        ...
        fma.rn.f32.bf16 acc, x0, y0, acc;
        fma.rn.f32.bf16 acc, x1, y1, acc;
        fma.rn.f32.bf16 acc, x2, y2, acc;
        fma.rn.f32.bf16 acc, x3, y3, acc;
        fma.rn.f32.bf16 acc, x4, y4, acc;
        fma.rn.f32.bf16 acc, x5, y5, acc;
        fma.rn.f32.bf16 acc, x6, y6, acc;
        fma.rn.f32.bf16 acc, x7, y7, acc;
        fma.rn.f32.bf16 acc, x8, y8, acc;
        fma.rn.f32.bf16 acc, x9, y9, acc;
        fma.rn.f32.bf16 acc, x10, y10, acc;
        fma.rn.f32.bf16 acc, x11, y11, acc;
        fma.rn.f32.bf16 acc, x12, y12, acc;
        fma.rn.f32.bf16 acc, x13, y13, acc;
        fma.rn.f32.bf16 acc, x14, y14, acc;
        fma.rn.f32.bf16 acc, x15, y15, acc;
        mov.f32 $0, acc;
        }""",
        "=f,l,l,l,l,l,l,l,l",
        has_side_effects=False,
...
@dsl_user_op
def _create_evict_last_policy(*, loc=None, ip=None) -> cutlass.Uint64:
    """Create an L2 policy that retains every touched cache line."""
...
@dsl_user_op
def _red_global_add_f32x4_evict_last(
    destination: cute.Pointer,
    value_0: Float32,
    value_1: Float32,
    value_2: Float32,
    value_3: Float32,
    cache_policy: cutlass.Uint64,
    *,
    loc=None,
    ip=None,
) -> None:
    """Reduce four FP32 values while retaining the workspace line in L2."""
    ...
    llvm.inline_asm(
        ...
        [
            ...
            cutlass.Uint64(cache_policy).ir_value(loc=loc, ip=ip),
        ],
...
@dsl_user_op
def _red_global_add_f32x2_evict_last(
    destination: cute.Pointer,
    value_0: Float32,
    value_1: Float32,
    cache_policy: cutlass.Uint64,
    *,
    ...
    """Reduce two FP32 values while retaining the workspace line in L2."""
    ...
    llvm.inline_asm(
        ...
        "red.global.add.L2::cache_hint.v2.f32 [$0], {$1, $2}, $3;",
        "l,f,f,l",
        has_side_effects=True,
        is_align_stack=False,
        asm_dialect=llvm.AsmDialect.AD_ATT,
...
class FlashAttentionDSABackwardSm107H128D576TwoCTA:
    ...
    # Prefetch stationary O rows before the softmax statistics consume them.
    O_PREFETCH_MAX_TOPK = 100_000
    PDL_MAX_TOPK = 128
    DSINK_BLOCK_Q = 32
    DSINK_THREADS = 128
    DSINK_UNROLL = 4
    ...
    def __init__(
        ...
        if max_topk == 128 and not short_sq:
            self.DQ_EPI_BATCH_CHUNKS = 2
        if enable_score_row_interleave and max_topk not in (512, 1024, 1152, 2048):
            raise ValueError("score-row interleave requires max_topk>=512")
        full_k1152 = max_topk == 1152 and not short_sq
        self.INTERLEAVE_SCORE_ROWS = enable_score_row_interleave or full_k1152
        if enable_wide_k2048_finalize and max_topk != 2048:
            raise ValueError("wide finalization is only valid for max_topk=2048")
    ...
    @cute.jit
    def _copy_sparse_k_d128_pair_interleaved(
        self,
        mKV: cute.Tensor,
        destination_rows: cute.Tensor,
        destination_row_0: Int32,
        destination_row_1: Int32,
        kv_index_0: Int32,
        kv_index_1: Int32,
        batch_idx: Int32,
        d_offset: Int32,
        ...
        """Copy two valid D128 rows, alternating their 128-byte halves."""

        source_row_full_0 = mKV[kv_index_0, None, (0, batch_idx)]
        source_row_full_1 = mKV[kv_index_1, None, (0, batch_idx)]
        source_row_0 = cute.make_tensor(
            cute.make_ptr(
                self.element_dtype,
                (source_row_full_0.iterator + d_offset).llvm_ptr,
                cute.AddressSpace.gmem,
                assumed_align=16,
            ),
            cute.make_layout((self.K_CHUNK,)),
        )
        source_row_1 = cute.make_tensor(
            cute.make_ptr(
                self.element_dtype,
                (source_row_full_1.iterator + d_offset).llvm_ptr,
                cute.AddressSpace.gmem,
                assumed_align=16,
            ),
            cute.make_layout((self.K_CHUNK,)),
        ...
        source_chunks = (
            cute.flat_divide(source_row_0, (8,)),
            cute.flat_divide(source_row_1, (8,)),
        )
        destination_chunks = (
            cute.flat_divide(destination_rows[destination_row_0, None], (8,)),
            cute.flat_divide(destination_rows[destination_row_1, None], (8,)),
        )
        for half in cutlass.range_constexpr(self.K_CHUNK // 64):
            chunk_index = half * self.KV_GROUP_SIZE + index_in_group
            for row in cutlass.range_constexpr(2):
                thread_source = thread_copy.partition_S(source_chunks[row][None, chunk_index])
                thread_destination = thread_copy.partition_D(destination_chunks[row][None, chunk_index])
                cute.copy(copy_atom, thread_source, thread_destination)
    ...
    def _issue_dkv_sweep(
        ...
        for chunk in cutlass.range_constexpr(self.H_TILE_CLUSTER // self.ROUND_K_HEADS):
            ...
            # The circular stage backs only MMA operand A.  Rubin can return
            # its credit as soon as the tensor core finishes reading A,
            # rather than waiting for the complete dV/dK accumulation.
            with cute.arch.elect_one():
                _tcgen05_commit_smem_a_read(
                    round_pipeline.sync_object_empty.get_barrier(round_consumer_state.index),
                    round_pipeline.consumer_mask,
                )
    ...
    def _resolve_kdq_row_indices(
        ...
        for local_n in kdq_local_n:
            ...
            kv_index = Int32(-1)
            if index_in_group == Int32(0):
                if global_n < topk:
                    kv_index = tile_indices[local_n]
            source_lane = role_tidx % Int32(32) - index_in_group
            kv_index = cute.arch.shuffle_sync(kv_index, source_lane)
    ...
    def _get_stats_workspace(self, workspace: cute.Tensor, total_q: Int32, num_heads: Int32):
        ...
        sink_probability_iter = cute.recast_ptr(workspace.iterator + plane_bytes, dtype=self.acc_dtype)
        ...
        return (
            ...
            cute.make_tensor(sink_probability_iter, layout),
        )
    ...
    def __call__(
        ...
        sum_odo, sink_probability = self._get_stats_workspace(
            workspace_LSE_OdO,
            mQ.shape[2][0],
    ...
    def _store_dq_epi_scalar_direct(
        ...
        if mtx < self.MATH_THREADS_PER_CTA:
            ...
            num_batches = num_chunks // batch_chunks
            batch_unroll = 1 if self.max_topk == 128 else num_batches
            for batch_index in cutlass.range(num_batches, unroll=batch_unroll):
                batched_values = []
                ...
                for slot in cutlass.range_constexpr(batch_chunks):
                    ...
                    converted_bits = cute.recast_tensor(converted_values, cutlass.Uint16)
                    ...
                    # The T2R layout assigns adjacent heads at a fixed D coordinate to
                    # each thread, so output stores follow those scalar coordinates.
                    for scalar_index in cutlass.range_constexpr(cute.size(chunk_values)):
    ...
    def _drain_dkv(
        ...
        if cutlass.const_expr(self.RETAIN_DKV_IN_L2):
            cache_policy = _create_evict_last_policy()
        ...
        if cutlass.const_expr(self.max_topk >= 512):
            ...
            for i in cutlass.range_constexpr(8):
                ...
                if global_row < topk:
                    ...
                    r_topk[i] = Int32(-1)
        # Canonicalize active positive-OOB indices once. Both TMEM panels then
        # need only the remaining nonnegative test before forming an atomic
        # destination; negative sentinels and duplicate indices are unchanged.
        for i in cutlass.range_constexpr(8):
            if r_topk[i] >= seqlen_kv:
                r_topk[i] = Int32(-1)
        ...
        for i in cutlass.range_constexpr(8):
            ...
            if kv_index >= Int32(0):
                ...
                if cutlass.const_expr(self.RETAIN_DKV_IN_L2):
                    _red_global_add_f32x4_evict_last(
    ...
    def convert_dkv(
        ...
        dsink_partials = cutlass.Array(
            Float32,
            self.num_threads_D_convert * self.num_threads_seq,
            space=cutlass.AddressSpace.smem,
            alignment=128,
        )
        tidx, tidy, _ = cute.arch.thread_idx()
        seq_block_idx, _, batch_idx = cute.arch.block_idx()
        # Programmatic dependent launch: this grid launches while the main
        # kernel's last wave drains, so wait for the main grid's completion
        # (workspace atomics and stats-plane writes visible) before any read.
        # Trigger after the wait: the next call's zero_init only becomes
        # launchable once the main kernel is complete, and its own entry wait
        # still orders it behind this kernel's workspace reads.
        if cutlass.const_expr(self.max_topk <= self.PDL_MAX_TOPK):
        ...
        if seq_id < seqlen:
            ...
            # The dK tail accumulates in logical order. Sixteen lanes convert
            # and store aligned groups of four values.
            if tidx < Int32(16):
                d = Int32(self.D_HEAD) + tidx * Int32(4)
                tail_values = _load_f32x4(acc_row.iterator + d)
                packed01 = _dq_pack_bf16x2(tail_values[0], tail_values[1])
                packed23 = _dq_pack_bf16x2(tail_values[2], tail_values[3])
                _store_bf16x4_ordinary(
                    out_row.iterator + d,
                    packed01,
                    packed23,
    ...
    def _load_score_kv_indexed(
                ...
                if index_in_group == Int32(0):
                    if topk_slot < topk:
                        if cutlass.const_expr(direct_indices):
                            ...
                            kv_index = tile_indices[logical_n]
                source_lane = tidx % Int32(32) - index_in_group
                kv_index = cute.arch.shuffle_sync(kv_index, source_lane)
        ...
        if cutlass.const_expr(wave == 0):
            ...
            include_tail = True
        # Declared before the dynamic all-valid arm: CuTe requires variables
        # assigned only by the fallback arm to have a stable join type.
        local_n = Int32(0)
        kv_index = Int32(-1)
        if cutlass.const_expr(self.INTERLEAVE_SCORE_ROWS):
            assert rows_per_group == 2
            row_valid_0 = row_kv_index[0] >= 0 and row_kv_index[0] < seqlen_kv
            row_valid_1 = row_kv_index[1] >= 0 and row_kv_index[1] < seqlen_kv
            if row_valid_0 and row_valid_1:
                for chunk in chunk_ids:
                    destination_rows = cute.composition(
                        destination[None, None, None, chunk],
                        cute.make_layout((self.N_TILE_CTA, self.K_CHUNK)),
                    )
                    self._copy_sparse_k_d128_pair_interleaved(
                        mKV,
                        destination_rows,
                        row_local_n[0],
                        row_local_n[1],
                ...
                if cutlass.const_expr(include_tail):
                    for row_iteration in cutlass.range_constexpr(2):
                        self._copy_k_tail_row(
                            mKV,
                            kt_destination,
                            row_local_n[row_iteration],
                            row_kv_index[row_iteration],
                            batch_idx,
                            index_in_group,
                            copy_atom,
                            thread_copy,
                ...
                for row_iteration in cutlass.range_constexpr(2):
                    ...
                    kv_index = row_kv_index[row_iteration]
                    row_valid = kv_index >= 0 and kv_index < seqlen_kv
                    if row_valid:
                        for chunk in chunk_ids:
                            destination_rows = cute.composition(
                                destination[None, None, None, chunk],
    ...
    def _publish_dsink_stats(
        ...
        if reducer_tidx < Int32(self.H_TILE_CTA):
            row = reducer_tidx
            head = rank * Int32(self.H_TILE_CTA) + row
            sink_value = Float32(attn_sink[head, (0, batch_idx)])
    ...
    def zero_init(
        ...
        # Programmatic dependent launch: this grid is launched with the
        # stream-serialization relaxation, so its zero stores must not land
        # until the prior grid on the stream (the previous call's convert
        # kernel, which still reads this FP32 workspace, or a caller op that
        # writes inputs) has completed and flushed.  Wait FIRST, then signal
        # the main kernel's early launch: the trigger ordering guarantees the
        # main grid only becomes launchable after every producer preceding
        # this call is complete, so its unguarded input TMA/gather reads stay
        # correct while its launch and prologue overlap the zeroing stores.
        if cutlass.const_expr(self.max_topk <= self.PDL_MAX_TOPK):
            cute.arch.griddepcontrol_wait()
            cute.arch.griddepcontrol_launch_dependents()
    ...
    def kernel(
        ...
        pds_push_ready_mbar = storage.pds_push_ready_mbar.data_ptr()
        ...
        # Score-tail prefetch and dQ-tail consumption can overlap across
        # pipeline generations, so their swizzled views need separate slabs.
        ...
        if tidx == Int32(0):
            ...
            # dS-side twin of the p_ready split above: the relay's dS push
            # sources only the peer-slot half of the dS image, written
            # exclusively by the two non-local-half math warps.  A dedicated
            # two-arrival push-readiness barrier lets the dS DSM push (and
            # the landing -> relay -> dK-issue chain behind it) start as soon
            # as those bytes are fenced.  Full-image readers (the offloaded
            # dK-tail) keep the four-arrival pds_ready barrier.
            cute.arch.mbarrier_init(pds_push_ready_mbar, self.MATH_WARPS // 2)
            # The relay's P push reads only the p_xchg half, written by the
            # two peer-half math warps; the local-half warps' visibility to
            # the tail issuer rides the p_free commit instead.  Arming the
            # ready barrier with two arrivals lets the DSM push start as soon
            # as the xchg bytes are fenced, without waiting for the local
            # half's stores.
            cute.arch.mbarrier_init(p_ready_mbars, self.MATH_WARPS // 2)
            if cutlass.const_expr(split_regime):
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
                ...
                for score_iter in cutlass.range(Int32(0), my_tile_count):
                    ...
                    if is_dq_owner:
                        if cutlass.const_expr(self.max_topk == 128):
                            ...
                            ktdq_half = tidx % Int32(2)
                            ktdq_global_n = (walk_base + walk_step * score_iter) * Int32(self.N_TILE) + ktdq_row
                            ktdq_kv = Int32(-1)
                            if ktdq_half == Int32(0):
                                if ktdq_global_n < topk:
                                    ktdq_kv = staged_indices[ktdq_row, index_slot]
                            ktdq_source_lane = tidx % Int32(32) - ktdq_half
                            ktdq_kv = cute.arch.shuffle_sync(ktdq_kv, ktdq_source_lane)
                            ...
                            ktdq_v0, ktdq_v1 = self._load_kt_dq_tail_values(
                                ...
                                tidx,
                                ktdq_kv,
                                reuse_local_score=not split_regime,
                            )
                            pipe_kscore.producer_acquire(gather_state)
                            self._gather_kdq_indexed(
                                mKV,
                            ...
                            if cutlass.const_expr(not split_regime):
                                # The kscore release retires the score UMMA,
                                # but other gather warps can still be reading
                                # the current slab for dQ. Finish those reads
                                # before any warp starts the next-tile refill.
                        ...
                        cute.arch.fence_view_async_shared()
                    if score_iter != my_tile_count - Int32(1):
                        # Prefetch the next tile while MMA still holds the current score-B
                        # generation, before the next gather consumes its rows.
                        if cutlass.const_expr(self.KV_PREFETCH):
                        ...
                        if (tidx & Int32(31)) == Int32(0):
                            pipe_kscore.producer_commit(gather_state)
                        gather_state.advance()
                    # Long top-k schedules keep the dK tail on the gather warps to avoid
                    # delaying the dKV drain. Topk128 uses the idle reducer warpgroup.
                    if cutlass.const_expr(split_regime):
                        if has_dkv:
                            cute.arch.mbarrier_wait(pds_ready_mbars, score_iter & Int32(1))
                            self._accumulate_dk_tail_wmma(
                    ...
                    elif cutlass.const_expr(self.max_topk != 128):
                        if score_iter == my_tile_count - Int32(1):
                            cute.arch.mbarrier_wait(pds_ready_mbars, score_iter & Int32(1))
                            self._accumulate_dk_tail_wmma(
                                dkt_warp_mma,
            ...
            r_p_packed = cute.recast_tensor(r_p, cutlass.Uint32)
            ...
            # In the exposed TOPK128 staged-index path, Ld16x256b maps each
            # warp as (lane % 4, lane // 4): lanes with the same lane % 4
            # own different head bands over the same eight sparse N
            # coordinates. Resolve each slot in the first four lanes, then
            # broadcast it to the other head owners.  Longer rows retain
            # their pre-wait global loads, whose latency is already hidden.
            math_lane = mtx % Int32(32)
            validity_source_lane = math_lane % Int32(4)
            validity_index_leader = math_lane < Int32(4)
            for loop_iter in cutlass.range(my_tile_count):
                ...
                if cutlass.const_expr(self.max_topk == 128):
                    ...
                    for band in cutlass.range_constexpr(8):
                        ...
                        row = Int32(-1)
                        if validity_index_leader:
                            if slot < topk:
                                if cutlass.const_expr(split_regime):
                                    row = mTopkIdxs[slot, (token_idx, batch_idx)]
                                else:
                                    row = staged_indices[hoist_band_n[band], loop_iter & Int32(1)]
                        row = cute.arch.shuffle_sync(row, validity_source_lane)
                        if slot < topk:
                            if row >= Int32(0) and row < mKV.shape[0]:
                                r_valid_band[band] = True
                else:
                    # Full-length rows make every slot in every issued tile
                    # active. Take one uniform branch per tile and avoid the
                    # eight redundant slot-bound predicates; ragged rows keep
                    # the original inactive-slot guard.
                    if topk == Int32(self.max_topk):
                        for band in cutlass.range_constexpr(8):
                            slot = tail_tile_index * Int32(self.N_TILE) + hoist_band_n[band]
                            row = mTopkIdxs[slot, (token_idx, batch_idx)]
                ...
                for h_group in cutlass.range_constexpr(4):
                    ...
                    for pair in cutlass.range_constexpr(4):
                        ...
                        r_score[i0] = v0
                        r_score[i1] = v1
                        r_p_packed[i0 // 2] = _dq_pack_bf16x2(v0, v1)
                if cutlass.const_expr(split_regime):
                ...
                if loop_iter == Int32(0):
                    ...
                    for h_group in cutlass.range_constexpr(4):
                        r_delta[h_group] = softmax_stats[hoist_group_local_h[h_group], 1] * scale_softmax
                for h_group in cutlass.range_constexpr(4):
                    scaled_delta = r_delta[h_group]
                    for pair in cutlass.range_constexpr(4):
                        i0 = hoist_band_indices[h_group][2 * pair]
                        i1 = hoist_band_indices[h_group][2 * pair + 1]
                        d0, d1 = cute.arch.fma_packed_f32x2(
                            (r_dp[i0], r_dp[i1]),
                        ...
                        r_p_packed[i0 // 2] = _dq_pack_bf16x2(d0, d1)
                ...
                if not math_owns_local_half:
                    with cute.arch.elect_one():
                        cute.arch.mbarrier_arrive(pds_push_ready_mbar)
                with cute.arch.elect_one():
            ...
            # Programmatic dependent launch gate: everything above (O
            # prefetch, folded LSE, O*dO statistics) reads only caller inputs
            # and is free to overlap the zero_init grid.  From here on the
            # reducers write the stats plane the previous call's convert
            # kernel may still be reading, publish dSink in the single-query
            # build, and atomically drain into the zero-initialized FP32 dKV
            # workspace -- all of which require the prior grids complete.
            if cutlass.const_expr(self.max_topk <= self.PDL_MAX_TOPK):
                cute.arch.griddepcontrol_wait()
            if is_dq_owner:
            ...
            for loop_iter in cutlass.range(drain_count):
                ...
                # Topk128 without split clusters runs the dK tail on reducer warps
                # 8..11, which reach the required pds_ready boundary after statistics.
                # Longer top-k schedules keep the tail on the gather warps.
                if cutlass.const_expr(self.max_topk == 128 and not split_regime):
                    if rtx < Int32(128):
            ...
            if relay_lane == Int32(0):
                rank_block_offset = rank * Int32(self.PDS_BLOCK_ELEMENTS)
                peer_half_offset = (Int32(1) - rank) * Int32(self.PDS_BLOCK_ELEMENTS)
                for loop_iter in cutlass.range(my_tile_count):
                    ...
                    if cutlass.const_expr(self.max_topk >= 1024):
                        _cpasync_bulk_s2cluster(
                            p_xchg_raw.iterator,
                            p_blocks_raw + rank_block_offset,
                            landing_mbars,
                    ...
                    cute.arch.mbarrier_wait(pds_push_ready_mbar, loop_iter & Int32(1))
                    cute.arch.mbarrier_arrive_and_expect_tx(landing_mbars + 1, self.PDS_BLOCK_BYTES, peer_cta_rank_in_cluster=peer_rank)
                    if cutlass.const_expr(self.max_topk >= 1024):
                        _cpasync_bulk_s2cluster(
                            ds_image_raw + peer_half_offset,
                            ds_blocks_raw + rank_block_offset,
                            landing_mbars + 1,
                            self.PDS_BLOCK_BYTES,
                            peer_rank,
                        )
    ...
    def _make_shared_storage(
        ...
        class SharedStorage:
            ...
            pds_push_ready_mbar: cute.struct.MemRange[cutlass.Int64, 1]
    ...
    def _issue_dq_rounds_early(
        ...
        for round_index in cutlass.range_constexpr(self.D_ROUNDS):
            ...
            if cutlass.const_expr(round_index == 0):
                ...
                if cutlass.const_expr(dq0_free_pipeline is not None):
                    # DQ0 reads the first K-loan half only as operand A, so
                    # its refill gate can signal on Rubin's A-read completion.
                    with cute.arch.elect_one():
                        _tcgen05_commit_smem_a_read(
                            dq0_free_pipeline.sync_object_full.get_barrier(dq0_free_commit_state.index),
                            dq0_free_pipeline.producer_mask,
                        )
        ...
        # Both dQ panels consume score-K loan storage only as operand A.
        # Release it after the A read, while DQ1 may still be accumulating.
        with cute.arch.elect_one():
            _tcgen05_commit_smem_a_read(
                kscore_pipeline.sync_object_empty.get_barrier(kscore_consumer_state.index),
                kscore_pipeline.consumer_mask,
            )
    ...
    def _prefetch_kv_rows_l2(
        ...
        if cutlass.const_expr(self.max_topk == 128):
            ...
            if line_half == Int32(0):
                if global_n < topk:
                    kv_index = tile_indices[local_n]
            source_lane = role_tidx % Int32(32) - line_half
            kv_index = cute.arch.shuffle_sync(kv_index, source_lane)
            if kv_index >= Int32(0) and kv_index < seqlen_kv:
            ...
            for wave in cutlass.range_constexpr(1):
                ...
                if linear < Int32(8 * 8):
                    ...
                    if line == Int32(0):
                        if global_n < topk:
                            kv_index = tile_indices[local_n]
                    source_lane = role_tidx % Int32(32) - line
                    kv_index = cute.arch.shuffle_sync(kv_index, source_lane)
                    if kv_index >= Int32(0) and kv_index < seqlen_kv:
                        row_base = mKV.iterator + cute.crd2idx((kv_index, Int32(0), (0, Int32(0))), mKV.layout)
                        _prefetch_kv_row_l2(row_base + line * Int32(64))
            if role_tidx < Int32(8):
                local_n = rank * Int32(self.N_TILE_CTA) + role_tidx
                global_n = tile_index * Int32(self.N_TILE) + local_n
                kv_index = Int32(-1)
                if global_n < topk:
                    kv_index = tile_indices[local_n]
                if kv_index >= Int32(0) and kv_index < seqlen_kv:
                    row_base = mKV.iterator + cute.crd2idx((kv_index, Int32(0), (0, Int32(0))), mKV.layout)
                    _prefetch_kv_row_l2(row_base + Int32(self.D_HEAD))

    @cute.jit
    def _load_kt_dq_tail_values(
        self,
        mKV: cute.Tensor,
        s_kt_score_rows: cute.Tensor,
        batch_idx: Int32,
        rank: Int32,
        role_tidx: Int32,
        pre_kv_index: Int32,
        reuse_local_score: cutlass.Constexpr[bool],
    ):
        """Load this thread's two K-tail vectors into registers.

        Non-split builds reuse the current score-tail slab for rank-owned rows
        and keep global loads for peer-owned rows. The caller must rendezvous
        all gather warps before overwriting the shared score-tail slab.
        """
        seqlen_kv = cute.size(mKV, mode=[0])
        ...
        if pre_kv_index >= Int32(0) and pre_kv_index < seqlen_kv:
            local_base = rank * Int32(self.N_TILE_CTA)
            is_local = row >= local_base and row < local_base + Int32(self.N_TILE_CTA)
            if cutlass.const_expr(reuse_local_score):
                if is_local:
                    score_row = s_kt_score_rows[row - local_base, None]
                    score_chunks = cute.flat_divide(score_row, (8,))
                    score_chunk = rank * Int32(4) + chunk_base
                    cute.copy(
                        vector_load,
                        score_chunks[None, score_chunk],
                        values_0,
                    )
                    cute.copy(
                        vector_load,
                        score_chunks[None, score_chunk + Int32(1)],
                        values_1,
                    )
                else:
                    ...
                    cute.copy(
                        vector_load,
                        cute.local_tile(source_row, (8,), (chunk_base,)),
                        values_0,
                    )
                    cute.copy(
                        vector_load,
                        cute.local_tile(source_row, (8,), (chunk_base + Int32(1),)),
                        values_1,
                    )
            else:
                source_row_full = mKV[pre_kv_index, None, (0, batch_idx)]
    ...
    def _gather_kt_dq(
        ...
        if cutlass.const_expr(pre_kv_index is not None):
            ...
            source_lane = role_tidx % Int32(32) - half
            kv_index = cute.arch.shuffle_sync(kv_index, source_lane)
        chunk_base = half * Int32(2)
        if kv_index >= Int32(0) and kv_index < seqlen_kv:
            source_row_full = mKV[kv_index, None, (0, batch_idx)]
    ...
    def _accumulate_dk_tail_wmma(
        ...
        # Programmatic dependent launch gate: the FP32 tail atomics below
        # deposit into the zero-initialized workspace columns, so the prior
        # grids must be complete.  On the gather-warp builds this is the
        # role's first workspace access; the wait is a no-op within a few
        # cycles once the dependency has resolved, so re-executing it every
        # tile costs nothing measurable.
        if cutlass.const_expr(self.max_topk <= self.PDL_MAX_TOPK):
            cute.arch.griddepcontrol_wait()
        rank_of_cta = cute.arch.make_warp_uniform(cute.arch.block_idx_in_cluster())
        if cutlass.const_expr(self.RETAIN_DKV_IN_L2):
            tail_cache_policy = _create_evict_last_policy()
        for half_step in cutlass.range(2, unroll=1):
            n_half = Int32(half_step)
            if rank_of_cta != Int32(0):
                n_half = Int32(1 - half_step)
            ...
            # Use one statically-unrolled deposit body for both CTA ranks.
            # This removes the duplicated rotated body from the large fused
            # kernel; both ranks retain the same paired FP32 contributions.
            for pair_index in cutlass.range_constexpr(cute.size(tCrC) // 2):
                ...
                if pair_ok:
                    ...
                    if row >= Int32(0) and row < seqlen_kv:
                        ...
                        if cutlass.const_expr(self.RETAIN_DKV_IN_L2):
                            _red_global_add_f32x2_evict_last(
                                pointer,
                                pair_values[0],
                                pair_values[1],
                                tail_cache_policy,
    ...
    def _store_dqt_epi(self, t_dqt, dqt_tiled_mma, dq, query, batch, rank, mtx):
        ...
        if mtx < self.MATH_THREADS_PER_CTA:
            ...
            cute.arch.fence_view_async_tmem_load()
            assert cute.size(r_dqt) % self.DQ_WIDE_STORE_VALUES == 0
            # The dQ tail T2R layout exposes adjacent pairs for streamed stores.
            for pair_index in cutlass.range_constexpr(cute.size(r_dqt) // 2):
                i0 = 2 * pair_index
                i1 = i0 + 1
                pair_head = Int32(cute.get(dqt_coordinates[i0], mode=[0]))
                pair_d0 = Int32(cute.get(dqt_coordinates[i0], mode=[1]))
                head1 = Int32(cute.get(dqt_coordinates[i1], mode=[0]))
                d1 = Int32(cute.get(dqt_coordinates[i1], mode=[1]))
                if head1 == pair_head and d1 == pair_d0 + Int32(1) and (pair_d0 & Int32(1)) == Int32(0):
                    packed = _dq_pack_bf16x2(r_dqt[i0], r_dqt[i1])
                    destination = cute.recast_ptr(
                        dq.iterator
                        + cute.crd2idx(
                            (
                                Int32(self.D_HEAD) + pair_d0,
                                pair_head,
                                (query, batch),
                            ),
                            dq.layout,
                    ...
                    cute.arch.store(destination, packed, cop="cs")
                else:
                    dq[Int32(self.D_HEAD) + pair_d0, pair_head, (query, batch)] = self.element_dtype(r_dqt[i0])

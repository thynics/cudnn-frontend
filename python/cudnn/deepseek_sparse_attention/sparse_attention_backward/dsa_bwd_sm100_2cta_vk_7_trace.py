"""vk_7_trace: vk_7 with the canonical-K pricing span set (refined trace).

Instrumentation-only sibling of vk_7.  Same 31-name budget trades as
vk_2_trace, adapted to the W1' topology (the TMA waits live on W19 now):
  in:  RK_ACQ(i) x6 gather kscore acquires; MAT_ACQ(m,q) x3 W17 round
       acquires (two inside ROUTE_K = the rendezvous credit share, one
       in the train); MAT_WAIT(m,q) on W19's six per-tile tma waits
       (supply flight exposure at the commit relay); PDS_WAIT(i) and
       GRAD_SUP_WAIT(i) at the leader's grads-block head.
  out: LOAD_QDO, LOAD_STATS, MATH_BAR1, WAIT_S, WAIT_dP.
Purpose: price the canonical-K bet -- ROUTE_K's residual composition
after W2 (credit wait vs post-A data drain), the remaining math slack
(M2's receivable), the period, and the train gap ledger.
Payloads: RK_ACQ constant 0 (runtime branches); others loop_iter/
issue_seq; several spans per tile share payloads -- bucket by
timestamp, not payload.
...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    * The gradient GEMMs of tile p are issued inside tile t=p-1's S/dP
    ...
    def _issue_prev_grads_head_v2(
        ...
        grad_sup_token = _iket.range_start(
            "GRAD_SUP_WAIT(i)",
            issue_seq,
        )

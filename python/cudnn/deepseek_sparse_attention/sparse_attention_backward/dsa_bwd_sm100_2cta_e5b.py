...
E5b-0: K-staging TMA consumer (fork of e4ca).  The score-K supply is
replaced wholesale: a producer pre-stages the sparse KV rows of every
score tile into a contiguous GMEM tensor (``mKStage``, per (token,
batch) rows [topk_padded, D512] bf16, score tile t = the contiguous
row slice [t*64, (t+1)*64), invalid/hole rows zero-filled), and each
K(t) fill collapses to ONE 32 KiB CG2 TMA box per CTA riding a
PipelineTmaUmma kscore pipe: producer_acquire arms the transaction
barrier on the leader CTA and the two collective TMA boxes complete it
directly.  The e4ca E-stack machinery (the per-chunk publish barriers,
the crew's chunk-wise gather fills, the dP release train) is dead and
deleted;
S and dP revert to the plain whole-tile issue gated by one kscore
consumer_wait before S.  The E4b lending machinery survives intact:
the loan generations still alternate with K on the same one-stage pipe
(hand-rolled producer protocol -- the loan bytes complete on their own
loan mbars, the pipe stage barrier takes one counting arrive per CTA),
the borrowed dO_r1 h0 parcel keeps its borrow-TMA/borrow_free mbars,
and the steady-state K TMA is gated on borrow_free so it never
overwrites the borrowed tenant living in score_kv bytes 16-32K.
Everything else (ring, math, reduce, dQ epilogue, kdq sparse gather,
dKV drain) is byte-identical to e4ca.
...
the score K tile arrives per KV tile as one staged TMA box; gradient
operands stream through a two-slot round ring.  dQ accumulates in TMEM
...
class FlashAttentionDSABackwardSm100TwoCTA(FlashAttentionDSABackwardSm100):
    ...
    def __call__(
        ...
        # E5b: pre-staged contiguous score-K rows.  The probe passes a
        # [tokens, topk_padded, D512] bf16 tensor; the canonical kernel
        # view is (N=topk_padded, K=D512, L=(token, batch)) so score
        # tile t of a token is the contiguous row slice
        # [t*N_TILE, (t+1)*N_TILE).
        mKStage = cute.make_tensor(
            mKStage.iterator,
            cute.make_layout(
                (mKStage.shape[1], mKStage.shape[2], (mKStage.shape[0], 1)),
                stride=(
                    mKStage.stride[1],
                    mKStage.stride[2],
                    (mKStage.stride[0], 0),
        ...
        # E5b: whole-tile score-K TMA atom.  Same trick as the
        # stationary-A pair: a one-stage [N, D512] B layout whose inner
        # swizzle and byte image are identical to the four-chunk
        # score_b view, so one TMA box lands exactly the bytes the
        # score/dP fragments read.  The B-side helper splits the N64
        # tile across the two CTAs (N32 x D512 = 32 KiB per CTA -- the
        # score_kv parcel); the CG2 load op folds both boxes'
        # completion onto the leader CTA's transaction barrier, which
        # is the PipelineTmaUmma contract.
        kstage_tiler = (
            self.H_TILE_CLUSTER,
            self.N_TILE,
            self.D_HEAD,
        )
        kstage_b_layout_staged = sm100_utils.make_smem_layout_b(
            score_tiled_mma,
            kstage_tiler,
            self.element_dtype,
            1,
        )
        assert cute.cosize(kstage_b_layout_staged) == cute.cosize(
            score_b_layout_staged
        )
        assert kstage_b_layout_staged.inner == score_b_layout_staged.inner
        kstage_b_layout = cute.select(
            kstage_b_layout_staged,
            mode=[0, 1, 2],
        )
        kstage_tma_load_op = cpasync.CopyBulkTensorTileG2SOp(
            tcgen05.CtaGroup.TWO
        )
        tma_atom_kstage, tma_tensor_kstage = cute.nvgpu.make_tiled_tma_atom_B(
            kstage_tma_load_op,
            mKStage,
            kstage_b_layout,
            kstage_tiler,
        ...
        kstage_stage_bytes = cute.size_in_bytes(
            self.element_dtype,
            kstage_b_layout,
        )
        assert kstage_stage_bytes == 32768
...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    Per-CTA roles: warps 0-3 gather the dQ-A images (E5b: warp 0 also
    drives the staged score-K TMA pipe -- the K tile itself arrives as
    one TMA box from the pre-staged mKStage tensor); warps 4-7 run the
    softmax backward and publish P/dS; warps 8-15 drain the fused
    dV+dK partial sums to the f32 workspace; warp 16 issues every GEMM
    and manages pipeline credits; warp 17 feeds the round ring from
    the stationary panels; warp 18 relays P/dS across the cluster;
    warp 19 relays ring TMA completions.
    ...
    def _specialize_shared_storage(
        ...
        class SharedStorageV2:
            ...
            # E4b lending: [0] = borrow TMA landing (expect_tx, one arm
            # per tile); [1] = borrow_free (one tcgen05 commit arrival
            # per tile, multicast to both CTAs).
            borrow_mbars: cute.struct.MemRange[cutlass.Int64, 2]
    ...
    @cute.jit
    def _load_score_kv_tma(
        self,
        tma_atom_kstage: cute.CopyAtom,
        t_kstage_gmem: cute.Tensor,
        t_kstage_smem: cute.Tensor,
        kscore_pipeline,
        kscore_producer_state: pipeline.PipelineState,
        kscore_full_mbar: cute.Pointer,
        tile_index: Int32,
        rank: Int32,
    ):
        """E5b-0 whole-tile score-K fill: one CG2 TMA box per CTA.

        producer_acquire waits the stage's empty barrier and (leader
        CTA only) arms the transaction barrier with both CTAs' bytes;
        the two collective CG2 TMA boxes then complete that barrier
        directly (producer_commit is a no-op on a TmaUmma pipe).  The
        stage barrier counts one arrive per CTA -- the loan
        generations sharing this pipe need the both-CTA fold -- and
        producer_acquire's internal arrive_and_expect_tx covers only
        the leader, so the follower adds its counting-only arrive
        here.  Executed by gather warp 0 of each CTA.
        """

        kscore_pipeline.producer_acquire(kscore_producer_state)
        cute.copy(
            tma_atom_kstage,
            t_kstage_gmem[None, tile_index, 0],
            t_kstage_smem[None, 0],
            tma_bar_ptr=kscore_pipeline.producer_get_barrier(
    ...
    def _load_score_kv_tma_gated(
        ...
        """Steady-state K TMA fill with the E4b lending gate.

        The whole-tile box lands all 32 KiB at once, so unlike the old
        split crew gather it may not launch while the borrowed dO_r1
        h0 tenant still lives in score_kv bytes 16-32K: after the pipe
        lease (producer_acquire), wait borrow_free(t-1) -- the
        leader's tcgen05 commit fired after dV_r1 pass a -- before
        issuing the copy.  Prologue K(0)/K(1) use the ungated variant
    ...
    def kernel(
        ...
        tma_atom_kstage: cute.CopyAtom,
        tma_tensor_kstage: cute.Tensor,
        kstage_b_layout_staged: cute.ComposedLayout,
        kstage_stage_bytes: cutlass.Constexpr[int],
    ):
        ...
        if warp_idx == Int32(self.LOAD_WARP):
            ...
            cpasync.prefetch_descriptor(tma_atom_qt)
            cpasync.prefetch_descriptor(tma_atom_dot)
            cpasync.prefetch_descriptor(tma_atom_kstage)
        ...
        # E5b: raw kscore pipe barriers (full = +0, empty = +1) for the
        # hand-rolled loan-generation producer protocol and the tail.
        kscore_mbars = storage.kscore_mbars.data_ptr()
        borrow_mbars = storage.borrow_mbars.data_ptr()
        ...
        # E5b: whole-tile TMA view of the same score_kv bytes (the
        # one-stage [N32, D512] layout is byte-identical to the
        # four-chunk score_b view; host-side asserts pin the identity).
        k_n_tma = storage.score_kv.get_tensor(
            kstage_b_layout_staged.outer,
            swizzle=kstage_b_layout_staged.inner,
        )
        loan_quad = (
            cute.make_tensor(
                cute.recast_ptr(
        ...
        # E5b: staged score-K TMA partition.  The B-side CG2 split
        # hands each CTA its own [N32, D512] half of score tile t; the
        # rest_n mode of t_kstage_gmem indexes the tile.
        g_kstage = cute.local_tile(
            tma_tensor_kstage,
            cute.select(
                (self.H_TILE_CLUSTER, self.N_TILE, self.D_HEAD),
                mode=[1, 2],
            ),
            (None, None, (token_idx, batch_idx)),
        )
        rank_g_kstage = rank_score_mma.partition_B(g_kstage)
        ...
        t_kstage_smem, t_kstage_gmem = cpasync.tma_partition(
            tma_atom_kstage,
            block_coord_vmnk[1],
            b_cta_layout,
            cute.group_modes(k_n_tma, 0, 3),
            cute.group_modes(rank_g_kstage, 0, 3),
        )
        ...
        # E5b: the kscore stage barrier counts one arrive per CTA.
        # K generations: the leader's producer_acquire arms the
        # transaction (its internal arrive) and the follower's gather
        # warp 0 adds a counting-only arrive; loan generations: each
        # CTA's gather warp 0 arrives once after its local loan bytes
        # landed (both-CTA fold, e4ca's 2x128-thread commit collapsed).
        kscore_commit_group = pipeline.CooperativeGroup(
            pipeline.Agent.Thread,
            atom_thr_size,
        )
        ...
        # E5b: TMA producer / UMMA consumer.  tx_count is BOTH CTAs'
        # boxes: the CG2 TMA routes each CTA's complete_tx onto the
        # leader CTA's stage barrier, and producer_acquire arms the
        # expectation on the leader only (PipelineTmaUmma contract).
        pipe_kscore = pipeline.PipelineTmaUmma.create(
            num_stages=1,
            producer_group=kscore_commit_group,
            consumer_group=leader_group,
            tx_count=kstage_stage_bytes * atom_thr_size,
        ...
        if tidx == Int32(0):
            ...
            # E4b: [0] = borrow TMA landing (expect_tx, one arm per
            # tile); [1] = borrow_free (one tcgen05 commit arrival).
            cute.arch.mbarrier_init(borrow_mbars, 1)
            cute.arch.mbarrier_init(borrow_mbars + 1, 1)
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            if tile_count > Int32(0):
                # Prologue: K(0), one whole-tile TMA box (E5b).  Only
                # warp 0 drives the kscore pipe; warps 1-3 are freed
                # from the K supply entirely.
                if warp_idx == Int32(0):
                    self._load_score_kv_tma(
                        tma_atom_kstage,
                        t_kstage_gmem,
                        t_kstage_smem,
                        pipe_kscore,
                ...
                if tile_count == Int32(1):
                    ...
                    # Loan generation (hand-rolled TmaUmma producer):
                    # warp 0 waits the stage's empty barrier in place
                    # of producer_acquire (which would arm a
                    # transaction nobody delivers to this barrier --
                    # the loan bytes complete on loan_tma_mbars), then
                    # arrives the leader's stage barrier once per CTA
                    # after its local loan halves landed.
                    if warp_idx == Int32(0):
                        cute.arch.mbarrier_wait(
                            kscore_mbars + 1,
                            gather_state.phase,
                    ...
                    # first recycled score stage still carries K(1).  The
                    # pipe lease alone gates the fill: no borrowed tenant
                    # exists yet, so the borrow gate is skipped.
                    next_iter = Int32(1)
                    if warp_idx == Int32(0):
                        self._load_score_kv_tma(
                            tma_atom_kstage,
                    ...
                    for score_iter in cutlass.range(
                        ...
                        # Loan generation (hand-rolled, see the tc==1
                        ...
                        if score_iter == tile_count - Int32(1):
                            ...
                            # arrival the old loan (empty wait), the
                            ...
                            # counter (K fills consumed 0..tc-3; this
                            # waits tc-2).
                            if warp_idx == Int32(0):
                                cute.arch.mbarrier_wait(
                                    kscore_mbars + 1,
                            ...
                            # Steady K TMA: the whole-tile box lands
                            # all 32K at once, so the copy is gated on
                            # borrow_free(t-1) after the pipe lease
                            # (the old split crew gather only gated
                            # its second half).
                            next_iter = score_iter + Int32(1)
                            if warp_idx == Int32(0):
                                self._load_score_kv_tma_gated(
                                    tma_atom_kstage,
                                    t_kstage_gmem,
                                    t_kstage_smem,
                                    pipe_kscore,
                ...
                # Tail: observe the final borrowed dO generation's true
                # UMMA source-read completion (the last consumer
                # release).  PipelineTmaUmma.producer_tail would call
                # producer_acquire and arm a dangling transaction, so
                # wait the empty barrier directly -- the exact wait the
                # e4ca producer_tail performed.  Publish the fact to
                # the math epilogue before it reuses score_kv as dQ
                # staging.
                if warp_idx == Int32(0):
                    cute.arch.mbarrier_wait(
                        kscore_mbars + 1,
            ...
            if is_leader_cta:
                ...
                relay_phase = Int32(0)
                # E4b: multicast mask for the borrow_free retirement
                # commit (self | peer), same recipe as the kscore
                # pipe's UMMA release.
                borrow_peer_mask = (
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    # E5b: base form -- one whole-tile lease gates both
                    # S and dP (the staged TMA box lands all four
                    # chunks at once, so the finer-grained fill gates
                    # are meaningless).
    ...
    def _issue_prev_grads_tail_v2(
        ...
        dkv_done_pipeline,
        dkv_producer_state: pipeline.PipelineState,
        borrow_mbars: cute.Pointer,
        borrow_peer_mask: Int32,
        issue_seq: Int32,
    ):
        ...
        # Retire the borrowed parcel: the steady K TMA fill of K(t+1)
        # waits this commit before overwriting score_kv 16-32K.
        with cute.arch.elect_one():
            tcgen05.commit(
                borrow_mbars + 1,
                borrow_peer_mask,

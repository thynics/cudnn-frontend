"""DeepSeek Sparse Attention backward, two-CTA SERIAL chain, kq3 build.

final_ser_kq3 = final_ser_kq2 + the relay-0 chain surgery ("1+2+3 in
one shot"), targeting the 1.18-1.23us relay-0 gate:

  1. Rep(8) T2R (baseline's atom; its whole T2R+softmax span is
     0.256us vs Rep(4)'s 0.352+0.352).  Rep(8) forfeits the
     warp-uniform n-half that the owner-split publish required, so:
  2. Image-only publish: P gains a whole-image buffer (p_image,
     funded exactly by deleting the vestigial ds_xchg and retiring
     p_xchg: net 0 B); math stores each fragment once, branch-free;
     grouped-stats banding retires to the assumption-free fallback.
  3. The relay lanes assemble the block operands: own half via a
     LOCAL bulk copy (image -> blocks) under a local arm, peer half
     via the DSM send under the remote arm; landing mbars count 2 and
     expect 2x4096 tx, flipping when BOTH halves are complete.  The
     downstream contract (relay0/relay1 on the leader) is unchanged.
  (count-4 closes come from kq2; the W18-wake fold was evaluated and
  dropped -- the wake now fronts two bulk issues and folding it into
  a math lane would delay the dS phase by the inter-warp close skew.)
...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    (dO0..dO3, Q0..Q3, KdQ pair) streaming through the 10-gen round
    ring.  P is published early (own p_ready arrival + immediate relay
    ...
    # so the ring carries 8 pure-TMA/bulk gens/tile in consumption
    # order: dO0,dO1, dO2,dO3, Q0,Q1, Q2,Q3.  8 % 2 == 0 keeps the
    # static gen->slot map tile-invariant (phase iron law).
    ROUND_GENS_PER_TILE = 8
    ROUND_STAGES = 2

    MMA_DONE_STAGES = 2

    # kq3 rev2: Rep(4) retained (see _make_score_tmem_load), so the
    # grouped-stats banding stays valid and enabled.
    ...
    def _make_score_tmem_load(self, score_cta_shape, score_epi_tile):
        ...
        # kq3 rev2: Rep(8) FAILED to compile -- the V2 score
        # accumulator's packed TMEM fragment is (32,2,16)-shaped and
        # only the Rep(4) atom's (64,16) tile divides it (baseline's
        # Rep(8) rides a different CG1 M64 accumulator geometry).
        # Moving to Rep(8) requires an unpacked 64-column S/dP layout,
        # which the serial machine could fund by dropping the now-
        # useless S/dP ping-pong (64+64+256+128 = 512 exactly) -- that
        # is the follow-up knife (kq4), pending the T2R atom-space
        # investigation.  kq3 keeps Rep(4); its remaining content is
        # the image-only publish + relay double-copy assembly.
    ...
    def kernel(
        ...
        # kq3: P publishes as a whole image (like dS); the image store
        # view is byte-identical to the block layouts (same asserts as
        # ds_image above), so the halves move as raw layout-preserving
        # bulk copies -- own half locally into p_blocks, peer half via
        # DSM.  The xchg staging buffers are deleted.
        p_image_store = storage.p_image.get_tensor(
            score_store_domain,
            swizzle=score_store_layout.inner,
        )
        ...
        if tidx == Int32(0):
            ...
            # kq3: each landing mbar is armed TWICE per tile -- by the
            # peer's relay (remote DSM half, expect_tx 4096) and by the
            # local relay (own-half bulk copy into the blocks, another
            # 4096) -- so it flips when both halves of the operand
            # buffer are complete.
            cute.arch.mbarrier_init(landing_mbars, 2)
            cute.arch.mbarrier_init(landing_mbars + 1, 2)
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            # kq3: image-only publish -- every warp stores its whole
            # fragment into the P/dS images; no owner split, no xchg
            # staging (the Rep(8) ownership is not n-half-uniform, and
            # the halves are moved by layout-preserving bulk copies on
            # the relay lanes instead).
            t_rs_p_image = thread_copy_r2s.partition_D(
                p_image_store
            )
            assert cute.size(t_rs_p_image, mode=[4]) == 1
            t_rs_p_image_tile = t_rs_p_image[
                None, None, None, None, 0
            ]
            r_score = cute.make_rmem_tensor(
                score_coordinates.shape,
            ...
            for loop_iter in cutlass.range(tile_count):
                ...
                # kq3: publish P as a whole image (no owner branch);
                # the relay lanes move the halves into p_blocks.
                r_p_store = thread_copy_r2s.retile(r_p)
                assert t_rs_p_image_tile.shape == r_p_store.shape
                cute.copy(
                    tiled_copy_r2s,
                    r_p_store,
                    t_rs_p_image_tile,
                ...
                # kq3: whole-image dS store only (dQ B operand + DSM/
                # local-bulk source); the ds_blocks halves are filled
                # by the relay lane's bulk copies.
            ...
            if tile_count > Int32(0):
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    # g0..g7: dO_r0, dO_r1, Q_r0, Q_r1 panel fills in
                    # the serial chain's consumption order, pipelined
                    # depth 2 (no score_kv loan: dO_r0 is two ordinary
                    # ring generations).
                    ...
                    # the gather warps.  The ring is 8 pure-TMA/bulk
                    # gens per tile, all committed by the commit warp.
            ...
            if relay_lane == Int32(0):
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    # kq3: own-half local bulk (p_image -> p_blocks)
                    # under a LOCAL arm, then peer-half DSM under the
                    # REMOTE arm; the landing mbar (init count 2)
                    # flips when both halves of p_blocks are complete.
                    cute.arch.mbarrier_arrive_and_expect_tx(
                        landing_mbars,
                    ...
                    _cpasync_bulk_s2cluster(
                        p_image_raw
                        + rank * Int32(self.PDS_BLOCK_ELEMENTS),
                        p_blocks_raw
                        + rank * Int32(self.PDS_BLOCK_ELEMENTS),
                        landing_mbars,
                        self.PDS_BLOCK_BYTES,
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    # kq3: own-half local bulk (ds_image -> ds_blocks)
                    # replaces the deleted math-side ds_local store.
                    cute.arch.mbarrier_arrive_and_expect_tx(
                        landing_mbars + 1,
                        self.PDS_BLOCK_BYTES,
                    )
    ...
    def _issue_grads_kq_v1(
        ...
        dO/Q quadrants stream through the 8-gen round ring; the K_dQ

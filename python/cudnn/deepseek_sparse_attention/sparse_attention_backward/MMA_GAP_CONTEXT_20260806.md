...
5. （vk_1 trace 复盘，"判决 1–5"）：pacer = W17 供给链自身饱和（6.65≈周期）；
   ...
   **【R0 判定更新，双数据集】**"W17 饱和"降级：包络确实填满周期（vk_2 上
   6.919/7.089=97.6%），但其内容 73% 是等待（vk_2：MAT_ACQ 3.708 + MAT_WAIT
   1.368 / 6.92；vk_1：2.386+0.990 / 6.65）——绑定结构是信用环，不是 W17 的
   工作量。"kdq 压关键路径"仅在 vk_1 基座成立（贴合 +0.03）；vk_2 上 kdq 早于
   需求 +0.554 就绪，**绑定门随基座漂移，成对判读纪律因此升级为必须**。

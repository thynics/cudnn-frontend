...
(v_s1 trace-instrumented build).

Identical logic to dsa_bwd_sm100_2cta_v_s1 (dO slice-streaming + loan
retirement, spec = V_S1_SPEC.md); adds IKET software annotations (role
marks and issue/wait spans) for pipeline trace capture.  Carries the
final_trace standard span set on unchanged machinery, the v_w3_2_trace
refined spans (RK_ACQ/MAT_ACQ) on the ring/kscore paths, and new
v_s1-specific spans on the dO stream: DPW(i,c) leader chunk-slot
waits, DOS_ACQ(i,c) warp-1 credit waits, DOS_BRIDGE(i,p) warp-1 TMA
completion bridging, KDQ(i) warp-1 kdq gather slide.  The annotations
are no-ops unless the IKET runtime is loaded.
"""
...
class FlashAttentionDSABackwardSm100TwoCTA(FlashAttentionDSABackwardSm100):
    ...
    def __call__(
        ...
        # v_s1: the dO panel is no longer token-resident.  A 2-stage member
        # of the same score-A layout family stages the four D128 chunks per
        # tile (chunk c -> slot c%2), fed by the v0-proven per-chunk dO TMA
        # atom (score_a_layout box, dp CG2 tiled-mma, cluster-aware): the
        # exact construction the ancestor kernels ran on silicon.
        ...
        assert stream_a_layout_staged.inner == score_a_layout_staged.inner
        assert (
            cute.size_in_bytes(
                self.element_dtype,
                cute.select(stream_a_layout_staged, mode=[0, 1, 2]),
            )
            == score_a_stage_bytes
        )
        # The TMA atom is built from the score family's per-stage layout;
        # the staging view is the 2-stage member.  Their per-stage layouts
        # must be identical (outer and swizzle) for the box bytes to match
        # the dP fragment's read order.
        assert str(
            cute.select(stream_a_layout_staged, mode=[0, 1, 2])
        ) == str(score_a_layout)
        tma_atom_do_chunk, tma_tensor_do_chunk = (
...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    v_s1 (spec: V_S1_SPEC.md): the dO panel is a 2-stage chunk stream
    instead of a token-resident 64KB panel, and the score_kv dO_r0
    loan is retired.

    Per-CTA roles: warps 0-3 gather the sparse K tile and the dQ-A
    images, with warp 1 additionally producing the four dO chunk TMAs
    per tile into the 2-slot stream staging; warps 4-7 run the softmax
    backward and publish P/dS; warps 8-15 drain the fused dV+dK
    partial sums to the f32 workspace; warp 16 issues every GEMM and
    manages pipeline credits; warp 17 feeds the round ring (Q halves
    from the resident panel or GMEM, all four dO quadrants from GMEM);
    warp 18 relays P/dS across the cluster; warp 19 relays ring TMA
    completions.

    Steady-state schedule per KV tile (rotated): S(t) and dP(t) issue
    first (dP consumes chunk c of the dO stream from slot c%2), then
    tile t-1's gradients (two dQ rounds, eight dV/dK passes, all A
    operands from the ten-generation round ring).  The kdq rendezvous
    hands the gather-filled dQ-A images to the ring through a
    count-128 hardware arrival; the score K tile is released after dP
    and K(t+1) loads immediately (kscore is single-generation).  dQ
    stays TMEM-resident across all tiles and stores through a
    ...
    # v_s1 dO chunk stream: four 16KB chunks per tile through two slots.
    DOSTREAM_STAGES = 2
    ...
    def _specialize_shared_storage(
        ...
        class SharedStorageV2:
            ...
            dostream_mbars: cute.struct.MemRange[cutlass.Int64, 4]
            ...
            # Raw single-phase-per-tile barriers.  The stationary pair now
            # covers Q only; dO readiness is per-chunk via the stream.
            stationary_tma_mbars: cute.struct.MemRange[cutlass.Int64, 1]
            ...
            dostream_tma_mbars: cute.struct.MemRange[cutlass.Int64, 2]
    ...
    def _issue_dp_chunk_v_s1(
        ...
        """Issue one dP D128 chunk from stream slot ``chunk % 2``.

        The accumulate-flag sequence is bit-identical to the resident
        _issue_score_chunks_v7 traversal: only chunk 0's first k-block
        initializes the accumulator.
        """

        mma = tiled_mma.with_()
        mma.set(tcgen05.Field.ACCUMULATE, chunk != 0)
        k_blocks_per_chunk = cute.size(a_fragment, mode=[2])
    ...
    def _issue_dp_streamed_v_s1(
        ...
        dostream_pipeline,
        dostream_state: pipeline.PipelineState,
        issue_seq: Int32,
    ):
        """Issue dP over the four streamed dO chunks.

        Chunk c waits its stream slot and releases it right after its
        k-blocks issue (umma arrive: the slot frees when the source reads
        physically complete).  The dostream state advances outside the
        accumulator ping-pong branch (v7 rule: pipeline states never
        cross the dynamic stage branch).  DPW(i,c) spans each chunk-slot
        wait; dP_ISSUE(i) spans the whole streamed issue for row identity
        with the resident-panel builds.
        """
        ...
        for chunk in cutlass.range_constexpr(self.K_CHUNKS):
            dpw_token = _iket.range_start(
                "DPW(i,c)",
                issue_seq * Int32(4) + Int32(chunk),
            )
            dostream_pipeline.consumer_wait(dostream_state)
            _iket.range_end(
                dpw_token,
                issue_seq * Int32(4) + Int32(chunk),
            )
            if producer_state.index == Int32(0):
                self._issue_dp_chunk_v_s1(
                    tiled_mma,
                    accumulator_0,
                    a_fragment,
                    b_fragment,
            ...
            dostream_pipeline.consumer_release(dostream_state)
            dostream_state.advance()
        _iket.range_end(
            mma_issue_token,
            issue_seq,
        )
        ...
        done_pipeline.producer_commit(producer_state)
        producer_state.advance()
        return producer_state, dostream_state

    @cute.jit
    def _stream_do_chunk_pair_v_s1(
        self,
        first_chunk: cutlass.Constexpr[int],
        tma_atom_do_chunk: cute.CopyAtom,
        t_dochunk_gmem: cute.Tensor,
        t_dochunk_smem: cute.Tensor,
        dostream_tma_mbars: cute.Pointer,
        dostream_pipeline,
        dostream_acq: pipeline.PipelineState,
        dostream_com: pipeline.PipelineState,
        dostream_phase: Int32,
        score_a_stage_bytes: cutlass.Constexpr[int],
        issue_seq: Int32,
    ):
        """Warp 1: stream dO chunks (first_chunk, first_chunk+1) of one tile.

        Acquire arms the raw per-CTA TMA barrier and launches the chunk
        box; completion is bridged into the cluster-visible dostream
        pipeline by this warp (the same local-mbar -> pipeline-commit
        pattern warp 19 runs for the round ring).  The rank's M-half is
        ...
        for pair_index in cutlass.range_constexpr(2):
            chunk = first_chunk + pair_index
            slot = chunk % 2
            dos_acq_token = _iket.range_start(
                "DOS_ACQ(i,c)",
                issue_seq * Int32(4) + Int32(chunk),
            )
            dostream_pipeline.producer_acquire(dostream_acq)
            _iket.range_end(
                dos_acq_token,
                issue_seq * Int32(4) + Int32(chunk),
            )
            dostream_acq.advance()
            with cute.arch.elect_one():
                cute.arch.mbarrier_arrive_and_expect_tx(
    ...
    def kernel(
        ...
        tma_atom_do_chunk: cute.CopyAtom,
        tma_tensor_do_chunk: cute.Tensor,
        stream_a_layout_staged: cute.ComposedLayout,
    ):
        ...
        _ = trace_token_idx
        _ = trace_batch_idx
        # v_s1: the resident-panel dO TMA pair is superseded by the chunk
        # stream; Q keeps the stationary path.
        _ = tma_atom_do
        _ = tma_tensor_do
        ...
        if warp_idx == Int32(self.LOAD_WARP):
            cpasync.prefetch_descriptor(tma_atom_q)
            cpasync.prefetch_descriptor(tma_atom_do_chunk)
            cpasync.prefetch_descriptor(tma_atom_qt)
            cpasync.prefetch_descriptor(tma_atom_dot)
        ...
        dostream_tma_mbars = storage.dostream_tma_mbars.data_ptr()
        ...
        # v_s1: one 2-stage view serves both the dP A fragment (chunk c
        # reads slot c%2) and the chunk TMA destination partition.
        stream_do = storage.stream_do.get_tensor(
            stream_a_layout_staged.outer,
            swizzle=stream_a_layout_staged.inner,
        )
        ...
        # v_s1: per-chunk dO stream partition (the v0-proven pattern:
        # score_a box, dp CG2 mma, cluster-aware coords).  RestM is the
        # unique H128 pack; RestK carries the four D chunks.
        g_do = cute.local_tile(
            tma_tensor_do_chunk,
            cute.select(
                (
        ...
        rank_dp_mma = dp_tiled_mma.get_slice(rank)
        rank_g_do = rank_dp_mma.partition_A(g_do)
        t_dochunk_smem, t_dochunk_gmem = cpasync.tma_partition(
            tma_atom_do_chunk,
            block_coord_vmnk[2],
            a_cta_layout,
            cute.group_modes(stream_do, 0, 3),
            cute.group_modes(rank_g_do, 0, 3),
        )
        t_dochunk_gmem = t_dochunk_gmem[None, 0, None]
        ...
        # v_s1 dO chunk stream: gather warp 1 produces (one elected thread
        # per CTA, TMA completion bridged through dostream_tma_mbars); the
        # leader's dP chunks consume and release with umma arrives.
        pipe_dostream = pipeline.PipelineAsyncUmma.create(
            num_stages=self.DOSTREAM_STAGES,
            producer_group=load_elect_group,
            consumer_group=leader_group,
            barrier_storage=storage.dostream_mbars.data_ptr(),
            cta_layout_vmnk=cluster_layout_vmnk,
            defer_sync=True,
        )
        ...
        if tidx == Int32(0):
            ...
            cute.arch.mbarrier_init(dostream_tma_mbars, 1)
            cute.arch.mbarrier_init(dostream_tma_mbars + 1, 1)
            cute.arch.mbarrier_init(
                kdq_ready_mbar,
                self.GATHER_THREADS,
            )
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            # v_s1: the loan is retired, so kscore is one K generation per
            # tile and the loop is uniform (no tail recommit, no single-tile
            # special case).  Warp 1 additionally produces the four dO chunk
            # TMAs per tile; its branch is self-contained so no pipeline
            # state crosses the dynamic warp branch.  Warp 1's blocking
            # points (chunk-pair credits) all release strictly before the
            # collective duties that follow them (K commit, kdq rendezvous),
            # so the split cannot deadlock the gather group.
            ...
            if tile_count > Int32(0):
                if warp_idx == Int32(1):
                    ...
                    dostream_acq = pipeline.make_pipeline_state(
                        pipeline.PipelineUserType.Producer,
                        self.DOSTREAM_STAGES,
                    )
                    dostream_com = pipeline.make_pipeline_state(
                        pipeline.PipelineUserType.Producer,
                        self.DOSTREAM_STAGES,
                    )
                    dostream_phase = Int32(0)
                    # Prologue: K(0), then the first tile's chunk stream.
                    ...
                    (
                        ...
                        dostream_com,
                        dostream_phase,
                    ) = self._stream_do_chunk_pair_v_s1(
                        0,
                        tma_atom_do_chunk,
                        t_dochunk_gmem,
                        t_dochunk_smem,
                    ...
                    pipe_dostream.producer_tail(dostream_acq)
                else:
                    gather_state = pipeline.make_pipeline_state(
                        pipeline.PipelineUserType.Producer,
                        1,
                    ...
                    pipe_kscore.producer_tail(gather_state)
                # producer_tail observed K(last)'s release by dP(last).
                # Publish that fact to the math epilogue before it reuses
                # score_kv as dQ staging.
            ...
            if is_leader_cta:
                ...
                dostream_cons = pipeline.make_pipeline_state(
                    pipeline.PipelineUserType.Consumer,
                    self.DOSTREAM_STAGES,
                )
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    # dP(t) then early K recycle.  Each dP chunk gates on
                    # its stream slot (the first tile's dP waits only 16KB,
                    # not the former 64KB panel).
                    dp_prod, dostream_cons = self._issue_dp_streamed_v_s1(
                        dp_tiled_mma,
                        t_dp,
                        t_dp_pp,
                        score_do_fragment,
            ...
            if tile_count > Int32(0):
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    # pipelined depth 2.  v_s1: the loan is retired, so the
                    # round-0 dO pair rides the ring like everything else,
                    # and all four dO quadrants are GMEM TMAs (the resident
                    # panel that fed the own-half bulks is gone).
                    ...
                    for flat_gen in cutlass.range_constexpr(8):
                        ...
                        # v_s1: dO generations are always GMEM TMAs (the
                        # resident panel that fed the own-half bulks is
                        # gone).  For Q, the h==rank half is a contiguous
                        # byte-identical 16 KiB slice of this CTA's own
                        # stationary tensor (K-major [H64,D512] M-atom pair
                        # 4r+2c): one local bulk copy replaces the
                        # fragmented 64-row TMA box; the h==peer half keeps
                        # the GMEM TMA (L2-hot).
    ...
    def _issue_prev_grads_head_v2(
        ...
        The tail keeps v12's early dQ completion commit.  v_s1: the
        round-0 dV operands are ordinary round-ring generations (the
        score_kv loan is retired), so this half consumes four ring
        generations and kscore is untouched here.
        """

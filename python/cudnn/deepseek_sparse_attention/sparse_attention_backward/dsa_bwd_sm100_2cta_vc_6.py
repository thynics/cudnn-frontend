"""vc_6: vc_2 with independent W17/A and W19/B operand streams.

The active V2 kernel keeps vc_2's score-K/dO temporal alias, score, math,
reducer, and persistent-TMEM design.  Its former depth-2 A/B round ring is
split into two fixed-slot depth-1 pipelines without adding shared memory:
W17 owns every A generation and W19 owns every B generation.  Each producer
can therefore publish and prefill its next operand as soon as its own slot is
released, without a blocking acquire on the other half.
...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    """vc_6: independent fixed-slot A/B supply on the vc_2/v12 base.

    W17 and W19 own separate depth-1 round pipelines while the leader keeps
    the original A/B mathematical order.  The remainder of this docstring
    records the inherited v12 lineage.
    ...
    * kdq offload (inherited pre-vc_6 schedule): the four gather warps
      (idle ~80%) write both K_dQ
    ...
    * vc_6 keeps vc_2's score-K loan for round-0 dO and splits the remaining
      K_dQ/Q/round-1-dO traffic into independent W17/A and W19/B streams.
    ...
    * The gradient GEMMs of tile p are issued inside tile t=p-1's S/dP
    ...
    # vc_6: two independent fixed-slot streams are each phase-closed after
    # four generations per tile.  A/W17 carries g0/g2/g4/g6 and B/W19
    # carries g1/g3/g5/g7.  Their combined consumer order is unchanged:
    # kdq, Q_r0, dO_r1, Q_r1.
    ...
    def __init__(
        ...
        # vc_6 kdq offload handshake: the four gather warps plus W17/A and
        # W19/B rendezvous twice per tile.  The first rendezvous proves both
        # fixed-slot credits are held; the second proves both K_dQ images are
        # written, cp.async-drained, and fenced.
        self.kdq_barrier = pipeline.NamedBarrier(
            barrier_id=7,
            num_threads=(self.GATHER_WARPS + 2) * 32,
        )
    ...
    def _specialize_shared_storage(
        ...
        class SharedStorageV2:
            ...
            # vc_6: [A full, A empty, B full, B empty].
    ...
    def _gather_kdq_v8(
        ...
        Barrier A: W17/A and W19/B each hold their fixed-slot credit, so
        both round buffers are safe to overwrite.  The gather warps then
        write both K_dQ images, drain their own cp.async groups, fence,
        and barrier B hands each generation back to its owning producer
        for an independent commit.
        """
    ...
    @cute.jit
    def _fill_round_half_vc6(
        self,
        tensor_kind: cutlass.Constexpr[int],
        grad_round: cutlass.Constexpr[int],
        h_half: cutlass.Constexpr[int],
        stationary_q_raw: cute.Pointer,
        stationary_do_raw: cute.Pointer,
        round_buf_raw: cute.Pointer,
        round_tma_mbar: cute.Pointer,
        grad_a_stage_bytes: cutlass.Constexpr[int],
        tma_atom_qt: cute.CopyAtom,
        tma_atom_dot: cute.CopyAtom,
        t_qt_gmem: cute.Tensor,
        ...
        """Fill one vc_6 fixed A/B slot from its stationary panel."""
        ...
        if cutlass.const_expr(tensor_kind == 0):
            if cutlass.const_expr(self.OWN_HALF_BULK):
                if rank == Int32(h_half):
                    ...
                    cute.copy(
                        tma_atom_dot,
                        t_dot_gmem[None, grad_round, h_half],
                        t_dot_smem[None, 0],
            ...
            if cutlass.const_expr(self.OWN_HALF_BULK):
                if rank == Int32(h_half):
                    ...
                    cute.copy(
                        tma_atom_qt,
                        t_qt_gmem[None, grad_round, h_half],
                        t_qt_smem[None, 0],
                        tma_bar_ptr=round_tma_mbar,
                    )
            else:
    ...
    def _issue_dq_rounds_v2(
        ...
        round_pipeline_a,
        round_consumer_state_a: pipeline.PipelineState,
        round_pipeline_b,
        round_consumer_state_b: pipeline.PipelineState,
        issue_seq: Int32,
    ):
        """Issue dQ0 from A and dQ1 from B, then release each fixed slot.

        vc_6 uses distinct consumer states so either producer can refill its
        own slot as soon as the matching dQ issue completes.
        """
        ...
        round_pipeline_a.consumer_wait(round_consumer_state_a)
        _iket.range_end(
            wait_dq_token,
            packed_issue,
        )
        ...
        cute.arch.fence_view_async_tmem_store()
        round_pipeline_a.consumer_release(round_consumer_state_a)
        round_consumer_state_a.advance()

        packed_issue = packed_issue + Int32(1)
        wait_dq_token = _iket.range_start(
            "WAIT_dQ(i,r)",
            packed_issue,
        )
        round_pipeline_b.consumer_wait(round_consumer_state_b)
        _iket.range_end(
            wait_dq_token,
            packed_issue,
        )
        ...
        cute.arch.fence_view_async_tmem_store()
        round_pipeline_b.consumer_release(round_consumer_state_b)
        round_consumer_state_b.advance()
        return round_consumer_state_a, round_consumer_state_b
    ...
    def kernel(
        ...
        # vc_6 reinterprets vc_2's four round mbarriers as two independent
        # fixed-slot pipelines.  Each still receives one elected commit per
        # CTA, so the CG2 producer group remains load_elect_group.
        pipe_round_a = pipeline.PipelineAsyncUmma.create(
            num_stages=self.ROUND_STAGES,
        ...
        pipe_round_b = pipeline.PipelineAsyncUmma.create(
            num_stages=self.ROUND_STAGES,
            producer_group=load_elect_group,
            consumer_group=leader_group,
            barrier_storage=storage.round_mbars.data_ptr() + 2,
            cta_layout_vmnk=cluster_layout_vmnk,
            defer_sync=True,
        )
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            if is_leader_cta:
                ...
                round_cons_a = pipeline.make_pipeline_state(
                    pipeline.PipelineUserType.Consumer,
                    self.ROUND_STAGES,
                )
                round_cons_b = pipeline.make_pipeline_state(
                    pipeline.PipelineUserType.Consumer,
                    self.ROUND_STAGES,
                )
            ...
            # --- W17/A: stationary TMA once, then the four A generations.
            # W19 independently owns every B generation below.  A can begin
            # its next fill immediately after the leader releases A, while
            # the leader and W19 are still progressing the current B.
            _iket.mark("ROLE_KV_LOAD", rank)
            round_state_a = pipeline.make_pipeline_state(
                pipeline.PipelineUserType.Producer,
                self.ROUND_STAGES,
            )
            tma_phase_a = Int32(0)
            if tile_count > Int32(0):
                ...
                for loop_iter in cutlass.range(tile_count):
                    # A0: W17 owns K_dQ round 0.  The six-warp rendezvous
                    # proves W19 also owns B0 before gather writes either
                    # destination, then proves both images are complete.
                    route_k_token = _iket.range_start(
                        "ROUTE_K(i)",
                        loop_iter,
                    )
                    pipe_round_a.producer_acquire(round_state_a)
                    self.kdq_barrier.arrive_and_wait()
                    self.kdq_barrier.arrive_and_wait()
                    cute.arch.fence_view_async_shared()
                    with cute.arch.elect_one():
                        pipe_round_a.producer_commit(round_state_a)
                    round_state_a.advance()
                    _iket.range_end(
                        route_k_token,
                        loop_iter,
                    )

                    # A1/A2/A3: Q_r0.h0, dO_r1.h0, Q_r1.h0.  MAT_QDO stays
                    # on W17 only so the existing two-spans-per-tile trace
                    # contract and 31-name limit remain unchanged.
                    mat_qdo_token_0 = _iket.range_start(
                        "MAT_QDO(m,r)",
                        loop_iter * Int32(self.D_ROUNDS),
                    )
                    for pair_index in cutlass.range_constexpr(3):
                        if cutlass.const_expr(pair_index == 0):
                            grad_round = 0
                            tensor_kind = 1  # Q_r0
                        elif cutlass.const_expr(pair_index == 1):
                            grad_round = 1
                            tensor_kind = 0  # dO_r1
                        else:
                            grad_round = 1
                            tensor_kind = 1  # Q_r1
                        if cutlass.const_expr(pair_index == 1):
                            _iket.range_end(
                                mat_qdo_token_0,
                                loop_iter * Int32(self.D_ROUNDS),
                            )
                ...
                pipe_round_a.producer_tail(round_state_a)

        elif warp_idx == Int32(self.B_LOAD_WARP):
            # --- W19/B: independent fixed-slot producer.  It explicitly
            # acquires the stationary raw barriers before using local bulk
            # sources; a named-barrier handoff from W17 would not transfer
            # async-proxy completion semantics.
            round_state_b = pipeline.make_pipeline_state(
                pipeline.PipelineUserType.Producer,
                self.ROUND_STAGES,
            )
            tma_phase_b = Int32(0)
            if tile_count > Int32(0):
                ...
                for loop_iter in cutlass.range(tile_count):
                    # B0: K_dQ round 1, filled by the gather warps.
                    pipe_round_b.producer_acquire(round_state_b)
                    self.kdq_barrier.arrive_and_wait()
                    self.kdq_barrier.arrive_and_wait()
                    cute.arch.fence_view_async_shared()
                    with cute.arch.elect_one():
                        pipe_round_b.producer_commit(round_state_b)
                    round_state_b.advance()

                    # B1/B2/B3: Q_r0.h1, dO_r1.h1, Q_r1.h1.
                    for pair_index in cutlass.range_constexpr(3):
                        if cutlass.const_expr(pair_index == 0):
                            grad_round = 0
                            tensor_kind = 1  # Q_r0
                ...
                pipe_round_b.producer_tail(round_state_b)
    ...
    def _issue_prev_grads_head_v2(
        ...
        (
            round_consumer_state_a,
            round_consumer_state_b,

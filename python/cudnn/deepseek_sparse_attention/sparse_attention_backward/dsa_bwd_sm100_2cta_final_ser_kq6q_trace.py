...
_LEAN_SPANS = (
    ...
    "WAIT_RELAY(",
    "WAIT_P1_LANDING(",
    # kq6q-lean: leader fine-grained waits dropped -- 16 ring spans/tile
...
def _nanosleep_u32(
    ...
    *,
    ...
    """Warp nanosleep hint (sm70+).  kq6q: paces the reducer atomic
...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    # kq6q B plan: four independent K32 slots publish each old K64 panel
    ...
    def kernel(
        ...
        # kq6q compile-time release gates.  Fragments are built directly
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            # kq6q: SINGLE-LANE strict P-first relay.  The kq dual-lane
    ...
    def _issue_grads_kq_v1(
        ...
        # Preserve the H1 payload contract: even=P block 0, odd=dS.  The
        # split P-block-1 gate has its own static event name below so old
        # WAIT_RELAY decoders remain valid.
        ...
        # H9: block 1's rank-0-local landing overlaps the two block-0 passes.
        relay_wait_token = _iket.range_start(
            "WAIT_P1_LANDING(i)",
            issue_seq,
        )

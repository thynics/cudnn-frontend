"""SM100 two-CTA DSA backward v1 development candidate.
...
This self-contained module includes the common two-CTA host/layout base, the
verified v0 execution checkpoint, and the T1a vector-atomic implementation.
``FlashAttentionDSABackwardSm100TwoCTAV1A0`` remains the development entry;
the public interface is intentionally unchanged until promotion.
"""
...
class FlashAttentionDSABackwardSm100TwoCTAV1A0(
    ...
    def _atomic_dkv_from_context_v1(
        ...
        """Reduce one tile using only its immutable issue-time context."""
        ...
        if tidx < Int32(self.MATH_THREADS_PER_CTA):
            ...
            # T0a's ROW_MAJOR T2R gives each reducer thread one D row and
            # all N=0..63 values.  Transpose each 4-lane x 4-N register
            # block so every lane owns one N and four contiguous D values.
            # The destination D base is consequently 16-byte aligned, and
            # one red.v4 replaces four scalar result-returning atomics.
            ...
            for vector_index in cutlass.range_constexpr(
                ...
                peer_2 = cute.arch.shuffle_sync_bfly(
                    value_3,
                    offset=1,
                )
                peer_3 = cute.arch.shuffle_sync_bfly(
                    value_2,
                    offset=1,
                )
                stage_0 = value_0
                ...
                # Swap lane bit 1 with register-index bit 1.  The resulting
                # registers are D_base+[0,1,2,3] at N=value_base+lane.
                peer_0 = cute.arch.shuffle_sync_bfly(
                    stage_2,
                    offset=2,
                )
...
# production package remains wired to the verified v0 until this candidate
# passes lowering, PTXAS, deadlock, correctness, and performance gates.
FlashAttentionDSABackwardSm100TwoCTAV1 = (
    FlashAttentionDSABackwardSm100TwoCTAV1A0
)
# The isolated harness keeps the production interface text unchanged and
# imports this candidate under the canonical module filename.  Rebinding the
# legacy symbol here therefore selects A0 only inside the frozen scratch
# snapshot; the production v0 module and interface are untouched.

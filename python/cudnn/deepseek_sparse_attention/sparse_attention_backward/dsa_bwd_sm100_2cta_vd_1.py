"""vd_1: vc_2 with the round ring split into two single-producer lanes.

The active V2 kernel keeps vc_2's score, math, reducer, loan (score-K/dO
temporal alias), and persistent-TMEM design.  The change is confined to
the round-region supply topology: the single 2-stage W17 FIFO becomes TWO
independent 1-stage pipelines split by buffer parity --

    lane A (W17, buf A, tma mbar 0): kdq-r0, Q_r0-h0, dO_r1-h0, Q_r1-h0
    lane B (W19, buf B, tma mbar 1): kdq-r1, Q_r0-h1, dO_r1-h1, Q_r1-h1

Each lane has ONE producer warp and its own acquire/commit chain, so a
slow generation on one parity no longer head-of-line blocks the other,
and the two fills progress concurrently instead of through one warp's
serial issue loop.  Byte layout, buffer assignment, fragment views, and
per-pass consumption order are unchanged; the leader alternates waits
between the two pipes with a compile-time lane map.  The kdq offload
rendezvous grows from five warps to six (each lane warp holds its own
round credit).  Zero new SMEM: the four round_mbars split two per lane,
and the two rotating TMA mbars map one per lane.
...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    * The gradient GEMMs of tile p are issued inside tile t=p-1's S/dP
    ...
    def __init__(
        ...
        # v8 kdq offload handshake, vd_1 form: the four gather warps plus
        # BOTH lane warps (W17 lane A, W19 lane B) rendezvous twice per
        # tile (A: each lane warp holds its own kdq round credit; B: the
        # gather warps' K_dQ fills are written and fenced).
    ...
    def _issue_dq_rounds_v2(
        ...
        lane_a_pipeline,
        lane_a_state: pipeline.PipelineState,
        lane_b_pipeline,
        lane_b_state: pipeline.PipelineState,
        issue_seq: Int32,
    ):
        """Issue both persistent dQ rounds back-to-back (v1_deep_p lesson).

        vd_1: round r's K_dQ generation lives on lane r (A holds round 0,
        B round 1); each is released immediately after its issue so that
        lane's producer can begin its panel refills.
        """
        ...
        for round_index in cutlass.range_constexpr(self.D_ROUNDS):
            ...
            if cutlass.const_expr(round_index == 0):
                lane_a_pipeline.consumer_wait(lane_a_state)
            else:
                lane_b_pipeline.consumer_wait(lane_b_state)
            _iket.range_end(
                wait_dq_token,
                packed_issue,
            )
            ...
            cute.arch.fence_view_async_tmem_store()
            if cutlass.const_expr(round_index == 0):
                lane_a_pipeline.consumer_release(lane_a_state)
                lane_a_state.advance()
            else:
                lane_b_pipeline.consumer_release(lane_b_state)
                lane_b_state.advance()
        return lane_a_state, lane_b_state
    ...
    def kernel(
        ...
        # vd_1: the 2-stage round FIFO splits into two 1-stage lanes.
        # The four round_mbars carve exactly into 2 per lane; each lane
        # has ONE elected producer thread per CTA (W17 for A, W19 for B
        # -- the group only encodes the count, not the warp identity).
        pipe_lane_a = pipeline.PipelineAsyncUmma.create(
            num_stages=1,
        ...
        pipe_lane_b = pipeline.PipelineAsyncUmma.create(
            num_stages=1,
            producer_group=load_elect_group,
            consumer_group=leader_group,
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            if is_leader_cta:
                ...
                lane_a_cons = pipeline.make_pipeline_state(
                    pipeline.PipelineUserType.Consumer,
                    1,
                )
                lane_b_cons = pipeline.make_pipeline_state(
                    pipeline.PipelineUserType.Consumer,
                    1,
                )
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    if has_prev:
                        ...
                        lane_a_cons, lane_b_cons, dkv_prod = (
            ...
            lane_a_acq = pipeline.make_pipeline_state(
                pipeline.PipelineUserType.Producer,
                1,
            )
            lane_a_com = pipeline.make_pipeline_state(
                pipeline.PipelineUserType.Producer,
                1,
            )
            tma_phase_0 = Int32(0)
            ...
            if tile_count > Int32(0):
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    # g0: lane A's K_dQ round-0 credit.  vd_1: the six-warp
                    # rendezvous (4 gather + W17 + W19) requires BOTH lane
                    # warps to have acquired their own kdq credit at
                    # handshake A; handshake B returns "both images written,
                    # cp.async-drained, and fenced", after which each lane
                    # commits its own generation.
                    route_k_token = _iket.range_start(
                        "ROUTE_K(i)",
                        loop_iter,
                    )
                    pipe_lane_a.producer_acquire(lane_a_acq)
                    lane_a_acq.advance()
                    self.kdq_barrier.arrive_and_wait()
                    self.kdq_barrier.arrive_and_wait()
                    cute.arch.fence_view_async_shared()
                    with cute.arch.elect_one():
                        pipe_lane_a.producer_commit(lane_a_com)
                    lane_a_com.advance()
                    _iket.range_end(
                        route_k_token,
                        loop_iter,
                    ...
                    # lane A panel generations: Q_r0-h0, dO_r1-h0, Q_r1-h0.
                    # Synchronous per generation on tma mbar 0: within a
                    # lane, consecutive consumptions sit two passes apart,
                    # so the flight hides under the credit lead.
                    mat_qdo_token = _iket.range_start(
                        "MAT_QDO(m,r)",
                        loop_iter * Int32(self.D_ROUNDS)
                        + Int32(0),
                    )
                    for lane_gen in cutlass.range_constexpr(3):
                        if cutlass.const_expr(lane_gen == 0):
                            grad_round = 0
                            tensor_kind = 1  # Q_r0
                        elif cutlass.const_expr(lane_gen == 1):
                            grad_round = 1
                            tensor_kind = 0  # dO_r1
                        else:
                            grad_round = 1
                ...
                pipe_lane_a.producer_tail(lane_a_acq)

        elif warp_idx == Int32(self.IDLE_WARP):
            # --- vd_1 lane B producer: the parity twin of W17's supply
            # loop.  Owns buf B, tma mbar 1, and the kdq round-1 credit;
            # joins the six-warp kdq rendezvous.  No stationary prologue.
            lane_b_acq = pipeline.make_pipeline_state(
                pipeline.PipelineUserType.Producer,
                1,
            )
            lane_b_com = pipeline.make_pipeline_state(
                pipeline.PipelineUserType.Producer,
                1,
            )
            tma_phase_b = Int32(0)
            if tile_count > Int32(0):
                for loop_iter in cutlass.range(tile_count):
                    pipe_lane_b.producer_acquire(lane_b_acq)
                    lane_b_acq.advance()
                    self.kdq_barrier.arrive_and_wait()
                    self.kdq_barrier.arrive_and_wait()
                    cute.arch.fence_view_async_shared()
                    with cute.arch.elect_one():
                        pipe_lane_b.producer_commit(lane_b_com)
                    lane_b_com.advance()
                    # lane B panel generations: Q_r0-h1, dO_r1-h1, Q_r1-h1.
                    # Synchronous per generation on tma mbar 1: within a
                    # lane, consecutive consumptions sit two passes apart,
                    # so the flight hides under the credit lead.
                    mat_qdo_token = _iket.range_start(
                        "MAT_QDO(m,r)",
                ...
                pipe_lane_b.producer_tail(lane_b_acq)
    ...
    def _issue_prev_grads_head_v2(
        ...
        # dQ both rounds back-to-back; releases free the lane buffers for
        # the panel refills.
        lane_a_state, lane_b_state = self._issue_dq_rounds_v2(
            dq_tiled_mma,
            t_dq_0,
            t_dq_1,
            dq_kd_fragment_a,
    ...
    def _issue_prev_grads_tail_v2(
        ...
        return lane_a_state, lane_b_state, dkv_producer_state

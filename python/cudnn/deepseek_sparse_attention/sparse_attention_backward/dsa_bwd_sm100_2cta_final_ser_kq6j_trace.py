"""IKET-only derivative, kq6j, of the two-CTA SERIAL-chain KQ build.

final_ser_kq6j = final_ser_kq6f(r1) + L2 prefetch for exposed
ring generations G2/G3 (C.3), single variable.

Below: the final_ser_kq6f(r1) IKET twin docstring.
...
def _nanosleep_u32(
    ...
    *,
    ...
    """Warp nanosleep hint (sm70+).  kq6j: paces the reducer atomic
...
class FlashAttentionDSABackwardSm100TwoCTA(FlashAttentionDSABackwardSm100):
    ...
    TMEM_COLUMNS = 512
    # kq6j: inter-chunk pacing (ns) for the reducer atomic bursts.
...
class FlashAttentionDSABackwardSm100TwoCTAV2(
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    for flat_gen in cutlass.range_constexpr(8):
                        if cutlass.const_expr(flat_gen < 2):
                            ...
                            tensor_kind = 1  # Q_r1
                        h_half = flat_gen % 2
                        # kq6j C.3: G0/G1 are already in flight.  Before
                        # G2 can block on ring credit, prefetch the peer
                        # GMEM half of G2/G3 into L2; the own half remains
                        # a local bulk copy and is intentionally skipped.
                        if cutlass.const_expr(flat_gen == 2):
                            if rank == Int32(0):
                                cute.prefetch(
                                    tma_atom_dot,
    ...
    def _drain_dkv_v8(
        ...
        for i in cutlass.range_constexpr(8):
            ...
            # kq6j: pace the burst (concentration knife).

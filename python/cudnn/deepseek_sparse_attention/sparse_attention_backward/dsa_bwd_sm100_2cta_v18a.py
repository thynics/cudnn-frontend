...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    * The gradient GEMMs of tile p are issued inside tile t=p-1's S/dP
    ...
    def _specialize_shared_storage(
        ...
        # Byte-exact plan (per CTA), cap 232,448.  v18a (E3): ds_xchg
        # is retired (dead store target since v12 P1b -- ROUTE_dS
        # sources the byte-identical whole-image half), and the pds
        # pipeline splits into pds_P + pds_dS (+2 mbars) plus a second
        # count-128 ready mbar, all absorbed by the mbarrier pad:
        #   stationary Q + dO             131,072
        #   score K stage (epi alias)      32,768
        #   round region (2 x 16,384)      32,768
        #   P blocks: local+xchg+landing   12,288
        #   dS image + local+landing       16,384
        #   softmax stats                     512
    ...
    def kernel(
        ...
        # v18a (E3): ds_xchg retired (dead field; ROUTE_dS sources the
        # byte-identical whole-image half of ds_image, v12 P1b).
        softmax_stats = storage.stats.get_tensor(
            cute.make_layout(
                (self.H_TILE_CTA, 2),
                stride=(1, self.H_TILE_CTA),
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            for loop_iter in cutlass.range(tile_count):
                ...
                if owns_n:
                    ...
                    # Local dS block for the dVdK dS passes (moved here
                    # from the dS phase; same owns_n predicate and the
                    # same r_ds_store retile as v12/v17a).
                    cute.copy(
                        tiled_copy_r2s,
                        r_ds_store,
                        t_rs_ds_local_tile,
            ...
            if tidx % Int32(32) == Int32(0):
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    # (1-rank) half of the whole dS image (ds_xchg is
                    # retired in v18a).
                    if rank == Int32(0):
                        _cpasync_bulk_s2cluster(

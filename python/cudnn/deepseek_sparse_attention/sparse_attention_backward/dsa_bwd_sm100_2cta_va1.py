...
class FlashAttentionDSABackwardSm100A1(
    ...
    Chair verdict 2026-07-31 (a1_rev0_spec): PTX ISA 9.1 forbids mixing
    tcgen05 cta_groups inside one kernel, so the P1 gradient re-homing
    (dV/dK local to the producing CTA) forces the WHOLE kernel back to
    cta_group::ONE.  The cluster (2, 1, 1) survives purely as a
    co-scheduling / L2-dedup device: the two CTAs of a pair work on the
    SAME token, each owning one H64 head half (CTA rank == the baseline
    head_block_idx), and both gather the same K rows back-to-back so the
    pair-unique HBM bytes match the baseline while the second reader
    hits L2.  Honest expectation (ledger): A1 == baseline + K L2-dedup;
    the CG2 issue advantages are returned and the atomic tax comes back
    -- the target is PARITY, not a win.

    Structure (each CTA runs the full baseline-shaped pipeline):

    * Five tiled MMAs, all cta_group::ONE, tilers copied from the
      baseline bible dsa_bwd_sm100.py:30-39/:295-318:
        S   = Q  @ K^T   K-major x K-major, mma tiler (64, 64, 512)
        dP  = dO @ V^T   K-major x K-major, mma tiler (64, 64, 512)
        dV  = dO^T @ P   MN x K,  mma (128, 64, 64), cta (512, 64, 64)
        dK  = Q^T @ dS   MN x K,  mma (128, 64, 64), cta (512, 64, 64)
        dQ  = K^T @ dS^T MN x MN, mma (128, 64, 64), cta (512, 64, 64)
      dQ is emitted transposed ([D, H]); the mdQ [M, H, D] -> [D, H,
      (M, 1)] view follows baseline :272-274.
    * TMEM map A (512/512 zero-slack): S0 at column 0 with dP0 lane-
      paired at the same columns (+16 << 16 lane offset, baseline
      :2603-2605); S1/dP1 ping-pong at column 64; dKV slot A/B at
      128/192; dQ0..3 at 256/320/384/448.
    * SMEM: stationary Q/dO (2 x 65,536 B, dual-viewed as the K-major
      score A and the MN-major dV/dK A, baseline :838-842); score_kv
      full [N64, D512] panel (65,536 B, dual-viewed as the K-major
      score B and the MN-major dQ A, baseline :844-845); p_image /
      ds_image local full [H64, N64] images (2 x 8,192 B; the COL_MAJOR
      epi store image is byte-identical to the K-major B operands,
      baseline sP/sP_store + sdS/sdS_store/sdST).  The dQ epilogue
      stages through the then-dead score_kv (v12 P4 / b244255).
    * Supply machinery of the CG2 lineage is deleted: no W17 round ring
      (10 gens/tile), no W18 relay/DSM publish, no W19, no kdq gather
      offload, no OWN_HALF_BULK.  THREADS_PER_CTA 640 -> 544.
    * Handshakes (all CTA-local; both CTAs run their own MMA warp):
      kscore 1-stage (gather -> mma), s_done/dp_done 2-stage ping-pong
      (mma -> math), pds 1-stage (math -> mma; math is the producer,
      one commit per thread after its own fenced stores -- baseline
      compute_mma_P/dS producer-group form), dkv_done 2-stage with
      FOUR generations per tile (one per D128 chunk, slot = chunk % 2;
      producer_acquire(g) pairs with consumer_release(g - 2) on the
      same slot), dq_done 1-stage (mma -> math epilogue).
    * K-panel policy rev0: whole-panel wait (baseline
      load_mma_K_stage=1 semantics); no chunk-chase.  The v7 rotated
      schedule is dropped: with the K panel double-booked as the dQ A
      operand, S(t+1) may only load after tile t's dQ MMAs release the
      panel (baseline in-tile order, K released right after the dQ
      issues).
    * Reducer: 4 generations/tile, T2R -> fence -> release first,
      atomics after (baseline :2163-2176 shape); atomic targets are
      the full-width D panels (both CTAs RMW the same rows -- the
      baseline atomic tax, accepted).
    * math: packed f32x2 pair math + phased P-then-dS (E2, kept from
      v17a), score T2R atom Ld16x256b Rep(8) == baseline :1757-1760;
      SOFTMAX_GROUPED_STATS=False (the v9.3 grouped-stats proof is
      Rep4-specific; the coordinate fallback is assumption-free).
      Publish is stmatrix m8n8.x4.trans via get_smem_store_op with a
      build-time isinstance gate (host assert F).

    Register plan (setmaxnreg is warpgroup-scoped; at 544 threads the
    MMA warp 16 sits in a PARTIAL warpgroup and must not execute it):
      gather (warps 0-3, one full wg)  decrease -> 40
      math   (warps 4-7, one full wg)  increase -> 128
      reduce (warps 8-15, two full wg) increase -> 104
      mma    (warp 16)                 launch allocation (untouched)
    Balance at launch=96: dec supply 128 x 56 = 7,168 >= inc demand
    128 x 32 + 256 x 8 = 6,144.

    Host asserts A-F (chair spec) are enforced at trace time in
    __call__/kernel; assert A's full pointwise dual-view identity runs
    in gate_0b1_host_assert.py (0c-1 extension) -- in-file it takes the
    nearest compile-time form (cosize + swizzle-inner equality) plus
    the baseline :844-845 existence citation.
    """
    ...
    # Baseline-bible tilers (dsa_bwd_sm100.py:30-39).
    SCORE_MMA_TILER = (64, 64, 512)
    GRAD_MMA_TILER = (128, 64, 64)
    GRAD_CTA_TILER = (512, 64, 64)
    D_BLOCKS = 4  # 512 // 128 gradient D chunks
    DKV_GENS_PER_TILE = 4  # one dkv_done generation per chunk

    # TMEM map A (spec tmem_map_A): 512/512 zero slack.
    TMEM_S_OFFSET = 0
    TMEM_S1_OFFSET = 64
    TMEM_DP_OFFSET = 0
    TMEM_DP1_OFFSET = 64
    TMEM_DP_LANE = 16 << 16  # baseline :2603 lane pairing
    TMEM_DKV0_OFFSET = 128
    TMEM_DKV1_OFFSET = 192
    TMEM_DQ0_OFFSET = 256
    TMEM_DQ1_OFFSET = 320
    ...
    # S/dP TMEM ping-pong depth (kept from v17a) and the dkv_done
    # pipeline depth (2 stages over 4 gens/tile: acquire(g) waits
    # release(g-2), the SAME slot's previous chunk -- host assert E).
    SCORE_DONE_STAGES = 2
    MMA_DONE_STAGES = 2

    # Rep8 fragment grouping differs from the Rep4 proof; rev0 runs the
    # assumption-free per-value coordinate fallback (spec math_publish).
    # Re-derive the h-group constancy for Rep8 before flipping (rev2).
    SOFTMAX_GROUPED_STATS = False

    # Register plan (see class docstring).
    GATHER_REGS = 40
    MATH_REGS = 128
    REDUCE_REGS = 104

    # Source-native IKET provenance mark distinguishing the active A1
    # kernel from the dormant bring-up classes.
    ...
    def _make_score_tmem_load(self, score_cta_shape, score_epi_tile):
        """S/dP T2R atom: Ld16x256b Rep(8), baseline :1757-1760 exact.

        The CG1 M64 score accumulator is the baseline's own shape; the
        baseline reads it with Ld16x256bOp(Repetition(8)) over 128
        threads (32 values/thread) and publishes through the stmatrix
        m8n8.x4.trans store atom.  get_smem_store_op keys the publish
        atom off this atom's (num_dp=16, num_bits=256) ownership, which
        fires use_stmatrix_m8n8_4x's f32->bf16 clause (host assert F is
        the isinstance build gate in the math role).
        """

        return cute.make_copy_atom(
            tcgen05.copy.Ld16x256bOp(
                tcgen05.copy.Repetition(8)
            ),
            self.acc_dtype,
        )
    ...
    def __call__(
        ...
        """Compile preprocessing, the all-CG1 main kernel, postprocessing."""

        # Canonical logical views (identical to the established base).
        mQ = cute.make_tensor(
            mQ.iterator,
            cute.make_layout(
                (mQ.shape[1], mQ.shape[2], (mQ.shape[0], 1)),
        ...
        # [M, H, D] -> [D, H, (M, 1)] -- baseline :272-274 (dQ^T store).
        mdQ = cute.make_tensor(
            mdQ.iterator,
            cute.make_layout(
                (mdQ.shape[2], mdQ.shape[1], (mdQ.shape[0], 1)),
        ...
        # ------------------------------------------------------------------
        # Five CG1 tiled MMAs (spec mma_rebuild; baseline :295-318).
        # ------------------------------------------------------------------
        # S = Q @ K^T
        score_tiled_mma = sm100_utils.make_trivial_tiled_mma(
            self.element_dtype,
        ...
        # All five are single-CTA atoms: the cluster carries no MMA role.
        assert cute.size(score_tiled_mma.thr_id.shape) == 1
        assert cute.size(dp_tiled_mma.thr_id.shape) == 1
        assert cute.size(dop_tiled_mma.thr_id.shape) == 1
        assert cute.size(qds_tiled_mma.thr_id.shape) == 1
        assert cute.size(kds_tiled_mma.thr_id.shape) == 1

        # ------------------------------------------------------------------
        # SMEM layouts (spec smem_rebuild).
        # ------------------------------------------------------------------
        stationary_a_layout_staged = sm100_utils.make_smem_layout_a(
            score_tiled_mma,
            self.SCORE_MMA_TILER,
            self.element_dtype,
            1,
        )
        score_b_layout_staged = sm100_utils.make_smem_layout_b(
        ...
        # Transposed gradient-A views over the SAME stationary bytes
        # (baseline sdOT/sQT, :343/:357 + :838-842).
        dot_a_layout_staged = sm100_utils.make_smem_layout_a(
            dop_tiled_mma,
            self.GRAD_CTA_TILER,
            self.element_dtype,
            1,
        )
        qt_a_layout_staged = sm100_utils.make_smem_layout_a(
            qds_tiled_mma,
            self.GRAD_CTA_TILER,
            self.element_dtype,
            1,
        )
        # dQ-A dual view over the SAME score_kv bytes (baseline sK_2,
        # :348 + :844-845).
        k2_a_layout_staged = sm100_utils.make_smem_layout_a(
            kds_tiled_mma,
            self.GRAD_CTA_TILER,
            self.element_dtype,
            1,
        )
        # Gradient B operands over the local P/dS full images (baseline
        # sP :344 / sdS :365 / sdST :356).
        p_b_layout_staged = sm100_utils.make_smem_layout_b(
            dop_tiled_mma,
            self.GRAD_MMA_TILER,
            self.element_dtype,
            1,
        )
        ds_b_layout_staged = sm100_utils.make_smem_layout_b(
            qds_tiled_mma,
            self.GRAD_MMA_TILER,
            self.element_dtype,
            1,
        )
        dst_b_layout_staged = sm100_utils.make_smem_layout_b(
            kds_tiled_mma,
            self.GRAD_MMA_TILER,
            self.element_dtype,
            1,
        )
        # dQ epilogue staging tile (baseline :370-372/:394-400): one
        # [D128, H64] round staged in the then-dead score_kv (v12 P4).
        dq_epi_layout_staged = sm100_utils.make_smem_layout_epi(
            self.element_dtype,
            utils.LayoutEnum.from_tensor(mdQ),
            (self.GRAD_MMA_TILER[0], self.GRAD_MMA_TILER[1]),
            1,
        )

        # ------------------------------------------------------------------
        # Host assert A (nearest compile-time form): the dQ-A view and
        # the score-B panel must be the same physical bytes.  Full
        # pointwise identity idx_b(n, d) == idx_a(d, n) is verified by
        # gate_0b1_host_assert.py (0c-1 extension); the baseline ships
        # this exact pair on one buffer (sK/sK_2, dsa_bwd_sm100.py
        # :844-845), which is the existence proof.  Failure of the
        # pointwise gate downgrades A1 to the kdq streaming fallback
        # (chair spec host_asserts.A).
        # ------------------------------------------------------------------
        assert cute.cosize(score_b_layout_staged) == 32768
        assert cute.cosize(k2_a_layout_staged) == 32768
        assert (
            k2_a_layout_staged.inner == score_b_layout_staged.inner
        )
        assert cute.cosize(stationary_a_layout_staged) == 32768
        assert cute.cosize(qt_a_layout_staged) == 32768
        assert cute.cosize(dot_a_layout_staged) == 32768
        assert (
            qt_a_layout_staged.inner
            == stationary_a_layout_staged.inner
        )
        assert (
            dot_a_layout_staged.inner
            == stationary_a_layout_staged.inner
        )
        assert cute.cosize(p_b_layout_staged) == 4096
        assert cute.cosize(ds_b_layout_staged) == 4096
        assert cute.cosize(dst_b_layout_staged) == 4096
        # dQ epilogue staging must fit the dead score_kv allocation.
        assert (
        ...
        # TMA atoms.
        # ------------------------------------------------------------------
        tma_load_op = cpasync.CopyBulkTensorTileG2SOp(cg1)
        stationary_a_layout = cute.select(
            stationary_a_layout_staged,
            mode=[0, 1, 2],
        )
        ...
        stationary_stage_bytes = cute.size_in_bytes(
            self.element_dtype,
            stationary_a_layout,
        )
        assert stationary_stage_bytes == 65536
        ...
        # ------------------------------------------------------------------
        # Shared storage (spec smem_rebuild): stationary Q/dO 2 x 65,536,
        # score_kv full panel 65,536, P/dS local full images 2 x 8,192,
        # stats 512, mbars < 1,024.  Logical sum 214,528 B; cute.struct
        # may pad the tail to the 1,024 struct alignment (E3 lesson:
        # bracket, never exact-assert, and echo the actual value).
        # ------------------------------------------------------------------
        element_dtype = self.element_dtype

        @cute.struct
        class SharedStorageA1:
        ...
        # Host assert B: byte account closes with >= 17 KiB headroom.
        _a1_smem_bytes = SharedStorageA1.size_in_bytes()
        assert _a1_smem_bytes >= 214_528, (
            f"SharedStorageA1 {_a1_smem_bytes} B under the 214,528 B "
            "logical sum -- field accounting changed"
        )
        assert _a1_smem_bytes <= 215_040, (
            f"SharedStorageA1 {_a1_smem_bytes} B over 214,528 B "
            "+ 512 B tail padding -- field accounting changed"
        )
        assert _a1_smem_bytes <= self.MAX_SMEM_BYTES
        self.shared_storage = SharedStorageA1
        self.shared_storage_bytes = _a1_smem_bytes
        self.layout_report = {
            "stationary_a": str(stationary_a_layout_staged),
            "score_b": str(score_b_layout_staged),
            "qt_a": str(qt_a_layout_staged),
            "dot_a": str(dot_a_layout_staged),
            "k2_a": str(k2_a_layout_staged),
            "p_b": str(p_b_layout_staged),
            "ds_b": str(ds_b_layout_staged),
            "dst_b": str(dst_b_layout_staged),
            "dq_epi": str(dq_epi_layout_staged),
            "shared_storage_bytes": _a1_smem_bytes,
        }

        # Host assert C: TMEM map A tiles [0, 512) exactly; the only
        # column overlap is the S/dP lane pairing (16-lane groups).
        assert self.TMEM_S_OFFSET == 0
        assert self.TMEM_DP_OFFSET == self.TMEM_S_OFFSET
        assert self.TMEM_S1_OFFSET == self.TMEM_S_OFFSET + 64
        assert self.TMEM_DP1_OFFSET == self.TMEM_S1_OFFSET
        assert self.TMEM_DKV0_OFFSET == self.TMEM_S1_OFFSET + 64
        assert self.TMEM_DKV1_OFFSET == self.TMEM_DKV0_OFFSET + 64
        assert self.TMEM_DQ0_OFFSET == self.TMEM_DKV1_OFFSET + 64
        assert self.TMEM_DQ1_OFFSET == self.TMEM_DQ0_OFFSET + 64
        assert self.TMEM_DQ2_OFFSET == self.TMEM_DQ1_OFFSET + 64
        assert self.TMEM_DQ3_OFFSET == self.TMEM_DQ2_OFFSET + 64
        assert self.TMEM_DQ3_OFFSET + 64 == self.TMEM_COLUMNS
        assert self.TMEM_DP_LANE == 16 << 16

        # Host assert E (protocol constants; the phase pairing
        # producer_acquire(g) <-> consumer_release(g - 2) same-slot is
        # structural: gens alternate slots with parity 2 == stages).
        assert self.MMA_DONE_STAGES == 2
        assert self.DKV_GENS_PER_TILE == 4
        assert self.SCORE_DONE_STAGES == 2
        # The Rep8 grouped-stats hoist is underived; rev0 pins the
        # assumption-free coordinate fallback (spec math_publish).
        assert not self.SOFTMAX_GROUPED_STATS
    ...
    def convert_canonical(
        ...
        The A1 drain stores each thread's register-gathered FP32x4 quad
        at group index dp_idx//4 of its 128-column D panel (the
        production store_dKV addressing, one panel per chunk), so the
        workspace column order inside every panel is the baseline
        permutation; this is the baseline convert's dim_idx decode.
        The scramble is panel-base invariant, so the same formula
        covers the chunk-indexed panels.
        """
    ...
    def kernel(
        ...
        stationary_a_layout_staged: cute.ComposedLayout,
        score_b_layout_staged: cute.ComposedLayout,
        dot_a_layout_staged: cute.ComposedLayout,
        qt_a_layout_staged: cute.ComposedLayout,
        k2_a_layout_staged: cute.ComposedLayout,
        p_b_layout_staged: cute.ComposedLayout,
        ds_b_layout_staged: cute.ComposedLayout,
        dst_b_layout_staged: cute.ComposedLayout,
        dq_epi_layout_staged: cute.ComposedLayout,
        score_tmem_load: cute.CopyAtom,
        stationary_stage_bytes: cutlass.Constexpr[int],
        trace_buffer: Optional[cute.Tensor],
        trace_token_idx: Int32,
        trace_batch_idx: Int32,
    ):
        """A1 all-CG1 H-native backward (chair spec a1_rev0_spec)."""
        ...
        p_image_raw = storage.p_image.data_ptr()
        ...
        # Gradient-A transposed views over the SAME bytes (baseline
        # :838-845 recast pattern).
        s_qt = cute.make_tensor(
            cute.recast_ptr(
                stationary_q.iterator,
                qt_a_layout_staged.inner,
            ),
            qt_a_layout_staged.outer,
        )
        s_dot = cute.make_tensor(
            cute.recast_ptr(
                stationary_do.iterator,
                dot_a_layout_staged.inner,
            ),
            dot_a_layout_staged.outer,
        )
        s_k2 = cute.make_tensor(
            cute.recast_ptr(
                k_n.iterator,
                k2_a_layout_staged.inner,
            ),
            k2_a_layout_staged.outer,
        )
        # P/dS local full images: B-operand views ...
        s_p_b = cute.make_tensor(
            cute.recast_ptr(
                p_image_raw,
                p_b_layout_staged.inner,
                dtype=self.element_dtype,
            ),
            p_b_layout_staged.outer,
        )
        s_ds_b = cute.make_tensor(
            cute.recast_ptr(
                ds_image_raw,
                ds_b_layout_staged.inner,
                dtype=self.element_dtype,
            ),
            ds_b_layout_staged.outer,
        )
        s_ds_t = cute.make_tensor(
            cute.recast_ptr(
                ds_image_raw,
                dst_b_layout_staged.inner,
                dtype=self.element_dtype,
            ),
            dst_b_layout_staged.outer,
        )
        # ... and the COL_MAJOR epi store views (byte-identical to the
        # K-major B operands: baseline sP_store/sdS_store, production-
        # verified; asserted below at build time).
        score_store_layout = sm100_utils.make_smem_layout_epi(
            self.element_dtype,
            utils.LayoutEnum.COL_MAJOR,
            (self.H_TILE_CTA, self.N_TILE),
        ...
        assert (
            cute.cosize(score_store_layout)
            == cute.cosize(p_b_layout_staged)
        )
        assert (
            score_store_layout.inner == p_b_layout_staged.inner
        )
        assert (
            score_store_layout.inner == ds_b_layout_staged.inner
        )
        assert (
            score_store_layout.inner == dst_b_layout_staged.inner
        )
        s_p_store = cute.make_tensor(
            cute.recast_ptr(
                p_image_raw,
                score_store_layout.inner,
                dtype=self.element_dtype,
        ...
        # v12 (P4): the dQ epilogue stages each [D128, H64] round in the
        # score-K allocation, which is dead once dq_done commits (its
        # last readers are the final tile's S/dP/dQ MMAs, all tracked by
        # the same commit).
        s_dq_epi = cute.make_tensor(
            cute.recast_ptr(
                score_kv_raw,
        ...
        # Stationary Q/dO TMA partitions: each CTA loads its own H64
        # half (mode index == rank).
        g_q = cute.local_tile(
            tma_tensor_q,
            cute.select(self.SCORE_MMA_TILER, mode=[0, 2]),
            (None, None, (token_idx, batch_idx)),
        )
        g_do = cute.local_tile(
            tma_tensor_do,
        ...
        score_thr_mma = score_tiled_mma.get_slice(0)
        rank_g_q = score_thr_mma.partition_A(g_q)
        dp_thr_mma = dp_tiled_mma.get_slice(0)
        rank_g_do = dp_thr_mma.partition_A(g_do)
        t_q_smem, t_q_gmem = cpasync.tma_partition(
            tma_atom_q,
            0,
            cute.make_layout(1),
        ...
        # ------------------------------------------------------------------
        # MMA fragments (baseline :900-925 shapes, incl. the 5-mode
        # gradient-A reshape).
        # ------------------------------------------------------------------
        score_q_fragment = score_tiled_mma.make_fragment_A(
            stationary_q
        ...
        dot_fragment = self._reshape_grad_a_fragment_a1(
            dop_tiled_mma.make_fragment_A(s_dot)
        )
        qt_fragment = self._reshape_grad_a_fragment_a1(
            qds_tiled_mma.make_fragment_A(s_qt)
        )
        kd_fragment = self._reshape_grad_a_fragment_a1(
            kds_tiled_mma.make_fragment_A(s_k2)
        )
        p_fragment = dop_tiled_mma.make_fragment_B(s_p_b)
        ds_dk_fragment = qds_tiled_mma.make_fragment_B(s_ds_b)
        ds_dq_fragment = kds_tiled_mma.make_fragment_B(s_ds_t)

        # ------------------------------------------------------------------
        # Pipelines (ALL CTA-local: no cluster masks anywhere -- both
        # CTAs run the full role set independently).
        # ------------------------------------------------------------------
        one_thread_group = pipeline.CooperativeGroup(
            pipeline.Agent.Thread,
            1,
        )
        ...
        pipe_s_done = pipeline.PipelineUmmaAsync.create(
            ...
            producer_group=one_thread_group,
        ...
        pipe_kscore = pipeline.PipelineAsyncUmma.create(
            ...
            consumer_group=one_thread_group,
            barrier_storage=storage.kscore_mbars.data_ptr(),
            defer_sync=True,
        )
        # P/dS handoff: math is the producer (each thread commits after
        # its own fenced stores -- the baseline compute_mma_P/dS
        # producer-group form, dsa_bwd_sm100.py:2738-2770); the MMA warp
        # consumes and its release is a tcgen05 commit that tracks every
        # gradient MMA's completed READ of the images.
        ...
        # Per-CTA TMEM (CG1: no cross-CTA allocator coordination).
        tmem = utils.TmemAllocator(
            tmem_holding_buf_ptr,
            barrier_for_retrieve=self.tmem_alloc_barrier,
        ...
        grad_c_layout = dop_tiled_mma.make_fragment_C(
            dop_tiled_mma.partition_shape_C(
                self.GRAD_MMA_TILER[:2]
            )
        ).layout
        dq_c_layout = kds_tiled_mma.make_fragment_C(
            kds_tiled_mma.partition_shape_C(
                self.GRAD_MMA_TILER[:2]
            )
        ).layout
        # Host assert D: accumulator footprints.  TMEM addresses encode
        # the lane in bits [31:16] and the column in bits [15:0], so
        # cosize decomposes into (lane extent, column extent).  The M64
        # score accumulator spans 64 columns on the half-DP lane comb
        # (16-lane groups at 0/32/64/96 -> lane extent 112); the M128
        # gradient accumulators span 64 columns on all 128 lanes.
        assert cute.cosize(score_c_layout) % 65536 == 64
        assert cute.cosize(score_c_layout) // 65536 == 111
        assert cute.cosize(grad_c_layout) % 65536 == 64
        assert cute.cosize(grad_c_layout) // 65536 == 127
        assert cute.cosize(dq_c_layout) % 65536 == 64
        assert cute.cosize(dq_c_layout) // 65536 == 127
        ...
        # dP lane-pairs with S at the same columns (baseline :2603-2605).
        t_dp = cute.make_tensor(
            tmem_ptr + self.TMEM_DP_OFFSET + self.TMEM_DP_LANE,
            score_c_layout,
        )
        t_dp_pp = cute.make_tensor(
            tmem_ptr + self.TMEM_DP1_OFFSET + self.TMEM_DP_LANE,
            score_c_layout,
        )
        t_dkv = (
            cute.make_tensor(
        ...
        t_dq = (
            ...
            cute.make_tensor(
                tmem_ptr + self.TMEM_DQ2_OFFSET,
                dq_c_layout,
            ),
            cute.make_tensor(
                tmem_ptr + self.TMEM_DQ3_OFFSET,
                dq_c_layout,
            ),
        )
        ...
        # Register plan (see class docstring): warps 0-15 are four FULL
        # warpgroups; warp 16's warpgroup is partial at 544 threads, so
        # it keeps the launch allocation (setmaxnreg is wg-scoped).
        if warp_idx < Int32(self.GATHER_WARPS):
            cute.arch.setmaxregister_decrease(self.GATHER_REGS)
        elif warp_idx < Int32(self.REDUCE_WARP_BEGIN):
            cute.arch.setmaxregister_increase(self.MATH_REGS)
        ...
        # ==================================================================
        if warp_idx < Int32(self.GATHER_WARPS):
            # --- gather: the FULL [N64, D512] score/dQ K panel, one
            # generation per tile (baseline load_KV shape: 4 warps, 16
            # rows each, lane-0 index read + shuffle, 128-bit cp.async).
            # Both CTAs of the pair gather the SAME rows back-to-back:
            # pair-unique bytes match the baseline and the second reader
            # hits L2 (the only surviving cluster benefit).
            _iket.mark("ROLE_KV_LOAD", rank)
            local_tidx = tidx % Int32(32)
            local_warp_idx = tidx // Int32(32)
            kv_copy_atom = cute.make_copy_atom(
                cpasync.CopyG2SOp(
                    cache_mode=cpasync.LoadCacheMode.GLOBAL,
                ),
            ...
            kv_thread_copy = kv_tiled_copy.get_slice(
                local_tidx % Int32(8)
            )
            ...
            rows_per_warp = self.N_TILE // self.GATHER_WARPS
            r_topk_rows = cute.make_rmem_tensor(
                (rows_per_warp,),
                cutlass.Int32,
            )
            if tile_count > Int32(0):
                full_tiles = (topk % Int32(self.N_TILE)) == Int32(0)
                # Prologue: the LAST tile index first (the only tile
                # that can be partial), baseline load_KV order.
                tile_index = tile_count - Int32(1)
                load_k_token = _iket.range_start(
                    "LOAD_K(i)",
                    Int32(0),
                )
                for i in cutlass.range_constexpr(rows_per_warp):
                    row = Int32(i * self.GATHER_WARPS) + local_warp_idx
                    idx = tile_index * Int32(self.N_TILE) + row
                    topk_idx = Int32(-1)
                    if local_tidx == Int32(0):
                        if idx < Int32(self.max_topk):
                            topk_idx = mTopkIdxs[
                                idx,
                                (token_idx, batch_idx),
                            ]
                    r_topk_rows[i] = cute.arch.shuffle_sync(
                        topk_idx,
                        0,
                    )
                ...
                if full_tiles:
                    self._load_kv_rows(
                        ...
                        async_copy_atom=kv_copy_atom,
                        async_thr_copy=kv_thread_copy,
                    )
                else:
                    self._load_kv_rows(
                        mKV,
                ...
                # Remaining tiles are full by construction.
                for loop_iter in cutlass.range(
                    ...
                    for i in cutlass.range_constexpr(rows_per_warp):
                        row = (
                            Int32(i * self.GATHER_WARPS)
                            + local_warp_idx
                        )
                        idx = (
                            tile_index * Int32(self.N_TILE) + row
                        )
                        topk_idx = Int32(-1)
                        if local_tidx == Int32(0):
                            if idx < Int32(self.max_topk):
            ...
            # --- math: stats, per-tile softmax + local publication, dQ
            # epilogue.  Baseline compute() T2R/publish skeleton
            # (dsa_bwd_sm100.py:1707-1898) with the v17a E2 math kept:
            # packed f32x2 pair arithmetic and the phased P-then-dS
            # structure.  All publishes go to the LOCAL full images --
            # no n-half ownership, no xchg, no DSM.
            ...
            # T2R skeleton (baseline :1757-1787): atom-core slices, one
            # tmem copy per ping-pong base (identical static layouts).
            t_score_core = t_score[(None, None), 0, 0]
            t_score_pp_core = t_score_pp[(None, None), 0, 0]
            t_dp_core = t_dp[(None, None), 0, 0]
            t_dp_pp_core = t_dp_pp[(None, None), 0, 0]
            s_p_store_slice = s_p_store[None, None, 0]
            s_ds_store_slice = s_ds_store[None, None, 0]
            ...
            score_source = score_thread.partition_S(t_score_core)
            ...
            score_source_pp = score_copy_pp.get_slice(
                mtx
            ).partition_S(t_score_pp_core)
            dp_copy = tcgen05.make_tmem_copy(
                score_tmem_load,
                t_dp_core,
            )
            dp_thread = dp_copy.get_slice(mtx)
            dp_source = dp_thread.partition_S(t_dp_core)
            ...
            dp_source_pp = dp_copy_pp.get_slice(
                mtx
            ).partition_S(t_dp_pp_core)

            # Logical (h, n) coordinates per fragment value, in store
            # order (baseline :1767-1777): identity composed with the
            # store-image layout, partitioned by the same T2R copy.
            c_score = cute.make_identity_tensor(
                (self.H_TILE_CTA, self.N_TILE)
            )
            c_score = cute.composition(
                c_score,
                s_p_store_slice.layout,
            )
            score_coordinates = score_thread.partition_D(c_score)
            c_dp = cute.make_identity_tensor(
                (self.H_TILE_CTA, self.N_TILE)
            )
            c_dp = cute.composition(
                c_dp,
                s_ds_store_slice.layout,
            )
            dp_coordinates = dp_thread.partition_D(c_dp)

            # Publish atom: host assert F build gate -- the store must
            # lower to stmatrix m8n8.x4.trans (the v8 scalar STS.U16
            # pathology fails the build instead of the run).
            smem_store_atom = sm100_utils.get_smem_store_op(
                utils.LayoutEnum.COL_MAJOR,
                self.element_dtype,
                self.acc_dtype,
            ...
            # 64x64 FP32 accumulator over 128 threads = 32 values each.
            assert cute.size(r_score) == 32
            ...
            for loop_iter in cutlass.range(tile_count):
                ...
                # E2 math (kept from v17a): packed f32x2 pair math in a
                # phased P-then-dS structure.  MATH_SOFTMAX/MATH_STORE
                # emit two instances per tile: payload 2i (P phase) and
                # 2i+1 (dS phase).
                ...
                # SOFTMAX_GROUPED_STATS=False is pinned by a host
                # assert in __call__: the v9.3 h-group constancy proof
                # was derived for the Rep4 fragment and does NOT
                # transfer to Rep8 (re-derive in rev2 before wiring a
                # grouped branch back in).
                # Assumption-free per-value coordinate lookup with
                # baseline-literal packed pairs (baseline :1818-1831):
                # adjacent fragment values pair; each fetches its own
                # per-row lse from the coordinate tensor (ground truth
                # under any T2R atom).
                for pair in cutlass.range_constexpr(16):
                    ...
                    # Keep FP32 P live for the dS phase (the baseline
                    # keeps tTR_rS the same way).
                    r_score[v0] = p_0
                    r_score[v1] = p_1
                ...
                # The pds gate: wait for the MMA warp's tcgen05-tracked
                # release of tile t-1's images (its gradient MMAs have
                # READ the bytes).
                ...
                r_p_store = thread_copy_r2s.retile(r_p)
                assert cute.size(t_rs_p) == cute.size(r_p_store)
                cute.copy(
                    tiled_copy_r2s,
                    r_p_store,
                    t_rs_p,
                ...
                r_ds_store = thread_copy_r2s.retile(r_ds)
                assert cute.size(t_rs_ds) == cute.size(r_ds_store)
                cute.copy(
                    tiled_copy_r2s,
                    r_ds_store,
                    t_rs_ds,
                ...
                # P/dS stay finite; dQ is protected by the same
                # zero-filled K panel (dual view) and dKV garbage
                # columns are dropped by the drain predicates
                # (global_row < topk, kv_index >= 0).
                cute.arch.fence_view_async_shared()
                _iket.range_end(
                    math_store_token,
                    phase_ds_payload,
                )
                # pds commit: one release-semantics arrive per math
                # thread after its own fenced stores (baseline
                # compute_mma_P/dS producer form).  MATH_BAR1 brackets
                # it for trace-contract continuity.
            ...
            # dQ epilogue: wait for the last dQ generation (committed by
            # the MMA warp right after the final tile's dQ issues -- the
            # v12 S1 form), then store all four [D128, H64] rounds via
            # SMEM staging in the dead score_kv + one bulk TMA each.
            if tile_count > Int32(0):
                pipe_dq_done.consumer_wait(dq_done_state)
                for dq_round in cutlass.range_constexpr(
                    self.D_BLOCKS
                ):
                ...
                # v12 (S1): the pds producer_tail stays BELOW the
                # epilogue -- it is only a drain of outstanding producer
                # credit and must not serialize the per-token tail.
            ...
            # --- reduce: four dkv_done generations per tile (one per
            # D128 chunk, slot = chunk % 2).  Per-chunk mechanics are
            # the baseline reducer verbatim (Ld16x256b Rep-4 T2R split
            # across two warp groups, register-gathered FP32x4 quads,
            # preloaded KV indices, T2R -> fence -> release FIRST and
            # the atomic burst after, baseline :2163-2176); the atomic
            # targets are the FULL-width D panels -- both CTAs of the
            # pair RMW the same rows (the baseline atomic tax).
            ...
            for loop_iter in cutlass.range(tile_count):
                ...
                dkv_wait, dkv_rel = self._drain_dkv_a1(
                    t_dkv[0],
                    t_dkv[1],
                    mdKV_acc,
                    mTopkIdxs,
            ...
            # --- MMA warp: BOTH CTAs run one (the leader/follower CG2
            # asymmetry is gone).  Baseline in-tile order: S(t), dP(t),
            # pds wait, four dQ blocks, K-panel release (a tcgen05
            # commit that tracks the S/dP/dQ reads), then the four
            # dkv_done chunk generations, then the pds release (tracks
            # every image read).  The v7 rotated schedule is
            # structurally impossible here: the K panel is double-
            # booked as the dQ A operand, so S(t+1)'s panel load must
            # wait for tile t's dQ issues (whole-panel-wait policy,
            # spec k_panel_policy).
            _iket.mark(
            ...
            if tile_count > Int32(0):
                # Stationary Q/dO TMA (the deleted W17's only surviving
                # duty): issue both loads, wait both completions.
            ...
            for loop_iter in cutlass.range(tile_count):
                ...
                # P/dS images of THIS tile published by math.
                wait_pds_token = _iket.range_start(
                    "WAIT_PDS(i)",
                    loop_iter,
                )
                pipe_pds.consumer_wait(pds_cons)
                _iket.range_end(
                    wait_pds_token,
                    loop_iter,
                )

                # dQ: four persistent [D128, H64] accumulators
                # (ACCUMULATE=False on the first tile only).
                dq_accumulate = loop_iter > Int32(0)
                for m_block in cutlass.range_constexpr(
                    self.D_BLOCKS
                ):
                    dq_packed = (
                        loop_iter * Int32(self.D_BLOCKS)
                        + Int32(m_block)
                    )
                ...
                # v12 (S1): on the LAST tile, commit dq_done right after
                # the dQ issues -- tcgen05.commit tracks their
                # completion, and the epilogue reads only the dQ TMEM
                # columns [256, 512), disjoint from the dKV slots the
                # remaining chunk passes write.  (The matching advance
                # runs after the loop; commit alone leaves the pipeline
                # state untouched, keeping the loop-carried state
                # type-stable.)
                if loop_iter == tile_count - Int32(1):
                    pipe_dq_done.producer_commit(dq_done_prod)

                # K panel free: the release is a tcgen05 commit tracking
                # the S/dP/dQ reads, so gather may start K(t+1) safely.
                pipe_kscore.consumer_release(kscore_cons)
                kscore_cons.advance()

                # dV/dK: four chunk generations, slot = chunk % 2.  The
                # acquire of generation g waits the reducer's release of
                # generation g-2 == the same slot's previous chunk.
                for chunk in cutlass.range_constexpr(
                    self.DKV_GENS_PER_TILE
                ):
                    pipe_dkv_done.producer_acquire(dkv_prod)
                    dkv_packed = (
                        loop_iter
                        * Int32(self.DKV_GENS_PER_TILE)
                        + Int32(chunk)
                    )
                ...
                # Image release: tcgen05 commit tracking every gradient
                # MMA's read of p_image/ds_image.
        ...
        # ==================================================================
        # Common tail: CTA rendezvous, cluster exit sync, TMEM release.
        # ==================================================================
        tmem.relinquish_alloc_permit()
        self.cta_barrier.arrive_and_wait()
        cute.arch.cluster_arrive()
    ...
    @cute.jit
    def _reshape_grad_a_fragment_a1(
        self,
        fragment: cute.Tensor,
    ) -> cute.Tensor:
        """Insert the baseline's dummy mode into a gradient-A fragment.

        make_fragment_A over a (512, 64, 64)-tiled MN-major view yields
        (MMA, REST_M, MMA_K, STAGE); cute.gemm wants the 5-mode
        (MMA, 1, REST_M, MMA_K, STAGE) profile so the per-issue slice
        [None, None, m_block, k_block, 0] leaves a rank-2 (MMA, 1)
        operand (baseline :907-925).
        """
    ...
    def _issue_score_full_k_a1(
        ...
        """One full-K512 CG1 score GEMM into one ping-pong accumulator.

        Baseline issue-loop form (dsa_bwd_sm100.py:1455-1463): flat
        runtime k-mode, unroll 4.
        """
    ...
    def _issue_score_a1(
        ...
        """Issue one S or dP CG1 GEMM into the stage-selected slot.

        The accumulator ping-pongs between two TMEM bases selected by
        the producer stage index; the pipeline state itself never
        crosses the dynamic stage branch (only the pure-side-effect
        GEMM loop does).
        """
    ...
    @cute.jit
    def _issue_dq_block_a1(
        self,
        kds_tiled_mma: cute.TiledMma,
        t_dq: cute.Tensor,
        kd_fragment: cute.Tensor,
        ds_dq_fragment: cute.Tensor,
        m_block: cutlass.Constexpr[int],
        accumulate: cutlass.Boolean,
    ) -> None:
        """Issue one persistent [D128, H64] dQ^T block (baseline
        :1564-1612 form: per-block ACCUMULATE seeded by tile order)."""

        mma = kds_tiled_mma.with_()
        mma.set(tcgen05.Field.ACCUMULATE, accumulate)
        for k_block in cutlass.range(
            0,
            cute.size(ds_dq_fragment, mode=[2]),
            unroll=2,
        ):
            cute.gemm(
                mma,
                t_dq,
                kd_fragment[None, None, m_block, k_block, 0],
                ds_dq_fragment[None, None, k_block, 0],
                t_dq,
            )
            mma.set(tcgen05.Field.ACCUMULATE, True)
    ...
    @cute.jit
    def _issue_dvdk_chunk_a1(
        self,
        dop_tiled_mma: cute.TiledMma,
        qds_tiled_mma: cute.TiledMma,
        t_dkv: cute.Tensor,
        dot_fragment: cute.Tensor,
        qt_fragment: cute.Tensor,
        p_fragment: cute.Tensor,
        ds_dk_fragment: cute.Tensor,
        chunk: cutlass.Constexpr[int],
    ) -> None:
        """One D128 chunk of dV then dK into one dKV slot.

        dV = dO^T[chunk] @ P overwrites the slot (ACCUMULATE=False on
        the first pass), dK = Q^T[chunk] @ dS accumulates on top --
        the baseline dKV0..3 pass pairing (:1489-1536) collapsed onto
        the chunk's slot.
        """

        mma_dv = dop_tiled_mma.with_()
        mma_dv.set(tcgen05.Field.ACCUMULATE, False)
        for k_block in cutlass.range(
            0,
            cute.size(p_fragment, mode=[2]),
            unroll=2,
        ):
            cute.gemm(
                mma_dv,
                t_dkv,
                dot_fragment[None, None, chunk, k_block, 0],
                p_fragment[None, None, k_block, 0],
                t_dkv,
            )
            mma_dv.set(tcgen05.Field.ACCUMULATE, True)

        mma_dk = qds_tiled_mma.with_()
        mma_dk.set(tcgen05.Field.ACCUMULATE, True)
        for k_block in cutlass.range(
            0,
            cute.size(ds_dk_fragment, mode=[2]),
            unroll=2,
        ):
            cute.gemm(
                mma_dk,
                t_dkv,
                qt_fragment[None, None, chunk, k_block, 0],
                ds_dk_fragment[None, None, k_block, 0],
                t_dkv,
            )
    ...
    def _drain_dkv_a1(
        ...
        """Drain one tile's FOUR dkv_done generations (chunk c -> slot
        c % 2 -> full-width D panel c).

        Per-chunk mechanics are the baseline reducer verbatim
        (Ld16x256b Rep-4 T2R split across two warp groups, register-
        gathered FP32x4 quads, ONE shared KV-index preload, T2R ->
        fence -> release FIRST, atomics after: baseline :2163-2176 /
        v11 single-slot liveness).  Split wait/release states let each
        release trail its own fence.
        """

        packed_base = issue_seq * Int32(self.DKV_GENS_PER_TILE)
        ...
        # ONE shared KV-index preload per tile, as independent loads
        # overlapping the first T2R (baseline reducer pattern).
        ...
        for chunk in cutlass.range_constexpr(
            ...
            packed_issue = packed_base + Int32(chunk)
            ...
            if cutlass.const_expr(chunk % 2 == 0):
                cute.copy(
                    tiled_t2r_0,
                    thread_source_0,
                    thread_values,
            ...
            for i in cutlass.range_constexpr(8):
                ...
                if kv_index >= Int32(0):
                    ...
                    tile_row_c = tile_row[None, chunk]
                    tile_row_c = cute.flat_divide(
                        tile_row_c,
                        (4,),
                    )
                    target_frg = tile_row_c[None, dp_idx // 4]
                    cute.arch.atomic_add(
    ...
    def _store_dq_a1(
        ...
        """Store one [D128, H64] dQ^T round: T2R + quantize + SMEM
        staging in the dead score_kv + one bulk TMA (baseline store_dQ
        :2434-2483 mechanics with the v12 P4 source-side completion).
        """

        t_dq_core = t_dq[(None, None), 0, 0]
        tmem_load_atom = cute.make_copy_atom(
        ...
        tiled_t2r_dq = tcgen05.make_tmem_copy(
            tmem_load_atom,
            t_dq_core,
        )
        thread_t2r_dq = tiled_t2r_dq.get_slice(mtx)
        t_tr_c_dq = thread_t2r_dq.partition_D(c_dq)
        t_tr_r_dq = cute.make_rmem_tensor(
            t_tr_c_dq.shape,
            self.acc_dtype,
        )
        t_tr_t_dq = thread_t2r_dq.partition_S(t_dq_core)
        cute.copy(tiled_t2r_dq, t_tr_t_dq, t_tr_r_dq)
        r_dq_bf16 = self.quantize(t_tr_r_dq, 4)
        cute.arch.fence_view_async_tmem_load()

        # Row-wise staging copy (baseline :2467-2471): with Ld32x32b
        # Rep(8) each thread owns one full 64-column DP row.
        thread_layout = cute.make_ordered_layout(
            (self.GRAD_MMA_TILER[0], self.GRAD_MMA_TILER[1]),
            (0, 1),
        )
        s_dq_comp = cute.composition(s_dq_epi, thread_layout)
        s_dq_row = cute.composition(
            s_dq_comp[mtx, None],
            cute.make_layout(t_tr_c_dq.shape),
        )
        cute.autovec_copy(r_dq_bf16, s_dq_row)

        cute.arch.fence_view_async_shared()
        self.math_barrier.arrive_and_wait()

        g_dq = cute.local_tile(
            tma_tensor_dq,
            cute.select(self.GRAD_MMA_TILER, mode=[0, 1]),
            (None, None, (token_idx, batch_idx)),
        )
        g_dq_tile = g_dq[None, None, round_index, rank]
        t_dq_smem, t_dq_gmem = cpasync.tma_partition(
            tma_atom_dq,
            0,
            cute.make_layout(1),
            cute.group_modes(s_dq_epi, 0, 2),
        ...
        if mtx < Int32(32):
            cute.arch.fence_view_async_shared()
            cute.copy(tma_atom_dq, t_dq_smem, t_dq_gmem)
            cute.arch.cp_async_bulk_commit_group()
            cute.arch.cp_async_bulk_wait_group(0, read=True)
        # Source-side completion broadcast: round r+1 may overwrite the
        # staging only after the engine has READ round r's bytes.
        self.math_barrier.arrive_and_wait()
    ...
    def _zero_dq_a1(
        ...
        """Write the required all-zero dQ half when no tile is issued."""

        elements = self.D_HEAD * self.H_TILE_CTA
        for i in cutlass.range(elements // self.MATH_THREADS):
            linear = Int32(i) * Int32(self.MATH_THREADS) + mtx
            d_index = linear % Int32(self.D_HEAD)
            head = linear // Int32(self.D_HEAD)
            mdQ[
                d_index,
                rank * Int32(self.H_TILE_CTA) + head,
                (token_idx, batch_idx),
            ] = self.element_dtype(0.0)

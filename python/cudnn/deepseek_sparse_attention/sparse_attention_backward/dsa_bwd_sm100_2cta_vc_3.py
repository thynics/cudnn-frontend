"""vc_3: vc_2 double borrow -- the score-K stage also carries Q(round0).

The active V2 kernel keeps vc_2's score, math, reducer, and persistent-TMEM
design.  vc_2 reinterpreted the dead 32-KiB score-K stage as the two round-0
dO operands (one kscore loan generation per tile).  vc_3 reuses the SAME
stage a second time inside each gradient head: after the round-0 dV passes
release the borrowed dO, the gather role refills it with the two round-0 Q
panels, and the round-0 dK passes consume that second loan generation.  The
one-stage kscore pipeline therefore carries K -> dO(r0) -> Q(r0) -> K (three
generations per steady tile), and the A/B round ring shrinks to
kdq/dO(round1)/Q(round1) -- six generations per tile, parity preserved.
The vc_2 tail-tile "unchanged-bytes recommit" is retired: the loan now
alternates dO/Q contents, so the final tile's pair are real refills.
Known cost (measured on vc_2): the second loan pushes K(t+2)'s refill later;
steady K-ready margin was mean 1.41 / min 0.99 us, so the acceptance gate is
release period plus S_ISSUE start jitter against the vc_2 run.
...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    * The gradient GEMMs of tile p are issued inside tile t=p-1's S/dP
    ...
    def kernel(
        ...
        # vc_3: the second (Q) loan generation reuses the same score_kv
        # halves through the Q_T TMA atom.
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            if tile_count > Int32(0):
                ...
                if tile_count == Int32(1):
                    ...
                    pipe_kscore.producer_commit(gather_state)
                    gather_state.advance()
                    # vc_3: second loan generation -- Q(r0) for the round-0
                    # dK passes (the fill helper is source-agnostic).
                    pipe_kscore.producer_acquire(gather_state)
                    loan_phase = self._fill_score_loan_do_r0_vc2(
                        rank,
                        warp_idx,
                    ...
                    for score_iter in cutlass.range(
                        ...
                        pipe_kscore.producer_commit(gather_state)
                        gather_state.advance()
                        # vc_3: second loan generation -- Q(r0) for the
                        # round-0 dK passes.  Its acquire waits the dV
                        # passes' UMMA reads of the borrowed dO.
                        pipe_kscore.producer_acquire(gather_state)
                        loan_phase = self._fill_score_loan_do_r0_vc2(
                            rank,
                            warp_idx,
            ...
            if tile_count > Int32(0):
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    # g2..g5: dO_r1, Q_r1 panel fills, pipelined depth 2.
                    # vc_3: BOTH round-0 panels (dO and Q) are supplied
                    # through score_kv by the gather role -- the ring
                    # carries six generations per tile, parity preserved.
    ...
    def _issue_prev_grads_head_v2(
        ...
        # vc_3: the round-0 dK passes consume the SECOND kscore loan
        # generation (Q(r0) republished into score_kv by the gather role
        # after the dV passes released the borrowed dO).  The round ring
        # no longer carries Q(r0).
        kscore_pipeline.consumer_wait(kscore_consumer_state)
        dkv_issue_token = _iket.range_start(
            "dVdK_ISSUE(i,r,p)",

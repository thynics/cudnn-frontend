# v16 = final.py + two import-time env levers (DSA_V16_EAGER, DSA_V16_PDS_HOIST,
# both default ON; only the literal string "0" disables).  Both levers off
# reproduces final.py's staged IR exactly.  Protocol notes: V16_RUNNER_NOTES.md.
import math
import os
...
class FlashAttentionDSABackwardSm100TwoCTAV2(FlashAttentionDSABackwardSm100TwoCTA):
    ...
    # v16 levers (import-time env, both default ON; 0 restores final behavior)
    # EAGER: round-ring panel commits move from W17's lagged wait (which held
    #   the ring at effective depth 1) to the idle warp 19, which commits each
    #   gen as soon as its TMA/bulk lands.  W17 keeps acquire + kdq commits,
    #   mirroring the pds pipeline's existing split-producer pattern.
    # PDS_HOIST: W18 commits pds full right after pds_ready instead of after
    #   both DSM pushes, taking the push-issue window out of the leader's dQ
    #   gate.  Cross-CTA halves stay protected by landing->relay_mbars gates.
    COMMIT_WARP = 19
    V16_EAGER = os.environ.get("DSA_V16_EAGER", "1") != "0"
    V16_PDS_HOIST = os.environ.get("DSA_V16_PDS_HOIST", "1") != "0"
    ...
    def kernel(
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            if tile_count > Int32(0):
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    with cute.arch.elect_one():
                        pipe_round.producer_commit(round_com)
                    round_com.advance()
                    if cutlass.const_expr(self.V16_EAGER):
                        # panel-gen commits live on warp 19; keep this warp's
                        # commit state aligned with the ring sequence
                        for _skip_gen in cutlass.range_constexpr(8):
                            round_com.advance()
                    ...
                    for flat_gen in cutlass.range_constexpr(8):
                        ...
                        if cutlass.const_expr(
                            (not self.V16_EAGER) and flat_gen > 0
                        ):
                    ...
                    if cutlass.const_expr(not self.V16_EAGER):
                        cute.arch.mbarrier_wait(
                            round_tma_mbars + 1,
                            tma_phase_1,
                        )
            ...
            if tidx % Int32(32) == Int32(0):
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    if cutlass.const_expr(self.V16_PDS_HOIST):
                        # commit pds full as soon as math data is ready: the
                        # leader's dQ gate no longer serializes behind the two
                        # DSM push issues; pushed halves stay guarded by the
                        # landing->relay_mbars gates below
                    ...
                    if cutlass.const_expr(not self.V16_PDS_HOIST):
                        pipe_pds.producer_commit(pds_com)
                        pds_com.advance()
                    _mbarrier_wait_acquire_cluster(
                        landing_mbars,
        ...
        if cutlass.const_expr(self.V16_EAGER):
            if warp_idx == Int32(self.COMMIT_WARP):
                if tidx % Int32(32) == Int32(0):
                    # eager commit engine: split producer of the round ring
                    # (W17 keeps acquire + kdq commits, mirroring the pds
                    # pipeline's math-acquire/W18-commit split); each panel
                    # gen is committed the moment its TMA/bulk tx lands,
                    # instead of at W17's next fill issue
                    round_com_eager = pipeline.make_pipeline_state(
                        pipeline.PipelineUserType.Producer,
                        self.ROUND_STAGES,
                    )
                    eager_phase_0 = Int32(0)
                    eager_phase_1 = Int32(0)
                    for loop_iter in cutlass.range(tile_count):
                        # kdq gens 0-1 are committed by W17
                        round_com_eager.advance()
                        round_com_eager.advance()
                        for flat_gen in cutlass.range_constexpr(8):
                            if cutlass.const_expr(flat_gen % 2 == 0):
                                ...
                                eager_phase_0 = Int32(1) - eager_phase_0
                            else:
                                cute.arch.mbarrier_wait(
                                    round_tma_mbars + 1,
                                    eager_phase_1,
                                )
                                eager_phase_1 = Int32(1) - eager_phase_1
                            pipe_round.producer_commit(round_com_eager)
                            round_com_eager.advance()

...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    """v2: H-native all-CG2 pipeline with 2-pass H reduction.
    ...
    * All gradient A operands ([D128,H64] Q/dO quadrants and [D128,N64]
      ...
      2x16 KiB round region.  Gather W0-3 produce the two K_dQ generations
      and load W13 produces the eight Q/dO generations.  Ten one-stage
      logical pipelines preserve generation identity while a g -> g-2
      empty-barrier credit chain protects the two aliased physical buffers.
    ...
    * The gradient GEMMs of tile p are issued inside tile t=p-1's S/dP
    ...
    # Round-region generations per tile (fixed order, split producers):
    ...
    def _specialize_shared_storage(
        ...
        round_mbar_count = 2 * self.ROUND_GENS_PER_TILE
    ...
    def _fill_kdq_v2(
        ...
        All four gather warps (128 threads = 16 groups of 8) cover all 64 KV
        rows; each row contributes one 256-byte D-quarter slice at column
        offset 256*r + 128*rank. Invalid rows are zero-filled so the dQ GEMM
        sees exact zeros (0 x garbage would still be garbage, 0 x 0 is not).
        """

        index_in_group = gather_tidx % self.KV_GROUP_SIZE
        group_index = gather_tidx // self.KV_GROUP_SIZE
        groups_per_team = self.GATHER_THREADS // self.KV_GROUP_SIZE
        d_offset = Int32(
            round_index * self.D_TILE_CLUSTER
        ) + rank * Int32(self.D_TILE_CTA)
        rows_per_group = self.N_TILE // groups_per_team
        for row_iteration in cutlass.range_constexpr(rows_per_group):
            local_n = row_iteration * groups_per_team + group_index
    ...
    def _issue_dkv_generation_v2(
        ...
        """Wait, issue, and release one independent dV/dK generation."""
    ...
    def _issue_dq_round_v2(
        ...
        """Issue one persistent dQ round on its one-stage generation.
        ...
        packed_issue = (
            issue_seq * Int32(self.D_ROUNDS) + Int32(round_index)
        )
    ...
    def _tma_store_dq_staging_v2(
        ...
        """TMA-store one complete rank-owned H128 x D128 dQ slab."""
        ...
        # The same 32 KiB score-stage alias is reused by the next round.
        # Wait for the async proxy to finish reading it before overwriting.
        cute.arch.mbarrier_wait(
            dq_source_done_mbar,
            Int32(round_index % 2),
        )
    ...
    def _store_dq_round_tma_v2(
        ...
        """Stage one final dQ round from TMEM, then TMA-store it."""
    ...
    def _zero_dq_round_tma_v2(
        ...
        """Stage and TMA-store one all-zero dQ slab for T=0."""

        linear_index = local_tidx
        while (
            linear_index
            < self.H_TILE_CLUSTER * self.D_TILE_CTA
        ):
            head = linear_index // Int32(self.D_TILE_CTA)
            local_d = linear_index % Int32(self.D_TILE_CTA)
            s_dq_epi[head, local_d] = self.element_dtype(0.0)
            linear_index += Int32(self.MATH_THREADS)
    ...
    def kernel(
        ...
        assert dq_epi_bytes <= 32 * 1024
        # The score-K stage is dead after the final dP release. Reinterpret
        # the same 32 KiB allocation as the design-specified dQ TMA staging.
        ...
        pipe_round_0 = pipeline.PipelineAsyncUmma.create(
            ...
            producer_group=gather_group,
            consumer_group=leader_group,
            barrier_storage=round_mbars,
            cta_layout_vmnk=cluster_layout_vmnk,
            defer_sync=True,
        )
        pipe_round_1 = pipeline.PipelineAsyncUmma.create(
            num_stages=self.ROUND_STAGES,
            producer_group=gather_group,
            consumer_group=leader_group,
            barrier_storage=round_mbars + 2,
            cta_layout_vmnk=cluster_layout_vmnk,
            defer_sync=True,
        )
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            # --- gather: score B plus the two all-warp K_dQ generations.
            ...
            for loop_iter in cutlass.range(tile_count):
                ...
                # P0/P1: every gather thread participates in both fills and
                # commits. Kscore is published first so a physical-credit
                # stall here cannot block S(t)'s already-ready operand.
                route_k_token = _iket.range_start(
                    "ROUTE_K(i)",
                    loop_iter,
                )
                # Both gather generations use the same one-stage tile phase;
                # acquire/commit read this state but never advance it.
                producer_phase = (
                ...
                for round_index in cutlass.range_constexpr(
                    ...
                    if cutlass.const_expr(round_index == 0):
                        kdq_pipeline = pipe_round_0
                        wrap_credit_pipeline = pipe_round_8
                    else:
            ...
            if tile_count > Int32(0):
                ...
                for round_index in cutlass.range_constexpr(
                    ...
                    if cutlass.const_expr(round_index == 0):
                        pipe_round_0.producer_tail(round_tail_state)
                    else:
                        pipe_round_1.producer_tail(round_tail_state)
            ...
            if tile_count > Int32(0):
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    # P2..P9 share this tile's publish and predecessor-credit
                    # phases. Pipeline operations read but do not advance
                    # either state.
                    publish_state = pipeline.PipelineState(
                        self.ROUND_STAGES,
                        loop_iter,
                    ...
                    # P2..P9: quadrant TMA fills, buf alternation matches
                    ...
                    for grad_round in cutlass.range_constexpr(
                        ...
                        for tensor_kind in cutlass.range_constexpr(2):
                            for h_half in cutlass.range_constexpr(
                                ...
                                if cutlass.const_expr(generation == 2):
                                    round_pipeline = pipe_round_2
                                    credit_pipeline = pipe_round_0
                                elif cutlass.const_expr(
                                    generation == 3
                                ):
                                    round_pipeline = pipe_round_3
                                    credit_pipeline = pipe_round_1
                                elif cutlass.const_expr(
                                    generation == 4
                                ):
                                    round_pipeline = pipe_round_4
                                    credit_pipeline = pipe_round_2
                                elif cutlass.const_expr(
                                    generation == 5
                                ):
                                    round_pipeline = pipe_round_5
                                    credit_pipeline = pipe_round_3
                                elif cutlass.const_expr(
                                    generation == 6
                                ):
                                    round_pipeline = pipe_round_6
                                    credit_pipeline = pipe_round_4
                                elif cutlass.const_expr(
                                    generation == 7
                                ):
                                    round_pipeline = pipe_round_7
                                    credit_pipeline = pipe_round_5
                                elif cutlass.const_expr(
                                    generation == 8
                                ):
                                    round_pipeline = pipe_round_8
                                    credit_pipeline = pipe_round_6
                                else:
                                    round_pipeline = pipe_round_9
    ...
    def _issue_prev_grads_tail_v2(
        ...
        # The final dV pass is issued, so P and its landing block can recycle
        # while the two final dK passes still consume dS.

"""vk_4: vk_2 + M2 -- the softmax exp2 moves off MUFU onto a deg-6 FFMA chain.

Approximation-class substitution, user-adjudicated 2026-08-06 (precedent
lineage: deg6 approved / deg5 rejected, HANDOFF_20260803).  Both softmax
exp2 sites (the vg_5 full-width band and the ungrouped fallback -- they
must stay value-consistent) call _exp2_ffma_deg6: magic-number range
reduction (x = n + f, f in [-0.5, 0.5] via +1.5*2^23 round-to-nearest,
whose low 9 exponent bits are zero so bits(z)<<23 == n<<23 exactly),
Taylor deg-6 Horner in six FFMAs, exponent restored by integer-adding
n<<23 to the result bits, and a -126.0 clamp so masked columns underflow
to 2^-126 (~1.2e-38, below bf16 resolution) instead of wrapping.

Numeric gate (exact-f32 simulation, 600k-point sweep over [-126, 0]):
max rel err 2.166e-7 -- BETTER than MUFU.EX2's own ~2-ulp (2.4e-7)
error class, i.e. this is not a same-magnitude approximation but a
not-worse one.  All arithmetic f32; no mixed precision.

Why: SOFTMAX measured 2.047us/tile on the active base (vk_2_trace);
MUFU.EX2 serializes on the SFU pipe while the band's 32-wide ILP can
stream through the FMA pipe.  Conversion is partial while W17 binds the
period (expected -0.15~0.3ms now) and rises toward 1:1 if W17 is ever
relieved.

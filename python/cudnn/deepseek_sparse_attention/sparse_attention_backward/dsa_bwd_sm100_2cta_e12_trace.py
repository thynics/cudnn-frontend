    ...
    def kernel(
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            for loop_iter in cutlass.range(tile_count):
                ...
                # e12: count-4 publish -- each thread fences its own
                # stores, the warp sync orders siblings' fences before
                # the elected lane's arrive, so four arrives carry the
                # full 128-thread completion proof at ~1/30 the arrive
                # ritual cost.
                cute.arch.sync_warp()
                with cute.arch.elect_one():
                    cute.arch.mbarrier_arrive(

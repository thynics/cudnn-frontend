...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    def _issue_score_chunks_streamed(
        ...
        # E1b diagnostic: all four chunk gates hoisted OUT of the issue
        # loop (whole-tile handoff again, no streaming overlap).  Same
        # mbar protocol, same arrives -- isolates the in-loop wait
        # placement (leader register pressure) from the rest of E1.
        for chunk in cutlass.range_constexpr(self.K_CHUNKS):
            _mbarrier_wait_acquire_cluster(
                kchunk_mbars + chunk,
                kchunk_phase,

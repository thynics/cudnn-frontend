...
class FlashAttentionDSABackwardSm100TwoCTAV1A0(
    ...
    @cute.jit
    def _scatter_h_to_n_half_v1(
        self,
        captured: cute.Tensor,
        destination_rows: cute.Tensor,
        destination_full: cute.Pointer,
        vector_iteration: cutlass.Constexpr[int],
        rank: Int32,
        peer_rank: Int32,
        tidx: Int32,
    ) -> None:
        """Scatter one captured N32 half into its local or peer owner."""

        flat_vector = (
            tidx + Int32(vector_iteration * self.ROUTE_THREADS)
        )
    ...
    def _route_ds_full_rewrite_v1(
        ...
        destination_full: cute.Pointer,
        remote_first: cutlass.Constexpr[bool],
        route_phase: Int32,
        rank: Int32,
        peer_rank: Int32,
        tidx: Int32,
    ) -> None:
        """Capture one H-owner field, then rewrite it under N ownership."""
        ...
        if cutlass.const_expr(remote_first):
            # Both ranks issue their peer-owned N32 half first.  The following
            # local N32 rewrite can then cover the asynchronous DSM drain.
            if rank == Int32(0):
                self._scatter_h_to_n_half_v1(
                    captured,
                    destination_rows,
                    destination_full,
                    1,

"""DeepSeek Sparse Attention backward, two-CTA SERIAL chain, vkq6w.
...
Below: the vkq6v build docstring.

vkq6v = vkq6u + one performance-critical protocol restoration.  K_dQ no
longer consumes four generations of the four-slot round ring.  As in the
measured kq6q champion, gather warps borrow the two 16 KiB halves of
``score_kv`` for one ordinary kscore generation after dP releases score K;
W16 waits once, issues both dQ rounds, then releases that generation for the
next tile's score-K gather.

The round ring is again exactly 16 generations/tile: dO0..dO3 and Q0..Q3,
each split into two true-K32 generations.  W17 owns all 16 producer acquires,
W19 converts the 16 TMA completion barriers into the 16 producer commits,
and W16 consumes/releases the same 16 states in order.  There is no
gather-owned round generation, kdq slot-free handoff, or W19 skip.

This preserves vkq6u's 640-thread deadlock-safe register ledger, strict
P-first relay, spill containment, and native IKET event structure.  The
single intended performance variable is removal of the four-generation K_dQ
ring detour.

This is also a correctness repair for the D2 silent race in the 20-generation
design: the dQ-A layout spans D half-columns, so releasing one K_dQ slot
immediately after enqueue does not prove UMMA has finished reading every byte
that a later TMA may overwrite.  The score_kv loan remains owned until both
dQ rounds have been enqueued and the TMEM-store fence has executed.  Only
then does one kscore consumer_release emit the tcgen05 commit covering both
rounds; the next producer_acquire waits that asynchronous empty transition.
The fence alone is not treated as UMMA completion evidence.

Below: the vkq6u build docstring.
...
# Keep 26 ranges, four role marks, and the V2 provenance mark within IKET's
# 31-name limit.  W18's relay role is implicit, leaving one event slot for
# the P/dS consumer wait at the leader's gradient-chain boundary.
_LEAN_SPANS = (
    "LOAD_QDO",
    "LOAD_STATS",
    "LOAD_K(",

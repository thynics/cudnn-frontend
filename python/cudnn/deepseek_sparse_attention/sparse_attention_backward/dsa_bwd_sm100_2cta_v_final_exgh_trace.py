...
(v_final_exgh trace twin).

Identical logic to dsa_bwd_sm100_2cta_v_final_exgh (P-early split
exchange: the math publish splits into a P phase and a dS phase with
two count-128 ready mbars; the relay flies the P DSM as soon as the P
phase lands).  Adds IKET software annotations for pipeline trace
capture; on top of the final_trace span set, the split points and the
leader gradient-block gates carry their own spans (MATH_DSCOMP /
MATH_STORE2 / RELAY_DSW / PDS_WAIT / LAND_P / LAND_DS / KS2_WAIT; the
ready arrives fold into the two store spans).  To stay at the 31-name
decoder cap, the one-shot prologue/epilogue spans (LOAD_STATS /
LOAD_QDO / TAIL / DQ_EPI) and the dP consumer spans (WAIT_dP / T2R_dP)
are dropped relative to final_trace.  The annotations are no-ops
unless the IKET runtime is loaded.
"""
...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    def _specialize_shared_storage(
        ...
        class SharedStorageV2:
            ...
            # exgh: [0] = P-phase ready, [1] = dS-phase ready.
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            for loop_iter in cutlass.range(tile_count):
                ...
                if cutlass.const_expr(self.SOFTMAX_GROUPED_STATS):
                    ...
                    # exgh P phase: the bf16 convert rides the exp2
                    # loop so P publishes (and flies) before the dS
                    # arithmetic.
                ...
                # exgh: publish P alone and signal p_ready -- the relay
                # flies the P DSM under the dS phase below.  Each pair
                # of warps owns one N32 half, so the branch is
                # warp-uniform.
                r_p_store = thread_copy_r2s.retile(r_p)
                assert t_rs_p_local_tile.shape == r_p_store.shape
                assert t_rs_p_xchg_tile.shape == r_p_store.shape
                ...
                # exgh dS phase, under the P flight.
                math_dscomp_token = _iket.range_start(
                    "MATH_DSCOMP(i)",
                    loop_iter,
                )
                if cutlass.const_expr(self.SOFTMAX_GROUPED_STATS):
                ...
                math_store2_token = _iket.range_start(
                    "MATH_STORE2(i)",
                    loop_iter,
                )
            ...
            if tidx % Int32(32) == Int32(0):
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    # exgh: the P DSM is already in flight; wait for the
                    # math dS phase before routing dS.
                    relay_dsw_token = _iket.range_start(
                        "RELAY_DSW(i)",
                        loop_iter,
                    )
                    cute.arch.mbarrier_wait(

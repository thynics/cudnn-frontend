...
class FlashAttentionDSABackwardSm100:
    ...
    def _get_workspace_size_dKV(k: int, d: int, b: int, acc_dtype: Type[cutlass.Numeric]):
        ...
        # aug_7: + SM-indexed topk-compact dKV panels appended after the
        # accumulator region: 148 SMs (B200) x k rows x (d/2) fp32 columns.
        # Expressed as per-KV-row entry growth so the fixed (k, d, b,
        # acc_dtype) signature and both harness allocation sites (local
        # _interface_sm100.py and the remote allinone template) inherit it.

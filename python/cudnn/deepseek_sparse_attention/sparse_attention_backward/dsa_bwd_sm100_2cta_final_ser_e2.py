"""DeepSeek Sparse Attention backward, two-CTA SERIAL chain, E2 math.

final_ser_e2 = final_ser with the math loops ported to the BASELINE
idiom (dsa_bwd_sm100.py compute(), L1818-1885): packed-f32x2 FP32
arithmetic instead of scalar -- P phase uses one fused
``fma_packed_f32x2`` (S*scale + lse) per value pair, dS phase uses
``add_packed_f32x2`` + two ``mul_packed_f32x2``.  The grouped-stats
banding already pairs adjacent fragment indices within one h-group
(band j=2k/2k+1 -> indices base+4k/base+4k+1, same lse/delta), so the
pairs pack for free.  This is the ledger's E2 lever ("SOFTMAX 1.87 ->
<=1.1") and removes the known implementation gap between the final
family's scalar math and baseline's packed math: e2-vs-ser measures
the implementation delta; e2-vs-baseline leaves the purely structural
serial-chain exposure (collective close + DSM exchange + landing +
cluster acquire, and the full-width CG2 dP shadow being ~0.35us vs
baseline's ~1.4us M64 pair).  Numerics note: the fused FMA contraction
matches baseline (single rounding); final_ser's scalar mul+add path
may differ by one pre-downcast ulp.
...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    # Serial build: the round-0 dO quadrants are ordinary ring
    # generations (no score_kv loan), so the ring carries 10 gens/tile:
    # dO0,dO1, dO2,dO3, Q0,Q1, Q2,Q3, KdQ0,KdQ1.  10 % 2 == 0 keeps the
    ...
    def kernel(
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            if tile_count > Int32(0):
                ...
                # Serial steady state, no dO loan: score_kv cycles ONE
                # kscore generation per tile (the score K), so K(t+1)
                # gathers as soon as dP(t) releases K(t) -- under the
                # dV/dK window.  The kdq rendezvous fill closes the
                # tile's ring cycle (g8/g9), so it runs last; the load
                # warp reaches the rendezvous only after dK r1 frees
                # both ring credits, which covers the kdq copy with the
                # dK execution window.
            ...
            for loop_iter in cutlass.range(tile_count):
                ...
                if cutlass.const_expr(self.SOFTMAX_GROUPED_STATS):
                    ...
                    for h_group in cutlass.range_constexpr(4):
                        ...
                        # Baseline idiom: adjacent band pairs share the
                        # h-group's lse, so scale+bias fuses into one
                        # packed-f32x2 FMA per pair (E2).
                ...
                if cutlass.const_expr(self.SOFTMAX_GROUPED_STATS):
                    for h_group in cutlass.range_constexpr(4):
                        ...
                        # Baseline idiom: (dP + delta) * P * scale as
                        # packed-f32x2 add + two packed muls per pair.
                        for pair in cutlass.range_constexpr(4):
                            i0 = band_indices[h_group][2 * pair]
                            i1 = band_indices[h_group][2 * pair + 1]
            ...
            if is_leader_cta:
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    # Same-tile serial gradient chain:
                    # dV r0/r1 -> dK r0/r1 -> dQ.
            ...
            if tile_count > Int32(0):
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    # g8/g9: both K_dQ rounds in one fused gather pass
                    # (one index read per row; both stage credits held).
                    # The rendezvous sits at the END of the ring cycle:
                    # the credits free as dK r1 consumes Q2/Q3, so the
                    # kdq copy is covered by the dK execution window and
                    # dQ closes the tile.
                    pipe_round.producer_acquire(round_acq)
                    round_acq.advance()
                    pipe_round.producer_acquire(round_acq)
            ...
            if tidx % Int32(32) == Int32(0):
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    # allows (the chain itself, not this leg's order,
                    # bounds when relay 0 opens dV).
                    cute.arch.mbarrier_wait(
                        p_ready_mbars,
                        p_ready_phase,
                    )
    ...
    def _issue_grads_ser_v1(
        ...
        """Same-tile serial gradient chain: dV r0/r1 -> dK r0/r1 -> dQ.

        Every gradient A operand streams through the round ring (10
        gens/tile, no score_kv loan in this build).  Gate placement
        follows the chain: relay 0 (P published and exchanged
        cluster-wide) opens dV; relay 1 (dS likewise) opens dK; dQ
        needs only the local dS images, whose visibility relay 1
        already certifies for both CTAs.  The pds consumer handshake is
        taken right before dQ purely as buffer backpressure -- it is
        quiet by then because the relay commits pds at the dS send,
        which precedes the relay-1 arrival that dK r0 waited on.
        ...
        # dQ both rounds close the tile's chain, consuming the ring's
        # tail KdQ pair (rendezvous-filled under the dK window).

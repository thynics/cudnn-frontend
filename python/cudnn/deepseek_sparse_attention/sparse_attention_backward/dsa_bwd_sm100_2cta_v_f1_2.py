"""v_f1_2: vk_5 + batched dQ rounds (R4a -- the 0.632 issue-path probe).

Single lever on the vk_5 base.  The two persistent dQ rounds consumed
per-round wait->issue->fence->release cycles; the measured r0->r1 gap
was 0.632/tile against a 0.03 control (dVdK p1->p2, no pipeline ops
between issues) even though both kdq generations are committed by the
same rendezvous, making the second wait nominally free.  This rev waits
both generations up front (clone()d release state, the L7702-family
precedent), issues both rounds back-to-back, then fences once and
releases both.

Adjudicated semantic delta: consumer_release's tcgen05.commit tracks
every MMA issued before the call, so g0's empty signal now fires at
BOTH rounds' completion instead of r0's alone.  Net supply effect is
favorable: g2's acquire sees ~+0.15 (r1 issues ~0.5 earlier, its
completion largely overlaps r0's window) while g3's acquire sees ~-0.5
(g1's release registration no longer waits the old late r1 issue).
Arithmetic, per-round k_block order and accumulate-flag sequencing are
untouched (pure-reorder class; expected bit-identical outputs).

vk_5: vk_2 + W1' -- the load warp sheds its TMA-completion waits.
...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    * The gradient GEMMs of tile p are issued inside tile t=p-1's S/dP
    ...
    def _issue_dq_rounds_v2(
        ...
        packed_issue_0 = issue_seq * Int32(self.D_ROUNDS)
        packed_issue_1 = packed_issue_0 + Int32(1)

        # v_f1_2 (R4a): both kdq generations are committed by the same
        # rendezvous, so wait them both up front -- the second wait is
        # nominally free once the first clears.
        ...
        release_state = round_consumer_state.clone()
        ...
        # Back-to-back issues: no pipeline ops between the two rounds
        # (the dVdK p1->p2 0.03 control shape).
        ...
        mma_1 = dq_tiled_mma.with_()
        mma_1.set(tcgen05.Field.ACCUMULATE, accumulate)
        for k_block in cutlass.range_constexpr(
        ...
        cute.arch.fence_view_async_tmem_store()
        round_pipeline.consumer_release(release_state)

...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    # vpagealias_a: dQ still owns the two round stages first, but only K-r1
    # reuses them. K-r0 and dO-r1 alias stationary pages directly.
    ...
    def kernel(
        ...
        # K-r0 aliases Q score pages Q0/Q1.  For each CTA one page is the
        # rank-owned diagonal and the peer page is replaced by TMA below.
        direct_q_quad = (
            cute.make_tensor(
                cute.recast_ptr(
                    stationary_q_raw,
        ...
        # V-r1 similarly aliases the second 32 KiB of stationary dO.
        direct_do_quad = (
            cute.make_tensor(
                cute.recast_ptr(
                    stationary_do_raw + Int32(16384),
                    dkv_a_layout_staged.inner,
                    dtype=self.element_dtype,
                ),
                dkv_a_layout_staged.outer,
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            if is_leader_cta:
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    release_score_pages = (
                        has_prev or tile_count == Int32(1)
                    )
                    ...
                    if release_score_pages:
                        # Q's last source read is S; let its 16 KiB grad
                        # fill start while dP is still issuing.
                    ...
                    # The initial ScoreFull image is deliberately retained
                    # across score(0) and score(1).  Every later score image
                    # has a single consumer; the one-tile case transitions
                    # directly from score(0) to the tail gradients.
                    if release_score_pages:
                    ...
                    if has_prev:
                        ...
                        ds_cons.advance()
                        p_cons.advance()
                        # The final in-loop gradient deliberately retains
                        # GradFull for the tail.  Earlier gradients hand both
                        # pages back so W19 can restore ScoreFull.
                        if release_grad_pages:
                            alias_cons.advance()
            ...
            if tile_count > Int32(0):
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    # the gather warps as soon as g2(prev) recycles it and
                    ...
                    # g2/g3: only Q-r1 (K1) still reuses the round pair.
                    # Q-r0 and dO-r1 are direct stationary-page aliases.
                    for local_gen in cutlass.range_constexpr(2):
                        flat_gen = local_gen + 4
            ...
            w19_phase_1 = Int32(0)
            if tile_count > Int32(0):
                # Publish the pristine prologue as ScoreFull generation 0.
                # Q is committed as soon as its full TMA lands; dO keeps its
                # independent completion so the original split readiness is
                # not collapsed into one barrier.
                pipe_alias_q.producer_acquire(alias_acq)
                pipe_alias_do.producer_acquire(alias_acq)
                alias_acq.advance()
                cute.arch.mbarrier_wait(
                    stationary_tma_mbars,
                    alias_q_phase,
                ...
                cute.arch.fence_view_async_shared()
                with cute.arch.elect_one():
                    pipe_alias_q.producer_commit(alias_com)
                cute.arch.mbarrier_wait(
                    stationary_tma_mbars + 1,
                    alias_do_phase,
                )
                ...
                cute.arch.fence_view_async_shared()
                with cute.arch.elect_one():
                    pipe_alias_do.producer_commit(alias_com)
                alias_com.advance()
            for loop_iter in cutlass.range(tile_count):
                # ScoreFull survives score(0)+score(1).  Thereafter every
                # score release flips both pages to GradFull.  A non-final
                # in-loop gradient releases them for the score restore.
                if loop_iter > Int32(0) or tile_count == Int32(1):
                    (
                        alias_acq,
                        alias_com,
                        alias_q_phase,
                    ...
                    if (
                        loop_iter > Int32(0)
                        and loop_iter < tile_count - Int32(1)
                    ):
                        (
                            alias_acq,
                            alias_com,
                ...
                for flat_gen in cutlass.range_constexpr(2):
                    ...
                    commit_com.advance()
            if tile_count > Int32(0):
                pipe_alias_q.producer_tail(alias_acq)
                pipe_alias_do.producer_tail(alias_acq)
    ...
    def _transition_alias_pages_vb(
        ...
        # Acquire/restore Q independently.  dO is still being consumed by
        # dP/V1 at this point, so do not couple its EMPTY wait to Q's TMA.
        ...
        # The dO EMPTY may become ready later (after dP or V1).  Q's new
        # generation is already visible while this independent acquire/TMA
        # proceeds, which is the intended gradual page release.
    ...
    def _issue_score_chunks_v7(
        ...
        for local_chunk in cutlass.range_constexpr(2):
            ...
            for k_block in cutlass.range(
                ...
                mma.set(tcgen05.Field.ACCUMULATE, True)
        page_pipeline.consumer_wait(page_consumer_state)
        for local_chunk in cutlass.range_constexpr(2):
            if cutlass.const_expr(is_dp):
                chunk = local_chunk + 2
            else:
    ...
    def _issue_prev_grads_head_v2(
        ...
        if release_alias:
            q_alias_pipeline.consumer_release(alias_consumer_state)
        cute.arch.fence_view_async_tmem_store()
    ...
    def _issue_prev_grads_tail_v2(
        ...
        if release_alias:
            do_alias_pipeline.consumer_release(alias_consumer_state)

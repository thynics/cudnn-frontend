...
(final_new trace-instrumented build).

Identical logic to the final_new release build (rotation removed --
baseline-style dependency staircase); adds IKET software annotations
(role marks and issue/wait spans) for pipeline trace capture.  The
annotations are no-ops unless the IKET runtime is loaded.

Span-name mapping (all 31 final names kept, none added):
- S_ISSUE/dP_ISSUE unchanged.
- dVdK_ISSUE(i,r,p): p now numbers the dV-major order (0-3 = dV_r0
  a/b, dV_r1 a/b in the head; 4-7 = dK_r0 a/b, dK_r1 a/b in the
  tail), the e4d renumbering precedent.
- dQ_ISSUE/WAIT_dQ ride _issue_dq_rounds_v2 to the END of the train.
- MATH_SOFTMAX(i) covers the P-phase loop; MATH_PDS_ACQ(i) the
  P-phase pipe_p acquire; MATH_STORE(i) the P stores; MATH_BAR1(i)
  the p_ready arrive.  MATH_PD(i) narrows to the whole dS-phase
  (dS loop + pds acquire + dS stores + pds_ready arrive); the
  dS-phase gets no new names.
- ROUTE_K(i) moves with the kdq rendezvous to the END of W17's tile
  loop; MAT_QDO(m,r) keeps its positional two-span split over the
  ten-generation cargo (payload +0 = the dO panel block, flat gens
  0-3; payload +1 = the Q panel block, flat gens 4-7);
  ROUTE_P/ROUTE_dS wrap the two W18 relay stages; TAIL wraps the
  leader's post-loop drain block.
"""
...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    images; warps 4-7 run the softmax backward and publish P then dS;
    warps 8-15 drain the fused dV+dK partial sums to the f32 workspace;
    warp 16 issues every GEMM and manages pipeline credits; warp 17
    feeds the round ring from the stationary panels; warp 18 relays P
    then dS across the cluster; warp 19 relays ring TMA completions.

    Steady-state schedule per KV tile (final_new, NOT rotated): every
    iteration processes ONE tile start-to-finish -- S(t), dP(t), then
    tile t's gradients in dV-major order (dV_r0 a/b, dV_r1 a/b,
    dK_r0 a/b, dK_r1 a/b, dQ r0/r1 last), all A operands off the round
    ring.  Math publishes P early (P-phase, off S alone) and dS after
    dP (dS-phase); W18 relays P then dS as two back-to-back stages so
    the dV passes can start on P while dS is still in flight.  The
    ring carries TEN generations per tile ([dO_r0, dO_r1, Q_r0, Q_r1]
    h0/h1 pairs, then the kdq pair); the kdq rendezvous hands the
    gather-filled dQ-A images to the ring at the END of W17's per-tile
    loop.  score_kv is single-tenant: it holds only the score K tile,
    so K(t+1) fills as soon as dP(t)'s consumption commit releases it.
    ...
    def _specialize_shared_storage(
        ...
        class SharedStorageV2:
            ...
            kscore_epi_safe_mbar: cutlass.Int64
    ...
    def _gather_kdq_v8(
        ...
        Barrier A: the load warp holds both kdq round-stage credits, so
    ...
    def _issue_dq_rounds_v2(
        ...
        can begin the next tile's panel refills.
        """
    ...
    def kernel(
        ...
        """final_new staircase-schedule two-CTA backward (rotation removed)."""
        ...
        kscore_epi_safe_mbar = storage.kscore_epi_safe_mbar.ptr
        ...
        # P-only twin of pipe_pds: the math P-phase acquires its empty
        # barrier (WAR on p_blocks/p_xchg) and W18 commits its full
        # barrier after the P push is in flight (atom_thr_size=2 -- both
        # CTAs' W18 leaders arrive the leader-facing full barrier).
        ...
        if tidx == Int32(0):
            ...
            cute.arch.mbarrier_init(kscore_epi_safe_mbar, 1)
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            if tile_count > Int32(0):
                ...
                # Single-tenant score_kv: K(t+1)'s acquire passes as
                # soon as the leader's post-dP(t) kscore release retires
                # (S/dP source reads of K(t) complete) -- a full grads
                # train ahead of S(t+1).  The kdq rendezvous for tile t
                # follows the K(t+1) fill so S(t+1) never queues behind
                # the rendezvous (W17 reaches it only after dK_r1(t)).
                for gather_iter in cutlass.range(
                    tile_count - Int32(1),
                ):
                    next_iter = gather_iter + Int32(1)
                    load_k_token = _iket.range_start(
                        "LOAD_K(i)",
                        next_iter,
                    )
                ...
                # Tail tile: only the final kdq rendezvous remains.
                self._gather_kdq_v8(
                    mKV,
                    mTopkIdxs,
                    gather_kd_rows_0,
                ...
                # producer_tail observes K(T-1)'s true UMMA source-read
                # completion (the leader's kscore release after dP(T-1)
                # is UMMA-tracked).  Publish that fact to the math
            ...
            for loop_iter in cutlass.range(tile_count):
                # ==========================================================
                # P-phase: S alone -> P published.  The FP32 P stays in
                # r_score's registers for the dS-phase (no precision
                # regression: dS is computed from the FP32 exp2 result
                # exactly as the fused build).
                # ==========================================================
                ...
                # WAR on p_blocks/p_xchg: math(t) may overwrite them only
                # after the leader released pipe_p(t-1) (dV reads done).
                ...
                # ==========================================================
                # dS-phase: dP + saved FP32 P -> dS published.  MATH_PD
                # (narrowed) wraps the whole phase after T2R_dP.
                # ==========================================================
                ...
                # WAR on ds_blocks/ds_image: gated by pipe_pds(t-1)'s
                # release (dK/dQ reads done).
                ...
                # Whole-image dS store for the dQ B operand; the
                # cross-CTA dS half rides W18's ds_image slice push.
            ...
            # and released off the dK_r0-b commit, slot 1 off the dK_r1-b
            # commit, then both atomic bursts run back-to-back.  Split
            # wait/release states let each release trail its own fence.
            ...
            # --- leader MMA: staircase schedule (no rotation).  The
            # follower CTA's MMA warp executes no pipeline operation at
            # all (FA4 rule).
            _iket.mark(
            ...
            if is_leader_cta:
                ...
                # e4d split acquire/commit producer states (the W17
                # round_acq/round_com pattern) -- the dV-major head
                # holds both slot generations before either commits.
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    # Tile t's own gradients, dV-major, in this same
                    # iteration (tile 0 included; no has_prev split).
                    ...
                    # Free the P and dS buffer sets for math(t+1): both
                    # releases ride UMMA-tracked commits, so they retire
                    # only after the last dV (P) / dK+dQ (dS) source
                    # reads complete.
                ...
                # The last tile completed inside its own iteration; only
                # the dQ completion commit and the drains remain.  dQ r1
                # is the final UMMA in program order, so this commit
                # tracks exactly the same completion set as an in-loop
                # commit would.
                if tile_count > Int32(0):
            ...
            if tile_count > Int32(0):
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    # g0..g7: dO_r0, dO_r1, Q_r0, Q_r1 panel fills,
                    # pipelined depth 2 (dV-major consume order; the
                    # score_kv loan is gone, so dO_r0 rides the ring
                    ...
                    # g8/g9: both K_dQ rounds in one fused gather pass at
                    # the END of the tile (dQ issues last in the leader's
                    # train); one index read per row, both stage credits
                    # held.
            ...
            if tidx % Int32(32) == Int32(0):
                ...
                for loop_iter in cutlass.range(tile_count):
                    # Stage 1: P.  The math P-phase publishes off S
                    # alone, so this push (and the leader's dV train)
                    # never waits on dP.
                    cute.arch.mbarrier_wait(
                        p_ready_mbars,
                        p_ready_phase,
            ...
            w19_phase_1 = Int32(0)
            for loop_iter in cutlass.range(tile_count):
                # g0..g7: the panel fills.  Same gen walk as the load
                # warp: even flat gens ride mbar 0, odd ride mbar 1
                # (four flips per tile per mbar, tracked across tiles).
                for flat_gen in cutlass.range_constexpr(8):
                    ...
                    commit_com.advance()
                # g8/g9: the kdq pair, committed by the load warp's
    ...
    def _issue_grads_head_new(
        ...
        """Gradient block, first half: all four dV passes (dV-major).

        Gates for dV_r0 a: P published on both CTAs (pipe_p full), P
        peer half landed (relay_mbars), dkv slot 0 free, and the ring's
        dO_r0 h0 generation -- with the loan removed, dO_r0 rides the
        ring like every other panel.  Both dkv slot generations are
        acquired here (the 2-stage pipe's full budget); the tail owns
        both commits (e4d split-state machinery).
        """

        # P blocks and xchg staging of THIS tile are published (and its
        # DSM send issued): the dV train's invariant-I1 gate.
    ...
    def _issue_grads_tail_new(
        ...
        """Gradient block, second half: all four dK passes, then dQ.

        Slot 0 commits after dK_r0 pass b and slot 1 after dK_r1 pass
        b; both acquires already ran in the head.  dQ issues LAST: it
        gates on pipe_pds (dS published on both CTAs) and consumes the
        local ds_image plus the ring's kdq generations -- committed by
        W17's end-of-tile rendezvous.
        """
        ...
        # dS whole image published on both CTAs (W18 commit orders
        # after each CTA's math dS store): dQ's only new gate.
    ...
    def _drain_dkv_v8(
        ...
        arrivals) -- but returns to per-slot generations: slot 0 is
        waited, T2R'd, fenced, and released off its dK_r0-b commit, two
        ring passes before slot 1 (dK_r1 b); the dV-major head acquires
        both slots up front.  Split wait/release pipeline states allow
        ...
        # --- slot 0: first tail-committed generation (dK_r0 b).
        ...
        # --- slot 1: second tail-committed generation (dK_r1 b).

"""DeepSeek Sparse Attention backward, two-CTA rubin_3 Q_r1 half-pantry build.

rubin_3 = rubin_2 + the round-1 Q quadrants pinned next to the dO
pantry (design: RUBIN2_IMPL_NOTES_20260811.md, appendix branch A).
Q quadrants are token-invariant exactly like the dO quadrants, so the
two round-1 dK A operands (Q_r1 h0/h1) leave the supply ring too.
The full Q pantry (all four quadrants) exceeds the 334,848 B cap; the
half pantry (round 1 only) fits once the ring drops to depth 2.

  1. Q half pantry: ``stationary_q`` grows 64 KB -> 96 KB per CTA.
     The first 32,768 elements stay the rank-owned H64 sub-panel
     (byte-identical to rubin_2: the S A fragment reads it at offset
     0 through the same score_a view).  The next 16,384 elements are
     the Q pantry: two [D128, H64] quadrant slots in
     dkv_a_layout_staged form, holding THIS CTA's M-half of Q.T for
     the two round-1 dK passes (q = h), filled once per token by the
     same quadrant TMA that used to feed the round ring every tile.
     Slot construction is byte-for-byte isomorphic to the rubin_2 dO
     pantry (recast_ptr views over the field tail, same layout object,
     same TMA partition construction), so the CG2 shared-descriptor
     rank-same-address rule is satisfied the same way: same local
     address in both CTAs, each CTA holding its own M-half bytes.
  2. dK round-1 zero-copy: the two tail dK passes consume permanent
     ``make_fragment_A`` views of the Q pantry slots -- no ring waits,
     no releases.  The round-0 Q quadrants (dK head) stay on the ring.
  3. Supply ring 6 -> 4 generations per tile at depth 2
     (2 kdq + 2 Q_r0 quadrants; 4 mod 2 == 0 keeps the static
     gen->slot map tile-invariant).  W17 issues 2 panel fills per tile
     instead of 4; W19 waits 2 completions instead of 4; the ring
     struct drops round_buf_2 (-16 KB, paying for half the Q pantry).
  4. The Q pantry TMAs ride the stationary Q leg
     (stationary_tma_mbars + 0): its expect_tx grows by 2 x 16,384 B,
     so the first S now also waits on the Q pantry (a one-time 32 KB
     widening of the readiness semantics -- accepted, see notes).

Inherited from rubin_2 unchanged: the 128 KB stationary_do (dP
sub-panel + 4-slot dV-A pantry, dV all-pass zero-copy), single-face
P/dS publication, Rubin-only K1a register layout (64/64/128/120), and
the 334,848 B cap.
...
the score K tile is gathered per KV tile; dQ-A images and the round-0
Q.T quadrants stream through the round ring.  dQ accumulates in TMEM
across all KV tiles and stores through a staged TMA epilogue; dKV
drains per tile via red.global.add into an f32 workspace.
"""
...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    """Two-CTA rubin_3 kernel: 20 warps per CTA, five CG2 GEMMs per KV tile.
    ...
    the round ring from the stationary Q panel; warp 18 relays P/dS
    ...
    the ring through a count-128 hardware arrival.  rubin_3: the score
    K tile is a single-tenant kscore generation (released after dP);
    the four dV passes consume permanent pantry views of the 128 KB
    stationary dO panel and the two round-1 dK passes consume
    permanent Q-pantry views of the 96 KB stationary Q panel (zero
    per-tile transport for both); the ring carries 4 generations per
    tile (2 kdq + 2 Q_r0 quadrants) at depth 2; P/dS publish through a
    single face.  dQ stays TMEM-resident across all tiles and stores
    through a two-round staged TMA epilogue.
    """

    THREADS_PER_CTA = 640

    # rubin_2 is Rubin-only: the K1a register layout is the only
    # layout.  Dynamic pool stays at the launch allocation
    # (640 x 96 = 61,440): 4x32x48 (gather) + 4x32x64 (W16-19)
    # + 4x32x128 (math) + 8x32x120 (reduce) = 61,440.
    PRODUCER_REGS = 64
    REDUCE_REGS = 120
    ...
    # rubin_3: the two Q_r1 quadrant generations leave the ring too
    # (they are Q-pantry views now) -> 4 gens/tile.  Ring slots are
    # named struct fields with a static gen->slot binding, so
    # ROUND_GENS_PER_TILE % ROUND_STAGES == 0 is a hard phase law
    # (4 % 2 == 0: the gen->slot map is tile-invariant).
    ROUND_GENS_PER_TILE = 4
    ROUND_STAGES = 2
    ...
    # g2/g3).  Consumption order == fill order (FIFO):
    #   Q_r0 h0/h1  ->  slots (2%2, 3%2).  kdq also lands on slots
    # 0/1 within the same tile (earlier gens); the depth-2 credit
    # discipline covers the reuse, per the rubin_1 depth-2-era slot
    # reuse precedent.
    ...
    # P/dS publication is single-face: face 0 serves every tile and
    # its raw mbarriers flip once per tile (phase(t) = t % 2).
    PDS_FACES = 1

    MAX_SMEM_BYTES = 334_848
    ...
    # dV-A pantry geometry inside the 65,536-element stationary_do
    # field: elements [0, 32768) are the rank-owned H64 sub-panel (the
    # dP A view, byte-identical to rubin_1's stationary_do); elements
    # [32768, 65536) are four 8,192-element [D128, H64] quadrant slots
    # in dkv_a_layout_staged form.  Slot q = 2*r + h holds THIS CTA's
    # M-half of dO.T for dV pass (round r, h-half h): the same bytes
    # the rubin_1 ring TMA (t_dot_gmem[None, r, h]) delivered into
    # round_buf[slot] every tile, now delivered once per token.
    PANTRY_BASE_ELEMENTS = 32_768
    PANTRY_SLOT_ELEMENTS = 8_192
    DV_PASSES = 4

    # Q half-pantry geometry inside the 49,152-element stationary_q
    # field: elements [0, 32768) are the rank-owned H64 sub-panel (the
    # S A view, byte-identical to rubin_2's stationary_q); elements
    # [32768, 49152) are two 8,192-element [D128, H64] quadrant slots
    # in dkv_a_layout_staged form.  Slot q = h holds THIS CTA's M-half
    # of Q.T for dK tail pass (round 1, h-half h): the same bytes the
    # rubin_2 ring TMA (t_qt_gmem[None, 1, h]) delivered into
    # round_buf[slot] every tile, now delivered once per token.  The
    # round-0 Q quadrants (dK head) stay on the ring.
    Q_PANTRY_BASE_ELEMENTS = 32_768
    Q_PANTRY_SLOT_ELEMENTS = 8_192
    Q_PANTRY_SLOTS = 2
    ...
    def _specialize_shared_storage(
        ...
        # --- dV-A pantry geometry (host proof) -----------------------
        # One pantry slot must be exactly one dkv-A quadrant image
        # (the ring-slot byte image the dV fragment already consumes).
        assert self.PANTRY_SLOT_ELEMENTS == cute.cosize(
            dkv_a_layout_staged
        )
        # One quadrant image is one score-family D128 chunk's bytes
        # (16 KiB): the same 8,192-element count that the rubin_1
        # own-half bulk copy moved per generation.
        assert self.PANTRY_SLOT_ELEMENTS * (
            BFloat16.width // 8
        ) == cute.size_in_bytes(
            self.element_dtype,
            cute.select(dkv_a_layout_staged, mode=[0, 1, 2]),
        )
        # The dP sub-panel occupies exactly the first half of the
        # 65,536-element field and equals rubin_1's stationary_do.
        assert self.PANTRY_BASE_ELEMENTS == cute.cosize(
            score_a_layout_staged
        )
        # The four slots tile the second half exactly (no overlap, no
        # tail): [32768, 65536) = 4 x 8192.
        assert (
            self.PANTRY_BASE_ELEMENTS
            + self.DV_PASSES * self.PANTRY_SLOT_ELEMENTS
            == 65536
        )
        # Every slot base is 1024 B aligned (bf16: 8192 el = 16,384 B
        # stride on a 1024 B-aligned field base).
        for _q in range(self.DV_PASSES):
            assert (
                (
                    self.PANTRY_BASE_ELEMENTS
                    + _q * self.PANTRY_SLOT_ELEMENTS
                )
                * (BFloat16.width // 8)
            ) % 1024 == 0

        # --- Q half-pantry geometry (host proof), byte-isomorphic to
        # the dO pantry block above. -----------------------------------
        # One Q pantry slot must be exactly one dkv-A quadrant image
        # (the ring-slot byte image the dK fragment already consumes).
        assert self.Q_PANTRY_SLOT_ELEMENTS == cute.cosize(
            dkv_a_layout_staged
        )
        # One quadrant image is 16 KiB, the same byte count the TMA
        # expect_tx accounting (grad_a_stage_bytes) uses per slot.
        assert self.Q_PANTRY_SLOT_ELEMENTS * (
            BFloat16.width // 8
        ) == cute.size_in_bytes(
            self.element_dtype,
            cute.select(dkv_a_layout_staged, mode=[0, 1, 2]),
        )
        # The S sub-panel occupies exactly the first 32,768 elements
        # of the 49,152-element field and equals rubin_2's
        # stationary_q.
        assert self.Q_PANTRY_BASE_ELEMENTS == cute.cosize(
            score_a_layout_staged
        )
        # The two slots tile the field tail exactly (no overlap, no
        # tail): [32768, 49152) = 2 x 8192.
        assert (
            self.Q_PANTRY_BASE_ELEMENTS
            + self.Q_PANTRY_SLOTS * self.Q_PANTRY_SLOT_ELEMENTS
            == 49152
        )
        # Every slot base is 1024 B aligned.
        for _q in range(self.Q_PANTRY_SLOTS):
            assert (
                (
                    self.Q_PANTRY_BASE_ELEMENTS
                    + _q * self.Q_PANTRY_SLOT_ELEMENTS
                )
                * (BFloat16.width // 8)
            ) % 1024 == 0

        # Single fully-literal struct (Rubin-only profile): ring depth
        # 2, single P/dS face, 128 KB stationary_do (dP sub-panel +
        # dV-A pantry), 96 KB stationary_q (S sub-panel + Q half
        # pantry).  cute.struct bodies cannot carry conditional
        # annotations; there is only one profile so none are needed.
        @cute.struct
        class SharedStorageV2:
            ...
            # [0, 32768): rank-owned H64 sub-panel (S A view).
            # [32768, 49152): Q pantry, 2 x 8192-element round-1
            # quadrants.
            stationary_q: cute.struct.Align[
                cute.struct.MemRange[element_dtype, 49152],
                1024,
            ]
            # [0, 32768): rank-owned H64 sub-panel (dP A view).
            # [32768, 65536): dV-A pantry, 4 x 8192-element quadrants.
            stationary_do: cute.struct.Align[
                cute.struct.MemRange[element_dtype, 65536],
                1024,
            ]
            score_kv: cute.struct.Align[
        ...
        storage_bytes = SharedStorageV2.size_in_bytes()
        # Upper-bound-only assert with value echo (E3 lesson: never pin
        # the exact struct size against a hand computation).  Expected
        # neighbourhood: ~325,120 B (~325 KB) per the rubin_3 byte
        # ledger (rubin_2's 308,736 + 32,768 Q pantry - 16,384 ring
        # depth 3 -> 2).
        assert storage_bytes <= self.MAX_SMEM_BYTES, (
    ...
    def _issue_dq_round_gemm(
        ...
        """One dQ round GEMM over the K_dQ image x the dS whole image."""

        mma = dq_tiled_mma.with_()
        mma.set(tcgen05.Field.ACCUMULATE, accumulate)
        for k_block in cutlass.range_constexpr(
            cute.size(kd_fragment, mode=[2])
        ):
            cute.gemm(
                mma,
                t_dq,
    ...
    def _issue_dq_rounds_v2(
        ...
        can begin the panel refills.  The dS whole image lives in the
        single P/dS face.
        """
    ...
    @cute.jit
    def _relay_send_pds_face(
        self,
        rank: Int32,
        peer_rank: Int32,
        ready_mbar: cute.Pointer,
        landing_base: cute.Pointer,
        ready_phase: Int32,
        p_xchg_src: cute.Pointer,
        p_block_dst_0: cute.Pointer,
        p_block_dst_1: cute.Pointer,
        ds_image_src: cute.Pointer,
        ds_block_dst_0: cute.Pointer,
        ds_block_dst_1: cute.Pointer,
    ) -> None:
        """One face's P/dS cross-CTA DSM sends (relay warp, single lane).

        Byte-identical to the final machine's relay body, with every
        barrier/buffer pointer supplied per face.
        """

        cute.arch.mbarrier_wait(ready_mbar, ready_phase)
        cute.arch.mbarrier_arrive_and_expect_tx(
            landing_base,
    ...
    @cute.jit
    def _relay_land_face(
        self,
        landing_base: cute.Pointer,
        relay_base: cute.Pointer,
        landing_phase: Int32,
    ) -> None:
        """One face's landing waits + relay arrivals (P then dS)."""
    ...
    @cute.jit
    def _publish_pds_face(
        self,
        tiled_copy_r2s: cute.TiledCopy,
        r_p_store: cute.Tensor,
        r_ds_store: cute.Tensor,
        owns_n: cutlass.Boolean,
        t_rs_p_local_tile: cute.Tensor,
        t_rs_ds_local_tile: cute.Tensor,
        t_rs_p_xchg_tile: cute.Tensor,
        t_rs_ds_tile: cute.Tensor,
    ) -> None:
        """stmatrix publish of P/dS into one parity face's buffers.

        Pure copies only -- callable from inside the dynamic face branch
        (v7 ping-pong rule: no pipeline state crosses the branch).
        """
    ...
    def kernel(
        ...
        epi_safe_mbar = storage.epi_safe_mbar.ptr
        ...
        round_buf_raws = (
            storage.round_buf_0.data_ptr(),
            storage.round_buf_1.data_ptr(),
        )
        round_bufs = (
            storage.round_buf_0,
            storage.round_buf_1,
        ...
        # of every tile land there under the mod-2 map (0%2, 1%2).
        round_kd = (
            round_bufs[0].get_tensor(
                dq_a_layout_staged.outer,
                swizzle=dq_a_layout_staged.inner,
            ),
            round_bufs[1].get_tensor(
                dq_a_layout_staged.outer,
                swizzle=dq_a_layout_staged.inner,
            ),
        )
        # Panel generations rotate through every slot.
        round_quad = tuple(
            buf.get_tensor(
                dkv_a_layout_staged.outer,
                swizzle=dkv_a_layout_staged.inner,
            )
            for buf in round_bufs
        )
        # dV-A pantry: four permanent [D128, H64] quadrant views over
        # the second half of stationary_do, one per dV pass q = 2r + h.
        # Each view is a fixed-address clone of a round-ring slot
        # (identical layout object and swizzle), so its dkv-A fragment
        # obeys the CG2 shared-descriptor rank-same-address rule the
        # same way the ring slots do: same local address in both CTAs,
        # each CTA holding its own M-half bytes (delivered by the
        # rank-partitioned quadrant TMA at startup).
        pantry_quads = tuple(
            cute.make_tensor(
                cute.recast_ptr(
                    stationary_do_raw
                    + self.PANTRY_BASE_ELEMENTS
                    + q * self.PANTRY_SLOT_ELEMENTS,
                    dkv_a_layout_staged.inner,
                    dtype=self.element_dtype,
                ),
                dkv_a_layout_staged.outer,
            )
            for q in range(self.DV_PASSES)
        )
        # Q pantry: two permanent [D128, H64] quadrant views over the
        # tail of stationary_q, one per round-1 dK pass q = h.  View
        # construction is byte-for-byte isomorphic to the dO pantry
        # above (same recast_ptr form, same layout object, same
        # swizzle), so the CG2 shared-descriptor rank-same-address
        # rule is satisfied the same way: same local address in both
        # CTAs, each CTA holding its own M-half bytes (delivered by
        # the rank-partitioned quadrant TMA at startup).
        q_pantry_quads = tuple(
            cute.make_tensor(
                cute.recast_ptr(
                    stationary_q_raw
                    + self.Q_PANTRY_BASE_ELEMENTS
                    + q * self.Q_PANTRY_SLOT_ELEMENTS,
                    dkv_a_layout_staged.inner,
                    dtype=self.element_dtype,
                ),
                dkv_a_layout_staged.outer,
            )
            for q in range(self.Q_PANTRY_SLOTS)
        )
        # P/dS publication is single-face: face 0 serves every tile.
        p_blocks_storages = (storage.p_blocks_0,)
        p_xchg_storages = (storage.p_xchg_0,)
        ds_image_storages = (storage.ds_image_0,)
        ds_blocks_storages = (storage.ds_blocks_0,)
        p_blocks_raw_f = tuple(
            s.data_ptr() for s in p_blocks_storages
        )
        ds_blocks_raw_f = tuple(
            s.data_ptr() for s in ds_blocks_storages
        )
        ds_image_raw_f = tuple(
            s.data_ptr() for s in ds_image_storages
        )
        p_blocks_f = tuple(
            (
                cute.make_tensor(
        ...
        p_block_stage = p_blocks_f[0][0][None, None, None, 0]
        assert (
        ...
        p_xchg_raw_f = tuple(
            s.get_tensor(flat_pds_block_layout)
            for s in p_xchg_storages
        )
        ...
        t_qt_smem_slots = (t_qt_smem_0,) + tuple(
            cpasync.tma_partition(
                tma_atom_qt,
                block_coord_vmnk[2],
                a_cta_layout,
                cute.group_modes(round_quad[slot], 0, 3),
                cute.group_modes(rank_g_qt, 0, 3),
            )[0]
            for slot in range(1, self.ROUND_STAGES)
        )
        # dO quadrant TMA now targets the pantry slots (once per
        # token) instead of the round ring (once per tile).  The gmem
        # side is the same rank-partitioned [D-round, H-half] quadrant
        # family rubin_1 used for its ring dO arm.
        t_dot_pantry_0, t_dot_gmem = cpasync.tma_partition(
            tma_atom_dot,
            block_coord_vmnk[2],
            a_cta_layout,
            cute.group_modes(pantry_quads[0], 0, 3),
            cute.group_modes(rank_g_dot, 0, 3),
        )
        t_dot_pantry_slots = (t_dot_pantry_0,) + tuple(
            cpasync.tma_partition(
                tma_atom_dot,
                block_coord_vmnk[2],
                a_cta_layout,
                cute.group_modes(pantry_quads[q], 0, 3),
                cute.group_modes(rank_g_dot, 0, 3),
            )[0]
            for q in range(1, self.DV_PASSES)
        )
        # rubin_3: the round-1 Q quadrant TMA targets the Q pantry
        # slots (once per token) instead of the round ring (once per
        # tile).  The gmem side is the same rank-partitioned
        # [D-round, H-half] quadrant family (t_qt_gmem) the ring Q arm
        # uses for round 0.
        t_qt_pantry_slots = tuple(
            cpasync.tma_partition(
                tma_atom_qt,
                block_coord_vmnk[2],
                a_cta_layout,
                cute.group_modes(q_pantry_quads[q], 0, 3),
                cute.group_modes(rank_g_qt, 0, 3),
            )[0]
            for q in range(self.Q_PANTRY_SLOTS)
        )
        ...
        quad_fragments = tuple(
            dkv_tiled_mma.make_fragment_A(view)
            for view in round_quad
        )
        # dV-A zero-copy fragments: permanent, one per pass q = 2r + h.
        # Same construction as the ring quad_fragments, at fixed pantry
        # addresses -- no ring waits or releases ever touch them.
        dv_a_fragments = tuple(
            dkv_tiled_mma.make_fragment_A(view)
            for view in pantry_quads
        )
        # dK round-1 zero-copy fragments: permanent, one per h-half.
        # Same construction as the ring quad_fragments, at fixed
        # Q-pantry addresses -- no ring waits or releases ever touch
        # them.  dK round 0 stays on the ring (quad_fragments).
        dk_q_fragments = tuple(
            dkv_tiled_mma.make_fragment_A(view)
            for view in q_pantry_quads
        )
        # Single-face B operands.
        dq_ds_fragment = dq_tiled_mma.make_fragment_B(
            ds_image_f[0]
        )
        p_fragments = (
            dkv_tiled_mma.make_fragment_B(p_blocks_f[0][0]),
            dkv_tiled_mma.make_fragment_B(p_blocks_f[0][1]),
        )
        ds_fragments = (
            dkv_tiled_mma.make_fragment_B(ds_blocks_f[0][0]),
            dkv_tiled_mma.make_fragment_B(ds_blocks_f[0][1]),
        )
        ...
        if tidx == Int32(0):
            ...
            # Per-face raw barriers ([P, dS] pairs / one ready per face).
            for face in cutlass.range_constexpr(self.PDS_FACES):
                cute.arch.mbarrier_init(
                    landing_mbars + 2 * face,
                    1,
                )
                cute.arch.mbarrier_init(
                    landing_mbars + 2 * face + 1,
                    1,
                )
                cute.arch.mbarrier_init(
                    relay_mbars + 2 * face,
                ...
                cute.arch.mbarrier_init(
                    relay_mbars + 2 * face + 1,
                    2,
                )
                cute.arch.mbarrier_init(
                    pds_ready_mbars + face,
                    self.MATH_THREADS,
                )
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            if tile_count > Int32(0):
                # kscore is single-tenant -- exactly one K generation
                # per tile.  acquire(t+1) is gated by the leader's
                # deferred release of K(t) (fires when the S/dP source
                # reads truly complete).
                ...
                if tile_count == Int32(1):
                    ...
                    # Window w's kdq is gathered right after window w+1's
                    # K load is committed (the K acquire gate -- dP(w)
                    # release -- has already passed by then), matching the
                    # final machine's real-time placement.
                ...
                # producer_tail observes K(last)'s true UMMA source-read
                # completion (the last dP).  Publish that fact to the math
                # epilogue before it reuses score_kv as dQ staging.
            ...
            t_rs_ds_f = tuple(
                thread_copy_r2s.partition_D(store)
                for store in ds_image_store_f
            )
            for _t in t_rs_ds_f:
                assert cute.size(_t, mode=[4]) == 1
            t_rs_ds_tile_f = tuple(
                _t[None, None, None, None, 0] for _t in t_rs_ds_f
            )
            n_owner = cute.arch.make_warp_uniform(
                Int32(
                    cute.get(
            ...
            owns_n = n_owner == rank
            # Per-face publish targets (ds_xchg retired: the dS exchange
            # sources from the ds_image face directly).
            aligned_p_blocks_ptr_f = tuple(
                cute.make_ptr(
                    self.element_dtype,
                    blocks[0].iterator.toint(),
                    blocks[0].memspace,
                    assumed_align=16,
                )
                for blocks in p_blocks_f
            )
            aligned_ds_blocks_ptr_f = tuple(
                cute.make_ptr(
                    self.element_dtype,
                    blocks[0].iterator.toint(),
                    blocks[0].memspace,
                ...
                for blocks in ds_blocks_f
            )
            p_local_store_f = tuple(
                cute.make_tensor(
                    cute.recast_ptr(
                        ptr,
                        score_store_layout.inner,
                ...
                for ptr in aligned_p_blocks_ptr_f
            )
            ds_local_store_f = tuple(
                cute.make_tensor(
                    cute.recast_ptr(
                        ptr,
                        score_store_layout.inner,
                ...
                for ptr in aligned_ds_blocks_ptr_f
            )
            aligned_p_xchg_ptr_f = tuple(
                cute.make_ptr(
                    self.element_dtype,
                    xchg.iterator.toint()
                ...
                for xchg in p_xchg_raw_f
            )
            p_xchg_store_f = tuple(
                cute.make_tensor(
                    cute.recast_ptr(
                        ptr,
                        score_store_layout.inner,
                ...
                for ptr in aligned_p_xchg_ptr_f
            )
            t_rs_p_local_f = tuple(
                thread_copy_r2s.partition_D(store)
                for store in p_local_store_f
            )
            t_rs_ds_local_f = tuple(
                thread_copy_r2s.partition_D(store)
                for store in ds_local_store_f
            )
            t_rs_p_xchg_f = tuple(
                thread_copy_r2s.partition_D(store)
                for store in p_xchg_store_f
            )
            for _t in t_rs_p_local_f:
                assert cute.size(_t, mode=[4]) == 1
            for _t in t_rs_ds_local_f:
                assert cute.size(_t, mode=[4]) == 1
            for _t in t_rs_p_xchg_f:
                assert cute.size(_t, mode=[4]) == 1
            t_rs_p_local_tile_f = tuple(
                _t[None, None, None, None, 0]
                for _t in t_rs_p_local_f
            )
            t_rs_ds_local_tile_f = tuple(
                _t[None, None, None, None, 0]
                for _t in t_rs_ds_local_f
            )
            t_rs_p_xchg_tile_f = tuple(
                _t[None, None, None, None, 0]
                for _t in t_rs_p_xchg_f
            )
            r_score = cute.make_rmem_tensor(
            ...
            for loop_iter in cutlass.range(tile_count):
                ...
                # Publish P/dS with stmatrix into the single face.
                # Each pair of warps owns one N32 half, so the owns_n
                # branch inside the helper is warp-uniform.
                r_p_store = thread_copy_r2s.retile(r_p)
                r_ds_store = thread_copy_r2s.retile(r_ds)
                self._publish_pds_face(
                    tiled_copy_r2s,
                    r_p_store,
                    r_ds_store,
                    owns_n,
                    t_rs_p_local_tile_f[0],
                    t_rs_ds_local_tile_f[0],
                    t_rs_p_xchg_tile_f[0],
                    t_rs_ds_tile_f[0],
                )
            ...
            if is_leader_cta:
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    if has_prev:
                        ...
                        issue_seq = loop_iter - Int32(1)
                        # Single face: its raw relay mbars are armed
                        # once per tile, so phase(t) = t % 2.
                        grads_relay_phase = issue_seq % Int32(2)
                        (
                            round_cons,
                            dkv_prod,
                            pds_cons,
            ...
            if tile_count > Int32(0):
                with cute.arch.elect_one():
                    # Q leg tx grows by the Q half pantry: the
                    # rank-owned H64 sub-panel (S view) plus the two
                    # round-1 pantry quadrants (dK tail A views),
                    # 64 KB + 2 x 16 KB.
                    cute.arch.mbarrier_arrive_and_expect_tx(
                        stationary_tma_mbars,
                        score_a_stage_bytes * self.K_CHUNKS
                        + self.Q_PANTRY_SLOTS * grad_a_stage_bytes,
                    )
                    # dO leg tx is doubled: the rank-owned H64
                    # sub-panel (dP view) plus the four pantry
                    # quadrants (dV-A views), 64 KB + 4 x 16 KB.
                    cute.arch.mbarrier_arrive_and_expect_tx(
                        stationary_tma_mbars + 1,
                        score_a_stage_bytes * self.K_CHUNKS
                        + self.DV_PASSES * grad_a_stage_bytes,
                    )
                cute.copy(
                    tma_atom_q,
                ...
                # Pantry fills: this CTA's rank-partitioned M-half of
                # dO.T for each dV pass q = 2r + h -- the same
                # t_dot_gmem[None, r, h] source the rubin_1 ring dO
                # arm used every tile, issued once per token.
                cute.copy(
                    tma_atom_dot,
                    t_dot_gmem[None, 0, 0],
                    t_dot_pantry_slots[0][None, 0],
                    tma_bar_ptr=stationary_tma_mbars + 1,
                )
                cute.copy(
                    tma_atom_dot,
                    t_dot_gmem[None, 0, 1],
                    t_dot_pantry_slots[1][None, 0],
                    tma_bar_ptr=stationary_tma_mbars + 1,
                )
                cute.copy(
                    tma_atom_dot,
                    t_dot_gmem[None, 1, 0],
                    t_dot_pantry_slots[2][None, 0],
                    tma_bar_ptr=stationary_tma_mbars + 1,
                )
                cute.copy(
                    tma_atom_dot,
                    t_dot_gmem[None, 1, 1],
                    t_dot_pantry_slots[3][None, 0],
                    tma_bar_ptr=stationary_tma_mbars + 1,
                )
                # rubin_3 Q pantry fills: this CTA's rank-partitioned
                # M-half of Q.T for the two round-1 dK passes (q = h)
                # -- the same t_qt_gmem[None, 1, h] source the ring
                # Q_r1 arm used every tile, issued once per token.
                # They ride the Q leg (stationary_tma_mbars + 0).
                cute.copy(
                    tma_atom_qt,
                    t_qt_gmem[None, 1, 0],
                    t_qt_pantry_slots[0][None, 0],
                    tma_bar_ptr=stationary_tma_mbars,
                )
                cute.copy(
                    tma_atom_qt,
                    t_qt_gmem[None, 1, 1],
                    t_qt_pantry_slots[1][None, 0],
                    tma_bar_ptr=stationary_tma_mbars,
                )
                # Split readiness: S needs only Q, dP needs only dO, so the
                # leader can issue the first S one TMA earlier.  The dO
                # ready arrive now also certifies the pantry (the first
                # dV pass runs strictly after the first dP), on both
                # CTAs via the count-2 leader mbar.  rubin_3: the Q
                # ready arrive likewise certifies the Q pantry (the
                # first dK tail pass runs strictly after the first S);
                # the first S now waits on 32 KB more (the Q pantry) --
                # a one-time, accepted widening of its gate.
                cute.arch.mbarrier_wait(
                    stationary_tma_mbars,
                    Int32(0),
                )
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    # g2/g3: Q_r0 h0/h1 panel fills, pipelined at ring
                    # depth ROUND_STAGES (rubin_3: the dO quadrants
                    # live in the dO pantry and the Q_r1 quadrants in
                    # the Q pantry; only kdq and Q_r0 ride the ring).
                    # Both panel commits are delegated to W19.  Re-arm
                    # safety: gen G's arm follows
                    # producer_acquire(G), i.e. the leader's release
                    # of gen G-ROUND_STAGES, whose consumption ordered
                    # after W19's wait cleared this slot's previous
                    # completion (for slots 0/1 the chain passes
                    # through the mbar-less kdq rendezvous commits of
                    # g0/g1) -- the mbar is never re-armed while
                    # pending.
                    for flat_gen in cutlass.range_constexpr(2):
                        grad_round = 0
                        h_half = flat_gen % 2
            ...
            if tidx % Int32(32) == Int32(0):
                ...
                for loop_iter in cutlass.range(tile_count):
                    # Single face: its raw barriers flip once per tile.
                    face_phase = loop_iter % Int32(2)
                    self._relay_send_pds_face(
                        rank,
                        peer_rank,
                        pds_ready_mbars,
                        landing_mbars,
                        face_phase,
                        p_xchg_raw_f[0].iterator,
                        p_block_raw_ptrs_f[0][0],
                        p_block_raw_ptrs_f[0][1],
                        ds_image_raw_f[0],
                        ds_block_raw_ptrs_f[0][0],
                        ds_block_raw_ptrs_f[0][1],
                    )
                    pipe_pds.producer_commit(pds_com)
                    pds_com.advance()
                    self._relay_land_face(
                        landing_mbars,
                        relay_mbars,
                        face_phase,
                    )
            ...
            # One raw-mbar phase per ring slot: a slot's mbar flips
            # once per arm; rubin_3's panel walk hits each slot once
            # per tile (g2 -> slot 0, g3 -> slot 1), so each phase
            # local flips once per tile.  Per-slot named locals kept
            # as the safe carrier (rubin_2 precedent).
            w19_phase_0 = Int32(0)
            w19_phase_1 = Int32(0)
            for loop_iter in cutlass.range(tile_count):
    ...
    def _issue_prev_grads_head_v2(
        ...
        dv_fragment_h0: cute.Tensor,
        dv_fragment_h1: cute.Tensor,
        quad_fragment_dk_h0: cute.Tensor,
        quad_fragment_dk_h1: cute.Tensor,
        ...
        rubin_3 (unchanged from rubin_2): the dQ K images (g0/g1) and
        the Q_r0 quadrants (g2/g3) are ring generations; the dV
        round-0 A operands are the permanent pantry fragments
        ``dv_fragment_h0/h1`` -- no ring wait or release surrounds
        their issue.  All P/dS B operands live in the single face.
        The tail keeps v12's early dQ
        completion commit; the epi_safe barrier (published after the
        kscore producer tail) protects score_kv from the dQ epilogue
        until dP of the last tile has truly read K.
        """
        ...
        _mbarrier_wait_acquire_cluster(relay_base, relay_phase)

        dkv_done_pipeline.producer_acquire(dkv_producer_state)
        # dV round-0: the permanent dO.T pantry views (zero-copy; no
        # ring machinery).
        ...
        # dK round-0: the Q_r0 quadrant pair from the ring (g2/g3).
        round_pipeline.consumer_wait(round_consumer_state)
        self._issue_dkv_pass_v2(
            dkv_tiled_mma,
            t_dkv_0,
    ...
    def _issue_prev_grads_tail_v2(
        ...
        dv_fragment_h0: cute.Tensor,
        dv_fragment_h1: cute.Tensor,
        dk_fragment_h0: cute.Tensor,
        dk_fragment_h1: cute.Tensor,
        p_fragment_0: cute.Tensor,
        p_fragment_1: cute.Tensor,
        ds_fragment_0: cute.Tensor,
        ds_fragment_1: cute.Tensor,
        ...
        rubin_3: BOTH round-1 A operands are permanent pantry
        fragments -- dV from the dO pantry (rubin_2), dK from the Q
        pantry (new).  No ring wait or release remains in the tail;
        the ring's per-tile business ends with the dK head passes.  B
        operands live in the single P/dS face.
        """

        dkv_done_pipeline.producer_acquire(dkv_producer_state)
        # dV round-1: the permanent dO.T pantry views (zero-copy).
        self._issue_dkv_pass_v2(
            dkv_tiled_mma,
            t_dkv_1,
            dv_fragment_h0,
        ...
        # dK round-1: the permanent Q.T pantry views (zero-copy; the
        # rubin_2 ring generations g4/g5 no longer exist).
        self._issue_dkv_pass_v2(
            dkv_tiled_mma,
            t_dkv_1,
            dk_fragment_h0,

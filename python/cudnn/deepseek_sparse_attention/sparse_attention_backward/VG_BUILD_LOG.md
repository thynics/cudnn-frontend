...
| v_w3_4 r2 | 序幕刀 1：topk 标量链 + gen0 索引 pre-barrier 前提 | 9.449 | 8.356 | 1.1308 | **null，归档**（同 run v_w3_2 腿 9.4495 @ 8.398：Δ +0.0006ms ≈ 零，双指标矛盾；四 case 逐位同）。判读：标量链原已被 init 窗实际隐藏或 head 绑定者在别处——与 K(0) 驳回案"真绑定者=stationary 面板路径"自洽。序幕家族仅剩 Q 面板 per-chunk 就绪一刀（−0.02~0.04 量级）。r1 曾命中 DSL 边界：未装饰 helper 的动态 if 在显式 scf.if 区域不被 AST 重写（then_block 上下文）——入 DSL 暗礁清单 |
...
|---|---|---|---|
| **dsa_bwd_sm100_2cta_final.py**（无 trace 交付版，7,335 行） | 9.450131 | −0.0007ms | 4/4，max_abs 三腿逐位相同 |
| **dsa_bwd_sm100_2cta_final_trace.py**（采集版，7,690 行） | 9.450707 | −0.0002ms | 同上 |

由 v_w3_2 机械生成：删休眠类 V0/V1A0（AST 定界，保留类间模块级助手）、release
版剥全部 IKET（代理类 + 327 行 span 语句 + 退化守卫坍缩）、战役标签注释按块
清除、docstring 产品化。**逻辑不变性 = 独立路径 AST 恒等审计**（对原文件做
AST 级变换与交付文件 dump 逐字节相等；审计拦截生成器两个真实缺陷：类间助手
误删、退化守卫不一致）。性能不变性 = 三腿同 service 硅上验收（Δ < 0.001ms）。
final_trace 的采集链路已实跑（decoded JSON 已产出）。

**SM100 战役闭章数字**：10.572 → 9.441（−10.7%），ratio 1.2786 → 1.1368；
超越 baseline 判定 Rubin-locked（四项应收：M2 / ve_1 / K3 环深 / CG2 drain
优势变现）。

...
stores); DQ_EPI measured 2x12us of the 28.5us per-token tail, and vg_6
adjudicated the cost as scatter, not TMA flight.  vk_3 flips the staging
to the accumulator's own (D, H) orientation and reuses the publish
recipe end to end: a forced Ld16x256b(Rep4) T2R atom keys
get_smem_store_op's stmatrix.m8n8.x4.trans f32->bf16 clause (build-gated
by the same isinstance assert as the P/dS publish -- a scalar fallback
fails the compile, not the run), and the gmem transpose moves into the
TMA view over the established [D, H, L] mdQ tensor (256B row segments
unchanged).  Store values are bit-identical: the same per-value bf16
convert, vectorized.  Barriers, the single 32KB staging in dead score-K,
and the one bulk TMA per round are all untouched.

Register note: the bf16 fragment adds 64 regs over the 128-reg f32
fragment on math warps; the scalar path already lived beyond the budget,
so the spill gate is the run (SASS autopsy pre-registered), not the
build.  The dormant V0 epilogue path is orientation-corrected in place.
...
class FlashAttentionDSABackwardSm100TwoCTA(FlashAttentionDSABackwardSm100):
    ...
    def __call__(
        ...
        # vk_3: the TMA pairs the (D, H)-oriented staging with the
        # established [D, H, L] mdQ view (D contiguous in gmem); per-row
        # 256B segments are unchanged, only the box orientation flips.
        ...
        assert cute.cosize(dq_epi_layout_staged) == (
        ...
        # vk_3: the epilogue staging flips to the accumulator's own
        # (D, H) orientation -- COL_MAJOR, D contiguous, exactly the
        # publish pair (COL_MAJOR destination + Ld16x256b T2R) that the
        # stmatrix clause of get_smem_store_op is keyed for.  The gmem
        # transpose moves into the TMA view (strides are free).
    ...
    def _store_dq_epi_tma_v12(
        ...
        if mtx < self.MATH_THREADS_PER_CTA:
            # vk_3: force the 16-DP T2R atom (the reducer's Ld16x256b
            # Rep4 construction) so get_smem_store_op fires its
            # stmatrix.m8n8.x4.trans f32->bf16 clause instead of the
            # scalar CopyUniversalOp fallback (the v9.3 lesson).  The
            # dq_tmem_load parameter stays in the signature untouched.
            epi_load_atom = cute.make_copy_atom(
                tcgen05.copy.Ld16x256bOp(
                    tcgen05.copy.Repetition(4)
                ),
                self.acc_dtype,
            )
            tiled_t2r = tcgen05.make_tmem_copy(epi_load_atom, t_dq)
            ...
            cute.copy(tiled_t2r, thread_source, thread_values)
            cute.arch.fence_view_async_tmem_load()
            # The scalar path wrote 128 STS.U16 per thread through the
            # (head, local_d) transpose; the staging is now C-oriented
            # (D, H) so the same bf16 converts leave through stmatrix.
            r_dq = cute.make_rmem_tensor(
                thread_coordinates.shape,
                self.element_dtype,
            )
            for value_index in cutlass.range_constexpr(
                ...
                r_dq[value_index] = self.element_dtype(
                    thread_values[value_index]
                )
            ...
            # v9.3 premise as a BUILD gate (mirrors the publish): a
            # silent scalar fallback must fail the compile, not the run.
            assert isinstance(
                smem_store_atom.op,
...
class FlashAttentionDSABackwardSm100TwoCTAV0(
    ...
    def _store_dq_round_v0(
        ...
        for value_index in cutlass.range_constexpr(
            ...
            # vk_3: the staging flipped to (D, H) with the host layout;
            # this dormant V0 path keeps its scalar stores, reindexed.
            s_dq_epi[
                local_d,
                head,
            ] = self.element_dtype(thread_values[value_index])

"""DeepSeek Sparse Attention backward, two-CTA SERIAL chain, kq6g.

final_ser_kq6g = final_ser_kq6k with the complete dKV reduce issue path
moved from per-thread red.global.add.f32x4 to row-contiguous 512-byte
cp.reduce.async.bulk groups.  Each reducer warpgroup streams through two
private 512-byte FP32 SMEM rows; one thread per warpgroup issues the bulk
reduces and keeps at most two groups in flight.  TMEM, T2R, pipeline
generations, slot order, and f32 elementwise atomic-add semantics stay
unchanged.

Shape (a) is used because it needs 128 x 512-byte operations per CTA/tile;
the warp-private 128-byte alternative needs 512 operations and risks the
async bulk-group queue.  The two-row x two-warpgroup staging footprint is
exactly 2 KiB.

Below: inherited lineage docstrings.
    ...
    def _drain_dkv_v8(
        ...
        for i in cutlass.range_constexpr(8):
            ...
            for row_in_chunk in cutlass.range_constexpr(4):
                ...
                # The issuer owns the bulk-group queue.  Waiting down to
                # one group before the WG crosses this barrier makes the
                # parity-selected row stage safe to overwrite.
                if dp_idx == Int32(0):
                ...
                if dp_idx == Int32(0):
                    ...
                    # Commit an empty group for an invalid row as well, so
                    # double-buffer reuse remains group-parity exact while
                    # the bulk operation itself is skipped.
                    cute.arch.cp_async_bulk_commit_group()

"""v_w3_3: v_w3_2 + M2 retest -- deg-6 FFMA exp2 on the new base.

The M2 receivable (vk_4, expected-null on the vk_2 base: math slack
absorbed the SOFTMAX cut while W17 held 97.6% duty) retests on a base
that has since taken three W17-side knives (vk_5 W1' -0.100, vk_7 W2
-0.168, v_w3_2 R2 -0.086).  The multi-lens review established the loop
threads math publish once per tile (g2-acquire <- dQ-r0 MMA <- PDS_WAIT
<- math publish), so the retest criterion is whether the shortened
SOFTMAX now moves the train head.  Code delta = vk_4's verbatim: one
inline-asm helper (user-adjudicated deg6, max rel err 2.166e-7, better
than MUFU.EX2's own error class; -126 clamp for masked columns) and the
two exp2 call sites (band + fallback, value-consistent).

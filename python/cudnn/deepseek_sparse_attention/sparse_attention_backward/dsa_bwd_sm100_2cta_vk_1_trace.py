"""vk_1_trace: vk_1 with the supply-wait spans broken out (refined trace).

Instrumentation sibling of vk_1 (= vg_5 + independent producer state per
pds pipeline, the validated S1 fix that restores IKET trace capture).
Carries the refined span trade inside the fixed 31-name budget:
RK_ACQ(i), MAT_ACQ(m,q) and MAT_WAIT(m,q) in (kscore credit wait, ring
credit wait, TMA completion wait -- the supply-row decomposition);
LOAD_QDO, LOAD_STATS and MATH_BAR1 out (prologue-only or sub-0.05us).
MATH_SOFTMAX, MATH_PDS_ACQ and both T2R spans kept: acceptance signals
for the exp band (vg_2/vg_5) and split publish (vg_1).

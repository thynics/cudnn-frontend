...
# vfinal_aug_7: the dKV red.global bursts are gone (topk-compact TMA store
# in the drain + scatter-accumulate tail).  The aug_6 pacing knobs are
# retired; kept as inert constants so sweep tooling does not NameError.
REDUCE_PACE_EVERY = 4
...
def _read_smid(
    *,
    ...
    """aug_7: physical SM id -- selects this CTA's private compact panel."""
...
class FlashAttentionDSABackwardSm100TwoCTA(FlashAttentionDSABackwardSm100):
    ...
    def __call__(
        ...
        # aug_7: topk-compact dKV workspace -- 148 SM-indexed panels of
        # [seqlen_kv_r rows x 256 fp32 cols] appended after the accumulator
        # region of workspace_dKV (see _get_workspace_size_dKV).  Panel
        # cols 0..127 = this CTA's slot-0 stripe (acc sub_tile rank),
        # 128..255 = slot-1 stripe (acc sub_tile 2+rank), column scramble
        # preserved verbatim so convert_canonical stays untouched.
        seqlen_kv_r = cute.round_up(mKV.shape[0], 8)
        d_r = cute.round_up(problem_shape[2], 8)
        compact_iter = cute.recast_ptr(
            workspace_dKV.iterator
            + seqlen_kv_r * d_r * (self.acc_dtype.width // 8),
            dtype=self.acc_dtype,
        )
        mdKV_compact = cute.make_tensor(
            compact_iter,
            cute.make_layout(
                (seqlen_kv_r, 256, 148),
                stride=(256, 1, seqlen_kv_r * 256),
            ),
        )
        dkv_cmp_stage_layout = cute.make_layout(
            (8, 128),
            stride=(128, 1),
        )
        tma_atom_dkv_compact, tma_tensor_dkv_compact = (
            cpasync.make_tiled_tma_atom(
                cpasync.CopyBulkTensorTileS2GOp(),
                mdKV_compact,
                dkv_cmp_stage_layout,
                (8, 128),
            )
        )
        ...
        self.kernel(
            ...
            stationary_a_layout_staged,
            mdKV_compact,
            tma_atom_dkv_compact,
            tma_tensor_dkv_compact,
        ).launch(
            grid=(
                2 * problem_shape[0],
                1,
...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    def kernel(
        ...
        mdKV_compact: cute.Tensor,
        tma_atom_dkv_compact: cute.CopyAtom,
        tma_tensor_dkv_compact: cute.Tensor,
    ):
        ...
        # aug_7: SM-indexed compact-panel ownership.  A CTA occupies its SM
        # until it retires (tail included, 1 CTA/SM at this smem/thread
        # footprint), so successor CTAs cannot overlap the panel.  Modulo
        # 148 guards out-of-range smid; >148-SM parts would alias (risk R4).
        smid_panel = cute.arch.make_warp_uniform(
            _read_smid() % Int32(148)
        )
        ...
        # aug_7: dKV compact staging -- the dead ds_xchg 4 KiB (never
        # stored or loaded anywhere in V2; the -n_owner-shifted VIEW at
        # the math role is left untouched) reinterpreted as one
        # [8 rows x 128 cols] fp32 chunk, plain (128,1) stride, no swizzle.
        s_dkv_stage = cute.make_tensor(
            cute.recast_ptr(
                storage.ds_xchg.data_ptr(),
                dtype=self.acc_dtype,
            ),
            cute.make_layout((8, 128), stride=(128, 1)),
        )
        round_kd = (
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            # aug_7: FULL completion (read=False, unlike the read-only
            # staging gate) of every compact bulk-store group, so the
            # common-tail cta_barrier orders the panel writes before the
            # scatter readback.
            cute.arch.cp_async_bulk_commit_group()
        ...
        # this readback.  red.global (NOT plain RMW): concurrently-tailing
        # clusters on other tokens may target the same kv_index row; the
        # cross-launch contract (stream serialization before
        # convert_canonical) is unchanged.  Rows >= topk and kv_index == -1
        # slots hold garbage in the panel and are skipped, mirroring the
        # aug_6 drain predicates.
        if tile_count > Int32(0):
            compact_panel = mdKV_compact[None, None, smid_panel]
            total_segments = tile_count * Int32(64 * 64)
            tail_iters = (
                total_segments + Int32(self.THREADS_PER_CTA - 1)
            ) // Int32(self.THREADS_PER_CTA)
            for tail_iter in cutlass.range(tail_iters):
                seg = tidx + tail_iter * Int32(self.THREADS_PER_CTA)
                if seg < total_segments:
                    row = seg // Int32(64)
                    quad_col = seg % Int32(64)
                    kv_index = Int32(-1)
                    if row < topk:
                        kv_index = mTopkIdxs[
                            row,
                    ...
                    if kv_index >= Int32(0):
                        src_quads = cute.flat_divide(
                            compact_panel[row, None],
                            (4,),
                        )
                        r_cmp = cute.make_rmem_tensor(
                            (4,),
                            self.acc_dtype,
                        )
                        cute.autovec_copy(
                            src_quads[None, quad_col],
                            r_cmp,
                        )
                        stripe = quad_col // Int32(32)
                        dkv_row = mdKV_acc[
                            None,
                            kv_index,
                            (0, batch_idx),
                        ...
                        sub_quads = cute.flat_divide(sub_row, (4,))
                        cute.arch.atomic_add(
                            sub_quads[
                                None,
                                quad_col % Int32(32),
    ...
    def _drain_dkv_v8(
        ...
        """Drain both rank-owned dKV slots into the topk-compact workspace.

        aug_7: consumer_wait/T2R/fence/consumer_release per slot is
        byte-identical to aug_6 (MMA backpressure and the slot-0 head
        start unchanged).  Only the burst destination changed: register
        quads land in the 4 KiB ds_xchg staging and leave as 8 chunked
        [8x128] fp32 bulk TMA S2G stores per slot into this CTA's
        mdKV_compact panel.  The KV-index preload and the mdKV_acc
        scatter moved to the in-kernel tail phase.  Historical text below:
        ...
        # aug_7: KV-index preload removed -- the drain no longer touches
        # mdKV_acc or mTopkIdxs; predication happens in the tail phase
        # (rows >= topk / kv_index == -1 leave garbage in the compact
        # panel by design, exactly like the aug_6 dropped-garbage rule).
        ...
        # aug_7: deposit the slot-0 panel into the topk-compact workspace
        # as 8 x [8 rows x 128 cols] fp32 chunks (4 KiB = ds_xchg staging).
        # Chunk b: panel rows 8b..8b+7, owner warp group b//4 (T2R band
        # v2 = b%4); columns keep TODAY'S quad scramble (quad k at column
        # 4*(dp_idx//4)+k), so the tail scatter reproduces byte-identical
        # mdKV_acc row images and convert_canonical stays untouched.
        g_cmp_tiles = cute.local_tile(
            tma_tensor_dkv_compact,
            (8, 128),
            (None, None, smid_panel),
        )
        for chunk in cutlass.range_constexpr(8):
            if wg_idx == Int32(chunk // 4):
                for v0 in cutlass.range_constexpr(2):
                    ...
                    cute.autovec_copy(
                        rdkv_frg_0,
                        stage_quads[None, dp_idx // 4],
                    )
            cute.arch.fence_view_async_shared()
            self.reduce_pace_barrier.arrive_and_wait()
            g_cmp_chunk = g_cmp_tiles[
                None,
                None,
                tile_index * Int32(8) + Int32(chunk),
                0,
            ]
            t_smem_cmp, t_gmem_cmp = cpasync.tma_partition(
                tma_atom_dkv_compact,
                0,
                cute.make_layout(1),
                cute.group_modes(s_dkv_stage, 0, 2),
                cute.group_modes(g_cmp_chunk, 0, 2),
            )
            if rtx < Int32(32):
                cute.arch.fence_view_async_shared()
                cute.copy(
                ...
                cute.arch.cp_async_bulk_commit_group()
                cute.arch.cp_async_bulk_wait_group(0, read=True)
            # Staging-reuse gate: chunk b+1 overwrites the 4 KiB only
            # after the engine has READ chunk b (dQ-epilogue idiom,
            # aug_6:1548-1552, with the 256-thread reduce_pace_barrier).
            self.reduce_pace_barrier.arrive_and_wait()
        ...
        # aug_7: slot-1 panel -> compact stripe 1 (acc sub_tile 2+rank).
        for chunk in cutlass.range_constexpr(8):
            if wg_idx == Int32(chunk // 4):
                for v0 in cutlass.range_constexpr(2):
                    i = 2 * (chunk % 4) + v0

...
(final_new: rotation removed -- baseline-style dependency staircase).
...
final_new removes final's rotated leader schedule: every iteration
processes ONE tile start-to-finish (S -> dP -> dV passes -> dK passes
-> dQ).  The math role splits into a P-phase and a dS-phase publishing
through two 1-stage pipes (pipe_p / pipe_pds), W18 relays P then dS as
two back-to-back stages, and the grads train runs dV-major with e4d's
split dkv acquire/commit producer states.  The score_kv loan is gone:
dO_r0 travels through the round ring like every other panel (ten ring
generations per tile), leaving score_kv a single-tenant K buffer so
K(t+1) fills immediately after dP(t)'s consumption commit.
"""
    ...
    def kernel(
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            ...
            for loop_iter in cutlass.range(tile_count):
                ...
                # ==========================================================
                # dS-phase: dP + saved FP32 P -> dS published.
                # ==========================================================

"""vc_1: SM100 two-CTA DSA backward with swap-AB and four-way tiling.

All five GEMMs use ``tcgen05.CtaGroup.TWO``.  Swapping the score operands
puts the head axis on the output columns: ``St = K @ Q.T`` and
``dPt = V @ dO.T``.  Each kv bundle is split into four h32 sub-tiles and Q
and dO are supplied as streams for those sub-tiles.

For each sub-tile the math path consumes S first, publishes local P, then
consumes dP together with P and publishes local dS.  The gradient path is
scheduled as four stages so the full operands do not occupy shared memory at
once: ``dV += P @ dO``, ``dK += dS @ Q``, and ``dQ += K @ dS``.  dO/Q and
the K view used by dQ are streamed or gathered; the peer half of dS needed by
dQ is exchanged through DSM, while P and the producer-owned dS stay local.

The steady-state leader order is ``score(0); score(t)+grads(t-1); grads(3)``.
Thus score, P/dS math, gradient MMA, and reduction can overlap across adjacent
sub-tiles without requiring a full-size single gradient stage in SMEM.
"""
...
class FlashAttentionDSABackwardSm100TwoCTAV2(
    ...
    """vc_1 four-sub-tile swap-AB data plane.
    ...
    * The leader is dr-major; the chase pieces for bundle t+1 are pinned
    ...
    #
    # Sub-tile head map (forced by the streamed strip ownership -- CTA r
    # supplies H[c*64+r*32 : +32) for c in {0,1} -- and the hardware CG2
    ...
    # K residency is score A and the Q/dO strip ring is score B.  The
    # gradient stream moves to the dkv-B operand (GMEM-
    ...
    def _specialize_shared_storage(
        ...
        #   dkv_b:   two half-wide [h32 x own-D64] stages  = 8,192
    ...
    def kernel(
        ...
        # vc_1: the boxes are STATIONARY_TILE_H(16)-row streamed
        # B-operand strips, partitioned through the helper MMA's B fraction.
        g_q = cute.local_tile(
            tma_tensor_q,
            cute.select(
                (
        ...
        if warp_idx < Int32(self.GATHER_WARPS):
            # --- gather, vc_1: the K residency fill --
            # eight rank-owned [own-kv64 x D64] pieces per bundle,
            # gathered once into the resident image.  The same four
            # gather warps also fill the kdq images.  After the peeled
            # first fill, steady state is r1(b-1) -> fill K(b+1) ->
            # r0(b).  This keeps the global kdq sequence unchanged but
            # moves the next K fill into the current bundle's grad tail.
            ...
            if tile_count > Int32(0):
                # Prologue: the first resident K image has no earlier
                # bundle in which it can be prefetched.
                ...
                # Each steady iteration services the current bundle's
                # kdq rendezvous while prefetching the next bundle's K.
                # The last bundle is peeled below because it has no next
                # resident image; pipeline-state advances stay static.
                for loop_iter in cutlass.range(
                    tile_count - Int32(1)
                ):
                    bundle_idx = tile_count - Int32(1) - loop_iter
                    # Complete r1(b-1) before r0(b), preserving the
                    # global r0/r1 named-barrier order.
                    ...
                    # The score(3) tcgen05 completion edge admits the
                    # next bundle's single resident-K fill here, ahead
                    # of the current bundle's late r0 rendezvous.
                ...
                # Peeled final bundle (bundle 0): preserve r1(prev),
                # r0(current), then the epilogue r1(current), with no
                # speculative K fill after the end of the stream.
            ...
            if is_leader_cta:
                ...
                # vc_1 head-outer bundle schedule:
                ...
                # issue window, then runs grads(t-1) immediately behind
                # it.  K is resident for all four passes; the bundle-head
                # kres wait admits the single eight-piece fill and the
                # release after score(3) gates the next bundle's fill.
                # The single round FIFO keeps kdq behind all 32 independent
                ...
                for loop_iter in cutlass.range(tile_count):
                    ...
                    for sub_tile in cutlass.range_constexpr(
                        ...
                        if cutlass.const_expr(
                            ...
                            # Release K immediately after score(3).  The
                            # AsyncUmma edge is delayed by tcgen05 until
                            # the final resident-K reads are complete,
                            # while grads(2), grads(3), and G5 can overlap
                            # the next bundle's gather.
            ...
            # --- load, vc_1: four Q/dO strip generations followed by
            # 36 round generations per bundle: g0..g31 are the 32
            ...
            if tile_count > Int32(0):
                for loop_iter in cutlass.range(tile_count):
                    ...
                    # Four score strip-pair generations, stage=t%2,
                    # are filled ahead of the round generations
                    ...
                    # dO -> buf A (barrier 0), Q -> buf B (barrier 1).
                    # Each gen carries the sub-tile's two h16 boxes on
                    # one barrier (expect_tx = 4,096 B).  The two-stage
                    # ring is software-pipelined so gen q is issued
                    # while gen q-1 completes.  MAT_QDO stays per
                    # sub-tile with payload bundle*4+t.
...
#   * audit: streamed-strip byte-identity with the staged score-B
...
# ======================================================================
# ======================================================================
# VC_1 RESIDENCY/STREAMING ADDENDUM
# ======================================================================
#
...
#     completion edge"); release sits after score(3), before grads(3),
#     so the next fill overlaps the whole grads/G5 tail;
...
#
# Wait-graph deltas (all other four-stage edges remain unchanged):
...
# publish, so W17 never delays grads(0); cross-bundle closes via
...
# Private harnesses historically select either the v0 or v1 symbol after
# remapping an experimental module.  Keep every selector on vc_1's active
# four-stage implementation; the explicit V2 symbol remains the direct entry.

"""v_w3_1: vk_7 + W3 -- the score-K index loads hoist to the iteration top.

W2's mirror on the score side (multi-lens review F10, adversarially
verified): _load_score_kv issues its two per-thread topk index loads
AFTER the kscore producer_acquire returns, so the first row copy eats
the full GMEM index latency inside the credit-gated critical section.
The steady iteration's head holds a 1.5-2.5us compound window (loan
acquire + loan TMA flight + K-acquire) with nothing for the gather
warps to do -- W3 moves the index loads (pure register state, 2 x Int32
per thread) to the iteration top, hiding them under that window.

The prefetch tile index is clamped to 0 on the final iteration (whose
K-leg does not exist; the two loads read valid tile-0 slots and are
discarded), so every consumed value is bit-identical to vk_7's.
...
class FlashAttentionDSABackwardSm100TwoCTA(FlashAttentionDSABackwardSm100):
    ...
    def _load_score_kv(
        ...
        row_local_n, row_kv_index = self._score_kv_row_indices(
            mTopkIdxs,
            token_idx,
            batch_idx,
            tile_index,
            ...
            if tile_count > Int32(0):
                ...
                if tile_count == Int32(1):
                    ...
                    for score_iter in cutlass.range(
                        ...
                        # W3: prefetch the next K tile's two per-thread
                        # topk indices at the iteration top -- their GMEM
                        # latency hides under the loan acquire + TMA
                        # flight + K-acquire compound window.  The tile
                        # index is clamped to 0 on the final iteration
                        # (no K leg there; the loads read valid tile-0
                        # slots and are discarded), so every consumed
                        # value is bit-identical to the unhoisted form.
                        w3_tile_index = (
                            tile_count - Int32(2) - score_iter
                        )
                        if w3_tile_index < Int32(0):
                            w3_tile_index = Int32(0)
                        (
                            w3_row_local_n,
                            w3_row_kv_index,
